...
| M1 | 工程底座、命令层、Operation 骨架 | 11 | 66 | 0 | 未开始 |
...
3. **M1 准备**：M1-01 标签已存在，删除旧实现需要产品负责人确认后执行；随后按 M1-02～M1-11 搭建工程底座。
...
| 任务 | 内容 | 怎么做 | 涉及文件 | 状态 | 提交 |
| --- | --- | --- | --- | --- | --- |
| M1-01 | 旧代码处置 | 标签 `legacy-2026-09` 已在远端（2026-09-26 核实）；【需产品负责人确认后执行】删除旧实现作为 M1 第一个独立提交（PLN-01 §5） | `backend/`、`agent/`、`frontend/` | 待办 | — |
...
| E-06-03 | 异步、工作流与事件 | 事件 `identity.user_changed.v1`（Outbox → Kafka，消费者按事件 ID 去重）。要点：无工作流。账号变更事务内写 Outbox 事件 `identity.user_changed.v1` 与 `audit.recorded.v1`（审计只经后者写入，DES-12）。 详见 [DES-09 §4](docs/design/09-账号与会话.md#4-异步与工作流) | `backend/internal/identity/adapter/workflow/`、`backend/internal/identity/adapter/event/`、`contracts/` | 待办 | — |
...
| E-17-03 | 异步、工作流与事件 | 事件 `media.consent_recorded.v1`、`media.consent_revoked.v1`（Outbox → Kafka，消费者按事件 ID 去重）。要点：无。 详见 [DES-20 §4](docs/design/20-真人授权.md#4-异步与工作流) | `backend/internal/media/adapter/workflow/`、`backend/internal/media/adapter/event/`、`contracts/` | 待办 | — |
...
| E-28-03 | Agent 服务 | 视频适配器 `omni_reference` 用途映射与提示词指代；输入输出按 `contracts/activities` 契约，离线评测纳入 `agent/evals/`。 | `agent/app/providers/` | 待办 | — |
...
| E-27-03 | 异步、工作流与事件 | 工作流 / Activity / 定时任务 `OperationWorkflow`。要点：OperationWorkflow（异步视频）：提交 → 轮询（退避 5 s → 30 s）→ 接管（下载、ffprobe、转码、缩略图、代理）→ 审核 → 完成。提示词由 Go 规则模板拼接（免费）；AI 优化提示词不… 详见 [DES-30 §4](docs/design/30-图生视频.md#4-异步与工作流) | `backend/internal/operation/adapter/workflow/`、`backend/internal/operation/adapter/event/`、`contracts/` | 待办 | — |
| E-27-04 | Agent 服务 | 视频适配器 `image2video` 模式（提示词由 Go 规则模板生成，不需 Skill）；输入输出按 `contracts/activities` 契约，离线评测纳入 `agent/evals/`。 | `agent/app/providers/` | 待办 | — |
