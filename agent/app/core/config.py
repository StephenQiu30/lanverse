...
class Settings(BaseSettings):
    ...
    media_location_rollback_seconds: int = Field(default=86400, ge=60, le=7 * 24 * 60 * 60)

    jwt_secret_key: SecretStr = SecretStr("development-only-jwt-secret-change-before-production")
    ...
    def validate_cross_setting_invariants(self) -> "Settings":
        if self.environment == "development":
            self.cors_origins = list(dict.fromkeys((*self.cors_origins, *DEVELOPMENT_CORS_ORIGINS)))
        ...
        if len(verification_secret.encode("utf-8")) < 32:
            raise ValueError("EMAIL_VERIFICATION_HMAC_SECRET must contain at least 32 bytes")
        if verification_secret == self.jwt_secret_key.get_secret_value():
            raise ValueError("EMAIL_VERIFICATION_HMAC_SECRET must not reuse JWT_SECRET_KEY")
        if (
            self.environment == "production"
            and verification_secret == DEVELOPMENT_EMAIL_VERIFICATION_HMAC_SECRET
        ):
            raise ValueError("EMAIL_VERIFICATION_HMAC_SECRET must be set in production")
        if self.smtp_enabled:
            ...
            if missing:
                raise ValueError(f"{', '.join(missing)} must be set when SMTP_ENABLED is true")
        ...
        if (
            ...
            raise ValueError(
                "PROVIDER_CREDENTIAL_FINGERPRINT_KEY must not reuse PROVIDER_CREDENTIAL_MASTER_KEY"
            )
        return self
