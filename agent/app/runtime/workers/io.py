...
from app.integrations.kafka import IO_TOPIC, KafkaIncomingMessage, consume_topic
...
@track_worker_inflight(topic=TOPIC_NAME, capacity=IO_WORKER_MAX_IN_FLIGHT)
...
async def _build_production_bible_with_lease(
    ...
    heartbeat_task = asyncio.create_task(_maintain_production_bible_lease(factory, prepared, stop))
    try:
    ...
    while True:
        try:
            ...
            try:
                ...
                continue
            if not renewed:
                raise ProductionBibleLeaseLost("Production Bible lease was fenced") from None
            lease_expires_at = now + PRODUCTION_BIBLE_LEASE_DURATION
            next_attempt_seconds = PRODUCTION_BIBLE_LEASE_HEARTBEAT_SECONDS
...
async def _draft_storyboard_with_lease(
    ...
    heartbeat_task = asyncio.create_task(_maintain_storyboard_draft_lease(factory, prepared, stop))
    try:
    ...
    while True:
        try:
            ...
            try:
                ...
                continue
            if not renewed:
                raise StoryboardDraftLeaseLost("Storyboard draft lease was fenced") from None
            lease_expires_at = now + STORYBOARD_DRAFT_LEASE_DURATION
            next_attempt_seconds = STORYBOARD_DRAFT_LEASE_HEARTBEAT_SECONDS
...
async def run_io_worker(settings: Settings) -> None:
    ...
    initialize_worker_metrics(topic=TOPIC_NAME, capacity=IO_WORKER_MAX_IN_FLIGHT)
    ...
    episode_planner = CodexLocalEpisodePlanner(
        codex_cli_path=settings.codex_cli_path,
        codex_model=settings.codex_model,
        max_concurrency=settings.codex_max_concurrency,
    )
    adaptation_provider = CodexLocalScriptAdapter(
        codex_cli_path=settings.codex_cli_path,
        codex_model=settings.codex_model,
        max_concurrency=settings.codex_max_concurrency,
    )
    storyboard_drafter = CodexLocalStoryboardDrafter(
        codex_cli_path=settings.codex_cli_path,
        codex_model=settings.codex_model,
        max_concurrency=settings.codex_max_concurrency,
