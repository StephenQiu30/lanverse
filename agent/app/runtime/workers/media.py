...
from app.integrations.kafka import MEDIA_TOPIC, KafkaIncomingMessage, consume_topic
...
MEDIA_WORKER_MAX_IN_FLIGHT = 1
MAX_MESSAGE_BYTES = 64 * 1024
WorkerResult = Literal["completed", "duplicate", "rejected", "requeued"]
TOPIC_NAME = MEDIA_TOPIC
CONSUMER_GROUP = "lanverse.media-workers.v1"
...
@track_worker_inflight(topic=TOPIC_NAME, capacity=MEDIA_WORKER_MAX_IN_FLIGHT)
...
async def run_media_worker(settings: Settings) -> None:
    ...
    initialize_worker_metrics(topic=TOPIC_NAME, capacity=MEDIA_WORKER_MAX_IN_FLIGHT)
    ...
    probe = RoutingMediaProbe(
        media_probe=FfprobeMediaProbe(timeout_seconds=settings.media_probe_timeout_seconds),
