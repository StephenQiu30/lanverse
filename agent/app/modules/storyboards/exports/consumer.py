...
def _lineage(
    ...
    values.extend(
        ("narrative_unit_version", item.unit_version_id, item.text_hash) for item in snapshot.units
    )
    values.extend(
        ("asset_version", item.asset_version_id, item.content_hash) for item in snapshot.assets
    )
    values.extend(
