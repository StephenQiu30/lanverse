...
async def create_batch(
    ...
    *,
    ...
    async with session.begin():
        ...
        production_bible_context = await resolve_confirmed_script_production_bible_context(
            session,
            confirmed_script_version_id=request.input_script_version_id,
            episode_number=content.position,
        )
        production_bible_id = (
            production_bible_context.bible_id if production_bible_context is not None else None
        )
        production_bible_revision = (
            production_bible_context.revision if production_bible_context is not None else None
        )
        production_bible_result_hash = (
            production_bible_context.result_hash if production_bible_context is not None else None
        )
        production_bible_world: list[dict[str, object]] = (
            [
                {
        ...
        if production_bible_context is not None:
            asset_unit_version_ids = await resolve_episode_storyboard_asset_units(
                session,
                episode_id,
                narrative_unit_version_ids={unit.unit_version_id for unit in narrative.units},
            )
            derived_state_ids = list(asset_unit_version_ids)
            if request.asset_state_ids and set(request.asset_state_ids) != set(derived_state_ids):
                raise ApiError(
    ...
    *,
    ...
    if run_token is not None:
        active_other_run = (
            ...
            and (batch.agent_lease_expires_at is None or batch.agent_lease_expires_at > claim_time)
        )
        if active_other_run:
