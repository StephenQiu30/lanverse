...
async def _validate_target(
    ...
    *,
    ...
    for reference in target.narrative_references:
        ...
        if reference.segment_end is not None and reference.segment_end > len(unit.exact_text):
            raise ApiError(
                ErrorCode.VALIDATION_FAILED,
...
async def _store_narrative_references(
    ...
    *,
    ...
    units = {unit.unit_version_id: unit for unit in narrative.units}
    missing = [value.unit_version_id for value in inputs if value.unit_version_id not in units]
    if missing:
        raise ApiError(
            ErrorCode.VALIDATION_FAILED,
            "Narrative reference is not current for the target episode",
...
async def merge_shots(
    ...
    *,
    ...
    async with session.begin():
        ...
        for reference in source_narrative_references:
            narrative_by_spec[reference.shot_spec_version_id].append(_narrative_input(reference))
        try:
...
def _evaluate_loaded_readiness(
    ...
    *,
    ...
    if coverage.status == "unavailable":
        ...
        if (
            any(
                reference.id in stale_reference_ids and reference.shot_id == shot.id
