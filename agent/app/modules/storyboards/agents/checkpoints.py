...
class DatabaseStoryboardCheckpointStore:
    ...
    async def save(self, checkpoint: StoryboardCheckpoint) -> None:
        try:
            checkpoint = StoryboardCheckpoint.model_validate(checkpoint.model_dump(mode="json"))
        ...
        async with self._session_factory() as session, session.begin():
            ...
            if batch.agent_lease_expires_at is None or batch.agent_lease_expires_at <= now:
                raise StoryboardCheckpointMismatchError("checkpoint draft lease has expired")
