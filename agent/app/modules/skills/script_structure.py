...
def _deterministic_scene_candidates(
    ...
    *,
    ...
    blocks = [
        ...
        if block.kind == "scene_heading"
        and _SCENE_HEADING.match(script_body[block.start_codepoint : block.end_codepoint].strip())
    ]
    ...
    for index, block in enumerate(blocks, start=1):
        ...
        source_end = blocks[index].start_codepoint if index < len(blocks) else len(script_body)
        ...
        for scene_start, scene_end, scene_key in selected_scene_ranges:
            ...
            for state in states:
                ...
                digest = hashlib.sha256(
                    (f"{entity_key}:{state_key}:{scene_key}:{mention[0]}:{mention[1]}").encode()
                ).hexdigest()[:24]
    ...
    *,
    ...
    def referenced_scene_key(raw_scene_key: str, source_start: int) -> str | None:
        ...
        if mapped is not None:
            ...
            if mapped_range is not None and mapped_range[0] <= source_start < mapped_range[1]:
                return mapped
        return scene_key_for_offset(source_start) or mapped

    candidate_payloads: list[dict[str, object]] = [
        cast(dict[str, object], candidate.model_dump(mode="json")) for candidate in selected_scenes
    ]
