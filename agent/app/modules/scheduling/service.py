...
def _response(schedule: Schedule) -> ScheduleResponse:
    return ScheduleResponse(
        ...
        misfire_policy=cast(Literal["skip", "run_once", "catch_up"], schedule.misfire_policy),
    ...
    *,
    ...
    try:
        if request.kind == "interval":
            ...
            next_fire_at = effective_from + timedelta(seconds=request.interval_seconds)
        else:
    ...
    async with session.begin():
        schedule, actor = await _owned_schedule(session, claims, schedule_id, for_update=True)
            ...
            try:
                if schedule.kind == "one_off":
                    ...
                    if current <= resume_from:
                        ...
                        schedule.next_fire_at = current + timedelta(seconds=periods * seconds)
    ...
    if existing is not None:
        ...
        return (
            ScheduleFireResponse(
                ...
                trigger_kind=cast(Literal["scheduled", "manual"], existing.trigger_kind),
                task=task,
            ),
            False,
        )
...
async def dispatch_claimed_schedule(
    ...
    *,
    ...
    for scheduled_for in plan.fire_times:
        normalized = scheduled_for.astimezone(UTC).isoformat().replace("+00:00", "Z")
...
async def record_dispatch_failure(
    ...
    *,
    ...
    if unrecoverable or schedule.failure_count >= 5:
        ...
        metric_reason = None
        schedule.next_attempt_at = now + timedelta(seconds=min(2**schedule.failure_count, 300))
    schedule.revision += 1
    schedule.updated_at = now
    await session.flush()
