...
async def finalize_production_bible_success(
    ...
    if not await fence_bible_run(
        ...
        raise ProductionBibleLeaseLost("Production Bible result no longer owns an active lease")
    now = datetime.now(UTC)
    try:
...
async def finalize_production_bible_failure(
    ...
    if not await fence_bible_run(
        ...
        raise ProductionBibleLeaseLost("Production Bible failure no longer owns an active lease")
    now = datetime.now(UTC)
