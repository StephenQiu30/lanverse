...
async def resolve_storyboard_planning_assets(
    ...
    *,
    ...
    if len(by_state) != len(unique_ids):
        return ()
    version_ids = [cast(UUID, by_state[state_id][0].current_version_id) for state_id in unique_ids]
    readiness = await resolve_asset_versions_readiness(
        session,
        workspace_id,
        project_id,
    ...
    *,
    ...
    for row in current:
        if row.narrative_unit_version_id not in narrative_unit_version_ids:
            continue
        units_by_state.setdefault(row.asset_state_id, []).append(row.narrative_unit_version_id)
    return {
