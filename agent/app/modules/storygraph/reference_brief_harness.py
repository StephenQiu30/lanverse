...
class ReferenceBriefHarness:
    """Execute the single-call Reference Brief candidate boundary."""
    ...
    def __init__(
        ...
        *,
        ...
        if (
            ...
            raise InvocationPolicyInvalid("Reference Brief execution budget is outside the release")
    ...
    async def execute(self) -> ReferenceBriefCandidate:
        prompt = json.dumps(
        ...
        candidate = await run_codex_process(
            ...
            guidance=self.bundle.guidance("compile_reference_brief", "default"),
            prompt=prompt,
            output_model=ReferenceBriefCandidate,
            timeout_seconds=self._max_execution_seconds,
            max_output_bytes=self._max_output_bytes,
            strict_output_schema=True,
        )
        if not isinstance(candidate, ReferenceBriefCandidate):
            raise CodexSchemaInvalid("Codex CLI returned the wrong Reference Brief schema")
        try:
            candidate.validate_for(self.stage_input)
        except ValueError as error:
            raise CodexSchemaInvalid("Reference Brief Candidate changed frozen input") from error
        return candidate
