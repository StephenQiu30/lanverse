...
async def resolve_subject(
    ...
    try:
        if subject.subject_type is SubjectType.MEDIA_VERSION:
            ...
            exists = await media_version_exists(session, workspace_id, subject.subject_id)
        elif subject.subject_type is SubjectType.SCRIPT_VERSION:
            from app.modules.scripts import script_version_exists

            exists = await script_version_exists(session, workspace_id, subject.subject_id)
            ...
            exists = await asset_version_exists(session, workspace_id, subject.subject_id)
        else:
