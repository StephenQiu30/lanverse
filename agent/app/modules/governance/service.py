...
async def check_rights(
    ...
    *,
    ...
    for proof in proofs:
        proof_ids.setdefault(proof.consent_revision_id, []).append(proof.media_version_id)
    ...
    for consent, revision in rows:
        ...
        try:
            proof_accessible = bool(revision_proofs) and all(
                [
                    await media_version_accessible(session, workspace_id, media_version_id)
        ...
        grants.append(
            ConsentGrant(
                ...
                revoked_at=(revision.created_at if revision.action == "revoke" else None),
            )
        )
...
async def check_rights_for_resolved_subjects(
    ...
    *,
    ...
    rows = await repository.list_current_consents_with_proofs_for_subjects(
        session,
        workspace_id,
        [(subject.subject_type.value, subject.subject_id) for subject in unique_subjects],
    )
    grouped: dict[
        tuple[str, UUID],
