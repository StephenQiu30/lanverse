...
async def _owned_consent(
    ...
    *,
    ...
    consent = await repository.find_consent(session, consent_id, for_update=for_update)
    if consent is None:
        raise _not_found()
    try:
        actor = await actor_context(session, claims, consent.workspace_id, capability)
...
def _proof_map(proofs: Sequence[ConsentProof]) -> dict[UUID, list[UUID]]:
    ...
    for proof in proofs:
        result.setdefault(proof.consent_revision_id, []).append(proof.media_version_id)
    return result
...
def _detail_response(
    ...
    summary = _summary_response(consent, current, proof_ids.get(current.id, []))
    return ConsentDetailResponse(
        **summary.model_dump(),
        revisions=[
            _revision_response(revision, proof_ids.get(revision.id, [])) for revision in revisions
        ],
    )
...
async def _load_detail(session: AsyncSession, consent: Consent) -> ConsentDetailResponse:
    revisions = await repository.list_revisions(session, consent.id)
    proofs = await repository.list_proofs(session, [revision.id for revision in revisions])
...
async def _same_registration(
    ...
    return (
        consent.subject_identity == request.subject_identity.model_dump(mode="json")
        and initial.scope == request.scope.model_dump(mode="json")
        and [proof.media_version_id for proof in proofs] == request.proof_media_version_ids
    ...
    async with session.begin():
        ...
        if existing is not None:
            ...
            inserted_id = await session.scalar(
                ...
                .on_conflict_do_nothing(constraint="uq_gov_consent_workspace_idempotency")
...
async def list_consents(
    ...
    *,
    ...
    for consent in consents:
        ...
        items.append(_summary_response(consent, revision, proofs.get(revision.id, [])))
...
async def get_consent(
    ...
    consent, _ = await _owned_consent(session, claims, consent_id, Capability.CONTENT_READ)
    return await _load_detail(session, consent)
...
async def revoke_consent(
    ...
    *,
    ...
    async with session.begin():
        ...
        current = await repository.find_revision(session, consent.current_revision_id)
        if current is None:
            raise ApiError(
                ErrorCode.INTERNAL_ERROR,
                "Consent current revision is unavailable",
