...
async def confirm_structure(
    ...
    async with session.begin():
        ...
        for position, candidate in enumerate(accepted_scene_candidates, start=1):
            ...
            scene = Scene(
                ...
                semantic_context={
                    ...
                    "production_tasks": [
                        task.model_dump(mode="json") for task in proposal.production_tasks
                    ],
                },
        ...
        if batch.production_bible_id is not None:
            ...
            state_by_key = {(state.entity_id, state.state_key): state for state in bible_states}
            ...
            for candidate in candidates:
                ...
                state = (
                    None if entity is None else state_by_key.get((entity.id, proposal.state_key))
                )
