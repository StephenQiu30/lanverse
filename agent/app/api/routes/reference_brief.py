"""Reference Brief Harness invocation route."""
...
from app.harness.grants import (
    ...
    verify_reference_brief_dispatch_authorization,
)
...
router = APIRouter(tags=["reference-brief"])
Service = Annotated[HarnessService, Depends(get_harness_service)]


@router.post(
    "/internal/storygraph/reference-brief/invocations",
    response_model=ReferenceBriefAttemptResult,
)
async def invoke_reference_brief(
    ...
    try:
        authorization = verify_reference_brief_dispatch_authorization(
    ...
    try:
        value, model = await service.reference_brief(invocation)
        candidate = value.model_dump(mode="json")
        result = ReferenceBriefAttemptResult.build(
