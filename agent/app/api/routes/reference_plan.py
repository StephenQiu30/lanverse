"""Reference Plan Harness invocation route."""
...
from app.harness.grants import (
    ...
    verify_reference_plan_dispatch_authorization,
)
...
router = APIRouter(tags=["reference-plan"])
Service = Annotated[HarnessService, Depends(get_harness_service)]


@router.post(
    "/internal/storygraph/reference-plan/invocations",
    response_model=ReferencePlanAttemptResult,
)
async def invoke_reference_plan(
    ...
    try:
        authorization = verify_reference_plan_dispatch_authorization(
    ...
    try:
        value, model = await service.reference_plan(invocation)
