...
def create_app(
    ...
    async def ready() -> dict[str, str]:
        ...
        if runtime_ready is not None and not runtime_ready():
            raise HTTPException(503, "agent_runtime_unavailable")
...
def create_configured_app(runtime_ready: Callable[[], bool] | None = None) -> FastAPI:
    settings = Settings.from_environment()
    repository = Repository(settings.database_url)
    app = create_app(repository, settings.secret, settings.task_queue, runtime_ready=runtime_ready)
...
def create_agent_app() -> FastAPI:
    """Compatibility alias for the canonical FastAPI application factory."""

    from app.api.application import create_agent_app as factory
