    ...
    def start(self, request: RunRequest) -> AgentRun:
        ...
        with self._lock:
            ...
            run = AgentRun(
                ...
                items=[
                    ProposalItem(
                        item_id=f"{request.stage}:review",
                        kind=f"{request.stage}_proposal",
                        value={"snapshot_ref": request.snapshot_ref, "requires_user_decision": True},
                        evidence=[{"line": 1, "start_offset": 0, "end_offset": 0}],
                    )
                ],
