...
def _valid_spec_payload() -> dict[str, object]:
    return {
        ...
        "visual": {
            ...
            "subject_placements": [{"subject_key": "hero", "placement": "画面左侧，面向站台尽头"}],
...
def _reference_unknown_dialogue(payload: dict[str, object]) -> None:
    cast(list[dict[str, object]], payload["dialogue_or_narration"])[0]["source_dialogue_id"] = (
...
def test_storyboard_hashes_are_canonical_and_fix_script_inputs() -> None:
    ...
    changed = storyboard_content_hashes(ShotSpec.model_validate(changed_payload), references)
