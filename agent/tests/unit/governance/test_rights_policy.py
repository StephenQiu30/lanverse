...
FIXTURE_FILE = Path(__file__).resolve().parents[2] / "fixtures/governance/consent_gate_cases.json"
...
def _grant(case: dict[str, object]) -> ConsentGrant:
    ...
    return ConsentGrant(
        ...
        status=(ConsentStatus.REVOKED if revoked_at is not None else ConsentStatus.ACTIVE),
        subject_identity_kind=SubjectIdentityKind(str(case["subject_kind"])),
        purposes=frozenset(
            str(value) for value in cast(list[object], typed_scope["authorized_purposes"])
        ),
        channels=frozenset(str(value) for value in cast(list[object], typed_scope["channels"])),
        regions=frozenset(str(value) for value in cast(list[object], typed_scope["regions"])),
...
def test_accepted_consent_fixtures_drive_the_rights_decision(
    ...
    assert result.consent_ids == ((_grant(case).consent_id,) if result.allowed else ())
...
def test_rights_policy_rejects_channel_region_and_inaccessible_proof() -> None:
    ...
    assert [blocker.code for blocker in evaluate_grants([inaccessible], baseline).blockers] == [
        "proof_unavailable"
    ]
