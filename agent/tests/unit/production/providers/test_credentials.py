...
def test_credential_cipher_factory_requires_two_independent_32_byte_keys() -> None:
    ...
    configured = Settings(
        ...
        provider_credential_master_key=SecretStr(urlsafe_b64encode(b"m" * 32).decode("ascii")),
        provider_credential_fingerprint_key=SecretStr(urlsafe_b64encode(b"f" * 32).decode("ascii")),
    )
