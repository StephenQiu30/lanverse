...
def test_golden_script_is_utf8_without_bom_and_has_normalized_newlines() -> None:
    fixture = load_golden_candidate()
    raw_bytes = fixture.full_script.encode("utf-8")

    assert not raw_bytes.startswith(b"\xef\xbb\xbf")
    raw_text = raw_bytes.decode("utf-8")
    assert "\r" not in raw_text
    assert not raw_text.endswith("\n\n")
    assert raw_text == fixture.full_script
...
def test_five_episode_boundaries_are_exact_contiguous_codepoint_slices() -> None:
    ...
    assert fixture.full_script == "\n\n".join(episode.source_text for episode in fixture.episodes)
    ...
    for episode, marker in zip(fixture.episodes, expected_markers, strict=True):
        ...
        assert (
            fixture.full_script[episode.source_start_codepoint : episode.source_end_codepoint]
            == episode.source_text
        )
...
def test_selected_episode_narrative_units_preserve_exact_source_ranges() -> None:
    ...
    for unit in fixture.narrative_units:
        ...
        assert (
            selected.source_text[unit.source_start_codepoint : unit.source_end_codepoint]
            == unit.exact_text
        )
...
def test_required_units_are_covered_and_optional_unit_has_an_approved_omission() -> None:
    fixture = load_golden_candidate()
    oracle = fixture.storyboard_oracle
    covered = {unit_id for shot in oracle.shots for unit_id in shot.narrative_unit_ids}
    required = {
        unit.narrative_unit_id for unit in fixture.narrative_units if unit.required_for_coverage
    }
    optional = {
        unit.narrative_unit_id for unit in fixture.narrative_units if not unit.required_for_coverage
    }
...
def test_shot_and_narrative_mapping_is_explicitly_many_to_many() -> None:
    ...
    assert all("nu-03-002" in by_id[shot_id].narrative_unit_ids for shot_id in one_to_many.shot_ids)
    ...
    assert set(many_to_one.narrative_unit_ids) <= set(by_id["shot-03-003"].narrative_unit_ids)
...
def test_every_prompt_token_has_one_ordered_fixed_asset_version_reference() -> None:
    fixture = load_golden_candidate()
    states = {state.state_id: state for asset in fixture.assets for state in asset.states}

    for shot in fixture.storyboard_oracle.shots:
        assert [ref.index for ref in shot.asset_refs] == list(range(1, len(shot.asset_refs) + 1))
