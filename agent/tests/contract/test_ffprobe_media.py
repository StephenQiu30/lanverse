...
async def test_ffprobe_inspects_real_image_bytes(
    ...
    assert REGISTRY.get_sample_value("lanverse_media_probe_results_total", labels) == (before + 1)
...
async def test_ffprobe_tool_unavailable_is_observable_without_leaking_the_path(
    ...
    assert REGISTRY.get_sample_value("lanverse_media_probe_results_total", labels) == (before + 1)
    span = next(span for span in exporter.get_finished_spans() if span.name == "media.ffprobe")
