...
async def test_private_minio_supports_the_complete_eight_operation_contract(
    ...
    put_before = REGISTRY.get_sample_value("lanverse_storage_operations_total", put_labels) or 0
...
async def test_partially_consumed_minio_stream_does_not_report_successful_bytes() -> None:
    ...
    try:
        ...
        bytes_before = REGISTRY.get_sample_value("lanverse_storage_bytes_total", byte_labels) or 0
        succeeded_before = (
            REGISTRY.get_sample_value("lanverse_storage_operations_total", succeeded_labels) or 0
        )
        cancelled_before = (
            REGISTRY.get_sample_value("lanverse_storage_operations_total", cancelled_labels) or 0
        )
        ...
        assert (
            REGISTRY.get_sample_value("lanverse_storage_bytes_total", byte_labels) == bytes_before
        )
        assert (
            REGISTRY.get_sample_value("lanverse_storage_operations_total", succeeded_labels)
            == succeeded_before
        )
        assert (
            REGISTRY.get_sample_value("lanverse_storage_operations_total", cancelled_labels)
            == cancelled_before + 1
        )
    finally:
        await storage.delete(key)
