...
def stored_objects(monkeypatch: pytest.MonkeyPatch) -> StoredObjects:
    ...
    async def stat_object(_: MinioObjectStorage, object_key: str) -> StorageObjectMetadata:
    ...
    def stream_object(_: MinioObjectStorage, object_key: str) -> AsyncIterator[bytes]:
...
async def test_owner_submits_idempotent_location_migration_and_reads_safe_status(
    ...
    headers, workspace_id = await _identity(client, email="media-location-owner@example.com")
    initialized = await _initialize_and_upload(
        client,
        headers,
        workspace_id,
    ...
    locations = await client.get(f"/api/v1/media/{version_id}/locations", headers=headers)
    ...
    other_headers, _ = await _identity(client, email="media-location-other@example.com")
    assert (
        await client.get(f"/api/v1/media/{version_id}/locations", headers=other_headers)
    ).status_code == 404

    async with session_factory() as session:
        migration_tasks = list(
            await session.scalars(select(Task).where(Task.task_type == "media_location_migration"))
        )
        assert len(migration_tasks) == 1
        event = await session.scalar(
            select(OutboxEvent).where(OutboxEvent.aggregate_id == migration_tasks[0].id)
        )
        assert event is not None
...
async def test_upload_rejects_invalid_declarations_and_never_creates_false_versions(
    ...
    initialized = await client.post("/api/v1/media/uploads", headers=headers, json=request)
