...
async def test_text_import_is_exact_idempotent_refreshable_and_writes_no_episode(
    ...
    headers, _, project = await _project(client, email="whole-script-owner@example.com")
    ...
    assert revision["raw_hash"] == hashlib.sha256(str(payload["text"]).encode()).hexdigest()
    ...
    refreshed = await client.get(f"/api/v1/document-revisions/{revision['id']}", headers=headers)
    assert refreshed.status_code == 200
    assert refreshed.json()["data"] == result
    listed = await client.get(f"/api/v1/projects/{project['id']}/script-documents", headers=headers)
...
async def test_format_problems_are_persisted_with_next_actions_before_materialization(
    ...
    headers, _, project = await _project(client, email="whole-script-format@example.com")
    endpoint = f"/api/v1/projects/{project['id']}/script-imports"
    ...
    assert [item["code"] for item in rejected.json()["data"]["issues"]] == ["number_gap"]
    assert rejected.json()["data"]["issues"][0]["next_action"] == ("renumber_episode_markers")
    assert candidate.status_code == 201
    assert candidate.json()["data"]["revision"]["analysis_status"] == ("ai_candidate_required")
    ...
    assert too_long.json()["data"]["revision"]["analysis_status"] == ("ai_candidate_required")
...
async def test_document_import_is_concurrency_safe_and_cross_workspace_hidden(
    ...
    headers, _, project = await _project(client, email="whole-script-concurrent@example.com")
    endpoint = f"/api/v1/projects/{project['id']}/script-imports"
    ...
    stranger_headers, _, _ = await _project(client, email="whole-script-stranger@example.com")
    hidden = await client.get(f"/api/v1/document-revisions/{revision_id}", headers=stranger_headers)
...
async def test_ready_markdown_is_previewed_before_importing_fixed_version(
    ...
    headers, workspace_id, project = await _project(client, email="whole-script-media@example.com")
    body = "第一集\n上传正文。\n\n第二集\n继续。".encode()
