...
async def _completed_batch(
    ...
    *,
    ...
    result = extraction_schemas.ScriptExtractionResult.model_validate({"candidates": candidates})
...
async def test_confirm_structure_lists_unresolved_required_and_blocking_candidates(
    ...
    assert [
        item["candidate_key"] for item in error["details"]["blocking_continuity_candidates"]
    ] == ["continuity-001"]
...
async def test_confirm_structure_is_concurrent_idempotent_ordered_and_private(
    ...
    batch_response = await client.get(f"/api/v1/extraction-batches/{batch_id}", headers=headers)
    assert batch_response.status_code == 200
    assert batch_response.json()["data"]["confirmed_script_version_id"] == confirmed_version["id"]
    ...
    assert [
        blocker["code"] for blocker in confirmed_delete.json()["error"]["details"]["blockers"]
    ...
    stranger_headers = {"authorization": f"Bearer {stranger.json()['data']['access_token']}"}
...
async def test_candidate_link_and_asset_delete_are_serialized(
    ...
    if linked.status_code == 201:
        ...
        assert deleted.json()["error"]["message"] == ("Asset has candidate decision references")
    else:
