...
from tests.contract.test_reference_brief_contract import reference_brief_candidate
from tests.contract.test_reference_brief_wire_contract import valid_invocation
...
def token(request: ReferenceBriefInvocation, *, attempt_id: str | None = None) -> str:
    claims = {
...
@pytest.mark.asyncio
async def test_reference_brief_api_returns_a_validated_candidate(
    client: httpx.AsyncClient,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setenv("AGENT_EXECUTION_SECRET", SECRET)
    request = valid_invocation()
    candidate = ReferenceBriefCandidate.model_validate(
        reference_brief_candidate("character_appearance")
    )

    async def execute(_: ReferenceBriefHarness) -> ReferenceBriefCandidate:
        return candidate

    monkeypatch.setattr(ReferenceBriefHarness, "execute", execute)
    authorization = token(request)
    response = await client.post(
        "/internal/storygraph/reference-brief/invocations",
        json=request.model_dump(mode="json"),
        headers={"X-Lanverse-Dispatch-Authorization": authorization},
    )

    assert response.status_code == 200
    result = ReferenceBriefAttemptResult.model_validate(response.json())
    assert result.status == "accepted"
    assert result.candidate_type == "reference_brief_candidate"
    result.validate_for(
        request,
        claim_version=1,
...
@pytest.mark.asyncio
async def test_reference_brief_api_preserves_runtime_uncertainty(
    client: httpx.AsyncClient,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setenv("AGENT_EXECUTION_SECRET", SECRET)
