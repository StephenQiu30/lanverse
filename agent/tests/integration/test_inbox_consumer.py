...
async def test_duplicate_delivery_has_one_business_effect(
    ...
    async with session_factory() as session:
        ...
        assert "AI extraction service is not configured" not in str(audit_events[-1].event_metadata)
...
async def test_database_rollback_leaves_message_safe_to_redeliver(
    ...
    async with session_factory() as session:
        ...
        audit_count = await session.scalar(
            select(func.count()).select_from(AuditEvent).where(AuditEvent.target_id == task_id)
        )
        assert task is not None
...
async def test_late_event_for_terminal_task_is_acknowledged_without_reexecution(
    ...
    async with session_factory() as session:
        task = await session.get(Task, task_id)
        deliveries = list(
            await session.scalars(select(InboxDelivery).order_by(InboxDelivery.received_at))
        )
        assert task is not None
        assert task.status == "failed"
        assert task.revision == 2
