...
async def test_export_preflight_blocks_uncovered_and_disabled_asset(
    ...
    blocked_headers, blocked_episode, _blocked_refs = await create_episode_with_confirmed_structure(
        client,
        session_factory,
    ...
    assert "COVERAGE_UNACCOUNTED" in {blocker["code"] for blocker in uncovered_data["blockers"]}
    ...
    assert "ASSET_DISABLED" in {blocker["code"] for blocker in disabled_data["blockers"]}
