...
Preserve each mention's `occurrence_role` exactly. Identity reconciliation may decide who a mention refers to, but it cannot turn `mentioned_only` into `actual` or infer physical presence from an alias, dialogue, memory, photograph, or other indirect reference. Scene locations always use `actual`.
