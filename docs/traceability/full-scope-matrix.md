...
| --- | --- | --- | --- | --- |
| 平台基线 | REQ-000、DES-000—005 | PRD-000 | PLAN-000 | 进行中：Go/Agent/frontend 固定，GORM、统一 HTTP envelope/状态码、基础 Schema、单 Kafka、MinIO、OpenAPI、HTTP 到事务的 workspace context、直接租户表 RLS 与策略启动校验已落地；Redis 业务接入、Elastic 投影、OTel、严格 Swagger 仍待实现 |
| A 手工事实主线 | M01—M05、M07—M09、M11、M14 | PRD-A | PLAN-A | 进行中：剧本解析草稿/批准、canonical GORM 物化、人物×集数与资产投影已可用并完成真实浏览器闭环；会话/路径/直接表租户边界基础已落地，跨租户/Worker/媒体对象负向验收、导入格式扩展、检索、完整镜头、交付预演待实现 |
