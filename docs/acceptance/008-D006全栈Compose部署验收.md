...
本地开发仍可直接运行 venv/npm 命令，只在需要 MinIO 时执行 `make minio-up`；该命令同时验证 Lanverse Compose 容器归属与端口健康。其他 project 的健康 MinIO 占用同端口时默认仍拒绝；只有用户明确授权并执行 `MINIO_REUSE_EXTERNAL=1 make minio-up` 才复用，凭据仅由调用进程提供，Makefile 不读取或持久化外部 secret；显式复用端点不健康时直接失败，不启动第二实例。完整开发环境通过 `make docker-dev-up` 启动。生产配置由 `.env.production.example` 描述，真实 `.env.production` 不提交，必需 secret 为空时快速失败；`make docker-prod-up` 固定执行 `--no-build --pull always --wait`。生产文件是基础 Compose 的薄覆盖层，不维护第二套完整服务定义或 `CONTAINER_*` 地址。
...
| MinIO Compose 归属与显式复用 | Red 架构测试先固定默认归属拒绝、显式 opt-in 和不健康快速失败；Green 以 `docker compose ps --status running -q minio` 区分当前 project。实测默认对 `baozi-minio` 退 端口退出 2，`MINIO_REUSE_EXTERNAL=1 make minio-up` 退出 0；只启动该已有容器，没有启动 Lanverse 的第二实例 |
| 复用 MinIO 真实契约 | MinIO 真实契约 2/2 通过；在独立 `lanverse-e2e` bucket 完成 PUT/stat/GET/presign/delete 和媒体 API 私有直传，凭据只进入测试子进程，未写入仓库或日志。本地 Lanverse API 以同样的进程级配置重启后，`127.0.0.1:8000/readyz` 返回 `ready`，PostgreSQL、Redis、RabbitMQ 与 MinIO 全部 `available` |
...
- 当前 9000 端口仍由已授权复用的 `baozi-minio` 发布，Lanverse Compose MinIO 保持停止；默认仍拒绝误用，显式复用命令、MinIO 2/2 和当前浏览器链已通过。该决定只适用本机开发/验收，不改变 production 必须显式配置存储凭据与网络边界的要求。
