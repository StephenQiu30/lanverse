...
2. 全新 PostgreSQL 16.15 与固定摘要 Temporal 服务同时运行 `go vet ./...`、`go test -p 1 ./...`：通过；`backend/tests/workflow` 包含真实 GORM Signal 事实旅程、真实 Temporal Start/Signal 与 Replay，最终用时 10.714 秒。
...
- Apply Human Gate Activity 的真实投影终态、拒绝/修改请求后的 Run 状态和崩溃重放仍需随 Worker 装配实现。
