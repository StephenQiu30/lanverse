...
- 执行人 / 复核人：Codex / 待复核
- Git commit：RED `3bd233b`，Worker 租户绑定 `2b47a29`，实现 `bc12b7d`，失败回执测试 `cdb19f0`
...
# 重启 API/operation-worker，刷新会话并重新查询 approved analysis
...
7. API 与 operation-worker 重启后，HttpOnly refresh 返回 200；approved analysis 仍返回 HTTP 200、3 集、TXT ParseReport 和 0 failed scopes，证明业务结果不依赖进程内状态。
...
- 页面刷新后可以恢复登录会话，且已批准事实可通过当前 API 读取，但前端尚未恢复最近 Project 上下文，因此刷新后的工作台不会自动重开已有项目。这是独立的项目导航/恢复缺口，不影响本文精确存储断言，必须在 A9/A10 前修复。
