...
- 状态：进行中；已完成含世界改编守恒边界的 Backend `PresetRelease`、4 个首批策展 Preset 目录、不可变 `ProjectPresetSelection` 合同、GORM Current Head 与认证读写 API、整体 Preset 切换确定性影响闭包、仓库 Skill Bundle 内容身份、八个生产 Input/Candidate Schema、包含视觉调用/结果的统一 Wire Schema、八变体 DefinitionCore/StageRelease、`resolve_visual_foundation` 跨语言严格合同、Agent vision Harness/私有媒体调用入口、Backend 媒体 Broker/HTTP Client、当前 production StoryGraph typed world read-set、Gate 2 exact source、faithful Input compiler、视觉执行生命周期及 GORM 生产持久化、Workflow/Temporal 生产节点，以及三个已采用外部参考的审计与隔离链。Visual Foundation 现在由 Gate 2 后的精确 StoryGraph 和不可变 Selection ID/revision/content hash 共同驱动，Worker 复用现有 Agent Client、对象存储 Broker、GORM Store 与持久 Release/Manifest/Invocation/Attempt/Authorization/Result/Candidate/Head；Activity 重试不重新绑定变化后的 Current Selection，Candidate 接收前仍执行 SERIALIZABLE 当前输入重编译校验。十三 Stage Core/StageSet、完整 SkillRelease/Control、rewrite eval/shadow/signature、真实下游 stale 写入、Reference Plan 与 Gate 3 仍未实现。
