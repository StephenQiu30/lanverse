...
- 状态：进行中；已完成 Backend `PresetRelease`、仓库 Skill Bundle 内容身份、七个现有生产 Input/Candidate Schema、SceneAnalysis Wire Schema 内容身份及七变体 P0 DefinitionCore，不表示十三 Stage Core/StageSet、Preset Inventory、Stage/Skill Release、视觉执行或 Gate 3 已实现。
- Git 基线/提交：本 DefinitionCore 切片基线 `7abe69650c10f97ca1049831f086c32a65a3f467`；本切片随本记录所在提交交付。
- Red 命令与失败：`go test ./tests/preset -count=1` 首先因 `backend/internal/preset/domain` 不存在而编译失败；Red 固定缺 NOTICE、六类能力或 Purpose Profile、错误视图角色、非内容定址引用、数字发布序号、非法日历日期、未知字段与 Hash 漂移必须拒绝。Bundle、Input/Candidate/Wire Schema 子项分别先因对应共享 Manifest/Registry/Decoder 不存在而编译失败。DefinitionCore 子项的 `go test ./tests/agent -run '^TestSceneAnalysisDefinitionCore' -count=1` 因 Builder、严格 Decoder 与合同 ID 不存在而编译失败，并固定缺失变体、未知 Release 字段、current 资源引用和 Core Hash 漂移必须拒绝。
- Green/定向验证：Preset 合同通过 `go test ./tests/preset -count=1`、`go vet ./internal/preset/... ./tests/preset`、`go test -race ./tests/preset -count=1` 与架构门。Bundle/Input/Candidate/Wire Schema/DefinitionCore 合同通过 `go test ./tests/agent -count=1`、`go vet ./internal/agent/... ./tests/agent` 及定向 Race Detector；Agent Ruff、Pyright 与 13 条 SceneAnalysis Schema/Wire/Contract/Harness 测试通过。`skill-creator` 的 `quick_validate.py` 使用仓库既有 Agent 虚拟环境返回 `Skill is valid!`。Go/Python 对同一 `build-storygraph` 资源集均重算 Bundle Hash `5b49be2fe6f0af9131f8fc02b267314c15968e9f8646456f1b75fcbfa86c16ef`，实际 Pydantic Output/Input/Wire Schema 根分别为 `305b7449a01557334b04fe67db545f41491c60b0747d0edc3c76f2d0f6cfacee`、`5f81015db1172d883a858f1aab0b948d3d3b93a84d55cc15ac6eec66310b3a57` 与 `f8da8db4f7b6f6c7e6ac09354b86be0c570fda9883ccbce2aa99ebcab987ddcf`；DefinitionCore 从这些前像和 Backend typed policy/invariant artifact 重算唯一 Core Hash。
- 全量 CI：Backend 通过 `test -z "$(gofmt -l .)"`、`go vet ./...` 与无外部环境变量的 `go test -count=1 ./...`；固定 `lanverse_test` 与本机 Homebrew Temporal `127.0.0.1:7233` 的有界恢复旅程真实通过（69.337 秒），未启动或重启环境。Frontend OpenAPI 无漂移、ESLint、TypeScript、25 files/73 tests 和生产构建通过。Agent Ruff lint/format、Pyright 与 Pytest `207 passed, 60 skipped` 通过；跳过项均明确要求隔离 PostgreSQL、本机 Temporal 或真实 Codex CLI，未冒充已执行。
- 真实输入/产物/事实对账：本切片只建立无数据库依赖的不可变领域值合同。Bundle Manifest 精确覆盖 `agent/skills/build-storygraph` 的 `NOTICE.md`、入口和全部 16 个 Reference，以每个文件的相对 POSIX 路径、字节长度与 SHA-256 形成来源/隔离/Bundle 三层根；旧 StoryGraph 和生产 SceneAnalysis Runtime 只接受同一 Bundle Content Hash。Input/Candidate Schema Manifest 直接从七个实际 Pydantic Stage Input/Model 的 JSON Schema 计算逐项 Hash，并与 Backend 已运行的 Stage/Profile/Schema Version 合同逐项对账；Wire Schema Manifest 同样直接覆盖实际 Invocation、Authorization Claims 与 Attempt Result。DefinitionCore 只引用发布前可独立存在的这些 Schema、严格 Candidate Validation、资源、执行、模型和空工具 Policy artifact，不包含 Bundle/Release/Signature/Control/运行激活引用。未创建表、migration、替代 SQL 事实源或兼容入口，也未启动/重启任何本机环境服务。
- 未覆盖条件与残余风险：项目自有 Bundle、七个现有 Input/Candidate/Wire Schema 与 P0 DefinitionCore 的真实内容身份已具备，但其余六个生产 Stage 的严格 Input/Candidate Schema、十三 Stage 完整 Core、外部 SourceInventory/review/rewrite、StageRelease/CandidateStageSet、Eval/Shadow、签名与 Control 尚未实现；4–6 个 Release Inventory、持久 Owner/API、Visual Foundation、Reference Plan、expected target set 与 Gate 3 也仍未完成，因此 `VPR-PRE-001–009` 和 `VPA-SUP/REL/VIS` 均保持未验收，当前 P0 Core 不能冒充正式 Release 或批准证据。
