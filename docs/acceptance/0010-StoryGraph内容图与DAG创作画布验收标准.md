...
- 未覆盖条件与残余风险：PresetRelease 守恒合同、4 个内置策展 Release、PresetSwitchImpact 确定性合同、不可变 Project Preset Selection + GORM Current Head + 认证 API、Selection-backed Visual Foundation 输入编译与接收前 stale 校验、Visual Foundation 严格合同、Agent vision Harness/私有媒体调用、Backend 媒体 Broker/Client、八变体 Registry/DefinitionCore/StageRelease、持久执行生命周期、Candidate 结果持久化、Workflow/Temporal 生产节点、Backend expected Reference Target set 及一次真实本机 Codex 图片 canary 已具备；真实下游 stale 写入、style-blind mutation、带真实图片附件的 MinIO→Backend→Agent 系统旅程、`plan_reference_assets` Candidate/节点、Reference Plan 原子发布与 Gate 3 尚未实现，因此 `VPR-PRE-005` 仍保持未勾选，且仍只验收 `VPA-VIS-002`，`VPA-VIS-001`、`VPR-PRE-009` 保持未勾选。八个 StageRelease、三个已采用参考的 SourceInventory、隔离安全审查、当前处置映射、七能力 Matrix 与运行时外部来源隔离已具备；但没有来源获准进入 rewrite，故当前 Matrix 合法为空，Golden/Adversarial Eval、Shadow 和独立签名没有合格输入且尚未形成完整吸收链，`VPA-SUP-005` 保持未勾选。其余五个未实现生产 Stage 的严格 Input/Candidate Schema、十三 Stage 完整 Core/CandidateStageSet、正式 SkillRelease 与追加式 Control 同样未实现。当前服务使用的最小 Skill/Control 身份尚未达到完整发布批准合同，不能冒充设计中的批准链；最终 `agent-browser` 仍只在全部开发完成后执行。
