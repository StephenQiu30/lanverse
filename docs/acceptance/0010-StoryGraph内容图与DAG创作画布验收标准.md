...
- 状态：进行中；已完成含世界改编守恒边界的 Backend `PresetRelease`、4 个首批策展 Preset 目录、整体 Preset 切换确定性影响闭包、仓库 Skill Bundle 内容身份、八个生产 Input/Candidate Schema、包含视觉调用/结果的统一 Wire Schema、八变体 DefinitionCore/StageRelease、`resolve_visual_foundation` 跨语言严格 Input/Candidate/Invocation/Authorization/AttemptResult 合同、Agent vision Harness/私有媒体调用入口与 Backend 媒体 Broker/HTTP Client，以及三个已采用外部参考的 SourceInventory、固定隔离审查与处置映射。七个文本 StageRelease 已由现有 SceneAnalysis 服务真实消费和持久化；视觉 Stage 已进入 Backend Registry、DefinitionCore 与 StageRelease 内容身份，Backend 可按冻结对象声明读取并传输，Agent 可验签、严格校验图片并返回 AttemptResult。不表示视觉 Stage 已接入持久调度服务或 Workflow，也不表示可由产品触发、十三 Stage Core/StageSet、完整 SkillRelease/Control、能力 rewrite/eval/shadow、项目 Preset 选择/真实 stale 写入或 Gate 3 已实现。
