...
- 状态：进行中；已完成 Backend `PresetRelease`、仓库 Skill Bundle 内容身份、七个现有生产 Input/Candidate Schema、SceneAnalysis Wire Schema、七变体 P0 DefinitionCore、对应正式 StageRelease 内容身份，以及三个已采用外部参考的 SourceInventory；StageRelease 已由现有 SceneAnalysis 服务真实消费和持久化。不表示十三 Stage Core/StageSet、完整 SkillRelease/Control、完整吸收评测、Preset Inventory、视觉执行或 Gate 3 已实现。
- Git 基线/提交：本 SourceInventory 切片基线 `ad231a075ee4bdd1cd730d1be0ffe17b840a5b59`；StageRelease 实现提交 `cb96c2a7b51ac65b876bbc545a7b9c94793693b7`，SourceInventory 实现提交 `5a58ccde8c1ab881d6b5eeca667c1a8449789dec`。
...
- Green/定向验证：Preset 合同通过 `go test ./tests/preset -count=1`、`go vet ./internal/preset/... ./tests/preset`、`go test -race ./tests/preset -count=1` 与架构门。Bundle/Input/Candidate/Wire Schema/DefinitionCore/StageRelease/SourceInventory 合同通过 `go test ./tests/agent -count=1`、`go vet ./internal/agent/... ./tests/agent`；SourceInventory 定向普通测试 0.726 秒、Race Detector 1.748 秒。三个来源的一手固定 GitHub 文件与许可证分别重算出 `7166/11344`、`33168/11345`、`11469/1067` 字节，并与 Inventory 中的六个 SHA-256 逐项相等；Inventory Root 为 `bd1222e69fa436511a1238045694001e03c7d8e07a074590ca088bac825fe67b`。StageRelease 定向 Race Detector 为 2.930 秒；真实 PostgreSQL `TestSceneAnalysisWorkflowPersistsStructureIdentityReviewAndReplays` 普通执行 9.603 秒、Race Detector 27.999 秒通过。`skill-creator` 的 `quick_validate.py` 使用仓库既有 Agent 虚拟环境返回 `Skill is valid!`。Go/Python Bundle Hash 为 `5b49be2fe6f0af9131f8fc02b267314c15968e9f8646456f1b75fcbfa86c16ef`，实际 Pydantic Output/Input/Wire Schema 根分别为 `305b7449a01557334b04fe67db545f41491c60b0747d0edc3c76f2d0f6cfacee`、`5f81015db1172d883a858f1aab0b948d3d3b93a84d55cc15ac6eec66310b3a57` 与 `f8da8db4f7b6f6c7e6ac09354b86be0c570fda9883ccbce2aa99ebcab987ddcf`。
- 全量 CI：Backend 通过 `test -z "$(gofmt -l .)"`、`go vet ./...` 与无外部环境变量的 `go test -count=1 ./...`；固定 `lanverse_test` 与本机 Homebrew Temporal `127.0.0.1:7233` 的有界恢复旅程真实通过（66.201 秒），未启动或重启环境。Frontend OpenAPI 无漂移、ESLint、TypeScript、25 files/73 tests 和生产构建通过。Agent Ruff lint/format、Pyright 与 Pytest `207 passed, 60 skipped` 通过；跳过项均明确要求隔离 PostgreSQL、本机 Temporal 或真实 Codex CLI，未冒充已执行。StageRelease 实现的 GitHub Actions run `34613732861` 及证据提交 run `34613985128` 均为 `Required / CI = success`；SourceInventory 实现 run `34615168858` 同样全部成功：Agent 38 秒、Backend 57 秒、Frontend 1 分 3 秒、Workflow 1 分 43 秒，聚合 `Required / CI` 6 秒通过。
- 真实输入/产物/事实对账：Bundle Manifest 精确覆盖 `agent/skills/build-storygraph` 的 `NOTICE.md`、入口和全部 16 个 Reference，以每个文件的相对 POSIX 路径、字节长度与 SHA-256 形成来源/隔离/Bundle 三层根；其 Canonical artifact 由 Backend 编译期嵌入并严格重算，不从用户目录或 current/latest 动态发现。SourceInventory 只记录设计已经采用的三个外部参考及其固定 GitHub commit/bytes/license/用途，不保存外部正文；三项 `absorption_mode` 均为 `reference_only`，`production_file_mappings[]` 均为空，因此它们不能成为生产 Bundle 文件。七个 P0 StageRelease 分别绑定 Core、Wire/Input/Output/Normalized Candidate、Normalizer、Prompt Compiler、实际 Reference 或 Rubric、入口加白名单资源集合、执行/模型/空工具 Policy 和精确运行镜像；正式服务不再手写另一份 Stage Hash 或资源映射，而是使用同一发布制品。既有 `agt_scene_analysis_releases` 继续作为唯一 SQL 事实源，通过 GORM 保存 Stage/Bundle/Image Hash 和 Loaded Paths；未新增表或字段、migration、手写 SQL、兼容双读或替代事实源，也未启动/重启任何本机环境服务。
- 未覆盖条件与残余风险：七个现有 P0 StageRelease 与三个已采用参考的 SourceInventory 内容身份已具备，但外部来源的隔离安全审查、rewrite mapping、Eval/Shadow 和独立签名尚未形成完整吸收链；其余六个生产 Stage 的严格 Input/Candidate Schema、十三 Stage 完整 Core/CandidateStageSet、正式 SkillRelease 与追加式 Control 同样未实现。当前服务使用的最小 Skill/Control 身份尚未达到完整发布批准合同，不能冒充设计中的批准链。4–6 个 Release Inventory、持久 Owner/API、Visual Foundation、Reference Plan、expected target set 与 Gate 3 也仍未完成，因此除 `VPA-SUP-001` 外其余 `VPR-PRE` 与 `VPA-SUP/REL/VIS` 保持未验收；最终 `agent-browser` 仍只在全部开发完成后执行。
