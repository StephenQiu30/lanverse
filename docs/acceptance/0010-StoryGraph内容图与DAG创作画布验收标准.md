...
- 状态：进行中；已完成 Backend `PresetRelease`、仓库 Skill Bundle 内容身份、七个现有生产 Candidate Schema 和 SceneAnalysis Wire Schema 内容身份，不表示 Preset Inventory、Stage/Skill Release、视觉执行或 Gate 3 已实现。
- Git 基线/提交：本 Wire Schema 切片基线 `dfd9f0311883c595a2dfdfc6ab7a47cb719dbb1c`；本切片随本记录所在提交交付。
- Red 命令与失败：`go test ./tests/preset -count=1` 首先因 `backend/internal/preset/domain` 不存在而编译失败；Red 固定缺 NOTICE、六类能力或 Purpose Profile、错误视图角色、非内容定址引用、数字发布序号、非法日历日期、未知字段与 Hash 漂移必须拒绝。Bundle 子项的 `go test ./tests/agent -run '^TestStoryGraphBundleManifestCoversEveryInstalledResource$' -count=1` 再因 `BuildStoryGraphBundleManifest`、合同 ID 与严格 Decoder 不存在而编译失败。Candidate Schema 子项的 `go test ./tests/agent -run '^TestSceneAnalysisCandidateSchemasMatchTheAgentModels$' -count=1` 因共享 Schema Manifest 类型、Registry 与严格 Decoder 均不存在而编译失败。Wire Schema 子项的 `go test ./tests/agent -run '^TestSceneAnalysisWireSchemasMatchTheAgentModels$' -count=1` 因共享 Wire Schema Manifest 类型、Registry 与严格 Decoder 均不存在而编译失败。
- Green/定向验证：Preset 合同通过 `go test ./tests/preset -count=1`、`go vet ./internal/preset/... ./tests/preset`、`go test -race ./tests/preset -count=1` 与架构门。Bundle/Candidate/Wire Schema 合同通过 `go test ./tests/agent -count=1`、`go vet ./internal/agent/... ./tests/agent`、两个 Schema Manifest 定向 Race Detector，以及 Agent Ruff、Pyright 与 12 条 SceneAnalysis Wire/Contract/Harness 测试；`skill-creator` 的 `quick_validate.py` 使用仓库既有 Agent 虚拟环境返回 `Skill is valid!`。Backend 以 Production Canonical JSON 严格 round-trip Preset/Schema Manifest；Go/Python 对同一 `build-storygraph` 资源集均重算 Bundle Hash `5b49be2fe6f0af9131f8fc02b267314c15968e9f8646456f1b75fcbfa86c16ef`，对七个实际 Pydantic Candidate Model 重算 Schema Set Hash `305b7449a01557334b04fe67db545f41491c60b0747d0edc3c76f2d0f6cfacee`，并从实际 Invocation、Dispatch Authorization Claims、Attempt Result 重算 Wire Schema Hash `f8da8db4f7b6f6c7e6ac09354b86be0c570fda9883ccbce2aa99ebcab987ddcf`。
- 全量 CI：Backend 通过 `test -z "$(gofmt -l .)"`、`go vet ./...` 与无外部环境变量的 `go test -count=1 ./...`；固定 `lanverse_test` 与本机 Homebrew Temporal `127.0.0.1:7233` 的有界恢复旅程真实通过（56.930 秒），未启动或重启环境。Frontend OpenAPI 无漂移、ESLint、TypeScript、25 files/73 tests 和生产构建通过。Agent Ruff lint/format、Pyright 与 Pytest `206 passed, 60 skipped` 通过；跳过项均明确要求隔离 PostgreSQL、本机 Temporal 或真实 Codex CLI，未冒充已执行。
- 真实输入/产物/事实对账：本切片只建立无数据库依赖的不可变领域值合同。Bundle Manifest 精确覆盖 `agent/skills/build-storygraph` 的 `NOTICE.md`、入口和全部 16 个 Reference，以每个文件的相对 POSIX 路径、字节长度与 SHA-256 形成来源/隔离/Bundle 三层根；旧 StoryGraph 和生产 SceneAnalysis Runtime 只接受同一 Bundle Content Hash。Candidate Schema Manifest 直接从七个实际 Pydantic Model 的 JSON Schema 计算逐项 Hash，并与 Backend 已运行的 Stage/Profile/Schema Version 合同逐项对账；Wire Schema Manifest 同样直接覆盖实际 Invocation、Authorization Claims 与 Attempt Result，而不是手写占位 Schema。未创建表、migration、替代 SQL 事实源或兼容入口，也未启动/重启任何本机环境服务。
