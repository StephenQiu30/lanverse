...
- [ ] `VPR-PRE-005`（`VP-I05`）：plan_reference_assets 先从正式制作世界机械冻结 Anchor/State/固定 Target seed inventory；Agent 只选择合法 Anchor State 并提出 fulfillment/用途规格，Backend 再机械计算最终 expected target set 与依赖。Agent 不得删除、增加、改名、合并 Target 或改写 Owner Ref。 最低证据：Seed root + Set equality。Backend 两遍 expected-set 编译器已从精确 Owner Set、P1 实际 Occurrence/Interaction 与候选 Anchor State 重算六类业务键，并拒绝缺失、多余、Scope/Occurrence 漂移及无实际 Occurrence 的 Target；跨 Go/Python 严格 Input/Candidate 与候选 Normalizer 已冻结 Seed Root、Anchor option、最终键集、PurposeProfile、依赖和 fulfillment rank，但真实 StoryGraph Input Compiler、Harness/Wire、Workflow 节点和 Gate 3 消费尚未接通，因此整体条款保持未勾选。
...
- 状态：进行中；已完成含世界改编守恒边界的 Backend `PresetRelease`、4 个首批策展 Preset 目录、不可变 `ProjectPresetSelection` 合同、GORM Current Head 与认证读写 API、整体 Preset 切换确定性影响闭包、仓库 Skill Bundle 内容身份、八个生产 Input/Candidate Schema、包含视觉调用/结果的统一 Wire Schema、八变体 DefinitionCore/StageRelease、`resolve_visual_foundation` 跨语言严格合同、Agent vision Harness/私有媒体调用入口、Backend 媒体 Broker/HTTP Client、当前 production StoryGraph typed world read-set、Gate 2 exact source、faithful Input compiler、视觉执行生命周期及 GORM 生产持久化、Workflow/Temporal 生产节点、Backend expected Reference Target set 两遍编译器，以及三个已采用外部参考的审计与隔离链。Visual Foundation 现在由 Gate 2 后的精确 StoryGraph 和不可变 Selection ID/revision/content hash 共同驱动，Worker 复用现有 Agent Client、对象存储 Broker、GORM Store 与持久 Release/Manifest/Invocation/Attempt/Authorization/Result/Candidate/Head；Activity 重试不重新绑定变化后的 Current Selection，Candidate 接收前仍执行 SERIALIZABLE 当前输入重编译校验。十三 Stage Core/StageSet、完整 SkillRelease/Control、rewrite eval/shadow/signature、真实下游 stale 写入、`plan_reference_assets` Candidate/节点、Reference Plan 发布与 Gate 3 仍未实现。
...
- 未覆盖条件与残余风险：PresetRelease 守恒合同、4 个内置策展 Release、PresetSwitchImpact 确定性合同、不可变 Project Preset Selection + GORM Current Head + 认证 API、Selection-backed Visual Foundation 输入编译与接收前 stale 校验、Visual Foundation 严格合同、Agent vision Harness/私有媒体调用、Backend 媒体 Broker/Client、八变体 Registry/DefinitionCore/StageRelease、持久执行生命周期、Candidate 结果持久化、Workflow/Temporal 生产节点、Backend expected Reference Target set、独立 Reference Plan Input/Candidate 语义合同及一次真实本机 Codex 图片 canary 已具备；真实下游 stale 写入、style-blind mutation、带真实图片附件的 MinIO→Backend→Agent 系统旅程、`plan_reference_assets` StoryGraph Input Compiler、Harness/Wire、Stage Registry/Release、持久执行和 Workflow 节点、Reference Plan 原子发布与 Gate 3 尚未实现，因此 `VPR-PRE-005` 仍保持未勾选，且仍只验收 `VPA-VIS-002`，`VPA-VIS-001`、`VPR-PRE-009` 保持未勾选。八个 StageRelease、三个已采用参考的 SourceInventory、隔离安全审查、当前处置映射、七能力 Matrix 与运行时外部来源隔离已具备；但没有来源获准进入 rewrite，故当前 Matrix 合法为空，Golden/Adversarial Eval、Shadow 和独立签名没有合格输入且尚未形成完整吸收链，`VPA-SUP-005` 保持未勾选。其余四个未实现生产 Stage 与尚未注册的 `plan_reference_assets` 的完整 Stage Input/Candidate、十三 Stage 完整 Core/CandidateStageSet、正式 SkillRelease 与追加式 Control 同样未实现。当前服务使用的最小 Skill/Control 身份尚未达到完整发布批准合同，不能冒充设计中的批准链；最终 `agent-browser` 仍只在全部开发完成后执行。
