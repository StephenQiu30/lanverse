...
`SG-I01`–`SG-I19` 的已勾选项只保留 2026-08-27 首次 `SG-D21` 接受后真实执行、且合同未被本次设计修改的证据。2026-08-29 新增或改写的媒体 Provider 合同必须从本次重新接受后的 `[ ]` 目标项重新产生证据；历史 Runware/环境变量实现、旧 Acceptance、mock、内存替身、跳过项、被禁用的 CI、兼容回退、静态推断或“理论可行”均不能抵扣。

每次勾选必须在本文件追加：实施任务与提交、真实命令、fixture/完整原稿、依赖版本与隔离方式、关键计数/Hash/Receipt、成功或失败结果、残余风险。涉及外部依赖时必须记录真实 PostgreSQL/GORM、Temporal、Kafka、Elasticsearch、ELK、MinIO、Codex CLI，以及对应 Seedream、GPT Image、Nano Banana 或 Seedance 精确 Profile；缺失凭据、额度或外部条件时保持 `[ ]`。

统一验收顺序为定向 Red→Green→Refactor → 架构/安全门禁 → 当时全量真实 CI → 独立提交。`agent-browser` 只能由 `SG-I35` 在 `SG-I34` 完成并提交后执行，不能提前替代任何非浏览器证据。

## 2. Cross-service Requirement Checklist

### 2.1 架构与事实源

- [x] `SG-ARC-001`（`SG-I01`、`SG-I03`–`035` 回归）：Backend 唯一业务 Writer，Agent/Frontend/event-worker 零 Owner 直写证据。
...
- [ ] `SG-ARC-004`（`SG-I01`、`SG-I06`–`031`）：真实 Temporal wait/signal/replay/restart 与无 Kafka/DB 第二 Workflow 证据。
- [x] `SG-ARC-005`（`SG-I01`、`SG-I03`、`SG-I32`）：四图 Schema/ID/DTO 不可互换证据。
- [ ] `SG-ARC-006`（`SG-I01`、`SG-I05`、全部 Agent 切片）：Agent Candidate-only 环境、依赖、网络和零业务写入证据。
- [ ] `SG-ARC-007`（`SG-I07`、`SG-I22`、`SG-I32`–`033`、`SG-I35`）：Browser 仅访问 Backend `/api/v1`，不直连媒体 Provider/私有对象地址的 bundle/import/网络证据。
- [ ] `SG-ARC-008`（`SG-I03`–`031`）：外部边界 Intent/Request/Job/Call/Receipt 稳定身份、known failure/unknown 与同 ID 对账故障矩阵。
- [ ] `SG-ARC-009`（`SG-I03`–`033`）：Workspace/Project Token Version/Membership 与防枚举权限矩阵。
...
- [x] `SG-GRF-008`（`SG-I03`、`SG-I17`、`SG-I28` 回归）：Compiler 只读已确认精确 Owner、污染源拒绝证据。
- [x] `SG-GRF-009`（`SG-I03`、`SG-I17`、`SG-I28` 回归）：Owner Set/Version/Head/Receipt/Outbox 单 GORM 事务故障注入。
...
- [ ] `SG-QRY-002`（`SG-I04`、`SG-I32`）：版本、lens、scope、depth/cursor、truncated/继续条件 contract。
- [ ] `SG-QRY-003`（`SG-I04`、`SG-I32`）：五类有界 Lens、大图分层和确定性结果 Hash。
...
- [ ] `SG-REV-006`（`SG-I11`、`SG-I14`、`SG-I16`、`SG-I19`、`SG-I26`、`SG-I28`–`031`）：八类 Gate 的显式 Owner Apply 与负向零写入证据。
...
- [ ] `SG-PRD-009`（`SG-I27`）：detail_shots 精确 READY AssetVersion/Artifact/Lineage/Style/view-role 门禁。
- [ ] `SG-PRD-010`（`SG-I28`）：Shot + ShotProductionBindingVersion + Receipt 全批 GORM 原子应用。
- [ ] `SG-PRD-011`（`SG-I12`、`SG-I28`–`031`）：Production/Image/Video/Asset 四类 Binding Owner/Record/API/Graph 不混用证据。
- [ ] `SG-PRD-012`（`SG-I17`、`SG-I28`）：Owner Apply 后 expected graph hash 编译与 unknown 同 Receipt 对账。
- [ ] `SG-VIS-001`（`SG-I23`–`031`）：`reference_asset|shot_frame|shot_video` strict union、按 kind 字段与旧入口拒绝。
- [ ] `SG-VIS-002`（`SG-I23`–`026`）：reference_asset 冻结身份/Specification/State/Style/view roles/Project Binding，Intent Gate 前零费用/零 Call。
- [ ] `SG-VIS-003`（`SG-I20`、`SG-I22`–`025`、`SG-I31`）：Catalog 只展示已注册真实 Factory；受控版本命令、未知/篡改 Preset 与任意 URL/Header/JSON 失败关闭。
- [ ] `SG-VIS-004`（`SG-I21`、`SG-I23`–`025`、`SG-I29`、`SG-I31`）：Provider 前 Intent/Price/Cost/Quota/Authorization/Binding/Job/Call 全部已提交。
- [ ] `SG-VIS-005`（`SG-I21`、`SG-I23`–`025`、`SG-I29`–`031`）：图片/视频私有 Staging、Backend 字节/Hash/媒体重验与 READY/QUARANTINED。
- [ ] `SG-VIS-006`（`SG-I26`）：composite 三视图 role/输入 Hash/READY/lineage QC 门禁。
- [ ] `SG-VIS-007`（`SG-I26`、`SG-I29`–`031`）：Image/Video 确定性 QC 与人工语义判断边界。
- [ ] `SG-VIS-008`（`SG-I26`、`SG-I29`–`031`）：Reference/Shot Frame/Shot Video 冻结 CandidateSet 的单一不可变 Selection 并发收敛。
- [ ] `SG-VIS-009`（`SG-I26`）：AssetVersion 精确绑定、不可变与历史版本引用。
- [ ] `SG-VIS-010`（`SG-I29`）：shot_frame 冻结正式 Shot/ProductionBinding/Occurrence/AssetVersion 完整性。
- [ ] `SG-VIS-011`（`SG-I29`）：只发布 ShotImageBindingVersion、单 Shot 局部重跑不改旧事实。
- [ ] `SG-VIS-012`（`SG-I20`、`SG-I22`）：Workspace Owner 配置/轮换/禁用 Connection/Credential/Profile/Project Binding 的权限、CAS、幂等、版本与重启证据。
- [ ] `SG-VIS-013`（`SG-I20`、`SG-I23`–`025`、`SG-I31`）：Seedream 5.0 Pro+、Seedance 2.0/2.0 Fast/2.0 Mini/2.5、GPT Image 2、Nano Banana 2 Lite/2/Pro/Legacy 精确 Profile/Capability/PriceQuote golden。
- [ ] `SG-VIS-014`（`SG-I20`、`SG-I22`）：Web 一次性 API Key、root-key Docker Secret、CredentialVersion AEAD/HMAC、零配置/错误 key/重启与 DB/API/log/cache/bundle 零泄漏。
- [ ] `SG-VIS-015`（`SG-I21`、`SG-I23`–`025`、`SG-I29`、`SG-I31`）：一个候选一个 ProviderCall、四候选四 Call、首次 dispatch 唯一发送与单终态输出故障矩阵。
- [ ] `SG-VIS-016`（`SG-I21`、`SG-I23`–`025`、`SG-I29`）：同步 Provider `outcome_unknown` 保留 Reservation 且禁止自动重提，迟到结果不覆盖终态。
- [ ] `SG-VIS-017`（`SG-I21`、`SG-I31`）：异步 remote task id 同身份查询、重启/Timer/Activity replay 与保留窗口到期显式阻塞。
- [ ] `SG-VIS-018`（`SG-I23`、`SG-I29`）：Seedream 5.0 Pro+ 精确协议、单 Call 单 Candidate、Staging/协议漂移与真实 reference/shot_frame。
- [ ] `SG-VIS-019`（`SG-I24`、`SG-I29`）：GPT Image 2 精确 Image API、`n=1`/单 b64_json、Usage/Staging 与真实 reference/shot_frame。
- [ ] `SG-VIS-020`（`SG-I25`、`SG-I29`）：Nano Banana 四模型精确 Interactions/Generate Content 合同、无试探/回退与真实 reference/shot_frame。
- [ ] `SG-VIS-021`（`SG-I30`–`031`）：shot_video 冻结 Shot/Production/Image Binding/Artifact/时长/比例/motion/Profile，capability mismatch 零费用且无静默改参。
- [ ] `SG-VIS-022`（`SG-I30`–`031`）：Video QC 后追加 ShotVideoBindingVersion，冻结完整 lineage/媒体 Hash 并投影 StoryGraph，局部重跑不改旧事实。
...
- [ ] `SG-LOG-002`（`SG-I04` 历史、`SG-I21`–`031`、`SG-I34` 新目标）：全链 trace/run/node/task/decision/provider job/call/receipt 关联与敏感字段零命中扫描。
- [x] `SG-LOG-003`（`SG-I04`、`SG-I34` 回归）：Logstash/Elasticsearch/Kibana 逐组件故障不改变业务事实、Workflow 或 Search 投影；Kafka 故障只属于业务事件链验证。

### 2.5 Frontend、运行质量与旅程

- [ ] `SG-FE-001`（`SG-I07`、`SG-I22`、`SG-I32`–`033`）：单 Next.js/npm + RTK Query + 生成 Client，无第二 Query/monorepo。
- [ ] `SG-FE-002`（`SG-I12`、`SG-I16`、`SG-I26`、`SG-I32`）：角色/地点卡派生 Owner 事实、State/Style 不重复身份与 Scene 区分。
- [x] `SG-FE-003`（`SG-I07`）：真实 Review Workbench，错误/unknown 无 mock/local success。
- [ ] `SG-FE-004`（`SG-I32`）：StoryGraph 路由、五 Lens、scope/version/focus 深链刷新。
- [ ] `SG-FE-005`（`SG-I32`）：React Flow/Dagre 随真实消费者引入，只读且布局/viewport 零回写。
- [ ] `SG-FE-006`（`SG-I32`）：Story Lens 与 Workflow Lens Query/DTO/ID/Adapter/renderer 分离。
- [ ] `SG-FE-007`（`SG-I32`）：Server fact/URL/local 状态边界，View Model 不复制到 Redux/Context/localStorage。
- [ ] `SG-FE-008`（`SG-I07`、`SG-I22`、`SG-I32`–`033`）：完整状态、键盘、焦点、列表替代与 reduced-motion。
- [ ] `SG-FE-009`（`SG-I33`）：类型化 Domain Intent + base/expected/idempotency，Owner Command 重编译且无 Graph JSON 直写。
- [ ] `SG-FE-010`（`SG-I22`）：Catalog 驱动 Provider Settings、Connection/Credential/Profile/Price/Binding 受控表单与 Secret 提交后 URL/cache/storage/log/bundle 零残留。
- [ ] `SG-FE-011`（`SG-I22`、`SG-I26`、`SG-I29`–`032`）：Reference/Shot Frame/Shot Video 的 Target/Candidate/QC/Cost/Selection/Apply 和零配置/root-key/stale/unknown 状态可区分。
- [ ] `SG-OPS-001`（`SG-I01` 起）：严格输入/事件/Provider/HTTP 解码、大小/深度/数字/UUID/Hash 负向证据。
- [ ] `SG-OPS-002`（`SG-I20`–`025`、`SG-I29`–`031`）：编译期 Provider Host/Region/redirect allowlist、SSRF、短生命周期 Credential 注入与 secret 零泄漏。
...
- [ ] `SG-OPS-009`（`SG-I34`、`SG-I35`）：完整实现与 CI 提交后才运行最终 agent-browser 的 Git 顺序。
- [ ] `SG-OPS-010`（`SG-I20`、`SG-I30`、`SG-I34`）：Backend Runtime Image 固定 `ffprobe`、Compose 仅 Backend/Frontend、root-key Secret 挂载、本机环境复用与 Provider API Key 零环境变量证据。
- [ ] `SG-JRN-001`（`SG-I08`–`017` 历史、`SG-I34` final）：至少两集 Document→Evidence→Bible→Episode/Scene/Beat/Occurrence/Claim→Core Graph 可反查。
- [ ] `SG-JRN-002`（`SG-I12`、`SG-I18`–`031`、`SG-I34`）：Web 配置三类图片/Seedance；跨集单 Asset、多 State、reference/detail/Shot/frame/video/四类 Binding/Graph 全链。
- [x] `SG-JRN-003`（`SG-I03`–`004`、`SG-I34` 回归）：Outbox→Kafka→Elastic→Search 深链与重复/乱序/重启/Reindex 收敛。
- [ ] `SG-JRN-004`（`SG-I34`）：完整原稿统计、人工抽查与全故障矩阵零重复事实/费用/配额。
- [ ] `SG-JRN-005`（`SG-I35`）：最终 agent-browser 旅程与 Web/API/PostgreSQL/Temporal/Kafka/Search/ProviderCall/Artifact 对账。
...
- [ ] `SGA-BND-006`（`SG-I20`、`SG-I27`、`SG-I34`）：Provider 配置/Secret/Target/Call/Staging/QC/Selection/Binding 全归 Backend；Agent Wire/env/dependency/network 中零媒体 Secret/Endpoint/字节/私有 URL/调用。
...
- [ ] `SGA-STG-004`（`SG-I08`–`010`、`SG-I13`、`SG-I15`、`SG-I18`、`SG-I27`）：无证据不补写事实的对抗/人工证据。
...
- [ ] `SGA-STG-007`（`SG-I27`）：detail_shots 精确 READY 资产并禁止改变已接受意图/身份/状态。
- [ ] `SGA-STG-008`（`SG-I18`、`SG-I27`、`SG-I28`）：Backend 生成序号/timecode/UUID/Owner/Binding，Agent 只给 Candidate。
...
- [ ] `SGA-EVD-005`（`SG-I08` fixture、`SG-I34` final）：两集开发 + 完整原稿统计和代表集人工细查。
...
- [x] `SGA-REP-001`（`SG-I10`、`SG-I27`）：模型 Review Issue 不冒充确定性 Gate/blocker。
- [x] `SGA-REP-002`（`SG-I10`、`SG-I27`）：Repair Patch 冻结 target/allowlist/base/邻接且不能改已发布 Graph。
- [x] `SGA-REP-003`（`SG-I10`、`SG-I27`）：expected Head 应用 N+1、幂等 Receipt 与并发单胜。
- [x] `SGA-REP-004`（`SG-I10`、`SG-I27`）：每轮重跑影响闭包 Gate/Review，有界预算耗尽不半成功。

### 3.4 Codex、错误、CI 与旅程

- [ ] `SGA-COD-001`（`SG-I05` 起、`SG-I34`）：真实本地 Codex ephemeral/read-only/ignore config/临时空目录与 Policy 模型。
...
- [ ] `SGA-OPS-004`（`SG-I08`–`010`、`SG-I13`、`SG-I15`、`SG-I18`、`SG-I27`、`SG-I34`）：各类至少一次真实 Codex，完整原稿无模型桩。
- [x] `SGA-OPS-005`（每个 `SG-Ixx`）：完整任务 Red→Green→Refactor、全量 CI、Evidence 与独立提交。
- [ ] `SGA-OPS-006`（`SG-I35`）：仅在 `SG-I34` 提交后运行最终 agent-browser，无新 Agent 实现。
- [ ] `SGA-JRN-001`（`SG-I08`–`011`、`SG-I34`）：完整原稿 Evidence→Story analyze/reconcile→review/repair→Bible Gate 可恢复。
- [ ] `SGA-JRN-002`（`SG-I12`–`017`、`SG-I34`）：confirmed Bible→episode analyze/reconcile→Owner Apply→Core Graph。
- [ ] `SGA-JRN-003`（`SG-I18`、`SG-I26`–`028`、`SG-I34`）：draft→needs_asset→READY AssetVersion→detail/review/repair，Agent 零媒体调用/Shot/Binding。
- [ ] `SGA-JRN-004`（`SG-I34`）：Bundle 滚动、deadline、runtime unavailable、迟到、Head/Repair 竞争恢复收敛。

## 4. 实施任务完成 Checklist

以下 35 项必须与 Plan 同序；`SG-I01`–`SG-I19` 只保留已提交的历史完成事实，`SG-I20`–`SG-I35` 是本次重新接受后全未勾选的新目标。每项只有在其映射 Requirement、定向验证、当时全量真实 CI、Acceptance Evidence 和独立 Git 提交均完成后才能勾选。
...
- [ ] `SG-I24`：GPT Image 2 `reference_asset` 精确 Adapter、真实调用、Staging/QC/CandidateSet 完成。
- [ ] `SG-I25`：Nano Banana 2 Lite/2/Pro/Legacy 四个 `reference_asset` 精确合同与真实调用完成。
- [ ] `SG-I26`：composite 三视图 QC/Selection/AssetVersion 与三类图片候选 Owner Apply 完成。
- [ ] `SG-I27`：精确 READY AssetVersion 的 detail_shots/Review/Repair 与 Agent 媒体隔离完成。
- [ ] `SG-I28`：Storyboard Gate/Shot/ProductionBinding/Graph 原子 Apply 完成。
- [ ] `SG-I29`：三类图片 Provider 全部完成 shot_frame/Selection/ShotImageBinding 与单 Shot 局部重跑。
- [ ] `SG-I30`：shot_video Target/Cost/Staging/ffprobe/Video QC/Selection/ShotVideoBinding/Graph 完成。
- [ ] `SG-I31`：Seedance 2.0/2.0 Fast/2.0 Mini/2.5 四个真实异步视频闭环完成。
- [ ] `SG-I32`：只读 Story Lens、React Flow/Dagre、有界查询与无写入完成。
- [ ] `SG-I33`：类型化 Domain Intent/Owner Command/重编译/Patch Diff 完成。
- [ ] `SG-I34`：完整原稿、四类 Provider 全模型、代表集人工细查、故障矩阵与全量真实 CI 证据已提交。
- [ ] `SG-I35`：最终 agent-browser Web Journey 与 Provider 设置、Backend/Owner/Temporal/Kafka/Search/ProviderCall/Artifact 对账已提交。
...
>
> 下方 2026-08-29 的 Runware、`IMAGE_PROVIDER`、`RUNWARE_API_KEY`、固定 Binding/Route/Test Evidence 同样只记录当时真实实现增量。它们证明不了当前 Provider 配置/Secret/Call、Seedream、GPT Image、Nano Banana、Seedance、图片/视频 Binding 或 Web 设置合同，也不给新 `SG-I20`–`SG-I35` 任何预先通过状态。
...
首次 `SG-D21` 建立时 188 个 Checklist 全部未勾选，随后按当时合同完成 `SG-I01`–`SG-I19`；这是 2026-08-29 通用媒体设计变更前的历史队列状态。其后的 Runware 增量仍保持未通过，并在本次重新接受后只作为重构输入，不决定新版 `SG-I20` 的内容或完成状态。
...
### 参考资产工作流付费准备（2026-08-29）

- Red 与独立测试：新增 `backend/tests/workflow/reference_asset_executor_test.go`，先因 Generation Workflow Executor 只接受 CandidateSet Source、构造函数缺少 Target Builder/Preparation Owner 而编译失败；实现后覆盖 exact approved output、目标选择、重投以及选择缺失/歧义时 Cost/Quota 调用为零。测试只位于 `backend/tests`，未与业务代码混放。
- 一节点一意图：既有 `uq_gen_intent_node_run` 防重复事实保持不变。节点输入必须是 `approved_storyboard_intents@1`，Config 必须且只能包含合法 `asset_id + asset_state_id`；Builder 返回的所有 Target 都重验 workspace/project/creator/source/hash，只有唯一匹配项可进入 Preparation。同一批准集的多个角色/状态由静态 DAG 多节点并行表达，不引入动态子图、数组顺序选择、循环多 Intent、Migration 或兼容入口。
- 幂等与付费边界：同一 WorkflowRun 的多个 reference 节点共享稳定 Target Build Receipt；每个 NodeRun 使用独立稳定 Preparation Key，并把 canonical Node Input Hash 与 Target ID/Hash 分别传入既有 Preparation Service。四张候选图对应四个计费/配额 Units；重投即使 Activity attempt 和外层幂等键改变，也仍命中相同 Target 与 Intent。选择缺失或歧义均发生在 Cost/Quota 前。
- 实际装配：单 Backend Composition Root 已装配真实 GORM ReferenceTargetBuilder 与 PreparationService，继续复用唯一 Catalog、Cost/Quota Owner 与 PostgreSQL 事实源；没有 Raw SQL、Migration、第二 ORM、额外 Backend Entrypoint、Agent Writer 或 Kafka Command。由于 Execution Claim、Provider Submit/Query、Staging 与 CandidateSet 尚未接完，当前不把该 Executor 注册进 System Catalog，避免用户启动一个只能永久 `RETRYING` 的付费节点。
- 验证：Red 命令按预期编译失败；Green 后 `go test -count=1 ./tests/workflow -run ReferenceAssetExecutor` 与 Generation/Architecture/Workflow 定向包均通过。最终在全新本机 PostgreSQL 测试库并复用现有 Temporal 执行 Backend `gofmt`、`go vet ./...`、`go test -count=1 -p 1 ./...` 全通过，Workflow 包 `121.773s`；Agent Ruff check/format、Pyright 与 Pytest 通过，结果 `39 passed, 4 skipped`，四项均为显式真实 Codex opt-in；Frontend OpenAPI 生成零漂移、lint、typecheck、18 个 Vitest 文件 54 项测试和 production build 全通过。开发/环境 Compose 独立渲染、开发默认 `backend + frontend`、环境默认零服务及三文件生产组合渲染通过；生产组合首次因 shell 未注入必填部署变量而真实失败，随后只用显式 render-only 假值重跑，不读取 `.env`。Backend 镜像构建及唯一 `/usr/local/bin/lanverse` 探针通过。Kafka/Elasticsearch/MinIO/Runware 未为本单元配置或重跑，不计外部集成通过。
- 日志与剩余范围：只复用已启动的 Logstash，不使用 Filebeat，也不启动或重启 Logstash。当前没有 Execution Claim、Provider Request/Job、远程 Runware、私有下载/Staging、Artifact/CandidateSet，因此 `SG-VIS-001`–`005`、`SG-OPS-002` 与 `SG-I20` 仍未勾选；全部开发完成前不运行 `agent-browser`。

### 参考资产任务提交与同一 Job 对账（2026-08-29）

- Red 与前向重放：扩展 `backend/tests/generation/provider_execution_test.go` 后，原 Preparation Receipt 在同一 Intent 已进入 `OUTCOME_UNKNOWN` 时真实失败为状态漂移。修正后 Preparation/Claim 重放不要求数据库停留在旧快照，而是校验 Target、Cost/Quota Owner ID/Receipt、创建时间、Claimant/Token/到期时间/Fencing 与 Revision 单调前进；绑定漂移或时间倒退继续失败关闭，原 Claim Authorization 不重签、不换身份。
- Workflow 执行链：`reference_asset` Executor 在既有 Target Builder/Preparation 后，以 `workflow-node:{node_run_id}` 领取 Execution Claim，并使用 NodeRun 稳定的 Claim/Submit Key。`CLAIMED|DISPATCHING` 只调用 Submit；`SUBMITTED|OUTCOME_UNKNOWN` 只携带已持久化 Provider Job ID 调用 Reconcile，查询 Key 随 Intent Revision 前进。受控 unknown 重投证明 Submit 恰好一次、后续只查询同一 Job；Job `RUNNING|UNKNOWN|SUCCEEDED|FAILED` 必须分别与 Intent `SUBMITTED|OUTCOME_UNKNOWN|SUCCEEDED|FAILED` 一致。
- 所有权与负向门禁：独立 Workflow 测试注入另一个 NodeRun 的 Claimant，Executor 在 Provider 调用前拒绝，Submit/Query 计数均为零。Request/Job/Intent 必须继续绑定同一 Target ID/Hash、Workspace、Project、WorkflowRun 与 NodeRun；一个 NodeRun 仍只拥有一个 Intent，没有动态子图、循环多 Intent、兼容路径或 Node Cache 远程防重。
- 实际装配与事实源：单 Backend Composition Root 把既有 GORM PreparationService 同时作为 Preparation/Claim Owner，并接入既有 ProviderService；所有 Request/Job/Receipt 继续写唯一 PostgreSQL/GORM Catalog，没有 Migration、DDL、Raw SQL、第二 ORM、第二 SQL 事实源、Agent Writer、Kafka Command 或额外 Backend 入口。生产 ProviderService 仍缺安全下载与私有 Staging Gateway，因此 `reference_asset` 节点继续不注册进 System Catalog，本证据不把内部接线宣称为真实 Runware 可用。
- 定向与 Backend 全量 CI：`go test -count=1 ./tests/workflow -run 'ReferenceAssetExecutor|ShotWorkflowSourceExecutors'` 通过；专属 PostgreSQL 中 `go test -count=1 ./tests/generation -run ProviderSubmissionAndReconcile` 通过；Generation/Workflow/Architecture 三域通过。最终在全新 Homebrew PostgreSQL `18.4` 数据库并复用现有 Temporal 执行 `test -z "$(gofmt -l .)"`、`go vet ./...`、`LANVERSE_TEST_DATABASE_URL=... LANVERSE_TEST_TEMPORAL_ADDRESS=127.0.0.1:7233 go test -count=1 -p 1 ./...` 全通过，Workflow 包耗时 `112.977s`。
- 跨项目、Compose 与镜像：Agent Ruff check/format、Pyright 与 Pytest 全通过，结果 `39 passed, 4 skipped`；四项仍只是显式真实 Codex CLI opt-in，本单元未修改 Agent。Frontend OpenAPI 生成零漂移、lint、typecheck、18 个 Vitest 文件 54 项测试与 Next.js production build 全通过。开发/环境 Compose 使用 `/dev/null` env 文件独立渲染，默认项目服务只有 Backend/Frontend、环境服务默认零启动；三文件生产组合用显式 render-only 假值渲染通过，没有读取 `.env`。Backend 镜像构建成功且 `/usr/local/bin` 只有可执行文件 `lanverse`，专项镜像已删除。
- 日志与剩余范围：只复用并探测已启动的 Logstash，不引入或运行 Filebeat，也不启动、重启或重新编排 Logstash；Kafka 仍只承载业务事件。真实 Runware Submit/Query、官方输出安全下载、私有 MinIO Staging、terminal Provider Receipt、Artifact/CandidateSet 与凭据化验收尚未完成，因此 `SG-VIS-001`–`005`、`SG-OPS-002` 与 `SG-I20` 保持未勾选。下一交付单元只进入 Staging/Artifact/Candidate 物化；全部开发完成前不运行 `agent-browser`。

### Runware 输出安全下载与私有 Staging（2026-08-29）

- Red 与独立测试：新增 `backend/tests/generation/runware_stager_test.go`，先因缺少生产 `ImageStagerConfig/NewImageStager` 编译失败，再固定成功下载/重放、同 Host 重定向、私网 DNS、私网重定向、Media Type、响应大小、PNG 尺寸和对象存储失败。测试只位于 `backend/tests`，未与生产代码混放。
- 下载与 SSRF 门禁：生产 Stager 只接受 `https://im.runware.ai` 且禁止凭据、Query、Fragment 和非 443 端口；每次初始请求及重定向都重新解析 DNS，拒绝 private/loopback/link-local/unspecified 地址。生产 Transport 不读取代理，Dialer 直接连接本次校验后的公共 IP，TLS 保留官方 Host 的 SNI/证书校验，从而不在校验后再次按未固定 DNS 地址连接。最多跟随五次允许的 HTTP Redirect，协议降级或 Host 漂移在下一次下载前失败关闭。
- PNG 与私有对象：响应必须为 200、`image/png`、未超 20 MiB；先解析尺寸/像素上限，再完整解码 PNG，实际宽高必须与冻结 Target 相同。对象 Key 固定为 `staging/{workspace_id}/{provider_job_id}/{image_uuid}.png`，MinIO Client 使用成熟 SDK 的 `If-None-Match: *` 条件写入并在写后重读验证 SHA-256/长度/Media Type；同 Key 同内容幂等返回，并发或串行的字节、Hash、长度、Media Type 漂移均拒绝且不覆盖已有对象。
- 配置与真实装配：`IMAGE_PROVIDER` 仅允许空或 `runware`。留空时 Config 不读取 `RUNWARE_API_KEY`/超时，也不构造假 Gateway；显式 `runware` 才要求 API Key 与 1–120 秒超时，并在唯一 Backend Workflow Composition Root 构造 Runware Gateway、私有 Stager 和现有 ProviderService。Secret 只传入 Bearer Header，不进入数据库、Target、Receipt、Temporal History 或日志。Compose 与安全示例只增加这三个显式变量，没有新服务、Binary 或环境容器。
- 真实 MinIO 与定向验证：复用已启动的本机 MinIO `127.0.0.1:9000` 执行 `LANVERSE_TEST_MINIO_*=... go test -count=1 ./tests/platform -run PrivateObjectEnsure`，真实证明相同私有对象重放收敛、两个不同内容的并发首次写入只有一个成功、内容/媒体漂移拒绝且测试对象被清理。`go test -count=1 ./tests/generation -run RunwareImageStager`、Config/Generation/Bootstrap/Architecture 定向包与 `go vet` 全通过；没有启动 MinIO 或任何环境容器。
- 完整 CI：并发条件写补强后，最终在另一全新 Homebrew PostgreSQL `18.4` 数据库并复用现有 Temporal 从头执行 Backend `gofmt`、`go vet ./...`、`go test -count=1 -p 1 ./...` 全通过，Workflow 包耗时 `119.399s`。Agent Ruff check/format、Pyright 与 Pytest 为 `39 passed, 4 skipped`，四项仍只是显式真实 Codex CLI opt-in；Frontend OpenAPI 零漂移、lint、typecheck、18 个 Vitest 文件 54 项测试和 production build 全通过。开发/环境/三文件生产 Compose 使用 `/dev/null` env 文件渲染通过；Backend 镜像只包含 `/usr/local/bin/lanverse`，专项镜像已删除。
- 日志与剩余范围：日志只发送到已启动的 Logstash，不使用 Filebeat，也没有启动、重启或重新编排 Logstash。真实 Runware 凭据化调用尚未执行；Workflow 在 Provider 成功后仍未调用既有 OutputMaterializationService，也未输出 canonical CandidateSet/注册 Catalog 节点，因此 `SG-VIS-001`–`005`、`SG-OPS-002` 与 `SG-I20` 继续保持未勾选。下一单元只接 terminal Result → READY Artifact/CandidateSet → Node Output；全部开发完成前不运行 `agent-browser`。

### 参考资产候选物化与确定性节点输出（2026-08-29）

- Red 与独立测试：扩展 `backend/tests/workflow/reference_asset_executor_test.go`，先因 Generation Workflow Executor 缺少 Provider Output Materializer 构造参数而编译失败；Green 后固定 unknown Submit 只发生一次、同 Job Reconcile 成功后立即物化、已成功 Intent 重投不再调用 Provider、两次成功返回相同 canonical `node-output-v1`，以及跨 Project CandidateSet 漂移失败关闭。测试继续只位于 `backend/tests`。
- 复用既有 Owner：Executor 没有新建 Artifact、Candidate、QC 或 CandidateSet 体系。生产 Composition Root 把既有 `OutputMaterializationService` 同时作为 CandidateSet Read Owner 与成功输出 Materializer；它按同一 Provider Job/Receipt 将私有 Staging 对象交给 Asset Owner 重读验证为 READY，再由现有 Generation Candidate Owner 注册确定性 QC 结果并构造 CandidateSet。唯一 PostgreSQL/GORM Catalog、单 Backend Binary 和现有 MinIO Client 保持不变；没有 Migration、DDL、Raw SQL、第二 ORM、第二事实源、Agent Writer 或新服务。
- 终态与重放：Executor 只在 Intent/Provider Job 为 `SUCCEEDED` 且 Generation Request、Provider Job、Provider Receipt ID 完整时调用 Materializer。返回的 CandidateSet 必须精确绑定当前 workspace/project/job/receipt、revision=1、合法内容 Hash、非空且无重复的 READY/QC Candidate；随后输出 `candidates:generation_candidate_set@1`。Materializer 错误或任一 ID/Hash/Artifact/QC 漂移直接向 Temporal 返回错误，不伪造 `SUCCEEDED`；`RUNNING|UNKNOWN` 继续返回 `RETRYING` 并只对账原 Job。
- 真实 Backend 门禁：在专属全新 Homebrew PostgreSQL `18.4` 数据库中复用已运行的 Temporal、MinIO 与 Homebrew Kafka `4.3.1`，执行 `test -z "$(gofmt -l .)"`、`go vet ./...` 和 `go test -count=1 -p 1 ./...` 全部通过；Generation `10.738s`、Eventing `23.756s`、StoryGraph `10.662s`、Workflow `111.625s`。测试数据库只由本单元创建，验收后精确删除。Logstash `127.0.0.1:5000` 仅做已运行实例的连通性复用；本轮没有配置 Elasticsearch/Kibana 集成变量，因此不把 ELK 查询或故障注入写成通过。
- 跨项目、Compose 与镜像：Agent Ruff check/format、Pyright 与 Pytest 通过，结果 `39 passed, 4 skipped`，四项仍是显式真实 Codex CLI opt-in，本单元未修改 Agent；Frontend OpenAPI 生成零漂移、lint、typecheck、18 个 Vitest 文件 54 项测试与 Next.js production build 全通过。开发/环境/生产 Compose 使用 `/dev/null` env 文件渲染，首次因 shell 临时变量只作用于第一条命令而失败，改为在同一子进程显式导出纯测试值后从头重跑通过。Backend/Frontend/Agent 三镜像均从当前工作区构建，单 `lanverse` Binary、Frontend standalone、Agent 非 root/固定 Codex/唯一 Bundle 探针全部通过；没有启动任何环境服务。
- 日志与剩余范围：日志继续由 Backend 直接发送到已启动的 Logstash，不使用 Filebeat，也没有启动、重启或重新编排 Logstash；Kafka 只承载业务事件。真实 Runware 凭据化调用、生产 Provider Binding、System Catalog 节点注册以及完整 reference asset 人工选择/AssetVersion 发布仍未完成，因此 `SG-VIS-001`–`005`、`SG-OPS-002` 与 `SG-I20` 保持未勾选。下一交付单元只补齐上述可用性门禁与真实 Runware reference asset 旅程；全部开发完成前不运行 `agent-browser`。

### 参考资产节点注册与 Provider 付费前门禁（2026-08-29）

- Red 与独立测试：先扩展 `backend/tests/authoring/graph_contract_test.go`、`backend/tests/workflow/reference_asset_executor_test.go`、`backend/tests/generation/provider_execution_test.go` 与物化测试；Red 真实表现为 `ProviderConfig`/`RequireConfiguredImageProviderBinding` 不存在、Catalog 仍为 `14.0.0` 且无 reference asset 节点，以及 Provider 不可用时仍会进入 Target/Preparation。测试继续只位于 `backend/tests`，未与业务代码混放。
- 正式 Catalog：发布新的不可变 `lanverse.production@15.0.0`，注册 `generation.reference_asset@1.0.0`。节点输入精确为 `intents:approved_storyboard_intents`，输出为 `candidates:generation_candidate_set`，Config 必须且只能包含 UUID `asset_id + asset_state_id`，Executor 固定 `activity.reference_asset_generation`、`cache=by_inputs`、`risk=external_ai`；旧 Catalog 内容未修改，也没有 provider-disabled 假成功路径。
- Binding 与付费边界：既有 Backend Provider Owner 继续按 Project 追加发布 Binding，Workflow 不隐式创建版本。ProviderService 新增只读授权门禁，最新 Binding 必须保持合法不可变 Hash，并与当前进程配置的 `provider_key`、`model_key`、`credential_ref` 完全一致。Executor 每次首次执行、unknown 重投和成功重放都在 Target Builder 前调用门禁；Provider 未启用、Binding 缺失或最新版本漂移时，Target、Intent、Cost/Quota Reservation、Claim、Submit/Reconcile 与 Materialization 调用计数全部为零。额外 Red 证明门禁通过后再追加漂移 Binding 会被首次创建 Request/Job 的同一事务拒绝，GenerationRequest 数和远程调用数仍为零；进程显式切换到新配置后才允许新 Request，已冻结旧 Request/Job 的对账不改路。
- 实际装配与事实源：唯一 Workflow Composition Root 只在 `IMAGE_PROVIDER=runware` 时配置固定 `runware`、`runware:z-image@turbo`、`env/runware_api_key`，继续复用现有 ProviderService、GORM Store 与单一 PostgreSQL Catalog。没有 Migration、DDL、Raw SQL、第二 ORM、第二 SQL 事实源、Agent Writer、Kafka Command、兼容 Gateway、新服务或额外 Backend 入口。
