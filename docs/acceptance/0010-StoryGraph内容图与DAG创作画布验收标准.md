...
- 状态：进行中；已完成含世界改编守恒边界的 Backend `PresetRelease`、4 个首批策展 Preset 目录、整体 Preset 切换确定性影响闭包、仓库 Skill Bundle 内容身份、八个生产 Input/Candidate Schema、包含视觉调用/结果的统一 Wire Schema、八变体 DefinitionCore/StageRelease、`resolve_visual_foundation` 跨语言严格 Input/Candidate/Invocation/Authorization/AttemptResult 合同、Agent vision Harness/私有媒体调用入口、Backend 媒体 Broker/HTTP Client、当前 production StoryGraph 的 Visual Foundation typed world read-set、Gate 2 已应用 Candidate/Design Gap 的 typed source snapshot、首个无附件/无 Gap 的 faithful Input compiler，以及三个已采用外部参考的 SourceInventory、固定隔离审查、处置映射、七能力 Capability Matrix 与运行时外部来源隔离。七个文本 StageRelease 已由现有 SceneAnalysis 服务真实消费和持久化；视觉 Stage 已进入 Backend Registry、DefinitionCore 与 StageRelease 内容身份，Backend 可从当前正式 StoryGraph 机械取得 Gate 2 roots，从当前 Bible Head 沿 exact Candidate Revision 重读已确认来源，并将同 Project roots、来源和精确策展 Release 编译成严格 faithful Agent Input；也可按冻结对象声明读取和传输媒体，Agent 可验签、严格校验图片并返回 AttemptResult。不表示视觉 Stage 已接入持久调度服务或 Workflow，也不表示可由产品触发、十三 Stage Core/StageSet、完整 SkillRelease/Control、rewrite eval/shadow/signature、项目 Preset 选择/真实 stale 写入或 Gate 3 已实现。
- Git 基线/提交：本 Visual Foundation 合同切片基线 `0990b2ca1b19cab113de62f3af0b26c4d65cc24d`；SourceMapping 实现提交 `0100fea72a6a7cf82eab8f42dfcf41740a7cb71c`，Preset 守恒合同实现提交 `a795b2da219d40b01e8b8f77863b4a0b7f4b613c`，Preset 切换影响闭包实现提交 `8ed4ed811b3d7af08f3a78a0d8d21c53fab3ab8f`，Visual Foundation 合同实现提交 `7df2903142d1344fc2a4feff1295743c03c99ba2`，图片输入校验实现提交 `093ad57fc3c05de1f3041b1fbffee7842374d39b`，Visual Foundation Harness 实现提交 `77b1b8965f1308506662adbb918093aba344fdb7`，媒体预算实现提交 `463178b639b2fd34c37c77420a6eeeca188dbb32`，Invocation 合同实现提交 `fe59b377c68a4f8d913b555484e19c3a1a35949a`，授权与结果合同实现提交 `f2e10445769a2675a8e3d3e19443fd329c2e2d1c`，视觉 Stage 注册实现提交 `19e13b78c124bab457757f2be659c006a08a361e`，Agent 媒体调用实现提交 `5f8232fa8af503e2f969e900958b8b933b9f46d7`，Backend 媒体传输实现提交 `1fdc5d8e968f15c5a0d7f1167e5b5ade295cbd7d`，首批策展 Preset 目录实现提交 `5fb2c9aeef8e65fafcd0953030d774e038dddc42`，外部能力吸收边界实现提交 `89679cd80e9a2e044fec4554f5250ba3193100f4`，视觉世界读集实现提交 `540a4b040aa5d178a08af817a5b21b8f0b17ea06`，Gate 2 视觉候选来源实现提交 `4bbc0c817c643e6b8b50ae99678dccba2852f101`，faithful 视觉输入编译实现提交 `d2113a9333b6fa297416e572aac8e2b22c68a5ef`。
...
- 未覆盖条件与残余风险：PresetRelease 守恒合同、4 个内置策展 Release、PresetSwitchImpact 确定性合同、Visual Foundation 严格候选/Invocation/Authorization/AttemptResult 合同、Agent vision Harness/私有媒体调用、Backend 媒体 Broker/Client、八变体 Registry/DefinitionCore/StageRelease 及一次真实本机 Codex 图片 canary 已具备，但持久 Preset Owner/API、项目 Preset 选择、真实 stale 写入、style-blind mutation、调用 Broker 的 Backend 持久 Invocation/Attempt/Result 服务、真实 MinIO→Backend→Agent 图片旅程、Candidate 结果持久化、Reference Plan、expected target set 与 Gate 3 尚未实现，因此仍只验收 `VPA-VIS-002`；`VPA-VIS-001`、`VPR-PRE-009` 保持未勾选，内置目录也不表示视觉阶段已可由产品运行。八个 StageRelease、三个已采用参考的 SourceInventory、隔离安全审查、当前处置映射、七能力 Matrix 与运行时外部来源隔离已具备；但没有来源获准进入 rewrite，故当前 Matrix 合法为空，Golden/Adversarial Eval、Shadow 和独立签名没有合格输入且尚未形成完整吸收链，`VPA-SUP-005` 保持未勾选。其余五个未实现生产 Stage 的严格 Input/Candidate Schema、十三 Stage 完整 Core/CandidateStageSet、正式 SkillRelease 与追加式 Control 同样未实现。当前服务使用的最小 Skill/Control 身份尚未达到完整发布批准合同，不能冒充设计中的批准链；最终 `agent-browser` 仍只在全部开发完成后执行。
