...
- ChangeRequest 合同阶段证据：`TestProductionWorldChangeRequestMatchesFrozenTargetEvidenceAndClosure` 证明 request 必须命中冻结 target、operation-specific reason、闭包内 Evidence 和服务器完整闭包，扩大 scope、伪造 Evidence、错型 target 或无审核依据均被拒绝；`TestProductionWorldRepairRootFollowsTheTypedOperation` 证明 root 只能落到三个既有制作阶段之一。该内部合同仍未打开 HTTP 决议。
