...
| A 手工事实主线 | M01—M05、M07—M09、M11、M14 | PRD-A | PLAN-A | 进行中：DOCX/Markdown/TXT 原件保全、确定性 ParseReport、剧本解析草稿/批准、canonical GORM 物化、人物×集数与资产投影、内存 Access Token/HttpOnly refresh 已可用并完成 production 浏览器闭环；会话/路径/直接表租户边界、Worker 消息租户/outbox 绑定、逻辑 Artifact/精确对象版本分离及 Fixture 顺序幂等已落地，媒体授权负向验收、刷新后项目恢复、边界手工修订、检索、完整镜头与交付预演待实现 |
