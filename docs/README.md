...
|---|---|---|---|
| `SG-D01` | [0010 StoryGraph 总设计](design/0010-StoryGraph内容图与DAG创作画布设计.md) | **已完成（2026-08-27；通用媒体 Provider/Shot 视频关系 2026-08-29 同步）**：用户已接受 StoryGraph、DAG、角色/地点视觉、四图与图片/视频生产关系边界 | 当前只作为总设计事实；文档门禁已推进到 `SG-D17` |
...
| `SG-D04` | [0001 完整设计基线](design/0001-AI短剧制作平台完整设计基线.md) | **已完成（2026-08-27；日志拓扑 2026-08-28、通用媒体 Provider 2026-08-29 同步）**：平台主干收口为 StoryGraph 与四图边界，并固定 Kafka 业务异步解耦、剧本/StoryGraph 检索、Backend → Logstash 日志链路和可空通用媒体 Provider | 仅解锁 `SG-D05`；不先改其他子模块 |
...
| `SG-D06` | [0003 系统总体架构](design/0003-系统总体架构.md) | **已完成（2026-08-27；日志拓扑 2026-08-28、通用媒体 Provider 2026-08-29 同步）**：重建 StoryGraph/四图系统图、两条 Compiler 链、单 Writer、Kafka 业务检索、直连 Logstash 日志和媒体 Provider 配置/执行事实边界 | 仅解锁 `SG-D07`；不把 StoryGraph 与 WorkflowDefinition 合并 |
...
| `SG-D09` | [2001 Backend 服务架构](design/2001-后端服务架构.md) | **已完成（2026-08-27；通用媒体 Provider 2026-08-29 同步）**：固定单 Backend/GORM Catalog、Temporal、私有 Agent、Provider Registry/Secret Resolver、Kafka/Search/ELK 目标与当前缺口 | 仅解锁 `SG-D10`；不引入 Migration、Raw SQL、第二 ORM 或第二 Writer |
...
| `SG-D17` | [0010 StoryGraph PRD](prd/0010-StoryGraph内容图与DAG创作画布产品需求.md) | **已重新接受（2026-08-29）**：Web Provider 配置、四类必接 Adapter、图片/视频 Binding 与真实模型验收均进入当前 MVP | 只解锁 `SG-D18` Requirement；仍不编码 |
| `SG-D18` | [0010 跨服务 Requirement](requirement/0010-StoryGraph内容图与DAG创作画布需求规格.md) | **已重新接受（2026-08-29）**：固定 Provider 配置/密钥/单次调用事实、三类图片协议、Seedance 视频协议、Web 设置与图片/视频 Binding 契约 | 只解锁 `SG-D19` Agent Requirement 复核；仍不编码 |
| `SG-D19` | [3003 Agent Contract Requirement](requirement/3003-StoryGraph剧本解析Harness与内置Skill需求规格.md) | **已复核接受（2026-08-29）**：Skill/Bundle、Wire、Stage/Shard、Candidate Revision、Codex CLI 与恢复契约不变；媒体 Provider Secret/Adapter/调用明确不进入 Agent | 只解锁 `SG-D20` 唯一总 Plan；仍不编码 |
...
| `VP-D02` | [0006 领域语言](design/0006-领域语言与模块命名规范.md) | **当前待同步**：固定 Scene Fact、Interaction、Reference Plan、组合 Binding 与 Scene Production Packet 规范名 | 不修改其他 Design，不创建 Wire/数据库对象 |
| `VP-D03` | [0001 完整设计基线](design/0001-AI短剧制作平台完整设计基线.md) | 待 `VP-D02` 完成后同步产品主链、Preset Owner 与 MVP 边界 | 不先改 StoryGraph 总设计 |
...
| `VP-D07` | [3003 Agent/Harness](design/3003-StoryGraph剧本解析Harness与内置Skill设计.md) | 待 `VP-D06` 完成后定义 `storygraph-stage-wire-v2`、固定 Skill Release 与新 Stage DAG | 不修改 Go/Python Wire 或 Bundle |
| `VP-D08` | [3002 Storyboard Harness](design/3002-本地-Codex-分镜智能体执行框架设计.md) | 待 `VP-D07` 完成后固定 Scene Production Packet 与视觉资产前置依赖 | 不改 Draft/Detail 合同 |
| `VP-D09` | [2002 Backend 领域设计](design/2002-后端领域模块功能设计.md) | 待 `VP-D08` 完成后固定 Owner、Record、Command、Receipt 和 StoryGraph Compiler 边界 | 不创建 GORM Model/API |
| `VP-D10` | [2051 Generation](design/2051-通用媒体Provider与Generation执行器设计.md) | 待 `VP-D09` 完成后增加 `approved_reference_plan` 严格 Target 来源与组合资产输出 | 不改 Generation Target |
| `VP-D11` | [2055 Human Gate](design/2055-Workflow公共HumanGate命令与恢复设计.md) | 待 `VP-D10` 完成后映射五个用户 Gate 的正式 Subject/Owner Apply | 不改 Workflow Definition |
| `VP-D12` | [1002 Frontend 功能](design/1002-前端功能模块设计.md) | 待 `VP-D11` 完成后固定 Guided Studio、状态时间线、覆盖矩阵和影响抽屉 | 不实现新页面 |
| `VP-D13` | [0010 唯一 PRD](prd/0010-StoryGraph内容图与DAG创作画布产品需求.md) | 待全部受影响 Design 重新接受后重排 MVP 产品范围 | 不派生 Requirement |
| `VP-D14` | [0010 Requirement](requirement/0010-StoryGraph内容图与DAG创作画布需求规格.md) 与 [3003 Agent Requirement](requirement/3003-StoryGraph剧本解析Harness与内置Skill需求规格.md) | 待 PRD 接受后建立跨服务与 Agent 可测试合同 | 不写 Plan |
| `VP-D15` | [0010 唯一 Plan](plan/0010-StoryGraph内容图与DAG创作画布实施计划.md) 与 [Acceptance](acceptance/0010-StoryGraph内容图与DAG创作画布验收标准.md) | 待 Requirement 接受后重排 P0–P4，所有新目标初始为 `[ ]` 并独立接受 | 不编码；历史 `SG-Ixx` 证据不抵扣 |
| `VP-I01` | P0 Scene Fact v2 首个垂直切片 | 只能从已接受 Plan 领取，执行 Red → Green → Refactor、定向验证、当时全量 CI、Evidence 与独立提交 | 不提前实现 Interaction、Preset 或媒体链 |
...
| `2051` | 通用媒体 Provider 与 Generation 执行器 | — | [通用媒体 Provider 与执行器设计](design/2051-通用媒体Provider与Generation执行器设计.md) | — | — | — | Backend 配置事实与 Secret Store 已验收，当前进入精确 ProviderCall/Receipt 实施 |
| `2055` | Workflow 公共 Human Gate 命令与恢复 | — | [公共 Human Gate 设计](design/2055-Workflow公共HumanGate命令与恢复设计.md) | — | — | — | `SG-D16` 已完成三阶段状态、八类显式 Owner Gate、Resume 与 Kafka/ELK 边界同步；等待统一派生文档 |
