...
| `SG-D17` | [0010 StoryGraph PRD](prd/0010-StoryGraph内容图与DAG创作画布产品需求.md) | **待重新同步（2026-08-29）**：把 Web Provider 配置、四类必接 Adapter、图片/视频完整旅程纳入当前 MVP | 未重新接受前不更新 Requirement、不编码 |
| `SG-D18` | [0010 跨服务 Requirement](requirement/0010-StoryGraph内容图与DAG创作画布需求规格.md) | **待重新同步（2026-08-29）**：重写 Runware/环境变量目标合同并增加精确 Provider 恢复、Secret、视频与 Binding 契约 | `SG-D17` 未完成前不开始 |
| `SG-D19` | [3003 Agent Contract Requirement](requirement/3003-StoryGraph剧本解析Harness与内置Skill需求规格.md) | **已完成（2026-08-27；待 `SG-D18` 后复核）**：固定 Skill 迁移/Bundle、Wire、Stage/Shard、Candidate Revision、Codex CLI 与恢复契约；媒体 Provider 变更不把 Secret/Adapter 职责移入 Agent | `SG-D18` 完成后确认无新增 Agent 合同，才解锁 `SG-D20` |
| `SG-D20` | [0010 唯一总 Plan](plan/0010-StoryGraph内容图与DAG创作画布实施计划.md) | **待重新同步（2026-08-29）**：保持 `SG-I01`–`SG-I19` 历史完成事实，重排剩余 Provider、图片、视频、Canvas 与最终验收任务 | `SG-D18` 未完成且 `SG-D19` 未复核前不开始；不得从旧 `SG-I20` 领取代码 |
| `SG-D21` | [0010 Acceptance Criteria](acceptance/0010-StoryGraph内容图与DAG创作画布验收标准.md) | **待重新同步（2026-08-29）**：保留历史 Evidence，新增目标 Requirement/Task Checklist 且初始全为 `[ ]` | 重新接受前不恢复编码；历史 Runware 证据不抵扣新目标 |

`SG-D01`–`SG-D21` 曾于 2026-08-27 依次接受，`SG-I01`–`SG-I19` 已通过各自完成门并分别回填 Evidence；2026-08-29 用户将 `2051` 从固定 Runware/环境变量改为可空启动、Backend 管理、Web 配置的通用媒体 Provider 服务，要求参考 CC-Switch 用内置预设实例化跨重启持久连接，并明确 Seedream 5.0 Pro+、Seedance 2.0+、GPT Image 2 与 Nano Banana 当前四个官方模型都是当前必接范围而不是计划或预留。`SG-D13` 已于 2026-08-29 重新接受，受影响 Design 同步完成；当前只解锁 `SG-D17` PRD 的重新同步，编码仍暂停。既有 Runware Target、Job、Stager、CandidateSet 和公开 Binding 命令只作为当前事实与重构输入，不抵扣新设计验收，也不形成兼容要求。必须继续按 `SG-D17`–`SG-D21` 更新统一 PRD/Requirement/Plan/Acceptance 并重新接受后才恢复编码；不得越过门禁领取旧 `SG-I20` 或提前实现 `shot_frame`、视频、Selection、AssetVersion 与 `agent-browser`。`plan/0007`、`plan/0008` 只表达未来 Platform Complete 目标，不是当前 StoryGraph 执行入口；旧 `3001/3002` PRD、Requirement、Plan 与 Acceptance 持续冻结。任一任务完成、回填 Evidence、通过当时全量 CI 并独立提交后才解锁下一项。
...
| `2051` | 通用媒体 Provider 与 Generation 执行器 | — | [通用媒体 Provider 与执行器设计](design/2051-通用媒体Provider与Generation执行器设计.md) | — | — | — | `SG-D13` 已重新接受并同步受影响 Design；等待 `SG-D17`–`SG-D21` 重新同步，编码仍暂停 |
