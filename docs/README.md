...
|---|---|---|---|
| `SG-D01` | [0010 StoryGraph 总设计](design/0010-StoryGraph内容图与DAG创作画布设计.md) | 用户单独接受 StoryGraph、DAG、角色/地点视觉与四图边界 | 不接受 `3003`，不移动 Skill，不编码 |
| `SG-D02` | [3003 Agent/Harness 子设计](design/3003-StoryGraph剧本解析Harness与内置Skill设计.md) | 只在 `SG-D01` 接受后，单独接受 Bundle、Stage、Shard、Candidate Revision 和 Codex 边界 | 不以 Agent 子设计反向改写 `0010` |
| `SG-D03` | [0006 领域语言](design/0006-领域语言与模块命名规范.md) | 先固定 StoryGraph、Asset/State/Version、Claim、Occurrence 和 Binding 规范名 | 下游 Design 不得自创同义词 |
...
`SG-D01` 是当前唯一激活步骤。`plan/0007`、`plan/0008` 只表达未来 Platform Complete 目标，不是当前 StoryGraph 执行入口；`SG-D11/SG-D12` 只同步两份 Design，旧 `3001/3002` PRD、Requirement、Plan 与 Acceptance 持续冻结，由 `SG-D17`–`SG-D21` 统一文档链取代。在 `SG-D20/SG-D21` 通过前，`0007/0008/1001/2002/3001/3002` 旧 Plan 中与 StoryGraph、Canvas、新 Human Gate、Agent Bundle 或视觉资产重叠的 Checklist 一律冻结，`2051/2055` 也不得绕过 `SG-D13/SG-D16` 进入编码。唯一代码实施顺序由 `0010` 的 `SG-Ixx` 维护，`3003` 只做 Agent 子任务映射。
...
| `0010` | StoryGraph 内容图、DAG 与视觉资产 | — | [内容图与 DAG 创作画布设计](design/0010-StoryGraph内容图与DAG创作画布设计.md) | — | — | — | Design 提案待用户接受，尚未派生或实施 |
...
| `3003` | StoryGraph 剧本解析 Harness 与内置 Skill | — | [Harness 与内置 Skill 设计](design/3003-StoryGraph剧本解析Harness与内置Skill设计.md) | — | — | — | 已冻结；仅在 `SG-D01` 接受后激活 `SG-D02` 评审 |
