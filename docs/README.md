...
| `SG-D04` | [0001 完整设计基线](design/0001-AI短剧制作平台完整设计基线.md) | **已完成（2026-08-27；日志拓扑 2026-08-28 修订）**：平台主干收口为 StoryGraph 与四图边界，并固定 Kafka 业务异步解耦、剧本/StoryGraph 检索和 Backend → Logstash 日志链路 | 仅解锁 `SG-D05`；不先改其他子模块 |
...
| `SG-D06` | [0003 系统总体架构](design/0003-系统总体架构.md) | **已完成（2026-08-27；日志拓扑 2026-08-28 修订）**：重建 StoryGraph/四图系统图、两条 Compiler 链、单 Writer、Kafka 业务检索与直连 Logstash 日志事实边界 | 仅解锁 `SG-D07`；不把 StoryGraph 与 WorkflowDefinition 合并 |
...
| `SG-D13` | [2051 通用媒体 Provider](design/2051-通用媒体Provider与Generation执行器设计.md) | **待重新接受（2026-08-29）**：固定 CC-Switch 风格内置预设、Web 创建持久连接、加密凭据，并强制真实接入 Seedream 5.0 Pro+、Seedance 2.0+、GPT Image 2 与 Nano Banana 当前四个官方模型 | 设计重新接受并同步下游文档前暂停编码；四类 Provider 均为完成条件，不保留 Runware 兼容入口 |
...
| `2051` | 通用媒体 Provider 与 Generation 执行器 | — | [通用媒体 Provider 与执行器设计](design/2051-通用媒体Provider与Generation执行器设计.md) | — | — | — | `SG-D13` 因 Preset/持久连接/Provider/Web/Secret/视频边界变更待重新接受；实现暂停 |
| `2055` | Workflow 公共 Human Gate 命令与恢复 | — | [公共 Human Gate 设计](design/2055-Workflow公共HumanGate命令与恢复设计.md) | — | — | — | `SG-D16` 已完成三阶段状态、七类显式 Owner Gate、Resume 与 Kafka/ELK 边界同步；等待统一派生文档 |
