...
| `SG-D19` | [3003 Agent Contract Requirement](requirement/3003-StoryGraph剧本解析Harness与内置Skill需求规格.md) | **当前唯一激活（2026-08-29）**：复核 Skill/Bundle、Wire、Stage/Shard、Candidate Revision、Codex CLI 与恢复契约，确认媒体 Provider Secret/Adapter/调用不进入 Agent | 完成复核前不开始 `SG-D20`、不编码 |
| `SG-D20` | [0010 唯一总 Plan](plan/0010-StoryGraph内容图与DAG创作画布实施计划.md) | **待重新同步（2026-08-29）**：保持 `SG-I01`–`SG-I19` 历史完成事实，重排剩余 Provider、图片、视频、Canvas 与最终验收任务 | `SG-D19` 未复核前不开始；不得从旧 `SG-I20` 领取代码 |
...
| `2051` | 通用媒体 Provider 与 Generation 执行器 | — | [通用媒体 Provider 与执行器设计](design/2051-通用媒体Provider与Generation执行器设计.md) | — | — | — | Design/PRD/Requirement 已重新接受；等待 `SG-D19`–`SG-D21`，编码仍暂停 |
