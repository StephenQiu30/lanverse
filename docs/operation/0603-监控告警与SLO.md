...
| 上游 | [0202 §1、§2、§3、§8](../requirement/0202-非功能需求规格.md)、[0304 工作流](../design/0304-工作流与生成操作.md)、[0601 环境与部署](0601-环境与部署.md)；功能 [F32 系统健康视图](../requirement/features/F32-系统健康视图.md) |
...
所有日志、指标、链路共享 `trace_id`、`project_id`、`operation_id` 维度（0202 OBS-01、OBS-02）；日志脱敏见 0307 §14。

## 2. SLI 与 SLO

服务时段：每日 9:00–24:00（0202 AVL-01）；窗口：自然月。
...
**错误预算策略**：月度可用性错误预算约 0.5% × 465 小时 ≈ 2.3 小时。当月消耗 > 50% 时暂停非紧急功能发布，优先修复稳定性问题；> 100% 时只允许修复类发布，并做复盘。重复计费与账本差异没有预算，出现即 S1（0402 §7）。
...
产品内的“系统健康视图”（F32）读取其中关键指标，阈值与本文告警一致。
...
| 告警 | 条件 | 级别 | 处置入口 |
| --- | --- | --- | --- |
| 服务不可用 | 外部拨测连续 3 次失败 | P1 | 0604 §5 RB-01 |
...
| 备份失败 | 最近一次全量或 WAL 归档失败 | P1 | 0604 §2 |
...
| 渲染失败 | 1 小时内失败 ≥ 3 | P2 | RB-09 |
| 预算超支 | 任一项目 `is_overrun = true` | P2 | 产品负责人 |
| 证书即将过期 | < 14 天 | P3 | — |
| 磁盘 / 内存 | 主机 > 85% | P3 | — |
| 依赖漏洞 | 每日扫描出现新的高危 | P3 | 0602 §9 |
