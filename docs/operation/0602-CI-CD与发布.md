...
| 上游 | [AGENTS.md](../../AGENTS.md)、[PROJECT.md §9](../../PROJECT.md)、[0202 §9、§13](../requirement/0202-非功能需求规格.md)、[0501 测试策略](../test/0501-测试策略.md)、[0503 AI 评测](../test/0503-AI评测方案.md)、[0601 环境与部署](0601-环境与部署.md) |
...
4. 迁移向前兼容一个版本，发布可在 15 分钟内回滚（0202 OPS-03）。
...
PR / push 到分支
  ...
  ├─ docs    ：链接与锚点校验、需求 / 非功能 / 功能编号校验、0502 与功能文件一致
  ├─ security：gitleaks → 依赖许可检查
  └─ e2e-smoke：Compose 启动全部组件 + 模拟供应商 → Playwright 冒烟（0501 §6）
合并到 main
...
| 单元、集成、工作流回放、前端组件测试 | 是 | 0501 §2 |
| Race Detector | 是 | AGENTS.md |
| 漏洞扫描（高危） | 是 | govulncheck、pnpm audit、pip-audit、Trivy |
| 生成物一致（wire、swag、OpenAPI 客户端、契约模型） | 是 | 0202 MNT-03 |
| 迁移可在空库与上一版本库执行 | 是 | 0302 §10 |
| Secret 扫描 | 是 | 0307 §13 |
| 文档编号与链接校验 | 是（docs 变更时） | 0402 §4.2 |
| AI 评测门禁 | 是（Skill / 模型变更时） | 0503 §6 |
| E2E 冒烟 | 是 | 0501 §6 |
| 覆盖率 | 否（核心包 < 80% 时评论提示） | 0202 MNT-02 |
...
6. 发布后冒烟 + 观察 30 分钟（错误率、延迟、队列积压、unknown 数；0603 看板）
7. 记录发布：版本、sha、迁移、执行人、结果（GitHub Release 或部署记录）
```

发布窗口：工作日 10:00–17:00，避开批量生成高峰；计划停机类变更按 0202 AVL-02 提前 24 小时通知。
...
| 对象 | 规则 |
| --- | --- |
| Temporal 工作流 | 修改已发布工作流的逻辑必须用 `workflow.GetVersion` 分支；回放测试通过（0304 §14）；删除旧分支前确认无在途执行 |
| Activity 契约 | 只做向后兼容变更（加可选字段）；破坏性变更新建 Activity 名（`provider.submit.v2`），旧名保留至在途执行结束 |
| Kafka 事件 | 加字段不升版；语义变化发布 `.v2` 并双写一个发布周期（0303 §6.1） |
| 公共 API | 加字段不升版；删除 / 改语义先标记废弃一个版本 |
| 前端与 API | 前端先兼容新旧响应；部署顺序为后端先、前端后 |
| Worker 部署 | 新旧 Worker 同时存在期间，靠上述版本分支保证兼容；滚动更新后旧 Worker 优雅退出（0304 §2） |
...
| 数据损坏 | 按 0604 恢复流程 | RTO ≤ 4 小时 |

## 8. 功能开关

- 需要分阶段启用的能力（新模型、V1 功能、境外出口）使用简单开关：`workspace.feature_flag`（组织级）或模型注册表的 `status`；开关变更写审计。
- 不引入第三方开关平台；开关在功能稳定后一个版本内移除。

（0302 同步项：新增 `workspace.feature_flag`：`key`、`enabled`、`org_id`，含必备列；MVP 可暂不建表，首次需要时通过迁移加入。）
...
- 基础镜像固定摘要；FFmpeg、libvips 版本升级需运行媒体黄金样例测试（0501）。

## 10. CI 中的密钥

- GitHub Actions 只保存：镜像仓库推送凭据、staging / production 部署凭据（Environment secrets，production 需审批）。
- CI 不持有任何模型供应商凭据；需要真实调用的评测由有权限的人手动触发，使用评测专用凭据与预算（0503 §7）。
