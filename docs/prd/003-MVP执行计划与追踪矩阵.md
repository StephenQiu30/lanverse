...
执行以 S0–S6 为排序轴，以 PRD-007 至 PRD-009 的 PT（Product Task）为产品范围与接受单位，以 PLAN-000 的 `DEV-*` 为工程排期、领取和完成单位；WP 只负责组合 PT/DEV，不再充当模糊任务。一个时间点只允许一个主切片处于本地工程实施状态；仅等待外部 API Key/额度的上游 Provider 验收作为发布门禁跟踪，不占用本地工程在制品，也不能被后续完成状态覆盖。每项 PT 采用 `确认范围 → 写失败验收/测试 → 最小实现 → OpenAPI/client → 前端闭环 → 真实依赖验证 → 产品接受`，不按“先做完后端所有表、再做前端”推进。
...
| D-004 | open（公开契约已收窄） | Image/Video Provider、参数、取消/查询能力和计价 | 产品/AI | S4 | 2026-07-31 已复核官方 SDK 5.0.43 的 `AsyncArk`、图片与视频 task 资源；公开 SDK 不证明模型开通。D-004 仍保持 open，唯一候选仍为北京地域的 `doubao-seedream-5-0-lite-260128`/`doubao-seedance-2-0-260128`；待真实账号确认权限/额度/并发、逐参数、取消/回调竞态和账单。当前不配置 `ARK_API_KEY`，不安装 SDK；AI Speech/Audio 不在范围 |
| D-005 | closed | 上线地区、授权模板、审核责任、内容检查、标识和留存 | 产品/合规 | PT-GOV-001/002 与 S2 accepted；S4 真实生成；S5 公开交付 | 2026-07-30 产品负责人确认 [DES-005 中国大陆工程基线](../design/005-S2供应商与合规准入设计.md)：面向中国大陆公众首发；产品负责人承担合规责任角色；无标识导出不进入 MVP；未成年人材料默认阻断；接受五个脱敏 fixture 及逐类最短必要删除/匿名化触发条件。该决策解锁 GOV-001/002 实现，不替代 S4/S5 真实 Provider、内容检查、标识样片及上线材料验收 |
| D-006 | closed | 默认复用本机 PostgreSQL/Redis/RabbitMQ，仅 MinIO 使用 Docker；完整环境仍可由根 `docker-compose-env.yml` 显式启动，业务由 `docker-compose.yml` 启动 | 开发负责人 | DEV-S0-04/S0 退出；S4 | 2026-07-29 实机复核四依赖 ready；日常只执行 `make minio-up`，完整 Compose 是隔离兜底；凭据只由本地 secret 注入 |
...
根 Makefile 由本地/CI 共用：`setup`、`dev-api`、`dev-frontend`、`scheduler`、`db-init`、`generate-api`、`lint`、`typecheck`、`test`、`check`、`docker-build`、`services-up/down`、`env-up/down`、`contract-minio`、`contract-rabbitmq`、`e2e-install`、`e2e`。外部契约命令只在对应真实服务进入业务后增加，缺少服务或凭据时快速失败，不进入默认 `check`；Worker 出现真实入口后才增加对应命令，不创建占位目标。
