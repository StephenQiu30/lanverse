...
| D-005 | open | 上线地区、授权模板、审核责任、内容检查、标识和留存 | 产品/合规 | PT-GOV-001/002 与 S2 accepted；S4 真实生成；S5 公开交付 | [DES-005 中国大陆候选基线与关闭清单](../design/005-S2供应商与合规准入设计.md)：脱敏授权 fixture、契约测试和留存/删除候选矩阵已形成；缺首发地区、责任人、模板/策略签认及最终期限或删除触发条件。未关闭不阻止 Task/剧本/私有媒体工程，但所有生成与交付 fail-closed |
...
根 Makefile 由本地/CI 共用：`setup`、`dev-api`、`dev-frontend`、`scheduler`、`db-init`、`generate-api`、`lint`、`typecheck`、`test`、`check`、`docker-build`、`business-up/down`、`env-up/down`、`contract-minio`、`contract-rabbitmq`、`e2e-install`、`e2e`。外部契约命令只在对应真实服务进入业务后增加，缺少服务或凭据时快速失败，不进入默认 `check`；Worker 出现真实入口后才增加对应命令，不创建占位目标。
