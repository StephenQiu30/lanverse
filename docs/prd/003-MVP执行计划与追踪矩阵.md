...
| D-003 | blocked | Text Provider、模型、参数、单价和测试额度 | 产品/AI | S2 | [DES-005 候选基线与关闭清单](../design/005-S2供应商与合规准入设计.md)：失败契约已由 `ee2cfdc` 实现并通过 CI；本地密钥已读取但北京按量端点仍返回 401，缺匹配账号/Base URL、成功脱敏契约、额度/账单证据及负责人接受记录 |
| D-004 | open | Image/Video/Speech Provider、参数、取消/查询能力和单价 | 产品/AI | S4 | 每类真实 API 契约和费用样例 |
| D-005 | blocked | 上线地区、授权模板、审核责任、内容检查、标识和留存 | 产品/合规 | S2/S5 | [DES-005 中国大陆候选基线与关闭清单](../design/005-S2供应商与合规准入设计.md)：脱敏授权 fixture、契约测试和留存/删除候选矩阵已形成；缺首发地区、责任人、模板/策略签认及最终期限或删除触发条件 |
...
根 Makefile 由本地/CI 共用：`setup`、`dev-api`、`dev-frontend`、`db-init`、`generate-api`、`lint`、`typecheck`、`test`、`check`、`docker-build`、`business-up/down`、`env-up/down`、`contract-minio`、`contract-text-provider`、`e2e-install`、`e2e`。外部契约命令必须显式执行，缺少真实服务或凭据时快速失败，不进入默认 `check`；Scheduler/Worker 出现真实入口后才增加对应命令，不创建占位目标。
