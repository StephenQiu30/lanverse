...
根 Makefile 由本地/CI 共用：`setup`、`dev-api`、`dev-frontend`、`scheduler`、`worker-io`、`worker-media`、`db-init`、`generate-api`、`lint`、`typecheck`、`test`、`check`、`docker-build`、`docker-dev-up/down/logs`、`docker-prod-up/down/logs`、`minio-up/down`、`contract-minio`、`contract-rabbitmq`、`e2e-install`、`e2e`。外部契约命令只在对应真实服务进入业务后增加，缺少服务或凭据时快速失败，不进入默认 `check`；六服务定义只存在于共享基础 Compose，生产文件只维护必要差异。
