...
执行以 S0–S6 为排序轴，以 PRD-007 至 PRD-009 的 PT（Product Task）为排期、领取和接受单位；WP 只负责组合 PT，不再充当模糊任务。一个时间点只允许一个主切片处于实施状态，每项 PT 采用 `确认范围 → 写失败验收/测试 → 最小实现 → OpenAPI/client → 前端闭环 → 真实依赖验证 → 产品接受`，不按“先做完后端所有表、再做前端”推进。

本文件是产品级执行分解，不替代代码级 Plan。[PLAN-000](../plan/000-MVP全栈实施总计划.md) 统一跨 PRD 的全栈实施基线，[PLAN-003](../plan/003-MVP工作包与交付门禁执行计划.md) 承接本 PRD 的具体编排；只有本 PRD 被接受、当前切片准入条件关闭后，对应 DEV/PT 任务才可进入实施。

## 2. 决策与准入清单

| 编号 | 必须决策 | 责任角色 | 阻塞切片 | 关闭证据 |
| --- | --- | --- | --- | --- |
| D-001 | 接受 REQ/DES/PRD 的 MVP 范围与非目标 | 产品负责人 | S0 | 文档状态与评审记录 |
| D-002 | OIDC issuer、audience、测试租户、登出/恢复入口 | 产品/后端 | S1 | 可用测试租户与令牌验证样例 |
| D-003 | Text Provider、模型、参数、单价和测试额度 | 产品/AI | S2 | 采购/测试结论与能力快照 |
| D-004 | Image/Video/Speech Provider、参数、取消/查询能力和单价 | 产品/AI | S4 | 每类真实 API 契约和费用样例 |
| D-005 | 上线地区、授权模板、审核责任、内容检查、标识和留存 | 产品/合规 | S2/S5 | 适用规则清单与责任人确认 |
| D-006 | RabbitMQ/MinIO 本机或显式 Compose 方式、非默认凭据和测试入口 | 开发负责人 | S0/S4 | 连接预检及真实契约测试可运行 |
| D-007 | 固定性能环境、验收剧本、供应商预算和故障演练窗口 | 产品/开发/QA | S4/S6 | 可重复 fixture、环境说明和预算 |

未关闭的决策允许做不依赖它的测试和内部契约，但不得把被阻塞切片标记完成。任何密钥只进入本地 secret 配置，不进入决策文档或 Git。
...
| --- | --- | --- | --- | --- |
| WP-00 | 全局 | 当前切片 D-001–007 | 用户确认 | 所需决策关闭，任务状态可进入 ready，密钥不入库 |
| WP-01 | S0 | PT-DAT-001/003、PT-OBS-001 | D-001 | setup 无污染、空库结构/租户约束、日志基线和架构测试通过 |
| WP-02 | S0 | PT-STO-001；Redis/RabbitMQ 连接基线 | D-006、WP-01 | PG/Redis/RabbitMQ/MinIO 就绪准确，OpenAPI client 无漂移 |
| WP-03 | S1 | PT-IDN-001–004、PT-PRJ-001–003 | D-002、WP-02 | 真实登录、空间隔离、Project/Episode 生命周期闭环 accepted |
...
6. Frontend：ViewModel 读取服务端状态与 next_action；Zustand 只保留未提交草稿/选择/布局；页面和测试分别进入规定目录。
...
一个 PT 只有满足以下条件才进入 ready：所属 Design/PRD 已接受；PRD-010 中所有对应叶子均有 Design 和验收归属；依赖 PT 已 accepted；相关决策关闭；用户结果、范围、非目标、事实所有者和交接明确；验收中的成功、失败、权限、并发、恢复 fixture 可执行；样例不含真实个人数据或密钥。WP 不能绕过其中任一 PT 的准入条件。
