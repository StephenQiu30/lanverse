...
| D-003 | blocked | Text Provider、模型、参数、单价和测试额度 | 产品/AI | S2 | [DES-005 候选基线与关闭清单](../design/005-S2供应商与合规准入设计.md)：本地密钥已读取但北京按量端点返回 401；缺匹配账号/Base URL、成功脱敏契约及额度/账单证据 |
| D-004 | open | Image/Video/Speech Provider、参数、取消/查询能力和单价 | 产品/AI | S4 | 每类真实 API 契约和费用样例 |
| D-005 | blocked | 上线地区、授权模板、审核责任、内容检查、标识和留存 | 产品/合规 | S2/S5 | [DES-005 中国大陆候选基线与关闭清单](../design/005-S2供应商与合规准入设计.md)：缺首发地区、责任人、授权模板和留存表确认 |
| D-006 | closed | 优先复用本机服务；完整环境由根 `docker-compose-env.yml` 显式启动，业务由 `docker-compose.yml` 启动，凭据只由本地 secret 注入 | 开发负责人 | DEV-S0-04/S0 退出；S4 | 2026-07-29 按用户决策移除 deploy 目录；PostgreSQL/Redis/RabbitMQ/MinIO 与 api/web 分文件管理；RabbitMQ、MinIO 使用已核验 digest，真实连接及契约测试仍是 DEV-S0-04 退出证据 |
