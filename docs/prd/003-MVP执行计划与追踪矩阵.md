...
| --- | --- | --- | --- | --- | --- |
| D-001 | open | 接受 MVP 范围/非目标、固定技术基线与 S0 工程边界 | 产品负责人/工程负责人 | S0 | REQ-001、DES-000/002、PRD-001/002/003 的 S0 范围评审记录与对应 commit |
| D-002 | open | OIDC issuer、audience、测试租户、登出/恢复入口 | 产品/后端 | S1 | 可用测试租户与令牌验证样例 |
...
| D-006 | open | RabbitMQ/MinIO 复用本机显式地址；缺失时使用 Compose profile，凭据只由本地 secret 注入 | 开发负责人 | DEV-S0-04/S0 退出；S4 | provisioning 方式、端口/env 契约与 `docker info`；真实连接及契约测试是 DEV-S0-04 退出证据 |
...
| WP-01 | S0 | PT-DAT-001/003、PT-OBS-001 | D-001（S0 基线） | setup 无污染、空库结构/租户约束、日志基线和架构测试通过 |
