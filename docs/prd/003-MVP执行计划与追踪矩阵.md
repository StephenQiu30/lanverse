...
根 Makefile 由本地/CI 共用：`setup`、`dev-api`、`dev-frontend`、`db-init`、`generate-api`、`lint`、`typecheck`、`test`、`check`、`docker-build`、`business-up/down`、`env-up/down`、`contract-minio`、`e2e-install`、`e2e`。外部契约命令只在对应真实服务进入业务后增加，缺少服务或凭据时快速失败，不进入默认 `check`；Scheduler/Worker 出现真实入口后才增加对应命令，不创建占位目标。
