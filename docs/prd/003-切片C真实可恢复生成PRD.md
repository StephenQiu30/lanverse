...
提交响应前超时、Kafka 重投/重平衡、Worker 在关键点重启、Redis 限流不可用、Provider 限流、重复/乱序回调、部分失败、取消未知、下载中断/哈希不一致、Provider 账户撤销、用量迟到。每个场景必须有用户可见状态和安全下一动作。
...
9. 限流拒绝/Redis 故障时任务可恢复且 Provider 调用为零；队列重投不产生第二次提交。
