...
- 数据库运行时仅用 asyncpg 参数化 SQL；物理 Schema 按表拆分标准 `.sql`，Alembic 只排序执行，不使用 ORM/Metadata/autogenerate。
...
├── migrations/
│   ├── env.py
│   ├── versions/0001_mvp.py
│   └── sql/0001_mvp/
│       ├── tables/01_projects.sql ... 20_delivery_versions.sql
│       ├── 90_foreign_keys.sql
│       └── 91_indexes.sql
...
| --- | --- | --- | --- |
| T-000 文档与数据库门禁 | 校验状态、追踪矩阵、20 表/53 FK/15 候选 UQ/13 partial UQ/4 partial index/22 JSONB exact-set；任一上游非 accepted 必须失败 | 只修正式文档、Schema、机器清单和 gate record | `python -B docs/design/scripts/verify_database_design.py` 与 gate verifier 通过；不创建应用目录 |
...
| T-002 SQL 与契约生成 | migration test 断言 20 个逐表 SQL、关系/索引文件及 catalog exact-set；contract test 断言 operation exact-set、Problem、幂等/ETag/202；OpenAPI 3.1→umi 生成先失败 | 完成 `0001_mvp`、asyncpg pool/transaction、OpenAPI 导出、`openapi2ts.config.ts`、request sender、生成目录和手写 RTK Query 层 | 隔离空库双 `alembic upgrade head`；catalog exact-set；`make contracts-check` 零 diff |
...
| T-004 剧本与分镜 | JSON Schema、哈希、版本确认、派生、旧输入冲突、6～10 镜头测试 Red | Mock TextProvider、Script/Asset/Shot handlers、Story UI；只实现已定义 Operation | 固定夹具生成并确认 Script/Shot，重复执行结果稳定 |
...
| 证据 | 命令/结果要求 | 覆盖 |
| --- | --- | --- |
| EV-000 | `python -B docs/design/scripts/verify_database_design.py`；result=passed | Design/数据库门禁 |
