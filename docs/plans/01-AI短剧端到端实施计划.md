...
| --- | --- | --- | --- |
| T-000 文档与数据库门禁 | 校验状态、追踪矩阵、根 SQL 文件/表/列/FK/partial、20 表/53 FK/15 候选 UQ/13 partial UQ/4 partial index/22 JSONB exact-set；任一上游非 accepted 必须失败 | 只修正式文档、SQL Design artifact、Schema、机器清单和 gate record | `python -B docs/design/scripts/verify_database_design.py` 与 PostgreSQL parser/gate verifier 通过；不执行 DDL、不创建应用目录 |
...
| 证据 | 命令/结果要求 | 覆盖 |
| --- | --- | --- |
| EV-000 | `python -B docs/design/scripts/verify_database_design.py` + PostgreSQL parser；result=passed | Design、根 SQL 与数据库门禁 |
