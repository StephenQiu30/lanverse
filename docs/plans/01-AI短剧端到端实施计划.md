...
每个任务完成需同时满足：目标测试 Green、相关 lint/type/build 通过、只提交本任务文件、记录命令与结果，并以 `git status --short` 证明没有残留本任务修改。功能切片提交顺序固定为 `test:` → `impl:`/`feat:` → 可选 `refactor:`/`docs:`。
...
根目录不创建 `src/`、`build/`、`data/`、`deploy/`、`env/` 或提交 `.env`。参考项目的 Agent/Route/Lib/Middleware/Types 职责分别进入业务模块 application/transport、`integrations/ai`、bootstrap/shared_kernel 和 OpenAPI 生成目录；不复制 Electron、WebSocket、`other` 或运行时 `test` 路由。其他环境只有在正式进入范围后才创建 `docker-compose-<env>.yml`，生产名称固定 `docker-compose-prod.yml`。
