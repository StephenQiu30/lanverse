...
| R2 | Agent 不能直接写业务数据：只输出提案（命令列表 + 差异）；用户点“应用”后由服务端以点击用户的身份、经与公共 API 相同的命令层逐条执行（权限、幂等、版本校验、审计与用户手动操作等价），逐条返回结果 | DES-01 §7.4 |
...
| R7 | 每次运行设置预算上限（token 与费用），超限终止并说明；Agent 自身 LLM 费用计入项目账本，并按 GEN-11 报价确认：开启会话时对“单会话 LLM 费用上限”（默认 ¥2）报价，用户确认后预留；会话内各次运行按实际费用累计，达到上限即停止，继续使用须对新的上限再次确认；每个上限（额度）独立结算：追加后旧额度在其进行中的运行结束时按实际结算并释放余额，当前额度在会话结束时结算（运行中断导致费用未知时，先按该次运行已确认的占用暂估结算，与供应商账单对账后多退，不向用户多收） | GEN-11、COST-01 |
| R8 | 调试面板仅管理员可见 | AGT-03 |

> R7 的默认上限金额待产品负责人确认（§4 REQ-37-Q1）。
...
设计层面的问题见 [DES-39](../design/39-对话式Agent.md) 的“待确认”（原 REQ-37-Q2 已移至 DES-39-Q1）。
