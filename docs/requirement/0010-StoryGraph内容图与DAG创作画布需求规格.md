...
| VPR-PRE-005 | plan_reference_assets 先从正式制作世界机械冻结 Anchor/State/固定 Target seed inventory，并绑定已持久化 VisualFoundationCandidate Revision/hash 与严格候选快照；Agent 只选择合法 Anchor State 并提出 fulfillment/用途规格，Backend 再机械计算最终 expected target set 与依赖。Agent 不得删除、增加、改名、合并 Target、改写 Owner Ref，或在 Gate 3 前依赖/伪造 Effective Snapshot Ref。 | Seed root + Set equality |
