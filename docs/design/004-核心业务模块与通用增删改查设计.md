...
- 已确认事实：平台自行维护登录账号；规范化邮箱只用于唯一登录，跨模块始终引用不可变 UserAccount ID。
...
- UserAccount：本地登录邮箱、Argon2 密码摘要、显示名称、头像、状态和 token_version；邮箱不是跨业务主键，密码摘要不得离开 identity 模块。
- Workspace：数据、成员和额度隔离边界；首次注册原子创建个人 Workspace。
...
| 对象 | 必要 CRUD | 业务规则 |
| --- | --- | --- |
| 当前用户 | 注册、登录、注销、改密、读取/修改显示资料、停用账户 | 注册原子建立个人空间；注销/改密撤销旧 JWT；停用不删除创作历史 |
