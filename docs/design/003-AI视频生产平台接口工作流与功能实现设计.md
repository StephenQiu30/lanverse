...
- Go Controller 的 Swagger 注释是当前 HTTP 文档唯一事实源；使用 `swag init` 生成 `backend/docs/swagger.json`，禁止手工维护 `openapi.json` 或第二份接口定义。
- Go API 的 `GET /api/swagger.json` 返回注释生成的 Swagger 文档，`GET /api/docs` 由固定版本 Swagger UI 渲染该文档。开发与受控运维环境启用文档路由；公网生产默认拒绝匿名访问。
...
- 前端 OpenAPI 生成器的 `schemaPath` 只指向绝对解析后的本地 `../backend/docs/swagger.json`；生成结果必须以 Swagger 注释和生成命令为准。
...
- Swagger JSON 生成结果与前端类型检查纳入 CI；CI 固定执行 `make -C backend swagger`、JSON 校验、`npm --prefix frontend run typecheck` 和生成后 diff 检查。
...
Python Agent 的 `/internal/agent-runs` start/get/cancel/health 只存在于 backend↔Agent 当前 JSON Schema 契约，不进入公共 Swagger UI、前端生成客户端、公共路由或 Ingress。Go `AgentExecutorPort` 是唯一调用方；mTLS 服务身份、network policy、request hash、body size 和 deadline 在解析 Run 正文前校验。生产关闭 FastAPI Swagger/ReDoc/OpenAPI 路由。
