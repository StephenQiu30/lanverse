...
> 状态：提案 v10，已固定无路径版本 HTTP、单一目标契约与 MVVC Controller 边界，待 API、数据与研发评审；后端使用 000 的 Python 主栈
> 日期：2026-08-22
...
4. Outbox 投递当前 TaskEnvelope，独立 Python Worker 幂等执行并通过应用结果端口写回权威模块。

对 HTTP 入口使用 000 定义的 MVVC：View/ViewModel 在前端，Controller 在后端 `transport/http/controllers`，Model 是无框架依赖的 Plain Object 领域对象。Controller 只执行 Header/Schema/身份到 Command/Query 的显式映射，每个方法只调用一个 Application Service 或 Query Handler；不直接使用 Repository、ORM、MinIO、RabbitMQ 或 Agent Harness。
...
- FastAPI/Pydantic 是 HTTP Schema 的唯一生成源，`operationId` 使用稳定 `<module>_<use_case>`，不得按函数重命名自动漂移。
