...
> 状态：提案 v17，已固定无路径版本 HTTP、本地单一 OpenAPI/Swagger、`@umijs/openapi + frontend/src/lib/request.ts`、GORM Repository、backend-only 单一 Kafka、Redis 协调、Elastic 派生检索/观测、Go MVVC Controller/Service 与私有 Python Agent 边界，待 API、数据与研发评审
> 日期：2026-08-22
...
4. Outbox 只向平台唯一 Kafka 集群投递当前 TaskEnvelope；非 Agent 任务由独立 Go Worker 执行，Agent Task 由 Go operation-worker 接管后调用不提供公共入口的 Python agent-service，结果最终经 Go Service 幂等写回权威模块。
...
对 HTTP 入口使用 000 定义的 MVVC：View/ViewModel 在前端，Controller 在 Go `backend/src/<module>/controller.go`，Model 是无框架依赖的 Go plain struct/value object。Controller 只执行 Header/Schema/身份到 Service 输入的显式映射，每个方法只调用一个 Service；不直接使用 pgx、MinIO、Kafka、Redis 或 Agent Harness。

任何界面都不能直接拼接数据库更新；任何 Worker 都不能绕过已批准计划修改镜头、选择或审批。

000 已固定 Go 为唯一业务后端：公共 API、领域命令、事务、Outbox、调度/对账和全部 Kafka/Elasticsearch 接入共享同一套 Go Service/Port；Python 是只在内网接受 backend AgentRun 调用并返回结构化结果的计算微服务，无 Kafka/Redis/Elasticsearch client、公共 API/Ingress。全部后台任务共用一个 Kafka 集群，Redis 不承担消息队列职责，Elastic 不承担消息或事实职责。本文中的 `*Workflow` 表示可恢复的应用流程，不等于默认采用 Temporal；首期由 PostgreSQL 状态机、Kafka、Go Scheduler/Reconciler 和幂等 Worker 实现。
...
- `frontend/openapi2ts.config.ts` 的 `schemaPath` 只指向绝对解析后的本地 `../backend/api/openapi.json`；`serversPath` 固定为 `src/api`，`requestLibPath` 注入 `@/lib/request` 和 `RequestOptions`。`@umijs/openapi` 只负责生成 tag 文件、函数和 `API` 类型；`frontend/src/api` 全目录禁止手改。
- 生成函数只能调用 `request<T>(url, { method, params, data, ...options })`。`frontend/src/lib/request.ts` 用 `axios.create` 建立唯一实例，返回 `response.data`，并统一 runtime base URL、默认超时、凭据/Authorization、`AbortSignal`、trace/request ID、单航班 401 刷新和 `ApiClientError(status/code/request_id/recovery_actions)`；业务提示、路由跳转、写请求重试和业务状态机不进入该文件。M01 由 `lib/auth/install-session-refresh.ts` 向 request adapter 注册一个调用生成 `identity_refresh_session` 的 callback，刷新调用带 `skipAuthRefresh` 防递归；request adapter 不硬编码任何业务/auth URL，也不导入 `src/api` 形成循环依赖。
...
Agent 不直接拥有“自动操作系统”的旁路权限，而是领域命令的规划器。M06 Service 是 AgentRun/Proposal/Decision 信封的唯一写入者；HTTP、业务 Workflow 或事件消费者只能通过它的公开端口请求运行。Python Agent Service 不进入公共 OpenAPI，不配置公共域名/Ingress，frontend、用户和第三方不能直接调用；其 FastAPI 路由只接受 operation-worker 的内网服务身份。
