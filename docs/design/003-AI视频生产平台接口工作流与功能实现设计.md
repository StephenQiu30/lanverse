...
> 状态：提案 v4，待 API、耐久任务与研发评审；后端使用 000 的 Python 主栈
> 日期：2026-08-20
...
| B | 独立 Agent Worker、实体/镜头提案、checkpoint、逐项接受、最小关系/影响接管视图 | Agent 复用 A 的查询与命令；关闭 Agent 后手工流程完整；显式 Scope 不随重排漂移；完整画布不作为前置 |
