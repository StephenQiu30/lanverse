...
- 管理端 OpenAPI 插件的 `schemaPath` 只指向绝对解析后的本地 `../backend/docs/swagger.json`；生成结果必须以 Swagger 注释和生成命令为准。
...
- Swagger JSON 生成结果与前端类型检查纳入 CI；CI 固定执行 `make -C backend swagger`、JSON 校验、`pnpm --dir admin run tsc` 和生成后 diff 检查。
...
Python Agent 的 `/internal/agent-runs` start/get/cancel/health 只存在于 backend↔Agent 当前 JSON Schema 契约，不进入公共 Swagger UI、管理端 client、公共路由或 Ingress。Go `AgentExecutorPort` 是唯一调用方；mTLS 服务身份、network policy、request hash、body size 和 deadline 在解析 Run 正文前校验。生产关闭 FastAPI Swagger/ReDoc/OpenAPI 路由。
