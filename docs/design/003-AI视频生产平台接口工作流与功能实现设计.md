...
> 状态：提案 v16，已固定无路径版本 HTTP、本地单一 OpenAPI/Swagger、`@umijs/openapi + Axios request.ts`、backend-only 单一 Kafka、Redis 协调、Elastic 派生检索/观测、Go MVVC Controller/Service 与私有 Python Agent 边界，待 API、数据与研发评审
> 日期：2026-08-22
...
4. Outbox 只向平台唯一 Kafka 集群投递当前 TaskEnvelope；非 Agent 任务由独立 Go Worker 执行，Agent Task 由 Go operation-worker 接管后调用不提供公共入口的 Python agent-service，结果最终经 Go Application Service 幂等写回权威模块。
5. Elasticsearch/观测后端不替代 Outbox：OTel 日志直接进入观测 data stream，approved/published 内容事件仍经 Outbox/Kafka 由 Go operation-worker 重建业务搜索 index；两者都不是权威事实。首期可共用一次 Elastic 部署，ToC 生产前拆为独立故障域。

对 HTTP 入口使用 000 定义的 MVVC：View/ViewModel 在前端，Controller 在 Go `transport/http/controllers`，Model 是无框架依赖的 Go plain struct/value object。Controller 只执行 Header/Schema/身份到 Command/Query 的显式映射，每个方法只调用一个 Application Service 或 Query Handler；不直接使用 pgx/sqlc、MinIO、Kafka、Redis 或 Agent Harness。

任何界面都不能直接拼接数据库更新；任何 Worker 都不能绕过已批准计划修改镜头、选择或审批。

000 已固定 Go 为唯一业务后端：公共 API、领域命令、事务、Outbox、调度/对账和全部 Kafka/Elasticsearch 接入共享同一套 Go Application Service/Port；Python 是只在内网接受 backend AgentRun 调用并返回结构化结果的计算微服务，无 Kafka/Redis/Elasticsearch client、公共 API/Ingress。全部后台任务共用一个 Kafka 集群，Redis 不承担消息队列职责，Elastic 不承担消息或事实职责。本文中的 `*Workflow` 表示可恢复的应用流程，不等于默认采用 Temporal；首期由 PostgreSQL 状态机、Kafka、Go Scheduler/Reconciler 和幂等 Worker 实现。
...
`data` 永远是业务表示；`meta` 只保存请求、分页、基线和并发元数据；`links` 只给同一 API 的受控链接。不得把 Kafka topic/partition/offset、Redis key、MinIO key、数据库行号、LangGraph checkpoint 或 Provider 原始响应放入公共信封。所有接口直接使用这一种响应包装；旧的仅 `data` 包装不在目标实现中读取或返回。
...
Operation 状态统一为：`accepted`、`queued`、`running`、`waiting_external`、`waiting_user`、`unknown`、`succeeded`、`partially_failed`、`failed`、`cancelled`。前端不读取 Kafka offset/lag、Redis lease、Task lease、Worker 进程或 LangGraph checkpoint 状态；`GET /api/operations/{id}` 和 SSE 都来自 PostgreSQL Operation。Operation 的 `result_refs` 只引用业务结果，不复制候选、报告或快照正文。
...
- `backend/api/openapi.json` 是唯一 HTTP Schema；格式固定为 OpenAPI 3.0.3 JSON，`info.version` 固定写 `current`，只表示必填文档元数据，不形成 URL、Header 或兼容版本。禁止再维护 `openapi.yaml`、运行时反向导出的 Schema 或远程 Schema URL。
- Go `oapi-codegen` 从该文件生成 strict server 类型；Go API 构建时嵌入同一文件，`GET /api/openapi.json` 原样返回，`GET /api/docs` 由固定版本 Swagger UI 渲染。开发与受控运维环境启用文档路由；公网生产默认拒绝匿名访问，且 Swagger UI 不生成另一份契约。
- 每个 operation 必须只有一个模块 tag 和显式稳定 `operationId: <module>_<use_case>`；路径必须从 `/api` 开始且不能包含 `v1`/版本段。Schema 名称使用业务名，不加 `V1`、`RequestDTO` 或 Go/TypeScript 框架后缀。
- 每个写接口必须声明 request body、`Idempotency-Key`、必要的 `If-Match`/`X-Workspace-Id`、成功响应、所有稳定业务错误、权限 Scope；流式/SSE 另有事件 Schema。成功响应显式使用 `200`、`201`、`202` 或 `204`，不依赖 generator 猜测 `2XX` range。
- `frontend/openapi2ts.config.ts` 的 `schemaPath` 只指向绝对解析后的本地 `../backend/api/openapi.json`；`serversPath` 固定为 `src/api`，`requestLibPath` 注入 `@/platform/http/request` 和 `RequestOptions`。`@umijs/openapi` 只负责生成 tag 文件、函数和 `API` 类型；`frontend/src/api` 全目录禁止手改。
- 生成函数只能调用 `request<T>(url, { method, params, data, ...options })`。`frontend/src/platform/http/request.ts` 用 `axios.create` 建立唯一实例，返回 `response.data`，并统一 runtime base URL、默认超时、凭据/Authorization、`AbortSignal`、trace/request ID、单航班 401 刷新和 `ApiClientError(status/code/request_id/recovery_actions)`；业务提示、路由跳转、写请求重试和业务状态机不进入该文件。M01 由 `platform/auth/install-session-refresh.ts` 向 request adapter 注册一个调用生成 `identity_refresh_session` 的 callback，刷新调用带 `skipAuthRefresh` 防递归；request adapter 不硬编码任何业务/auth URL，也不导入 `src/api` 形成循环依赖。
- View 只能调用 ViewModel；ViewModel 只导入 `frontend/src/api` 的生成函数，不导入 Axios/`request.ts`、不拼 URL、不复制 DTO，也不通过 RTK Query/第二个 endpoint registry 再包装同一 API。加载、取消、错误动作和必要的页面缓存属于 ViewModel，业务完成仍以 Go/PostgreSQL 响应为准。
- OpenAPI、Go 生成类型和前端生成 API 都提交 Git。CI 固定执行 Schema 校验、Go 生成、`npm --prefix frontend run openapi2ts`、typecheck/contract test 和生成后 `git diff --exit-code`；生成结果变化必须与契约、Controller、ViewModel 和测试同一提交。
- `/api` 只存在一套当前契约。字段、状态或同步/异步语义发生变化时，后端、OpenAPI、生成客户端、ViewModel、Contract 测试和 E2E 在同一发布单元更新；不新增 URL 版本、兼容 alias 或双 Schema。Elastic 搜索 alias 只用于当前索引原子切换，不承载旧 API/Schema。
- Contract 测试至少覆盖：唯一 operationId/tag、无版本路径、Swagger 展示与提交文件 hash 相同、前端生成无 diff、同键同 hash 回放、同键异 hash 冲突、缺/错 If-Match、跨 Workspace 404、未知字段拒绝、cursor 与 basis、202 Operation 回源和稳定错误信封。

### 2.8 私有 Agent Service 契约

Python Agent 的 `/internal/agent-runs` start/get/cancel/health 只存在于 backend↔Agent 当前 JSON Schema 契约，不进入公共 `backend/api/openapi.json`、Swagger UI、frontend client、公共路由或 Ingress。Go `AgentExecutorPort` 是唯一调用方；mTLS 服务身份、network policy、request hash、body size 和 deadline 在解析 Run 正文前校验。生产关闭 FastAPI Swagger/ReDoc/OpenAPI 路由。

Agent 反向调用只允许 run-scoped 的只读 Tool、载荷 capability 和 `POST /internal/agent-gates/model-calls:authorize`；最后一项由 Go 同时执行 Redis GCRA 与 M11 UsageGate，成功时返回单次、限 run/model/expiry 的 capability。它不是公共业务 API，也不允许 Agent 直接访问 Redis/M11 表。私有契约同样只有当前 Schema，不实现路径版本、兼容 decoder 或 callback。
...
| `GET /api/projects/{id}/narrative-search` | `SearchApprovedNarrative` | `q/phrase/content_unit_id/scene_id/node_type/character_subject_id/location_entity_id/requirement_type/cursor/limit`；返回 approved node refs、BM25 score、高亮、SourceAnchor、indexed narrative/knowledge basis、filter completeness、checkpoint/as_of/stale；Elastic 不可用返回 `503 search_unavailable`，不伪装零结果 |
...
| Agent Service 大载荷 ResultPort | `CreateStageResultPayloadCapability` → `CommitStageResultPayload` / `GetStageResultPayloadByCapability` | 按 run/item/request key/hash 幂等；校验 scope、size/hash 后登记不可变 payload ref；提交成功但响应丢失可回读原 ref，同 key 异 hash 冲突；未被有效 Result 引用的对象进入具名清理流程 |
| operation-worker 拉取 Agent Result | `AgentExecutorPort.GetRun` → `CommitAgentRunResult` | private HTTP 超时先按 run/key/hash 对账；payload 与目标 Preview 校验通过后，按 run/sequence/result hash 追加 Result、AgentProposal/Item、Result-Advisory membership 和 `agent.run.result_committed` Outbox；Advisory 本体按 logical key/hash 创建或回读，Publisher 事后投递回执 |
...
- 平台只部署一个 Kafka 集群，且只允许 Go backend 连接；业务事件、Operation、导入、Provider、媒体和 Agent invoke Task 只以 Topic/ACL/consumer group 做逻辑隔离，不允许另建 Broker 或另一种队列。Topic 和消息契约由 Go backend 统一治理。
- Topic、partition key、consumer group 和保留期只使用 000 §11.2 的固定表；事件类型不映射成新 Topic，也不添加版本后缀。
- Go Outbox Publisher 使用 `franz-go` 幂等 producer、`acks=all`；Outbox 记录 Broker ack 的 topic/partition/offset 仅供运维追踪，不进入公共 API。
- Go consumer 关闭 auto commit，在 PostgreSQL 短事务完成 Inbox 去重、Task/Attempt 或投影 checkpoint 接管和必要 Outbox 后提交 offset。`lanverse-search-projection` 是 operation-worker 中消费 `lanverse.domain.events` 的具名 group，不新增 Topic/进程；Python Agent 不消费或生产 Kafka 消息，服务身份没有 Broker 凭据。
- commit 前崩溃会重投，commit 后执行进程崩溃由 PostgreSQL Task lease/Scheduler 或 Agent checkpoint 恢复；两条路径都复用相同 message/task/run ID。
- retry 使用 PostgreSQL `available_at` 与新 Outbox 重投原 Topic；消费者不得 sleep 占住 partition，也不创建逐级 retry Topic。
- contract/hash 非当前、payload 超限或最大 attempt 用尽时，先创建 `DeadLetterCase`，再由 Outbox 发 `lanverse.dead-letter`；人工重放生成新 attempt，但复用原业务幂等键。
...
| `narrative.revision.approved` | 叙事 current 已切换并与 Outbox 同事务提交 | M03 发布 ApprovedNarrativeAnalysisView；`lanverse-search-projection` 回读 approved 节点/Anchor 幂等更新 Elastic；切片 B 请求新的 knowledge stage，切片 A 开放 M04 手工门禁；M05 等下游只读取 approved/current |
...
| `knowledge.production_requirement_inventory.rebuilt` | 新清单投影版本已原子切换 | 更新 completeness/failed scopes/unassessed 和批准预检；搜索 projector 以 M04 SearchEnrichmentView 更新 requirement filters，不复制 Inventory 事实 |
| `knowledge.requirement_readiness.rebuilt` | 新准备度投影版本已原子切换 | 更新缺口/项目摘要；不得改写需求决议 |
| `knowledge.entity_occurrence.rebuilt` | 新矩阵投影版本已原子切换 | 更新人物页、项目概览和批准预检；搜索 projector 更新 entity/unresolved-subject/location filters，失败只使 filter completeness stale |
...
### 4.4 Elastic 搜索投影与日志链路

`SearchApprovedNarrativeController` 只把 query/filter/cursor 映射到 Go `SearchApprovedNarrativeService`。Service 先用 M01 鉴权并从 PostgreSQL 解析允许的 workspace/project 与 exact approved NarrativeRevision set/hash；使用人物/地点/需求 filter 时还要取得 M04 current SearchEnrichment basis/completeness，再调用 `SearchIndexPort`。adapter 固定使用官方 `go-elasticsearch/v9`，所有 query 必须带上述 terms filter。公共 API 不接受 index 名、Elasticsearch DSL、script、analyzer 或任意 field 参数。

索引更新流程为：`事实 + Outbox` 同事务 → 唯一 Kafka `lanverse.domain.events` → operation-worker Inbox/checkpoint → 回读 approved M03/M04 Query 并冻结 narrative/knowledge basis hash → bulk index/delete → 再读 current basis → 仅同 basis 更新 PostgreSQL `ops_search_projection_checkpoint` → 提交 offset。跨 aggregate 事件不假设全局顺序；旧 basis 结果丢弃并按 current basis 重排。Elastic 超时只重试同一投影 item；私有内容重建使用新 `lanverse-private-content-<schema_hash>` 物理索引并在计数、revision set/hash、Anchor 抽样和跨租户/未发布负向校验后切换 alias。未来公共 published 内容使用独立 `lanverse-public-content-<schema_hash>` 索引族，不与私有文档混写。搜索故障不回滚 Narrative 批准，也不从日志恢复领域事件。

Go/Python 运行日志输出 ECS 兼容 JSON/OTel，经 Collector 直接进入独立 `logs/metrics/traces-lanverse.*` data stream，不经过 Kafka/Outbox。日志不得包含剧本正文、搜索 query 原文、Prompt、签名 URL、凭据或模型完整响应；必要诊断只记录 query hash、result count、latency、index schema/checkpoint 与 trace ID。首期不部署 Logstash、Debezium 或 Kafka Connect；ToC 生产前这些 data stream 迁入与业务搜索 Elasticsearch 不同的观测故障域。
...
Agent 不直接拥有“自动操作系统”的旁路权限，而是领域命令的规划器。M06 Application Service 是 AgentRun/Proposal/Decision 信封的唯一写入者；HTTP、业务 Workflow 或事件消费者只能通过它的公开端口请求运行。Python Agent Service 不进入公共 OpenAPI，不配置公共域名/Ingress，frontend、用户和第三方不能直接调用；其 FastAPI 路由只接受 operation-worker 的内网服务身份。

普通独立 Agent 用例可以由 HTTP Command 映射到 M06 `StartAgentRun`，但调用终点仍是 Go backend。`script_analysis` 必须走 §6 的专用路径：M03 先提交 AnalysisStage 与 `narrative.analysis_stage.requested` Outbox，M06 消费后在短事务中创建 `AgentRun`、child Operation 和 Agent Task Outbox；Go operation-worker 从唯一 Kafka 的 operation Topic 接管 Task 后，经 backend `AgentExecutorPort` private HTTP adapter 调用 Agent Service 的 start/get/cancel。Request 至少固定 parent AnalysisRun、root/child Operation、`stage`、`stage_generation`、input snapshot ref/hash、input/result contract hash、Command Schema descriptor/hash、Tool allowlist、模型/治理/用量上限和 trace。Agent Service 的 LangGraph 只执行一次 Run 内部的分析、只读 Tool 和提案图；checkpoint 不成为产品状态，Go 查询并校验 Result 后再经受限 ResultPort 提交。
...
Tool 输出有 JSON Schema、最大对象数量和版本引用。大型 Proposal payload 先经 M06 run/item-scoped 一次性 capability 暂存为不可变、内容寻址引用；Worker 不获得通用对象存储凭据，payload 不进入 Kafka 或 checkpoint。高消耗调用、选择、风险接受、审批和交付没有自动执行 Tool。
...
- 不在事务中调用 LLM、对象存储、Kafka、Redis 或提供商。
...
| 场景 | 机制 | Redis 锁是否允许 | 最终正确性 |
| --- | --- | --- | --- |
| 聚合更新、批准、主选、额度预留 | PostgreSQL expected revision、unique、row lock | 禁止 | PostgreSQL 事务 |
| Outbox/Inbox 与 Job 唯一 | PostgreSQL unique + lease | 禁止替代 | PostgreSQL 事务 |
| 同 scope 投影重建/媒体派生 | `DistributedLockPort` 短租约 + fencing | 允许 | 目标行比较 fence；重复结果幂等 |
| Scheduler 单例扫描 | Redis 短租约；丢锁立即停止 | 允许 | 数据库 claim 条件更新仍防重复 |
| Provider 提交 | 不用 Redis 锁判断是否已提交 | 只允许优化同 key 并发 | Job/Attempt/idempotency/unknown 对账 |
| MinIO 删除/LegalHold | PostgreSQL storage guard generation | 禁止替代 | M14 fence + MinIO exact version |

任何受 Redis 锁保护的 Go 工作都必须接收可取消 context、记录 owner/fencing token、在 `ttl/3` 续租并在失败时停止；下游原子写必须拒绝旧 fencing token。不得跨请求保存 Lease 对象，不实现可重入/公平/读写锁。

### 18.2 分布式限流矩阵

| owner | key 维度 | 算法/结果 | Redis 不可用 |
| --- | --- | --- | --- |
| M01 | IP/account/action | Redis GCRA；429 + Retry-After | 登录/验证码 fail closed |
| API 平台 | workspace/principal/operation | Redis GCRA；remaining/reset | 低成本已认证读使用更严格本地保底；写入 fail closed |
| M06 | workspace/model-profile | Redis GCRA + M11 UsageGate | 暂停新模型调用，Run 保持可恢复 |
| M08 | workspace/connection/capability | Redis GCRA + Provider backoff | Job 进入 retry_wait，不提交 Provider |
| M11 | workspace/policy/unit | 不用速率 key 代替额度 | 从 PostgreSQL 账本/Reservation 判定 |

RateLimit rule 必须引用当前 PolicyRevision；key 只使用稳定 ID/hash。限流拒绝不创建业务失败事实、不消耗 M11 额度；已经收单的 Provider/模型调用按实际证据入账。
...
项目名、文件名、搜索 query 原文、Prompt、评论内容和签名 URL 不进入标签。日志事件按 ECS 进入独立 Elastic data stream，包含 request/command/operation/task/job/attempt ID、trace ID 和安全 hash，支持从用户错误追到外部证据；PostgreSQL AuditEvent 仍是业务审计权威。

关键告警：unknown 任务积压、回调验签失败突增、Outbox 滞后、Kafka consumer lag/rebalance/offset commit 失败、dead-letter 积压、Redis lock obtain/refresh/lost、限流拒绝/降级、Task lease 过期、Elastic ingest/search error、剧本索引 checkpoint lag/重建失败、媒体隔离率、跨租户拒绝、用量估算偏差和交付校验失败。
...
| approved 剧本检索 | 中文/中英 BM25、短语/结构 filter、高亮/Anchor、exact approved revision filter、stale/unavailable、重复事件、索引删除后重建、跨 Workspace 零命中 |
...
| 切片 | 垂直功能 | 完成定义 |
| --- | --- | --- |
| A | Go 业务运行骨架、项目、整本来源导入/拆集物化、叙事校对、全剧清单/人物矩阵、approved 剧本检索、最小实体和手工镜头、Operation/Outbox | 无 Agent/真实 Provider 时可从真实多集文档确认剧集、稳定 ContentUnit、每集 Scene、人物/地点/道具/服装需求和人物×剧集矩阵，得到带锚点的批准叙事并可检索、形成至少 10 个镜头；Go Worker/搜索索引重建后任务和事实可恢复 |
