...
| R5 收敛 | 删除临时 re-export/allowlist，更新生成 client 与架构文档 | 完整质量门禁、文档追踪、`git diff --check` 全部通过。 |
...
- 2026-07-30 R2 已完成：`scripts` 的版本与提取/候选决议分别迁入 `versions`、`extractions` 能力包，根 `api.py` 仅组合 Router，根 `service.py`/`schemas.py` 已删除且未保留兼容转发层；定向回归 36 项、全量后端 73 项和前端 13 项通过，OpenAPI 生成结果无漂移，进入 R3 projects/identity 能力拆分。
- 2026-07-30 R3 已完成：`projects` 拆为项目、单集与生产快照能力，`identity` 拆为认证/账户与工作空间/授权能力；共享数据库、Session 与 ASGI client fixture 收敛到 `tests/conftest.py`，对应 integration 测试按相同能力目录组织。全量后端 77 项和前端 13 项通过，3 项外部基础设施契约按显式开关跳过，OpenAPI 生成结果无漂移，进入 R4 production/messaging 边界固定。
- 2026-07-30 R4 已完成：审计确认 production 当前仅有 Task 生命周期、messaging 的 Outbox/Inbox 共同构成消息交付生命周期，继续保持扁平模块；新增架构守卫禁止公开 contracts 依赖 FastAPI、SQLAlchemy 或模块内部实现，并要求新 GenerationRequest/Attempt 或独立交付生命周期进入前先显式更新边界。定向架构与任务/消息回归 14 项通过，进入 R5 收敛。
- 2026-07-30 R5 已完成：删除未使用的 `messaging/schemas.py` 转发层，将跨模块使用的 `TaskResponse` 与相关结果 DTO 归入 production 公开 contracts，跨模块内部导入 allowlist 保持为 0。当时完整质量门禁的后端 80 项、前端 13 项通过，3 项需显式外部基础设施的契约测试按开关跳过；OpenAPI client 重新生成无差异，DEV-ARCH-01 工程门禁解除。
