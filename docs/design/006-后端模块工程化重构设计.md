...
- 2026-07-30 R5 已完成：删除未使用的 `messaging/schemas.py` 转发层，将跨模块使用的 `TaskResponse` 与相关结果 DTO 归入 production 公开 contracts，跨模块内部导入 allowlist 保持为 0。最终 `make check` 后端 80 项、前端 13 项通过，3 项需显式外部基础设施的契约测试按开关跳过；OpenAPI client 重新生成无差异，DEV-ARCH-01 工程门禁解除。
