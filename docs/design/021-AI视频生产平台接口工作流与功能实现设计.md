...
> 状态：提案 v3，待 API、工作流与研发评审；运行语言服从 019 PoC 决策
> 日期：2026-08-20
> 产品边界：[018-AI 视频生产平台绿地产品与功能模块设计](018-AI视频生产平台绿地产品与功能模块设计.md)
> 系统架构：[019-AI 视频生产平台绿地系统架构设计](019-AI视频生产平台绿地系统架构设计.md)
...
3. 需要长时间或外部副作用的命令启动 Temporal Workflow。
4. Workflow 调用有幂等语义的 Activity，并把结果写回权威模块。

任何界面都不能直接拼接数据库更新；任何 Worker 都不能绕过已批准计划修改镜头、选择或审批。

019 当前把 Go 业务控制面 + Python/LangGraph Agent 处理面列为候选而非已接受选型。若纵向 PoC 通过，本文件中的 API、命令事务、业务 Workflow 和权威写入由 Go 承载，Agent/智能处理 Activity 由 Python 承载；若未通过，保持同一接口和领域约束改用 Python 单栈，不改变本文业务契约。
...
Operation 状态统一为：`accepted`、`queued`、`running`、`waiting_external`、`waiting_user`、`unknown`、`succeeded`、`partially_failed`、`failed`、`cancelled`。前端不读取 Temporal 内部状态；`GET /v1/operations/{id}` 和 SSE 都来自 PostgreSQL Operation。Operation 的 `result_refs` 只引用业务结果，不复制候选、报告或快照正文。

`workflow_id` 仅存在于服务端 Operation 记录和受限运维诊断中，普通客户端不把它当作资源 ID 或恢复依据。
...
| `generation.plan.approved.v1` | 计划、用量预留及 start_now Operation 已按处置冻结 | Outbox 启动与既有 Operation 绑定的 Workflow；hold 等待显式提交 |
...
部分页失败时 Workflow 返回 `partial` 并列出缺页；用户可继续校对成功内容或补充来源，不强迫整份重来。
...
若用户在 Workflow 运行期间批准了新的 NarrativeRevision，Workflow 仍可完成，但 Proposal 标记 `stale`，不能执行。

## 7. GenerationPlanWorkflow

GenerationPlan 的预检可以由 Workflow 执行，但批准是 PostgreSQL 中的短事务。批准命令接收 `execution_disposition`：

- `start_now`：批准事务通过 execution 端口同时创建 Operation 和启动 Outbox；Dispatcher 启动与该 Operation 绑定的 Workflow，失败时重投同一意图。
...
Webhook 端点只做：读取原始字节、验证签名/时间窗、按 ProviderEvent ID 去重、由 ExternalJob ID 反查内部 Job、保存 CallbackReceipt、向 Workflow 发信号。它不直接把 Job 标记成功。
...
5. ReviewDecision 写入数据库后向 Workflow 发信号。
...
在 Go + Python 候选架构中，Go 创建 `AgentRun` 并由 Temporal Go Workflow 调度专用 Python Activity；Python LangGraph 只执行一次 Run 内部的分析、只读 Tool 和提案图。输入输出使用 019 定义的版本化 Protobuf，LangGraph checkpoint 不成为产品状态。Python 没有业务库写权限，所有写 Tool 实际只生成 ProposalItem；接受时由 Go 重新鉴权、检查基线版本并调用权威命令处理器。

| Tool | 只读/写入 | 输出/命令 |
| --- | --- | --- |
| `get_project_context` | 只读 | 精简项目、版本、阻塞和用量上下文 |
| `analyze_source` | 异步提案 | 启动 Import/分析 Workflow |
...
Agent Tool 分为两类：只读 Tool 通过受限内部 Query API 获取最小上下文；提案 Tool 只构造命令载荷，不能调用写 API。产品级人工等待由业务 Workflow/Operation 表达，不在 LangGraph `interrupt` 中维护第二套批准状态。
...
- Worker Activity 结果由权威应用端口使用业务唯一键提交，Temporal 重放或 Python Activity 重试不会重复写事实；Python Agent/ML Worker 不直接写业务表。
- 资源预留使用条件更新或锁定 Scope 行，防止并发超限。
- 不在事务中调用 LLM、对象存储、Temporal 或提供商。
- “批准计划 + 资源预留”等跨模块原子用例由顶层用例编排器调用各模块写端口；编排器不能绕过端口直接写对方表。
- Operation 在启动 Workflow 前提交；启动意图通过 Outbox/协调器至少一次投递，以业务幂等键保证只对应一个 Workflow。
...
项目名、文件名、Prompt、评论内容和签名 URL 不进入标签。日志事件包含 request/command/workflow/job/attempt ID，支持从用户错误追到外部证据。

关键告警：未知任务积压、回调验签失败突增、Outbox 滞后、Workflow 无进展、媒体隔离率、跨租户拒绝、用量估算偏差和交付校验失败。
...
| Operation | 启动投递失败、Workflow 已运行但投影滞后、重复启动、父子进度汇总 |
...
| 切片 | 垂直功能 | 完成定义 |
| --- | --- | --- |
| A | 项目、来源直传、导入、叙事校对、Operation 骨架 | 可从真实文档得到带锚点的批准叙事版本；长操作重启可恢复 |
| B | 实体状态、参考、镜头方案、受限 Agent 首稿 | 可从同一命令生成和逐项接受镜头提案；显式 Scope 不随重排漂移 |
...
每个切片必须同时完成：领域测试、真实 PostgreSQL 集成测试、Workflow 故障测试、OpenAPI 契约、权限负向测试、指标和运维恢复说明。页面可用但这些条件未满足时，不算完成。
