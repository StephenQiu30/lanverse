...
- 状态：StoryGraph 演进同步前已冻结；待 `SG-D12` 重构并重新评审
- 日期：2026-08-25
...
- StoryGraph 演进门禁：当前设计和旧 5-Skill 派生文档只作为 `0010/3003` 的现状输入，不是它们的规范前置，也不再领取新实施任务；`SG-D12` 只同步并重新接受本 Design，旧派生文档持续冻结，由 `SG-D17`–`SG-D21` 统一文档链取代
