...
> 状态：提案 v6，已固定目标业务闭环、数据与接口边界，待产品、编剧、制片与研发联合评审
> 日期：2026-08-22
...
目标技术路径使用 Python。FastAPI 负责命令、查询和短事务；PostgreSQL 保存业务事实、提案和决议证据；Outbox 与 RabbitMQ 承载可靠异步任务。切片 A 关闭 Agent/LangGraph 时仍可通过确定性解析和人工命令完成全部五项输出；切片 B 才由独立 Agent Worker 运行三个受限 LangGraph stage。M03 `AnalysisRun` 与 root Operation 表达整本业务流程；每个 LangGraph checkpoint 只恢复 breakdown、narrative 或 knowledge 中的一个 child AgentRun，不恢复跨人工门禁的整本流程，也不成为人物资料或审批状态源。
...
所有路径使用无版本前缀 `/api`，通用响应、错误、Header、游标和幂等规则以 003 为准。HTTP Pydantic Schema 位于 `transport/http/schemas`，Controller 位于 `transport/http/controllers/<use_case>.py`；Controller 显式映射到 `application/commands` 中的 frozen dataclass，并只调用同名 `application/services/<use_case>.py`。
...
2. 每类权威事实只有一个模块写入所有者；跨模块只通过 Plain Object Command/Query、事件和稳定 revision ref 协作。
