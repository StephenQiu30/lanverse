...
> 状态：提案 v7，已固定 Go 业务/Python Agent 目标闭环、数据与接口边界，待产品、编剧、制片与研发联合评审
> 日期：2026-08-22
...
目标技术路径固定为 Go 业务后端 + Python Agent。Go API/Application Service 负责命令、查询、权限、短事务和全部业务事实写入；Go import/operation worker 负责确定性解析、投递和结果校验；PostgreSQL 保存业务事实、提案和决议证据；Outbox 与 RabbitMQ 承载可靠异步任务。切片 A 关闭 Python Agent/LangGraph 时仍可通过 Go 确定性解析和人工命令完成全部五项输出；切片 B 才由独立 Python agent-worker 运行三个受限 LangGraph stage。M03 `AnalysisRun` 与 root Operation 表达整本业务流程；每个 LangGraph checkpoint 只恢复 breakdown、narrative 或 knowledge 中的一个 child AgentRun，不恢复跨人工门禁的整本流程，也不成为人物资料或审批状态源。
...
所有路径使用无版本前缀 `/api`，通用响应、错误、Header、游标和幂等规则以 003 为准。HTTP Schema 唯一源位于 `backend/api/openapi.yaml`，oapi-codegen 生成 Go 边界类型；Controller 位于 `backend/internal/modules/<module>/transport/http/controllers/<use_case>.go`，显式映射到 `application/commands` 中的 Go plain struct，并只调用同名 `application/services/<use_case>.go`。
