...
- Go Domain 只使用标准库和本模块类型，不依赖 transport、database、Kafka、Redis、MinIO 或 Agent。
...
| 前端数据 | `@umijs/openapi` 生成 TypeScript API + Axios `request.ts` | ViewModel 只调用生成函数；统一 base URL、鉴权、超时、取消和错误映射，不手写 URL/DTO 或第二套 endpoint registry |
...
| 异步投递 | 单一 Apache Kafka 集群 + Go `franz-go` + PostgreSQL Outbox/Inbox | 全部后台任务只由 Go backend 接入一套消息基础设施；至少一次投递、分区有序、消费者组、重放、背压和死信；Agent 不安装 Kafka 客户端 |
...
| SVC-02 | `backend/cmd/operation-worker` | Go | Outbox Publisher、Scheduler、Reconciler、Inbox、数据库/Elasticsearch 投影、私有 Agent Run 调用与结果接管 | 替用户批准、猜测 unknown 成功、跨模块直接写表 | A |
...
| SVC-05 | `agent/src/lanverse_agent/entrypoints/server.py` | Python | 作为私有计算微服务幂等启动/查询/取消 AgentRun、执行 LangGraph Harness/Skill/模型/只读 Tool、返回 AgentResult | Kafka 客户端、公共 HTTP API/Ingress、前端直连、业务数据库、业务 Service、批准、生成执行、通用 MinIO 凭据 | B |
...
├── frontend/
│   ├── package.json
│   ├── openapi2ts.config.ts            # 只从 backend/api/openapi.json 生成
│   ├── src/
...
│   │   │       └── tests/
│   │   └── platform/
│   │       ├── http/request.ts          # 唯一 Axios instance/adapter；生成 API 专用
...
│   ├── go.sum
│   ├── api/
│   │   ├── openapi.json                 # 唯一公共 HTTP 契约；路径只用 /api
│   │   └── oapi-codegen.yaml            # Go strict server 生成配置
│   ├── contracts/
...
│   │       ├── database/
│   │       ├── messaging/
│   │       │   └── kafka/               # franz-go、Topic、producer/consumer、offset
│   │       ├── coordination/
│   │       │   ├── locks/               # redislock 租约、续租、fencing
│   │       │   └── ratelimit/           # redis_rate GCRA
│   │       ├── objectstorage/
│   │       ├── search/                  # go-elasticsearch；业务 SearchIndexPort 唯一 adapter
│   │       ├── cache/
│   │       ├── secrets/
...
│   ├── src/lanverse_agent/
│   │   ├── entrypoints/
│   │   │   └── server.py                # FastAPI/Uvicorn，仅私有 AgentRun 契约
...
- `backend/api/openapi.json` 是唯一 HTTP 契约；固定使用 OpenAPI 3.0.3 JSON。JSON 是为了让 `oapi-codegen` 与 `@umijs/openapi` 直接读取同一份本地文件，不再维护 YAML/JSON 转换产物，也不依赖启动中的 backend。
- Go API 的 `/api/openapi.json` 原样返回构建时嵌入的当前契约；`/api/docs` 使用固定版本 Swagger UI 渲染该文件。两者是开发/运维文档入口，不是第二套业务 API；生产默认只对受控内部身份开放。
...
View → ViewModel → frontend/src/api/<tag>.ts（生成）
                 → frontend/src/platform/http/request.ts（Axios）
                 → Go Controller → Application Service
```

- `frontend/openapi2ts.config.ts` 使用绝对解析后的本地 `../backend/api/openapi.json`，`serversPath` 固定为 `src/api`；`schemaPath` 不接受远程 URL 或环境变量，避免后端未启动、网络漂移或生成时拿到错误环境的 Schema。
- `requestLibPath` 直接注入 `import request, { type RequestOptions } from '@/platform/http/request'`；生成函数统一按 `request<T>(url, { method, params, data, ...options })` 调用，`request.ts` 返回 OpenAPI 响应 body，而不是 AxiosResponse。
- `request.ts` 是唯一 Axios instance：统一 runtime base URL、10 秒默认超时、cookie/Authorization、W3C request/trace 关联、`AbortSignal`、401 单航班刷新一次和稳定错误信封映射。M01 启动组合层通过 `install-session-refresh.ts` 注册调用生成 `identity_refresh_session` 的 callback，并以 `skipAuthRefresh` 防递归；`request.ts` 不硬编码 refresh/login 路径，也不反向导入生成 API。写命令不自动重试；幂等键、`If-Match` 与 workspace header 必须由生成签名/ViewModel 显式提供。
...
Go 不复制 Redisson 的大而全 API。`backend/internal/platform/coordination` 只暴露两个窄端口：
...
5. 搜索入口固定为 frontend/Agent → Go Search Controller → 具名 Go Search Application Service → `SearchIndexPort` → Elasticsearch，不增加公共搜索微服务。Go 每次先鉴权并解析允许的 workspace/project、visibility、publication/right basis 与 exact approved revision set，再强制注入 filter；frontend、Python Agent 和用户均无 Elasticsearch endpoint/API key。Agent 只能调用 run-scoped backend Search Tool；公共 ToC 查询也必须经过 Go 限流、可见性和 M14 权利过滤，不能直接暴露 Elasticsearch DSL。
...
| Go 集成测试 | pgx/sqlc、PostgreSQL、RLS、并发冲突、Kafka、Redis lock/rate limit、MinIO、Elasticsearch 投影/重建/租户过滤 |
...
| 架构测试 | Go Domain 不依赖 adapter；Python 不连接 Kafka/Redis/Elasticsearch/业务 DB/MinIO；模块无跨 owner 写入；部署只有单一消息集群；日志/搜索索引不成为业务事实 |
...
- [ ] Go plain struct 不依赖 HTTP、pgx、sqlc、Kafka、Redis、MinIO、LangGraph 或 Provider SDK。
- [ ] Controller 每个写入口只调用一个明确 Service；不存在万能 Manager/Service。
- [ ] 公共路径只使用 `/api`，`backend/api/openapi.json` 是 Swagger UI、Go strict server 与前端 API 的唯一 HTTP 类型源，生成后无 diff。
