...
- 管理端 OpenAPI 配置使用绝对解析后的本地 `../backend/docs/swagger.json`；生成前先运行 `make -C backend swagger`，不依赖启动中的 backend。
...
- [ ] 公共路径只使用 `/api`，Controller Swagger 注释生成的 `backend/docs/swagger.json` 是 Swagger UI 与管理端类型检查的唯一 HTTP 文档源，生成后无 diff。
