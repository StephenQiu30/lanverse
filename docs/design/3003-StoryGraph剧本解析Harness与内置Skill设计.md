...
Backend Candidate Repair Coordinator 已在上述边界内应用 Patch：Application 先在父 Candidate 中按稳定 Key 定位 frozen fragment、重算 base fragment hash、逐字段替换并重新执行严格 Story Reconciliation Candidate 校验；GORM Store 随后在一个 PostgreSQL 事务中锁定 expected Head、重验父 Revision、Review Revision 与成功 Repair Result，创建 N+1、CAS Head，写入复用的 `CommandReceipt`，并为受影响闭包写不可变 `StageInstanceStaleness`。闭包从被替换的 exact Candidate Revision 出发，既沿 Invocation 的 exact upstream revision id/hash，也沿 Backend Aggregate Origin 的 exact leaf revision id/hash 逐层传播；aggregate Stage 的 staleness 不伪造 Invocation 引用。已应用 Repair Invocation 作为新 Revision provenance 明确排除，未受影响分支不标 stale，原 Invocation/Result/Revision 不修改。闭包无法完整解码时整个事务回滚；同一 Patch 重放返回同一 Receipt，不同幂等键继续使用旧 expected Head 时冲突且不留 Receipt。当前尚未由 Temporal 创建 replacement Invocation，也未驱动有界 Gate/Review 重跑，这部分继续属于下一实现单元。
