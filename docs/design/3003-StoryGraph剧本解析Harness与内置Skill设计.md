...
基础 Target 的 Backend 持久执行复用现有 Release、Manifest、Invocation、Attempt、DispatchAuthorization、Result、CandidateRevision 与 CandidateHead；`compile_reference_brief` 及 `reference_brief_candidate` 只扩展这些既有 GORM Catalog 的语义闭集，不增加表、字段或 migration 文件。dispatch 事务和 accepted 事务都调用上述同一 facts loader，并要求重编译结果与冻结 Input 逐字段相同；accepted 重放返回同一 Candidate Revision，传输不可信只落 `outcome_unknown` 且不得产生 Candidate。该持久执行子阶段本身不注册 Workflow/Temporal 节点，不调用 Provider，也不放行有依赖 Target；后续 Temporal 屏障按下述合同复用它。

Temporal 接入使用单个 `activity.compile_reference_briefs` 基础波次屏障节点。节点只消费 Gate 3 已发布的 `visual_reference_owner_set`，先以 GORM 重读并验证该输出指向的 `visual_foundation_reference.confirm` CommandReceipt、Receipt 内容 Hash、当前 Approved Plan 与 Project Activation Head；随后按 `target_business_key` canonical 顺序选择 Plan 中无依赖且非 `not_generated` 的基础 Target，并逐项调用上述既有 Backend 持久执行服务。同一 Node/Stage 先发布一个覆盖完整基础波次的共享 Manifest，每个 Target Invocation 只引用其中自己的 semantic shard；合法 canonical business key 可以超过 200 字符，因此既有 Invocation/Read 的 `shard_key` 由 GORM Catalog 使用 `text` 保存完整值，不截断、不另存 Hash。任一 Target 为 rejected、`outcome_unknown`、事实漂移或依赖未就绪时节点失败并交由 Temporal 重试；已 accepted Target 依靠 Invocation/Candidate 幂等重放，不重复调用 Agent。全部基础 Target accepted 后，节点原样透传 Gate 3 Owner Set 作为下游屏障；每个 Brief 的 CandidateRevision/Head 仍是 PostgreSQL 唯一事实，不构造虚假的 Candidate Set、第二事实源或隐藏集合 Hash。
...
待审组附件由 Backend 从精确的 `ready_for_review` Staged Media 编译，按 Subject 的槽位顺序逐项匹配 scope、Execution、bundle index、slot key、媒体 revision/hash/digest，拒绝缺项、多项、重复或非就绪事实。每项只携带 `slot`（完整 VisionReviewSlot）、`provider_receipt_ref`、`media_type`、`byte_length`、`pixel_width/height`、`page_count/frame_count` 和 `rights_observation`；不携带对象存储地址、凭据或本地路径。当前已实现的 Generation Staged Media 为 PNG 静态图片，page/frame 均为 1；单图最多 10 MiB、单组最多 32 MiB，宽高 1–8192，单图最多 16777216 像素，与已有 PNG 解码策略一致。权利观察固定保留 `not_assessed`，Receipt 仅证明生成来源，不代表权利批准。Go/Python 验证同一闭合附件语义；附件本身不是完整 Invocation Input、内部送审授权或正式使用许可。后续完整 Input 必须同时绑定用途准入、Brief、Style/Policy 和依赖资产，transport 再核对真实只读 bytes。
...
Gate 2 的 MVP 不引入通用 Patch DSL，只接受四个语义操作：`revise_production_entity`、`rebind_scene_occurrence`、`revise_interaction`、`revise_continuity`。Gate Input 必须从同一冻结 Candidate 派生四组有序 `repair_targets`，Review Detail 只复制这份不可变 inventory，不能由前端或 Agent 补写。Backend 再从冻结的六视图机械计算并排序去重 `scene/entity/state/occurrence/interaction/continuity/ledger` 七类闭包；Scene、Occurrence、Interaction 或 Continuity 修复只扩展到直接 Interaction 和一跳 Continuity 邻接，不再从新增邻场递归扩散；Production Entity 修复覆盖该 Entity/State 的完整 shard。不存在的 key、操作与 key 类型不匹配、重复或未排序 target 一律拒绝。该闭包只是 repair Run 的授权输入，不直接写 Owner，也不允许 Agent 自报影响范围。Review 持久化、唯一 root repair Run、逐阶段 directive 注入、闭包保持检查和新 Gate Input 全部接通后，Gate 2 才固定公开 `approved|changes_requested|rejected`；任一环节无法形成可执行修复时必须拒绝决议，不能制造恢复死路。
