...
Bible Review/Repair 的冻结边界已经落地：Backend 本地 `bible-deterministic-gate-v1` 只根据冻结 Reconciliation Candidate 的 Key/引用结构产生排序 blocker，模型已有的 Conflict/Review Issue 不参与 Gate 计算；`review_storygraph` 输出不再包含任何 Gate/blocker 字段，只能返回属于目标 Candidate Evidence 集合的 Review Issue。`repair_candidate` 同时绑定目标 Candidate Revision 与产生目标 Issue 的 Review Candidate Revision，输入冻结目标 Issue、允许字段、规范片段 Hash、只读邻接和轮次上限；输出 Patch 只能命中该允许集，并按字段契约限定为 `text` 或 `strings` replacement。Bible 允许字段为显式集合，不含 Evidence、Identity Key、对象字段或任何 Graph 写入字段，且 Review/Repair Payload 都拒绝 `base_storygraph_version_id/hash`，因此尚未发布的 Candidate Repair 与已发布 StoryGraph 修改保持两条不相交的 DTO 路径。

Backend Candidate Repair Coordinator 已在上述边界内应用 Patch：Application 先在父 Candidate 中按稳定 Key 定位 frozen fragment、重算 base fragment hash、逐字段替换并重新执行严格 Story Reconciliation Candidate 校验；GORM Store 随后在一个 PostgreSQL 事务中锁定 expected Head、重验父 Revision、Review Revision 与成功 Repair Result，创建 N+1、CAS Head，写入复用的 `CommandReceipt`，并为受影响闭包写不可变 `StageInstanceStaleness`。闭包从被替换的 exact Candidate Revision 出发，既沿 Invocation 的 exact upstream revision id/hash，也沿 Backend Aggregate Origin 的 exact leaf revision id/hash 逐层传播；aggregate Stage 的 staleness 不伪造 Invocation 引用。已应用 Repair Invocation 作为新 Revision provenance 明确排除，未受影响分支不标 stale，原 Invocation/Result/Revision 不修改。闭包无法完整解码时整个事务回滚；同一 Patch 重放返回同一 Receipt，不同幂等键继续使用旧 expected Head 时冲突且不留 Receipt。

Temporal 驱动的 Bible Review/Repair 闭环也已落地为 `agent.story_review@1.0.0`：Workflow Node 冻结根 Candidate Revision 与 `max_repair_rounds=1..3`，Backend 为当前 Candidate Head 创建同一 `review_storygraph` Manifest identity 的不可变版本和 Review Invocation。每个 Review 结果都由 Backend 对当前 Candidate 重新执行 deterministic gate；Gate blocker、模型失败、不可修边界或预算耗尽都停在 `needs_review/failed`，不会产生 Node Output。存在可修 blocking Issue 时，Backend 只为排序后的首个 Issue 创建一次冻结 Repair Invocation；成功 Patch 通过既有原子 Repair Coordinator 产生 N+1，下一次 Temporal 持久轮询再为新 Head 创建父 Hash 相连的 Manifest V+1 和 replacement Review Invocation。只有新 Head 的 Gate 无 blocker 且模型 Review 无 blocking Issue 时，Node 才发布该精确 Revision id/hash/version。Repair Invocation 总数是持久预算事实，服务重启、重复 Activity 和并发领取均不能重置轮次；实现复用既有 `ShardManifest`、`AgentInvocation`、`StageCandidateRevision/Head`、`CommandReceipt` 与 `StageInstanceStaleness`，没有新增 Migration、Raw SQL、第二 ORM、第二数据库或第二 Workflow 引擎。
