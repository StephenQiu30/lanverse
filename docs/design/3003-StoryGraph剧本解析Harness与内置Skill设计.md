...
- 状态：设计队列中已冻结；仅在 `SG-D01` 接受后激活 `SG-D02` 评审
- 日期：2026-08-26
...
- 评审规则：必须先单独接受 0010（`SG-D01`），才能单独评审本提案（`SG-D02`）。`SG-D02` 接受前不移动目录、不修改 Execution Policy、不新增 Invocation Kind，也不建立兼容入口；后续只引用文档中心 `SG-Dxx` 与 0010 `SG-Ixx` 唯一顺序
