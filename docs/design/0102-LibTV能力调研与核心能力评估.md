...
- 状态：草案，待评审
- 日期：2026-09-25
- 目的：弄清 LibTV 的能力范围与实现设计，评估 Lanverse 真正要做的核心能力，并为 0201 的顶层架构提供依据
- 关联：[0101 产品定义与需求分析](0101-产品定义与需求分析.md)、[0201 系统架构设计](0201-系统架构设计.md)
...
2. **它的画布基于 React Flow**，节点按“资源 / 生成 / 编辑”三类动作组织，生成结果由后台任务按字段白名单回写到节点【事实】。这印证了 0301 的画布选型。
...
| # | 设计点 | 证据 | 我们的判断 |
| --- | --- | --- | --- |
| D1 | 画布基于 React Flow | 【事实】前端含 `react-flow__node / edge / minimap / handle / controls / background` 等类名 | 与 0301 D8 一致；React Flow 足以承载同类体验 |
...
| D5 | 连线表达输入 | 【事实】连线时根据上游节点类型自动填充下游参数（如风格素材节点连到图片生成节点时带入模型） | 连线 = 参考 / 输入；与 0201 “参考连线即参考组合”一致 |
...
优先级沿用 0101：MVP / V1 / V2 / 待定。“对照”列说明与 0101 的关系。

| 能力 | LibTV | Lanverse 定位 | 优先级 | 对照 0101 |
| --- | --- | --- | --- | --- |
...
| 对话式 Agent（一句话出片、自动编排） | LibTV Agent、100+ Skill | 顶层预留，功能后置 | V2 | CNV-07 扩展 |
...
| AGT-01 | 对话式 Agent：自然语言驱动与 UI 相同的命令 | V2 | D9 |
...
6. **持久化的流程编排**：Operation 的调度、重试、等待、对账与人工确认由工作流引擎统一承担（按 0301 第 3 版确定为 Temporal），工作流只由一方（Go）编写，AI 执行作为独立的 Activity。
7. **合规与来源是媒体属性**：每个媒体资产带来源（上传 / 生成 / 哪个 Operation）、合规状态、授权记录。

这些要求在 [0201](0201-系统架构设计.md) 中落地。

## 7. 待确认

| # | 问题 | 建议 |
| --- | --- | --- |
| R1 | 是否接受第 5.2 节补入 0101 的需求 | 评审时确认 |
| R2 | 是否需要登录后实测 LibTV（画布交互、Agent 质量、成本） | 建议 P0 期间由产品负责人登录实测，并补充本文 |
| R3 | Agent 是否在 V1 就提前做“单镜头改写 / 提示词建议”这类小范围能力 | 建议 V1 评估 |
