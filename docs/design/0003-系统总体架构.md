...
原 `0002` 架构决策已合并到本文；运行程序按真实隔离或扩缩容需要拆分，不预建目标目录。每个业务事实只有一个 Owner，Frontend/Agent/Provider/Projection 不直接写业务数据库；Temporal 管长任务恢复，Kafka 只传输已提交事件。完整边界以本文及服务子设计为准。
