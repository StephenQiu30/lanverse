...
Go Backend 是业务事实唯一 Writer；Python Agent 只生成 Candidate。PostgreSQL/GORM 是唯一 SQL 事实源，Temporal 保存跨步骤执行历史，MinIO 保存对象字节，Kafka 解耦已提交事件，Elasticsearch 保存可重建剧本/StoryGraph 检索投影，Logstash/Elasticsearch/Kibana 管理已脱敏结构化日志；日志不经过 Kafka。
...
flowchart TB
  subgraph FE[Frontend]
    ...
    R[Review]
    PS[Web Provider Settings]
    AC[Authoring Canvas - deferred]
  end

  API[lanverse Backend / single binary / only writer]
  PG[(PostgreSQL + GORM)]
  OS[(MinIO)]
  ...
  PR[Backend Media Provider Registry]
  PROV[Seedream / Seedance / GPT Image / Nano Banana]
  MED[Media]
...
→ ShotVideo GenerationTarget + GenerationCandidate + CandidateSelection
...
| `lanverse` Backend | 唯一 `backend/cmd/main.go` 同进程装配公共 REST/SSE、所有业务 Command/Query/Owner Apply、Temporal Workflow/Activity 和 Kafka/Search 投影；`replay`/`reindex` 作为同一 Binary 子命令运行 | 已实现单一 Binary；内部职责仍遵循模块边界和独立 readiness |
...
| 系统 | 保存内容 | 事实边界 |
|---|---|---|
| PostgreSQL/GORM | 业务聚合、不可变版本/Head、Run 业务投影、Candidate、Artifact 元数据、Provider Connection/Credential/Profile/Binding 版本、Review、Cost/Quota、Outbox/Inbox | 唯一 SQL 事实源和业务 Writer 目标；Credential 只保存密文 |
...
| Kafka | 已提交 Script/StoryGraph 业务事件 | 传输边界，不是业务查询源，也不承载日志 |
...
日志链路固定为 `JSON Log → fail-open TCP → Logstash → Elasticsearch Log Index → Kibana`。Logstash 不消费业务事件；无效记录只保留错误码与 SHA-256 后写入独立 Dead-letter Index。未脱敏剧本、Secret 与生成内容不进入日志索引，Logstash 故障不阻断业务事实提交。
...
Agent Candidate 与媒体生成都由 Backend 冻结输入和接收结果，但使用不同运行协议。媒体 Provider 能力必须允许零配置启动：Backend 内置无 Secret Preset Catalog，Workspace Owner 通过 Web 一次写入 API Key，Backend 以 Docker Secret root key 加密不可变 CredentialVersion，并发布精确 Project Provider Binding。浏览器、Agent、Temporal、Kafka 与 StoryGraph 均不能读取凭据或直连 Provider。

媒体 Adapter 当前强制覆盖 `volcengine_ark` 的 Seedream 5.0 Pro+/Seedance 2.0+、OpenAI GPT Image 2 和 Google Nano Banana 四个当前官方模型；这些不是动态插件、通配模型或后续预留。每个 Target 在 Cost/Quota 前解析并冻结 Connection/Credential/ModelProfile/Binding Version，按 Provider 真实同步/异步能力恢复，不自动 fallback。
...
→ GenerationRequest / ProviderJob / independent ProviderCall[]
→ Staging Output Validate
→ Artifact + GenerationCandidate[]
→ Quality Gate / HumanTask / ReviewDecision
→ CandidateSelection
→ Backend Owner Apply
→ AssetVersion / ShotImageBindingVersion / ShotVideoBindingVersion / StoryGraphVersion
```
...
10. 一个媒体候选对应一个 ProviderCall；只有首次 `PENDING → DISPATCHING` 可发送，未知结果按同一 Call/remote task 身份对账，不盲目重提。
