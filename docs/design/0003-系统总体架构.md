...
当前迁移入口已调整为 Go `lanverse-api`；尚未迁移的业务请求由该入口转发到内部 Python 兼容运行时。该拓扑只用于逐聚合替换，不能形成两套写入所有者，详见[迁移期后端语言切换策略](2003-迁移期后端语言切换策略.md)。
...
产品能力以两个发布门验收：`Guided MVP` 完成 Guided Studio 的可恢复生产闭环，`Platform Complete` 再补齐完整事件投影、Search/Cost/Quota/Observability、Canvas、协作与 Copilot。发布门不等同于迁移阶段编号；当前整合顺序以[平台迁移与服务整合计划](../plan/0007-平台迁移与服务整合计划.md)为准，详细产品边界见[产品范围与验收基线](../prd/0001-产品范围与验收基线.md)。
...
当前未落地或需迁移：Go Backend 五 Binary、独立 Agent、Temporal、Model Gateway、完整 Media Render、Debezium、Elasticsearch、Frontend Monorepo、TanStack Query/Zustand、Canvas、Yjs/Hocuspocus、统一 Review/QC/Quota 和完整 Compose。安全顺序见[平台迁移与服务整合计划](../plan/0007-平台迁移与服务整合计划.md)，资源级控制见[资源写入所有权迁移台账](../plan/0008-资源写入所有权迁移台账.md)。
