...
采用“前后端分离 + 领域模块化单体 + 独立工作流 Worker”。FastAPI API 与 Worker 共用一个 Python 代码库和领域模型，分别运行；PostgreSQL 保存业务事实，MinIO 保存媒体，Temporal Task Queue 分发跨分钟/小时、可重试和可恢复的生成任务。
...
flowchart LR
    ...
    TEMP --> IOW["I/O Worker / bounded threads"]
    TEMP --> MEDIAW["Media Worker / fixed slots"]
    IOW --> PG
    IOW --> MINIO
...
Temporal Task Queue 是 MVP 唯一消息队列，不再叠加 RabbitMQ、Redis Streams、Kafka 或 Celery。队列只分两类：`lanverse-io` 承载 Workflow 及供应商/MinIO I/O Activity，`lanverse-media` 承载 FFmpeg Activity，避免网络等待与媒体处理争抢同一并发额度。

阻塞式供应商或 MinIO SDK Activity 使用有界 `ThreadPoolExecutor`，其 `max_workers` 与 Worker 的 `max_concurrent_activities` 保持一致并通过环境配置。FFmpeg 由媒体 Worker 启动独立子进程，以 Activity 并发上限约束 CPU/内存；线程池不进入 FastAPI 请求处理，也不执行 CPU 密集媒体计算。扩容依据 Task Queue backlog、schedule-to-start 延迟、CPU/内存和供应商限流，不按请求数盲目加线程。
...
| 消息队列 | Temporal Task Queue | 持久任务、Worker 拉取、负载均衡和限流；不维护第二套 Broker |
| Worker 并发 | 有界 ThreadPoolExecutor + Temporal Activity 并发上限 | 隔离阻塞 I/O；媒体任务使用独立 Worker 和队列 |
...
- [Temporal Task Queue](https://docs.temporal.io/task-queue)、[Python Worker API](https://python.temporal.io/temporalio.worker.Worker.html)
