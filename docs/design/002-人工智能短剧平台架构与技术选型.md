...
│   │       └── repository.py
│   ├── integrations/         # provider/MinIO/FFmpeg adapters
│   └── worker.py             # RabbitMQ publisher/consumer composition root
└── tests/                     # 与 app 分离；只存放测试代码与夹具
    ├── unit/
    ├── integration/
    └── contract/
```
...
```text
frontend/
├── openapi2ts.config.ts      # OpenAPI URL 与 src/api 输出配置
├── tests/                    # unit/integration/e2e；禁止放入 src
└── src/
    ├── app/                  # router/providers/layout
    ├── api/                  # @umijs/openapi 生成；禁止手改
    ├── components/ui/        # shadcn/ui 源码组件；底层固定 Radix UI
    ├── features/<feature>/   # 业务页面、组件、view-model
    └── lib/                  # request、cn 等框架辅助
```
