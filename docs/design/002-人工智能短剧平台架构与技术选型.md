...
文本模型实际接入后可使用 LangChain 提供调用、消息和结构化输出能力。只有真实流程存在多步骤恢复、条件分支或人工中断时才引入 LangGraph；图节点只能接收可序列化的内部 DTO 并调用应用端口，不直接访问 Router、ORM 或供应商 SDK。
...
| AI 编排 | LangGraph（仅在真实多步骤用例需要时引入） | 无明确分支、恢复或人工节点时不安装；不替代平台 Task/RabbitMQ |
| 文本模型适配 | LangChain（实际文本服务接入时按需引入） | 当前不安装未使用依赖；图像/视频/语音不强套 ChatModel |
...
依赖只锁主/次版本策略和具体 lockfile，Design 不绑定可能快速淘汰的模型版本。当前不设置默认 AI 服务；实际业务接入时再确认服务、模型和配置。内部边界按真实用例拆分，禁止提前建立万能接口或空适配器。

LangChain 只在获得实际文本服务配置并开始首个真实契约测试时加入 `pyproject.toml` 和锁文件；LangGraph 还必须有已确认的多步骤恢复、条件分支或人工节点需求。不得为了技术选型先提交未使用依赖或空 `agents/chains/graphs` 目录。每次实际接入只新增当次需要的适配器、契约测试和能力配置，不复制业务状态机。
