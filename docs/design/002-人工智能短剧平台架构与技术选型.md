...
开发环境遵循 local-first。2026-07-30 工程基线改为：PyCharm 使用 Python 3.11.15 创建 `backend/.venv`，依赖由 pip 按提交的 requirements 锁安装；PostgreSQL 18.4、Redis 8.8.1、RabbitMQ 4.3.4 由 Homebrew 在本机运行；MinIO `RELEASE.2025-09-07T16-13-09Z` 是唯一默认使用的基础设施容器。Docker CLI/daemon 29.6.2 与 Compose 5.3.1 可用，但不会由 setup 隐式使用；日常开发只显式运行 `make minio-up`，完整 `docker-compose-env.yml` 仅作为隔离环境兜底。`setup` 只创建项目内 `.venv`、安装锁定依赖和执行 `npm ci`，不修改系统环境。
