...
开发环境遵循 local-first，以上版本来自 2026-07-28 本机只读盘点：Python 3.11.15 是已经存在的 uv 托管解释器，只复用、不重新下载；PostgreSQL 18.4 和 Redis 8.8.0 正在本地运行；Docker CLI/daemon 29.6.2 与 Compose 5.3.1 可用，但不会由 setup 隐式使用；RabbitMQ 与 MinIO 当前未提供本机命令或可核验容器，因此不虚构版本，保持为 D-006 待决定条件。`setup` 只预检和同步 `backend/.venv`，缺失时停止且不修改系统；Docker Compose 仅由用户显式调用。
