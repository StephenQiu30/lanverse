...
路由页面与只服务该页面的 ViewModel/Controller hook 放在 `app/(main)/pages/<feature>/`（Next.js App Router）；可复用业务组件放在 `components/<feature>/`；shadcn/ui 基础件放在 `components/ui/`。服务端状态由 RTK Query 管理（cache-and-network 策略、自动失效与重新获取），客户端本地草稿、选择和布局由 Redux Toolkit slice 保存。服务端业务状态不得复制到全局前端 Store 形成第二事实源。Vitest 与 Playwright 测试只进入 `frontend/tests/`，禁止在 `frontend/src/` 中放置 `*.test.ts(x)`、`*.spec.ts(x)` 或测试夹具。

`@umijs/openapi` 在开发或构建时从 `OPENAPI_SCHEMA_URL` 读取 FastAPI 的 `/openapi.json`，以 `serversPath: './src/api'` 直接生成请求函数和类型。业务代码直接使用 `src/api/`；只保留 `src/lib/request.ts` 作为基于 axios 的统一请求适配（base URL、auth 拦截器、错误映射），不再增加 `generated/services/client` 等重复目录。
...
| 领域 | 选择 | 原因 |
| --- | --- | --- |
| Python | Python 3.11.15 + uv 0.11.31 | 与本机有效解释器和工具一致；uv 只锁依赖、维护项目 `.venv` 和执行命令 |
...
| 对象存储 | 开发/MVP MinIO；生产可选 OSS | ObjectStoragePort 隔离 SDK 差异；私有对象、直传、hash 和迁移契约不变 |
...
| 交付 | Docker CLI 29.6.2；本地 Docker Compose；GitHub Actions | 与本机 CLI 一致；可重复环境，暂不引入 Kubernetes |

开发环境遵循 local-first，以上版本均来自 2026-07-27 本机只读盘点：Python 3.11.15 是已经存在的 uv 托管解释器，只复用、不重新下载；PostgreSQL 18.4 和 Redis 8.8.0 正在本地运行；Docker 仅有 CLI、daemon 未运行；RabbitMQ 与 MinIO 当前未提供本机命令或可核验容器，因此不虚构版本，保持为启动前待满足条件。`setup` 只预检和同步 `backend/.venv`，缺失时停止且不修改系统；Docker Compose 仅由用户显式调用。
...
实现时至少验证：目录与模块边界、空库初始化、真实 PostgreSQL/Redis/RabbitMQ/MinIO 集成、缓存故障回源、Schedule 并发防重、消息重投、供应商未知对账、存储契约、费用不变量、FFmpeg 样片和浏览器端到端流程。
