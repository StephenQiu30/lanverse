...
- 2026-08-13 前由 `metadata.create_all()` 建立、且尚无 `alembic_version` 的已有库，必须先备份，再做表、列、类型、默认值、约束和索引零差异校验。接管只允许五个已知完整签名：当前 51 表 Metadata 直接采用 head；分集计划 era 50 表结构采用 revision `7f3a9c1d2e84` 后升级；整剧文档 era 46 表结构采用 revision `4c8e2f7a9b31` 后升级；Provider-era 42 表结构采用 revision `8d9f2a6c4b71` 后升级；固定提交 `ce360d25^` 的 38 表历史结构采用 revision `95c0d24572c5` 后原子升级，依次补齐 Provider、整剧文档、分集计划与剧本改写增量。任一能力的部分表集、未知表、缺索引/约束或多余结构一律拒绝。`create_all()` 只可保留在隔离测试 fixture，不是部署或升级路径。
