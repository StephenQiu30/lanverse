...
- 2026-08-13 前由 `metadata.create_all()` 建立、且尚无 `alembic_version` 的已有库，必须先备份，再与当前 Metadata 做表、列、类型、默认值、约束和索引零差异校验，完全一致才允许采用 baseline；未知、缺索引/约束或多余结构一律拒绝。`create_all()` 只可保留在隔离测试 fixture，不是部署或升级路径。
