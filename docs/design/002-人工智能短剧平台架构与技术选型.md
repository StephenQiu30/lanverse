...
采用 LangGraph 表达“提取 → 校验 → 人工确认 → 继续”等有分支的 AI 步骤，LangChain 只提供文本模型调用、消息和结构化输出能力。图节点只能接收可序列化的内部 DTO 并调用应用端口，不直接访问 Router、ORM 或供应商 SDK。

LangGraph checkpoint 只保存一次 AI 推理流程的节点进度，`thread_id` 固定使用平台 `task_id`；PostgreSQL 的 Task/Attempt 仍是业务状态和对外查询的唯一事实，RabbitMQ 仍是唯一工作通知。不得把 LangGraph 当成第二消息队列、通用后台任务平台或费用/候选事实源。节点中的外部调用必须使用稳定 `provider_request_key` 并具备幂等恢复，图重放不得重复产生付费调用。

供应商按能力拆成 `TextModelPort`、`ImageGenerationPort`、`VideoGenerationPort` 和 `SpeechGenerationPort`，不设计一个接受任意字典的万能 AI 接口。LangChain 适配文本模型；图像、视频和语音由类型化 ProviderAdapter 直接适配供应商异步协议。Seedance 作为视频候选供应商时，通过火山方舟“创建任务 → 查询/回调 → 取消 → 下载”的异步契约接入，供应商任务 ID 落入 Attempt，最终视频进入 MinIO/OSS 并登记 MediaVersion/Lineage。
...
| AI 编排 | LangGraph（按真实多步骤用例增量引入） | 显式分支、可恢复节点和人工确认；不替代平台 Task/RabbitMQ |
| 文本模型适配 | LangChain Core + 所需供应商集成包 | 统一消息、结构化输出与模型调用；图像/视频/语音不强套 ChatModel |
| 数据 | PostgreSQL 18.4 + SQLAlchemy 2 async + asyncpg | 与本机服务端一致；事务、关系、JSONB、锁与成熟 ORM |
| 缓存 | Redis 8.8.0 + redis-py asyncio | 与本机服务端一致；cache-aside、有限 TTL、限流与异步连接池 |
...
开发环境遵循 local-first，以上版本来自 2026-07-28 本机只读盘点和官方镜像核验：Python 3.11.15 是已经存在的 uv 托管解释器，只复用、不重新下载；PostgreSQL 18.4 和 Redis 8.8.0 正在本地运行；Docker CLI/daemon 29.6.2 与 Compose 5.3.1 可用，但不会由 setup 隐式使用。D-006 已关闭：RabbitMQ 使用 `rabbitmq:4.3.4-alpine@sha256:c07a5e60f5429be18b3b7fd3a4dfa9a84c3372f88df084b6f2b22224192c360c`，MinIO 使用 `minio/minio:RELEASE.2025-09-07T16-13-09Z-cpuv1@sha256:13582eff79c6605a2d315bdd0e70164142ea7e98fc8411e9e10d089502a6d883`；缺少任一基础设施时显式运行根目录 `docker-compose-env.yml`。`setup` 只预检和同步 `backend/.venv`，缺失时停止且不修改系统。

依赖只锁主/次版本策略和具体 lockfile，Design 不绑定可能快速淘汰的模型版本。默认模型供应商在 PRD/采购验证后确定；内部端口先固定为 Text、Image、Video、Speech 四类能力。

LangGraph/LangChain 依赖只在 S2 首个真实文本提取图开始实现时加入 `pyproject.toml` 和锁文件；不得为了技术选型先提交未使用依赖或空 `agents/chains/graphs` 目录。后续每接入一个供应商，只新增该供应商适配器、契约测试和能力配置，不复制业务状态机。
...
外部事实按切片关闭，不一次性冻结全部未来设计：S0 前只接受 MVP/S0 工程边界并确定基础设施提供方式；本地 JWT 认证契约在 S1 前，Text Provider 与合规初选在 S2 前，Image/Video/Speech Provider 和额度在 S4 前，交付合规在 S5 前，性能/恢复环境在 S6 前。未到当前切片的决定保持 open，不阻塞已有 Plan 评审，也不得由实现者猜默认值。
