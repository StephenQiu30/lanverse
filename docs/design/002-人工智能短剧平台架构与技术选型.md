...
| 对象存储 | 开发/MVP 使用 `minio/minio:latest`；生产可选 OSS | 本地配置保持直接；ObjectStoragePort 隔离 SDK 差异，私有对象、直传、hash 和迁移契约不变 |
...
开发环境遵循 local-first。PyCharm 使用 Python 3.11.15 创建 `backend/.venv`，依赖由 pip 按提交的 requirements 锁安装；日常可直接复用本机 PostgreSQL、Redis、RabbitMQ，并按需执行 `make minio-up`。Docker 不会由 setup 隐式使用；需要完整开发环境时，基础 Compose 使用 `.env` 构建并启动全部依赖、server 与 web。生产在同一服务事实源上叠加薄覆盖层，使用 `.env.production`、隐藏基础设施端口、禁止现场构建并拉取已发布镜像。两套入口都通过健康依赖、持久卷和重启策略保证启动链，外部基础镜像使用 `latest`。`setup` 只创建项目内 `.venv`、安装锁定依赖和执行 `npm ci`，不修改系统环境。
