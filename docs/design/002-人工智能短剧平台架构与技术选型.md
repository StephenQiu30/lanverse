...
路由页面与只服务该页面的 ViewModel/Controller hook 直接放在 `app/(main)/<route>/`（Next.js App Router），例如 `projects/[projectId]/page.tsx`、`studio/[episodeId]/script/page.tsx`；不得增加会形成 `/pages` URL 的 `pages` 中间层。可复用业务组件放在 `components/<feature>/`；shadcn/ui 基础件放在 `components/ui/`。服务端状态由 RTK Query 管理（缓存键、tag 失效、按需重新获取与必要轮询），客户端本地草稿、选择和布局由 Redux Toolkit slice 保存。服务端业务状态不得复制到全局前端 Store 形成第二事实源。Vitest 与 Playwright 测试只进入 `frontend/tests/`，禁止在 `frontend/src/` 中放置 `*.test.ts(x)`、`*.spec.ts(x)` 或测试夹具。

`@umijs/openapi` 在开发或构建时从 `OPENAPI_SCHEMA_URL` 读取 FastAPI 的 `/openapi.json`，以 `serversPath: './src/api'` 直接生成请求函数和类型。业务代码直接使用 `src/api/`；只保留 `src/lib/request.ts` 作为基于 axios 的统一请求适配，S0 只实现 base URL/超时/错误映射，S1 本地 JWT 登录接入时再加 Bearer token 与 401 清理；不增加 `generated/services/client` 等重复目录。
...
- Temporal/Celery Beat/APScheduler JobStore：PostgreSQL Schedule/Fire + RabbitMQ 已覆盖 MVP 持久调度和执行投递，不建第二事实。
