...
`@umijs/openapi` 在开发或构建时从 `OPENAPI_SCHEMA_URL` 读取 FastAPI 的 `/openapi.json`，以 `serversPath: './src/api'` 直接生成请求函数和类型。业务代码直接使用 `src/api/`；只保留 `src/lib/request.ts` 作为基于 axios 的统一请求适配，S0 只实现 base URL/超时/错误映射，S1 真实 OIDC 接入时再加 token 与 401 恢复；不增加 `generated/services/client` 等重复目录。
...
外部事实按切片关闭，不一次性冻结全部未来设计：S0 前只接受 MVP/S0 工程边界并确定基础设施提供方式；OIDC 在 S1 前，Text Provider 与合规初选在 S2 前，Image/Video/Speech Provider 和额度在 S4 前，交付合规在 S5 前，性能/恢复环境在 S6 前。未到当前切片的决定保持 open，不阻塞已有 Plan 评审，也不得由实现者猜默认值。
