...
供应商接入按当前已实现能力建立窄接口，不设计一个接受任意字典的万能 AI Service，也不提前创建尚无调用方的四套 Port。S2 文本固定为 DeepSeek `deepseek-v4-pro` 非思考模式；S4 图片/视频方向分别固定为火山方舟 Seedream/Seedance，但只有进入 S4 并确认真实账号、精确模型、地域、额度、参数、查询/取消/回调和计价后才实现唯一火山适配器。Speech/Audio 仍在首个真实用例时决策。
...
固定技术选型表只记录当前必须安装和维护的工程依赖。S2 只加入 `langchain-deepseek==1.1.0` 和锁定的传递依赖，不额外安装顶层 `langchain`、LangGraph、本地模型运行时或火山 SDK。D-004 关闭并进入 S4 首个真实生成用例时，才把唯一火山方舟 SDK 的精确版本加入 `pyproject.toml` 与锁文件。不得提交未使用依赖、空 Port 或 `agents/chains/graphs` 目录；每次接入只新增当次需要的适配器、配置和契约测试，不复制 Task、消息或费用状态机。
...
外部事实按切片关闭，不一次性冻结全部未来设计：S0 前只接受 MVP/S0 工程边界并确定基础设施提供方式；本地 JWT 认证契约在 S1 前；S2 已确认 LangChain + DeepSeek 文本能力与合规基线；S4 前继续冻结 Seedream/Seedance 的精确账号契约并决定 Speech/Audio；交付合规在 S5 前，性能/恢复环境在 S6 前。未到当前切片的决定保持 open，不阻塞已有 Plan 评审，也不得由实现者猜默认值。
