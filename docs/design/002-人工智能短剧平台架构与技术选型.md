...
路由页面与只服务该页面的 ViewModel/Controller hook 放在 `app/pages/<feature>/`；可复用业务组件放在 `components/<feature>/`；shadcn/ui 基础件放在 `components/ui/`。ViewModel 由 TanStack Query 管理服务端状态，Zustand 只保存编辑器本地草稿、选择和临时布局。服务端业务状态不得复制到全局前端 Store 形成第二事实源。Vitest 与 Playwright 测试只进入 `frontend/tests/`，禁止在 `frontend/src/` 中放置 `*.test.ts(x)`、`*.spec.ts(x)` 或测试夹具。
