...
固定技术选型表只记录当前必须安装和维护的工程依赖。当前固定依赖不包含 AI 供应商 SDK、模型运行时或 AI 编排框架；只有已实现业务用例、真实服务配置和失败契约同时明确后才加入 `pyproject.toml` 与锁文件。LangChain 只在能实质减少真实文本调用适配代码时按需加入，LangGraph 只在已验收流程确有多步骤恢复、条件分支或人工中断时评估。不得为了技术选型先提交未使用依赖、供应商环境变量、空 Port 或 `agents/chains/graphs` 目录；每次接入只新增当次需要的适配器、配置和契约测试，不复制业务状态机。
