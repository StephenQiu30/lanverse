...
- 状态：proposed（完整平台范围；MVP-A 研究与已实施设计按对应 Acceptance 局部 accepted）
- 调研日期：2026-08-12；2026-08-14 增量复核
- 调研对象：GitHub AI 短剧/视频生产项目、墨鱼短剧 VibeReels 公开产品表面、Lanverse 当前实现
- 证据边界：公开页面、公开前端构建产物、公开 GitHub 仓库与本地源码；2026-08-14 在已授权墨鱼会话内只读审计，未创建/修改项目、未执行生成或付费、未写入第三方系统
...
### 3.5 2026-08-14 增量开源项目复核

本次补充优先审阅与“完整工作台、质量回路、成本和导出”直接相关的一手仓库。它们不替代上面的固定 commit 设计证据；实施前仍需为具体代码重新固定 commit 和逐文件审查。

| 项目 | 增量价值 | 采用决定 | 边界 |
| --- | --- | --- | --- |
| [LumenX](https://github.com/alibaba/lumenx) | Studio/Playground 分层、剧本/资产/分镜、多视角角色、批量候选、TTS、时间线、FFmpeg | 参考专业工作台与快速试验分层、用途能力目录和批量候选 | MIT；项目较新，README 不证明租户、任务恢复和成本完整性 |
| [Jellyfish](https://github.com/Forget-C/Jellyfish) | 候选确认、资产使用关系、ready 与 generating 分离、异步任务中心 | 继续作为候选/任务/门禁的高相关参考 | Apache-2.0；前端/版本历史仍需逐路径核验 |
| [LocalMiniDrama](https://github.com/xuanyustudio/LocalMiniDrama) | 缺失项补生成、局部重试、ZIP 导入导出、列表/画布同源 | 参考本地流、局部恢复和交接包体验 | MIT；单机/SQLite 不作为 SaaS 权限与审计底座 |
| [wind-comic](https://github.com/ChrisChen667788/wind-comic) | 质量循环、stale/cost、Yjs、FFmpeg、EDL/FCPXML/AAF | 参考后续质量回路、协作与专业导出方向 | MIT；年轻项目，名称/镜号弱身份不进入当前模型 |
| [aid-server](https://github.com/gzxx-2025/aid-server) | 统一任务、余额冻结/结算、运营和内容审核 | 参考成本与运营问题清单 | MIT；运行史不足，不作为成熟账本证明 |
| [dramai](https://github.com/hyyyyyyz/dramai) | 浏览器内创作、Dexie、ffmpeg.wasm、MP4/SRT/剪映 ZIP | 参考本地优先和客户端导出 | Apache-2.0；浏览器密钥、刷新恢复和协作边界需谨慎 |
| [AI Video Production Editor](https://github.com/LudwigKienle/ai-video-production-editor) | 连续性审核到返拍队列、专业时间线/导出 | 参考 Review Issue → 局部返工 | GPL-3.0；不得未经许可评审合入 |
| [MoneyPrinterTurbo](https://github.com/harry0703/MoneyPrinterTurbo) | TTS、字幕、BGM、素材和 FFmpeg 合成 | 继续作为后期横向证据 | MIT；不是剧本—资产—分镜一致性平台 |
| [drama-skills](https://github.com/worldwonderer/drama-skills) | 独立 coverage ledger、版本/hash 审查和 stale | 参考覆盖守恒与独立审查规则 | MIT；领域技能项目较新，不作为平台成熟度证明 |

Toonflow 仍只能作产品参考：其 LICENSE 含额外商业/去标条款，不能按标准 Apache-2.0 处理。DramaClaw 为 ELv2 source-available；BigBanana、CC BY-NC-SA 项目和无明确 LICENSE 的项目均不进入商业复用候选。

## 4. 墨鱼短剧 / VibeReels 公开产品审计

### 4.1 审计范围和可信度

本次以 [VibeReels 官网](https://drama.moyuai.com.cn/)、官网公开工作流图、公开前端构建/source map 和 2026-08-14 已授权会话内的只读页面为一手证据。未创建或修改项目、未触发任务/付费、未下载私有素材。公开/客户端证据可证明当前展示的数据层次、字段、入口和提示语义，不能证明后端算法、数据库、套餐权限、稳定性、账单准确性和真实生成质量。

因此以下审计是“公开表面 + 已授权只读产品审计”，不是付费生成、并发恢复或整集交付验收。
...
### 4.7 2026-08-14 已授权只读增量

只读项目工作区补充证明了以下客户端可见事实：

- 新建项目先在“漫剧”与“真人剧/3D 漫剧”之间选择，页面明确提示创建后不可修改；真人/3D 额外强调角色审核；
- 项目头部把剧本信息、资产配置、视频创作、项目设置、风格、协作者和成本放在同一上下文；
- 资产设计同时展示角色/场景/道具数量、出现 Episode、基础图、多剧情状态、音频、筛选/排序和可下载资产 JSON；
- 视频工作台以 Episode/Shot 组织导演信息、详细 Prompt、资产引用、上传/选择资源、生成历史、成功/失败、主选/星标和局部处理；
- 生成按钮在提交前显示可见费用档位；项目成本页按文本、图片、视频展示调用、模型、操作和成本；
- 基础图、状态图和视频分别选择模型，并可配置风格预设、Prompt 前后缀和项目参考图；
- 页面信息密度高、技术模型名直接暴露，且部分阶段文案与已有镜头/版本可能并存；批量问题视图、阶段/任务/结果分离和高级设置渐进披露值得优先设计。

![VibeReels 项目类型选择](assets/ai-short-drama-platform-research/2026-08-14-vibereels-audit/02-create-project-type.jpg)

![VibeReels 风格与模型设置](assets/ai-short-drama-platform-research/2026-08-14-vibereels-audit/03-style-model-settings.jpg)

增量证据与访问边界记录在[审计证据说明](assets/ai-short-drama-platform-research/2026-08-14-vibereels-audit/README.md)。
