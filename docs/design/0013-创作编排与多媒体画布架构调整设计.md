...
- 状态：已接受目标（2026-09-07 用户确认按新设计全面推进）；实现状态以验收证据为准
- 日期：2026-09-07
...
> 2026-09-23 技术修订：用户决定直接在 Lanverse 实现画布，参考 `basketikun/infinite-canvas` 的部分交互；具体渲染与适配以 [1003 画布方案](1003-多媒体创作画布与运行可视化设计.md) 为准。画布 AI 执行统一经现有 Agent 服务；Go 保留授权、正式资源、费用、账本与采纳。现有生成路径在按模型迁移前继续由当前 Owner 负责。本文件其他平台边界继续有效，方案细节待审阅。

## 1. 结论与变更范围

Lanverse 保留 Go 正式业务底座，将新创作流程按流程族逐步交给 Python Temporal Worker；受限 Harness 继续只执行有界专业任务。Go 拥有正式源版本、项目与资产、审阅决定、采纳结果、生成授权、费用和账本，Python 应用层拥有新流程的分析草案与运行证据。画布及后续迁移的 AI/供应商执行通过现有 Agent 服务统一进入；现有 Go Generation 调用在逐模型迁移前保留原执行 Owner。前端在现有 Next.js 应用中展示资源和执行投影，通过独立 CanvasOperation 编辑布局。

这是职责和协议调整，不以更换 ORM、前端查询库、消息基础设施或重排目录为目标。Go 与 Python 使用同一 Temporal 平台的不同流程类型和任务队列，不各自推进同一创作运行。

本轮由三份联动 Design 表达变更，各自只定义其负责的合同。用户已接受新方向，受影响的既有设计已增加明确的适用范围与优先级说明，保留迁移依据。文本与分镜 MVP 的执行范围和证据分别记录在 0013 实施计划及验收记录。
...
| Agent | Backend 私有、无持久存储的候选执行器 | 一个 Agent 应用服务内部保留可信应用、Workflow Worker 和受限推理模块边界 |
...
不在本次范围：重写所有旧 Workflow、迁移全部历史候选、批量改业务表名、完整支付/社区设计、自由代码节点、第二套调度器、多人 CRDT、创建未来目录。具体编码范围以用户随后确认的文本与分镜 MVP 为准。
...
| 前端已有 RTK Query，尚无真实画布页面 | `frontend/src/lib/server-state.ts`、`frontend/package.json` | 保留缓存体系，新画布只作为消费者 |
...
```mermaid
flowchart TB
  UI[Next.js 工作台与画布组件] -->|命令和查询| GO[Go 平台领域服务]
...
| 外部生成授权、正式 operation、UNKNOWN、媒体接管、费用 | Go Generation/Media/Cost/Quota | Go 保留正式 operation 与财务事实；画布及迁移后的 AI 执行经 Agent 调用供应商并回传身份/结果，不写 Go 业务库 |
...
现有 Go Generation operation 在完成逐模型迁移前仍由 Go adapter 负责供应商发送、请求身份、查询/回调去重、UNKNOWN、接管和费用协调。画布新增的 AI/供应商执行统一经现有 Agent 服务：Go Backend 校验项目权限与生成确认、固定报价和幂等 operation，再以签名内部命令交给 Agent；Agent 负责受限推理或可信 Provider adapter 调用并持久化执行/供应商回执，Go 仍负责正式 operation、费用结算、媒体接管与人工采纳。每个模型/能力只允许一个执行 Owner，不能让 Agent 与 Go 为同一 operation 各自发送。平台操作不能在返回 accepted 后依赖一次内存调用继续执行，提交与恢复必须持久化。
...
该一致切面仅覆盖平台持久资源/运行投影；独立布局附自身 revision，不宣称跨平台库、Agent 库与布局的全局事务快照。正式执行使用固定 InputSnapshot。前端快照恢复、鉴权流、RTK Query 与画布局部状态分工见 1003。
...
Frontend 沿用当前 Next.js 应用目录约定；画布组件随首个真实消费者落在 `frontend/src/components/canvas/`。开源项目的画廊/分组交互可按 1003 参考，其 Store、浏览器执行器与本地供应商设置不进入平台执行权威。
...
同步以变更的职责为单位，不重写全部文档，也不把原来已接受内容全部降级。3004 负责 Harness/专业流程的专项替代范围，1003 负责画布/工作台的专项替代范围；三份设计已联动接受；用户随后授权按新设计开发，并将当前实现范围收敛为文本与分镜 MVP。新增工作按 0013 实施计划推进，旧队列的历史完成记录不抵扣新架构验收。
...
- [Infinite Canvas 官方仓库](https://github.com/basketikun/infinite-canvas)：作为画布交互参考；领域数据和执行仍由平台实现，具体边界见 1003。
...
## 10. 当前 Backend MVP 范围

2026-09-07 用户将本轮收敛为“先完成文本与分镜闭环”：原稿导入、全稿事实与设定、可审阅文本分镜、正式采纳与恢复。图片/视频生成、完整画布、支付和成片不进入本轮。文本分镜确认只冻结导演意图、来源与覆盖，不自动提升为参考资产齐备的可生产镜头。旧视觉生产门继续约束后续真正媒体执行，不能为了文本 MVP 将其绕过。

实现先补齐已有正式源的精确读取及当前主链缺口，再接新 Python 流程与平台采纳。后端合同细化与执行顺序见 [0013 后端实施计划](../plan/0013-新架构后端实施计划.md)，验证直接复用本机环境。

## 11. 文本 MVP 的 Go 运行交接合同

新公开入口为 `POST /api/projects/{project_id}/creation-runs`、项目运行列表及 `GET /api/creation-runs/{run_id}`。创建固定 document_revision_id、source_hash、幂等键和服务端流程版本；调用者与权限来自现有登录身份。项目与用户共同约束幂等键，同键异输入返回冲突。只接受已经正式发布的精确源版本。

Go `production/creation` 拥有运行接受记录、固定路由及 HTTPS 命令 Outbox；它不执行创作步骤。运行和 Outbox 在同一 GORM 事务创建。运行状态区分 queued、delivery_unknown、delivery_blocked、accepted；accepted 只表示 Agent 已持久接受，并不表示创作完成。Query 返回固定源身份、交接状态、安全错误码及接受回执，不能将传输失败解释为创作失败。

新能力需要显式配置 `CREATION_AGENT_URL`；为空时创建返回 503，但历史查询继续可用。运行保存创建时的服务端地址与固定流程类型，后续配置变化不能把原运行切到另一种引擎。同一服务的部署地址迁移须显式声明原 origin，只改变传输路由；保留历史地址、原命令和回执身份，详见 0020。MVP 流程身份为 `lanverse.creation.text-storyboard.production`，确定性 Temporal Workflow ID 为 `lanverse:creation:{run_id}`。此身份专用于新 Python 流程，不能提交给旧 Go Worker。

Dispatcher 采用有租约、有 fencing token 的持久领取。先查询 Agent 的 `/internal/creation/commands/{command_id}`，不存在时重新核验原调用者当前权限，然后 POST `/internal/creation/commands`。查询/投递使用短时 HMAC 授权，绑定 method、path、production canonical body hash、过期时间；不持久化授权令牌。Agent 接受回执必须匹配 command/run/payload hash/流程类型/确定性 Workflow ID。HTTP 超时或结果无法判定保留原命令并退避重试；拒绝或权限撤销显示 delivery_blocked，可按原身份显式重投。权限撤销仍允许读取原接受回执进行对账，不允许首次提交新的创作工作。

Go API 进程拥有 Dispatcher 生命周期，退出时取消并等待，数据库会在领取租约过期后允许另一进程接续。此 HTTPS Outbox 不冒充 Kafka 业务事件。平台接受和 Python 的持久接受/Temporal 启动分别验收；HTTP 合同替身不能抵扣真实 Python 进程验收。

## 12. Python 命令接受与启动服务

可信入口位于 `agent/app/creation/`，独立于现有 `candidate_runtime` 进程。仅此入口安装 PostgreSQL/Temporal 可选依赖并持有 `CREATION_DATABASE_URL`、`CREATION_AGENT_SECRET` 与 Temporal 客户端；现有 Harness 入口不导入这些模块，也不接收这些凭据。服务默认不挂载公开业务接口、不调用模型、不写 Go 业务库。部署使用独立数据库及专用角色；不从平台 `DATABASE_URL` 回退取值。

命令采用第 11 节严格合同，拒绝额外字段、重复 JSON 键、非规范 UUID、非整数修订以及与 run_id 不一致的 Workflow ID。HMAC 绑定实际 HTTP 原始正文 SHA-256（Go 发出 production canonical JSON）、原始路径、method、audience 和最多 60 秒有效期。接受回执另使用 production canonical payload hash，避免将传输编码等同于业务身份。

同一事务写入不可变接受记录和启动 Outbox；command_id、run_id、workflow_id 均唯一。同命令同内容返回原 receipt_id/accepted_at，同身份异内容返回 409。仅凭原命令可查询持久回执，不依赖 Temporal 可用性。数据库迁移由显式命令执行，应用启动只验证迁移状态，不自动迁移平台数据库。

启动 Dispatcher 保存首次接受时的 Task Queue，使用数据库时钟、45 秒租约及递增 fence，单次网络尝试限制为 15 秒。先 describe 固定 Workflow ID，不存在才 start；使用 REJECT_DUPLICATE，响应不确定时继续查询原 ID。已存在的 Workflow 必须匹配类型、Task Queue 和 memo 中的 command_id/run_id/payload_hash；冲突持久标记 blocked，不能绑定错误历史。成功保存 Temporal run_id，未知结果持久退避，旧领取者不能覆盖新结果。数据库只调度启动交接，不调度创作步骤。

接受、启动和创作完成是三个不同事实。此服务不注册占位创作 Workflow，也不返回虚构草案；真实文本 Workflow、受限 Harness 调用及草案/事件事务在后续步骤实现。显式配置此入口前，应部署匹配队列的真实 Worker；仅验证 Temporal start 不抵扣模型与正式采纳验收。

Temporal 的 REJECT_DUPLICATE 仅在历史保留范围内有效（[官方身份与保留期说明](https://github.com/temporalio/documentation/blob/main/docs/encyclopedia/workflow/workflow-execution/workflowid-runid.mdx)）。因此启动前核验 Namespace 至少保留 24 小时，自动 start 只允许首次接受后 12 小时内；超窗仍先查询原 ID，存在则对账，不存在则持久阻塞，不把过期历史当成从未启动。服务 SDK 用法已通过 GitHub 插件复核 [Python SDK 官方说明](https://github.com/temporalio/sdk-python/blob/main/README.md)，实际 API 按已锁定版本和本机服务验证。
