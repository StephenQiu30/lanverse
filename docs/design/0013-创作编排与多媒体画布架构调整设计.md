...
新能力需要显式配置 `CREATION_AGENT_URL`；为空时创建返回 503，但历史查询继续可用。运行保存创建时的服务端地址与固定流程类型，后续配置变化不能把原运行切到另一种引擎。MVP 流程身份为 `lanverse.creation.text-storyboard.production`，确定性 Temporal Workflow ID 为 `lanverse:creation:{run_id}`。此身份专用于新 Python 流程，不能提交给旧 Go Worker。
