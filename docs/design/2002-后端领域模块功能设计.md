...
### Generation CandidateSet 与 Workflow Human Gate 切片

`CandidateSet` 在 MVP 中不是第二份持久聚合或新表，而是 Generation 从成功 `ProviderResultReceipt`、其全部 Source Output 对应的 `GenerationCandidate/QCReport` 与当前 READY Artifact 重建的不可变只读事实视图。Set ID 复用 Provider Job ID，Revision 固定为 `1`；成员只包含通过确定性 QC 的 Candidate，按 Candidate ID 排序，并使用与 `CandidateSelection.CandidateSetHash` 相同的 canonical Candidate Reference 结构计算 Content Hash。任一 Receipt Output 尚未物化、Candidate/QC/Artifact 漂移、当前权限失效或通过成员为空时均 fail closed，不能打开人工选择。

Workflow 的 `generation_candidate_set` Node Output 只保存 Set ID、Revision 与 Content Hash。`gate.generation_image_review` 打开时，Runtime 仍冻结完整 `node-input-v1`，但必须通过 Generation Application Interface 重新展开并核对 Set，随后由 Review Owner 把排序 Candidate ID 冻结到 `HumanTask`；客户端和 Workflow Repository 都不能查询 Generation 表或自报候选。`selected` ReviewDecision 生效时，Generation Human Gate Applier 只把 ReviewDecision ID 交给既有 `ApplySelection`，并要求返回的 `CandidateSelection` 与 Workflow Run/Node、HumanTask、CandidateSet Hash 和选中 Candidate 完全一致。Workflow Apply Receipt、Signal 与 Gate `node-output-v1` 绑定同一 `generation.candidate.select` Command Receipt，输出唯一指向 `generation_candidate_selection`。

本切片复用现有 Provider Result、Candidate/QC、Selection、Review、Workflow Signal、GORM Model Catalog 和单 PostgreSQL 事实源；不新增 CandidateSet 表、Migration、DDL/Raw SQL、通用事件路由或兼容 Subject。它只完成已物化 Provider Job 的候选集展开与选择生效，不在未选定首个真实图片 Provider 时实现 Provider Adapter、Generation 图片 Executor、Production Shot Binding、Temporal ShotWorkflow 或单 Shot 重跑。

### Production Shot 图片绑定切片

`CandidateSelection` 生效后，图片进入正式生产的唯一方式是由 Production/Storyboard Owner 执行 `storyboard.shot.bind_selected_image`。命令只接受正式 Shot ID、该 Workflow 输入已冻结的 Shot Revision/Content Hash、Generation CandidateSelection ID、Expected Current Binding Revision 与幂等键；Production 必须重读 Shot 并逐项核对，再通过自己声明的 `SelectedImageSource` Port 调用 Generation `RequireSelected`。它不能读取 Generation、Review 或 Asset 表，也不能接受调用方自报 Candidate、Artifact、Hash 或选择版本。Generation Adapter 必须把 Selection 中被选 Candidate 的精确 Candidate Revision、Artifact ID/Revision/SHA-256 与 Selection Content Hash 映射成 Production 所需的最小只读快照。

`ShotImageBindingVersion` 是 Production Owner 的追加式不可变事实。每个版本冻结 Workspace、Project、Episode、Shot 当前 Revision/Content Hash、CandidateSelection ID/Revision/Content Hash、Candidate ID/Revision、Artifact ID/Revision/SHA-256、Binding Revision 与 Content Hash；当前值由同一 Shot 的最大 Binding Revision 确定，不维护第二个可漂移的 current pointer。首绑要求 Expected Revision 为 `0`，替换要求精确匹配当前 Revision；并发同 Revision 只能有一个新版本成功。同一 Selection 只能绑定一个 Shot，同一幂等命令必须返回同一 Binding/Command Receipt，输入漂移失败关闭。

该切片只使用 Storyboard Application Port、Generation Adapter、Workflow Production Executor、GORM Model Catalog、共享 Command Receipt 与单 PostgreSQL 事实源；正式 `workflow-worker` 组合根装配真实 Selection/Asset Readiness 与 Binding Service。绑定前 Generation 会重新验证 Selection、QC 与 Artifact READY；绑定保存精确历史快照，后续 Render 仍须按该版本重新执行 Asset Readiness/Download Grant。不新增 Migration、Raw SQL、第二 ORM、通用 Binding 抽象、跨 Owner 大事务或兼容写路径。本切片不新增正式图片生成节点、System Catalog Shot 节点、Temporal ShotWorkflow、图片审核页面或单 Shot 重跑；Executor 只证明当正式图提供 Shot/Selection 冻结输入时可执行，测试 Catalog 不得冒充产品 Catalog。

### 正式 Shot Workflow 后半程切片

`lanverse.shot` 是与 Episode Catalog 并列的正式不可变 Node Catalog，不将 Shot 图片执行节点塞入现有 `lanverse.production` 后继续绑定 `lanverse.episode-production`。MVP 第一版只包含四个真实节点：`input.production_shot` 从 Production Owner 重读 active Shot；`input.generation_candidate_set` 消费该冻结 Shot 输出后从 Generation Owner 重建已成功物化的 CandidateSet；`human.generation_image_review` 打开现有候选审核并生成唯一 CandidateSelection；`production.shot_image_binding` 将该 Selection 绑定到同一 Shot。显式 Shot 输入边保证归档、越权或漂移的 Shot 在打开 HumanTask 前失败。编译器只从已发布 Revision 冻结的 Catalog Key 解析 Workflow Type：`lanverse.production` 对应 `lanverse.episode-production`，`lanverse.shot` 对应 `lanverse.shot-production`；Browser 和 Command 不能自报 Temporal Workflow Type。

`ShotProductionWorkflow` 必须作为独立 Temporal Workflow Type 注册，使用与 Episode 一致的确定性 Activity/Human Gate/Control/Failure/Completion 内核，但启动时必须拒绝 Episode Type 或错误版本。生产 Worker 显式路由 Generation 与 Production 两个 Executor；Generation Executor 只读 CandidateSet，Production Executor 只读 Shot/写 Binding，不建立可动态注册的通用插件层。两个 Source Node 都必须重新授权并校验 Workspace/Project、Revision 与 Content Hash；流转只通过 canonical `node-output-v1` 和冻结 `node-input-v1`，Workflow Repository 不读 Production/Generation 表。

本切片以真实 PostgreSQL/GORM、Temporal History/Replay、Review Decision、Generation Selection 和 Production Binding 验证首次图片绑定的 Shot 级闭环。它的 CandidateSet 必须已由成功 Provider Receipt 物化；不新增本地假图片、受控 Provider 生产分支、Migration、Raw SQL、第二数据库或第二调度器。真实图片 Provider/Generation Executor、Episode 动态扇出 `ShotWorkflow × N`、已绑定 Shot 替换图片所需的冻结 Binding Revision、Motion/Video/Render 和单 Shot 局部重跑仍属后续切片，不得因为已存在 `lanverse.shot-production` 名称就报告完整 ShotWorkflow 已完成。

### Shot Workflow v2 绑定目标与单 Shot 局部重跑切片

`lanverse.shot@1.0.0` 的 `expected_current_revision=0` 只能表达首绑，不允许修改已发布 Catalog 或在旧 Executor 中猜测当前版本。替换图片发布不可变 `lanverse.shot@2.0.0`：`input.production_shot@2.0.0` 在 Storyboard Owner 同一 GORM 事务内锁定 active Shot 并读取当前最大 `ShotImageBindingVersion`，同时输出冻结 `production_shot` 和 `production_shot_image_binding_target`。Target 以 Shot ID 为 Reference ID、下一 Binding Revision 为 Reference Version，Content Hash 包含 Shot Revision/Hash 与当前 Binding ID/Revision/Hash；未绑定时明确表示当前 Revision `0`，不增加 target 表或 current pointer。

`production.shot_image_binding@2.0.0` 必须同时消费 Shot、CandidateSelection 和 Binding Target，不再接受 Authoring 配置中的 Expected Revision。Storyboard Owner 在写入事务中重新锁定 Shot、重建当前 Target Hash，只有 Revision 和 Hash 都与节点冻结输入一致才追加新 Binding。并发重跑冻结同一 Target 时只允许一个写入者成功；已成功命令的 Receipt 重放仍返回原 Binding，不被后续版本误判为冲突。

单 Shot 局部重跑复用已有派生 Run 语义：源 Run 必须已终止，DefinitionVersion/RunInputSnapshot 不变，新 Run/Temporal ID 唯一；从 `shot` 根节点重跑四节点 Shot 图，从 Owner 重读最新 Target、打开新 HumanTask、产生新 CandidateSelection，最终追加 Binding Revision，不重跑 Episode Workflow。从下游节点尝试复用过期 Target 必须 fail closed；本切片的“重跑”是对已物化 CandidateSet 重新审核和绑定，不得报告为重新调用图片 Provider。

本切片不新增 Migration、DDL/Raw SQL、第二 SQL 事实源、兼容转发层或通用 Snapshot 框架。v1 Executor 只用于执行既有不可变 v1 Definition；v2 使用显式的 Source/Binding Executor 契约。真实云图片 Provider、Generation 生成 Executor、Episode 扇出、新候选生成和前端局部修复交互仍在本切片之外；本地 Codex CLI 仍只服务 Agent 文本/结构化 AI 调用。
