...
| Canvas（post-MVP，不进入当前 Gate） | 首个可操作/完整摘要/定位/交互反馈 | 未来 PoC 候选：≤2s / ≤5s / ≤1s / ≤100ms；95% 帧无 >100ms 冻结；状态 ≤5s | CUR-CAN-NFR-002/005/006 |
...
| 无障碍 | WCAG/P0 键盘路径 | WCAG 2.2 AA；不仅靠颜色/动画；当前 P0 不依赖画布 | 各模块 Accessibility NFR |
...
| Scripts | 100,000 code points/20,000 字/500 NarrativeUnit 仅作为性能观测样本 | 以真实分布观测，不作为导入或物化拒绝上限 |
...
flowchart TB
    ...
    API2 --> PG
    API1 --> PH["Production Harness<br/>LangGraph + PostgreSQL checkpoint"]
    API2 --> PH
    API1 --> SSE["SSE; polling fallback"]
    API2 --> SSE
    PH --> TW["Text worker pool"]
    PH --> PW["Provider worker pools"]
    PH --> MW["Media worker pool"]
    PH --> EW["Export worker pool"]
    PH --> RW["Projection/reconcile worker"]
...
| Accessibility | 当前 P0 列表/详情/播放/下载路径 WCAG 2.2 AA 和仅键盘验收；Canvas 在 post-MVP 单独验收 |
