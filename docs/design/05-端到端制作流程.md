...
updated: 2026-07-25
downstream: [DESIGN-06, DESIGN-07, PRODUCT-01, PLAN-01, ACCEPTANCE-01]
---
...
本切片同时交付可运行前端、API、PostgreSQL TaskJob Worker、LangChain Core AI 接入、Provider Adapter、MinIO 与 FFmpeg；只有整条链路通过验收才算完成。`PRODUCT-01` 只细化本闭环的用户价值和业务规则；`PLAN-01` 只实现本闭环及其测试。
...
无历史应用数据；只有[06 数据库表与迁移设计](06-数据库表与迁移设计.md)接受并通过 database_design_ready 后，本切片才创建逐表 SQL、Alembic `0001_mvp` 和根 `docker-compose.yml`。实现回滚按 frontend→API→Worker 反向停用新入口并恢复上一兼容制品，不停止或删除本机既有 PostgreSQL/MinIO。若真实 Provider 无法使用，可回退到 Mock 继续工程验证，但不能据此判定真实成片 Acceptance 通过。
