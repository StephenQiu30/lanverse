...
purpose: 把 MVP 核心设计组合为可由一份 PRD、一份 Plan 和一次 Acceptance 验证的完整用户闭环
...
updated: 2026-07-25
downstream: [DESIGN-06, PRODUCT-01, PLAN-01, ACCEPTANCE-01]
---
...
本切片同时交付可运行前端、API、数据库、Workflow、Provider Adapter、媒体存储与 FFmpeg；只有整条链路通过验收才算完成。`PRODUCT-01` 只细化本闭环的用户价值和业务规则；`PLAN-01` 只实现本闭环及其测试。
...
- 依赖：文本、图片、视频、TTS 每种模态恰有一个已批准真实 Provider，测试另有确定性 Mock。
...
| Worker 在任一步重启 | 轮询仍返回原 task_id 和已保存进度 | Temporal/OutboxEvent 恢复原 Workflow |
...
`PLAN-01` 必须把实现拆成可连续运行的 Red→Green 增量：
...
3. Red：重复派发、Worker 重启、unknown 和部分失败；Green：Temporal、OutboxEvent、图片/视频/TTS Mock。
...
无历史应用数据；本切片创建 `0001` 初始迁移和本地 Compose。实现回滚按 frontend→API→Worker 反向停用新入口并恢复上一兼容制品，不删除 PostgreSQL/Temporal/MinIO 卷。若真实 Provider 无法使用，可回退到 Mock 继续工程验证，但不能据此判定真实成片 Acceptance 通过。
...
- AC-ARCH-101-003：Plan 可并发重放同一命令 20 次并注入 Worker 重启，断言仍只有一个逻辑 Task/Workflow、一次初始 Provider 副作用、每槽一个 Candidate，且原 `task_id` 可继续查询。
...
以上 Design AC 只判断方案是否可执行；实际命令和运行结果必须由接受的 `PLAN-01` 实施后写入 `ACCEPTANCE-01`。
