...
`plan_reference_assets` 首版不拆成新的 Anchor Stage，也不执行两次模型调用。Backend 在单次 Invocation 前从同一 Owner Set 冻结 `reference_target_seed_inventory`：每个实际 Character 包含一个已知的 Identity-only Anchor 业务键、唯一 Identity/Specification、完整实际 State 选项及各 State 的 Scene/Occurrence coverage；Location、Prop、Scene 与 Interaction 包含可直接计算的固定 Target 业务键和完整输入闭包。种子清单及其 Canonical root、`p1_scope_keys[]`、已持久化 VisualFoundationCandidate Revision/hash 与严格候选快照、六类 PurposeProfile 一并进入严格 Input。Backend 必须从精确 Candidate Revision 重读候选字节并校验内容与 Production World/Preset lineage；Agent 不接收尚未发布的 EffectiveStyleSnapshot Ref。EffectiveStyleSnapshot 与 EffectivePolicySnapshot 只在 Gate 3 批准时根据同一 VisualFoundationCandidate 原子发布，不为本 Stage 预留 Owner 或制造伪正式版本。该种子不是一份可由 Agent 修改的预期 Plan，也不提前把某个 Character State 冒充已批准 Anchor。

Agent Candidate 只返回每个 Anchor 业务键选定的一个 `selected_state_ref`，以及派生后每个 Target 的 `target_business_key`、`target_kind`、`fulfillment`、PurposeProfile 范围内的 `design_focus[]`、完整 `forbidden_changes[]` 和 `depends_on_target_business_keys[]`。它不返回或改写 Identity/Specification/Scene/Occurrence/Interaction/Style Owner Ref；这些引用、coverage、约束和正式 Dependency Edge 均由 Backend 从冻结种子机械投影。Backend 先验证 Anchor 选择属于对应实际 State 选项，再按两遍算法得到最终 `expected_target_business_keys`，要求 Candidate Target 键集逐字节全等；随后要求 kind、依赖、fulfillment rank、design focus allowlist 与 forbidden changes 全等或属于预设允许范围。Candidate 缺失、多出、改名、合并 Target，选择非实际 State，或自行改变依赖都拒绝。只有通过该 Normalizer 的 Candidate Revision 才能进入 Gate 3；本 Stage 不创建 ApprovedReferencePlanVersion、不调用 Provider、不生成媒体。
