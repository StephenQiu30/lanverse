...
`plan_reference_assets` 首版不拆成新的 Anchor Stage，也不执行两次模型调用。Backend 在单次 Invocation 前从同一 Owner Set 冻结 `reference_target_seed_inventory`：每个实际 Character 包含一个已知的 Identity-only Anchor 业务键、唯一 Identity/Specification、完整实际 State 选项及各 State 的 Scene/Occurrence coverage；Location、Prop、Scene 与 Interaction 包含可直接计算的固定 Target 业务键和完整输入闭包。种子清单及其 Canonical root、`p1_scope_keys[]`、VisualFoundationCandidate Revision/hash、EffectiveStyleSnapshot Ref、六类 PurposeProfile 一并进入严格 Input。它不是一份可由 Agent 修改的预期 Plan，也不提前把某个 Character State 冒充已批准 Anchor。
