...
| 图片生成 | 火山方舟 Seedream 5.0 Lite，模型 ID `doubao-seedream-5-0-lite-260128`，由 `GenerationProvider` 隔离 |
| 兼容提示 | 模板记录原始模型；与当前生成模型不兼容时提示用户效果可能不同 |

来源公开可访问不等于允许采集或再发布。MVP 使用火山方舟北京区域；公开参考价 ¥0.22/张只用于预算基线，实际价格在激活和每次结算时记录。供应商字段不进入领域核心。
...
- 图片生成、任务状态和个人作品需要登录。
- 管理员接口需要独立权限并记录审计。
- MVP 使用管理员邀请账号，每人 5 张/日、全站 100 张/日，模型月度硬上限 ¥700，不实现支付。
...
2026-07-22 接受以下 MVP 基线：Seedream 5.0 Lite 北京区域；邀请制账号；每人 5 张/日、全站 100 张/日、月度模型硬上限 ¥700；DiffusionDB 1K；用户作品私有；主要运营法域为中国大陆；成人内容完全拒绝。

Design 已允许创建 PRD 和 Plan，但不等于允许真实联网或产生费用。外部采集仍需满足 `DESIGN-002` 的来源准入；真实生成还需配置凭证、确认最新条款/价格并由负责人启用 kill switch。

## 11. 参考资料

- [即梦 AI](https://www.jimeng.com/)
- [DiffusionDB Dataset Card](https://huggingface.co/datasets/poloclub/diffusiondb)
- [火山方舟 Seedream 4.0-5.0 指南](https://www.volcengine.com/docs/82379/1829186)
...
- 2026-07-22：v2.1.0 固化 Seedream 5.0 Lite、邀请制账号、额度预算和中国大陆运营基线，Design 转为 accepted。
