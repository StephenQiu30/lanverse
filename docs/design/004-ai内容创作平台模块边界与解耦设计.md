...
```text
frontend/                     # 只依赖生成的 API 契约，不导入后端实现
  src/{app,components/ui,lib,hooks}/
backend/
  apps/{api,worker,scheduler}/
  packages/
    contracts/
    core/{shared,identity,ingestion,catalog,search,creation,generation,asset,governance,workflows}/
    adapters/{infrastructure,sources,generation}/
  migrations/{module}/
  tests/
```
...
| `GenerationProvider` | 火山方舟 Seedream 5.0 Lite adapter |
...
只有独立扩容、故障隔离或部署节奏出现真实需求时才拆服务；拆分保留现有 Port、事件和数据所有权。Design 已接受：职责无重叠或循环依赖；供应商、RabbitMQ、MinIO 和搜索可替换；重复事件幂等；架构测试阻止越界导入；S0-S3 不提前实现视频、社区或支付。

## 12. 变更记录

- 2026-07-22：v2.0.0 将 creation、generation、asset、identity 纳入核心模块，明确模板发布与图片生成两个独立 Workflow。
- 2026-07-22：v2.1.0 固化首个 GenerationProvider；v2.2.0 增加 frontend/backend 顶层隔离和前端契约边界。
