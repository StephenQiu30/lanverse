...
updated: 2026-07-25
downstream: [DESIGN-04至DESIGN-07, PRODUCT-01, PLAN-01, ACCEPTANCE-01]
---
...
MVP 先实现确定性 Mock，并在 `PLAN-01` T-008 真实 smoke 前关闭全部启用真实 profile 的 Provider、模型、凭据、额度、地区/保留边界、查询/取消/幂等和失败计费决策。配置外 profile 或缺少必需默认 profile 必须启动失败。
