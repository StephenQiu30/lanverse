...
- 每张表只有一个 Go 模块拥有写入权。跨模块用例经公开 Service/Command 协调，不通过共享 sqlc row、pgx transaction 或跨模块 SQL 修改他模块表。
...
1. `backend/schema/current.sql` 是业务数据库唯一结构源，不采用 ORM Metadata，不建立增量结构历史链或数据回填链。
2. Go `backend/cmd/schema-init` 只允许对空业务库执行；建表、索引、约束、RLS、必要 PostgreSQL 对象和 schema fingerprint 必须由同一受审计命令一次完成。
