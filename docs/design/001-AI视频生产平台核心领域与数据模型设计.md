...
> 状态：提案 v11，已固定 Go 数据访问、单一 Kafka/Redis 协调、Elastic 派生读取数据边界、最终 Schema 与无历史数据兼容，待领域与数据评审
> 日期：2026-08-22
...
- 每张表只有一个 Go 模块拥有写入权。跨模块用例经公开 Service/Command 协调，不通过共享 database row、pgx transaction 或跨模块 SQL 修改他模块表。
...
- 领域事实、current 指针变更和 Outbox 记录必须在一个 PostgreSQL 事务内提交；不允许先发 Kafka 再补写数据库。
...
- Elasticsearch 只保存可重建检索文档和运行观测副本。日志、Trace、Elastic `_seq_no`、索引 alias 或命中结果不能替代 Outbox、AuditEvent、approved/current 或 PostgreSQL 投影基线。
...
| `nar_analysis_run` | project_id、source_manifest_hash、root_operation_id、analysis_contract_version、current_stage、current_gate、current_stage_generation、nullable current_child_agent_run_id、status、input_hash | 整本分析唯一根流程；root Operation 只由 M03 Service 完成，不由 child AgentRun 完成；取消/重试和迟到回执按 current stage generation 校验 |
...
- approved NarrativeRevision 的全文检索是 Elasticsearch 派生读模型，不增加 M03 业务事实表。检索文档只能从 approved/current Narrative 节点和 SourceAnchor 重建；Search Service 必须先从 PostgreSQL 解析允许的 exact revision set，再查询 Elastic。
...
LangGraph 等 Python Agent Runtime 的 thread/checkpoint 使用独立、可重建的运行存储，不属于 `agent_` 业务表；只初始化空存储的当前结构，结构不一致时拒绝启动，不建立 migration/兼容链。Go 业务查询只读取 AgentRun/Result/Proposal/Decision/Execution/Receipt，不能从 checkpoint 推断 stage、根 AnalysisRun、批准或 current 已完成。
...
| `ops_task` | operation_id、kind、schema_version、contract_hash、message_key、payload_ref、status、available_at、lease_owner/expires_at、attempt_count | 一个逻辑任务稳定 ID；重复投递复用同一行；Kafka offset 不表示任务所有权 |
| `ops_task_attempt` | task_id、attempt_no、worker_kind、started/ended_at、status、error_code | 只追加运行证据；不等同业务成功 |
| `ops_outbox_event` | event_id、topic、message_key、schema_version、contract_hash、payload_ref、trace_context、available_at、published_at、published_partition/offset、attempt_count | 与任一模块的领域事实同事务；topic 只能来自 000 固定清单；Broker ack 后才记发布坐标，失败重发同一 event/message key |
| `ops_inbox_receipt` | consumer_group、event_id、topic、partition/offset、payload_hash、received_at、result_ref | `(consumer_group, event_id)` 唯一，所有 Go 消费者先登记并接管 Task 再提交 offset；不是公共 API 收件箱 |
| `ops_dead_letter_case` | task/event_id、source_topic、consumer_group、reason_code、schema_version/contract_hash、payload_ref、first/last_seen_at、status、resolution_ref | 只保存安全摘要和受控载荷引用，不复制敏感正文；人工重放创建新 attempt 并复用业务幂等键 |
| `ops_search_projection_checkpoint` | projection_name、workspace_id、project_id、index_schema_hash、indexed_revision_set_hash、nullable indexed_knowledge_basis_hash、filter_completeness、source_event_checkpoint、status、as_of、nullable rebuild_operation_id、last_error_code | 只服务 `narrative_search`；同 project/schema 一个 current checkpoint；记录可重建进度和失败，不保存正文、不成为 Narrative/M04 current；知识不完整时全文可用但 entity/requirement filter 不得宣称完整 |
| `exec_provider_connection` | provider_code、credential_ref、region、status、policy_revision_id | 凭据只保存密钥引用 |
| `exec_provider_rate_policy_revision` | connection_id、nullable capability_id、action、rate_per_period、period_ms、burst、cost、effective_from/to、revision | 不可变规则；同 connection/capability/action 生效区间不重叠；Redis 只保存按本 revision 计算的过期 GCRA 计数 |
...
Kafka 与 Redis 运行态不扩展为第二套业务数据模型：全平台只有一组 `ops_outbox_event/ops_inbox_receipt/ops_dead_letter_case` 支撑唯一 Kafka 集群；模块不得建立私有 queue/outbox/inbox 表。Redis GCRA 决策不持有 lease，因此不存在 `rate_limit_lease` 业务表；需要审计时记录使用的 policy revision 和最终门禁结果。分布式锁也不建立通用业务锁表，owner token/TTL 只在 Redis；真正受保护的目标行或扫描 checkpoint 保存并比较最后接受的 fencing token，批准、主选、Job 唯一和用量预留继续使用各自 PostgreSQL 约束。
...
项目概览、叙事全局 occurrence、人物×剧集矩阵、生产清单摘要、依赖图、镜头进度、用量汇总和 Elasticsearch 剧本检索都是可重建读模型。投影更新失败不能阻止权威事务，但必须暴露修订、完整度、失败范围和滞后时间；关键审批不能仅依赖投影。Elastic 查询返回的 `indexed_revision_set_hash/checkpoint/as_of/stale` 必须与 PostgreSQL checkpoint 对照，不能把索引内部 sequence 当作产品版本。
...
1. `backend/schema/current.sql` 是业务数据库唯一结构源，不使用 ORM Metadata 自动建表或迁移，不建立增量结构历史链或数据回填链；GORM 仅用于 Repository 映射。
2. Go `LANVERSE_ROLE=schema-init go run ./cmd` 只允许对空业务库执行；建表、索引、约束、RLS、必要 PostgreSQL 对象和 schema fingerprint 必须由同一受审计命令一次完成。
3. 非空库只做严格 fingerprint 与结构校验；表、列、类型、默认值、约束、索引或 RLS 任一不一致即 readiness 失败，不自动增删改，也不识别或清理历史 revision marker。
4. 首个真实生产数据保留需求被接受前，Schema 变化通过重建开发、测试和预发布数据库落实。
5. Go API/Worker、Python Agent、JSON 和事件只使用同一发布单元的当前 Schema/Contract hash；非当前 `schema_version` 直接拒绝，不保留转换函数、过渡字段或多版本读取分支。
...
28. 删除整个 Elasticsearch 剧本索引后，从 approved NarrativeRevision/Anchor 重建新物理索引并切换 alias；重建期间业务写入和批准继续，查询明确 stale/unavailable，文档数、revision set/hash 与跨租户负向结果校验通过后才切换。
29. PostgreSQL 业务事实与 Outbox 同事务提交而 Elastic/OTel 故障时，命令仍成功且事件可重投；恢复观测或检索不得从日志补造 Outbox/AuditEvent。
