...
| 表 | 关键字段 | 约束 |
| --- | --- | --- |
| `nar_source_revision` | project_id、可选 content_unit_id、artifact_id、source_type、hash | 原件不可覆盖；整本首次拆集允许只绑定 Project |
| `nar_import_job` | project_id、source_manifest_hash、status、operation_id | 同解析配置可幂等复用；运行投影关联 Operation |
| `nar_analysis_input` | import_job_id、source_revision_id、ordinal、content_hash | 按序引用不可变来源，不复制正文 |
| `nar_parse_report` | parser_version、page_status_json、warnings | 追加报告 |
| `nar_episode_breakdown_revision` | import_job_id、status、rule_version、segmentation_hash、coverage_hash | approved 后不可改，current CAS |
| `nar_episode_candidate` | breakdown_revision_id、temporary_key、ordinal、title、anchor_start/end、rule_code、confidence、decision | 同修订 key/ordinal 唯一；边界无重叠并覆盖守恒 |
...
| `nar_narrative_revision` | import_job_id、breakdown_revision_id、mapping_id、based_on_id、status、summary、content_hash | draft/proposed/approved/current 显式；current 唯一，正式下游只读 approved/current |
...
| `nar_production_element_mention` | narrative_revision_id、content_unit/scene/beat/dialogue_id、element_type、surface_text、participation_kind、anchor_ids、status | Mention 不因实体决议删除；正式 M04 只读 approved/current 修订 |
...
| `pk_production_requirement_revision` | item_id、subject_type/ref、type、purpose、variant、effective_scope_id、quantity/unit、evidence_refs、decision、status | decided 后不可变；不含 readiness；不创建媒体 |
...
| `pk_production_coverage_decision` | narrative_revision_id、coverage_subject_type/ref、production_relevance、result、requirement_revision_ids、status、actor/reason | 创建即不可变；新 decision supersede 旧版；Inventory 不反写此权威事实 |
...
| `pk_production_requirement_inventory_item` | inventory_projection_id、coverage_subject_type/ref、nullable coverage_decision_id、requirement_revision_ids、coverage_status、unassessed_reason | coverage universe 每个成员恰有一行；缺权威决定时 decision_id 为空并具名 unassessed reason；无效引用也为 unassessed；可链接零到多个需求修订 |
...
| `agent_run` | session_id、operation_id、graph_id/version、input_contract_version、input_snapshot_hash、usage_cap、governance_evaluation_id、status、runtime_thread_ref、started/completed_at | 业务侧运行索引；不复制 checkpoint；不是 GenerationJob |
| `agent_proposal` | session_id、agent_run_id、based_on_versions、summary、expires_at | 基线变化后过期；同 run/result hash 幂等唯一 |
| `agent_proposal_item` | proposal_id、sequence、target_module、command_type、command_schema_version、command_payload_json、payload_hash、based_on_refs_json、evidence_refs_json、risk_level、decision、target_result_ref | M06 只拥有提案信封与决定；payload Schema、字段校验和接受后的目标结果由 target_module 版本化并拥有 |
...
AgentProposalItem 不能包含数据库补丁，只能包含目标模块公开、版本化的领域命令。`target_module + command_type + command_schema_version` 决定 payload Schema；M06 只验证信封、哈希、证据引用和通用风险字段，目标模块负责 payload 字段、不变量、权限和写入。接受时业务后端重新鉴权、校验每项 based_on 引用与 payload hash、生成幂等键，并把目标命令结果写回 `target_result_ref`；不得把无主通用 JSON 直接落表执行。LangGraph 等 Agent Runtime 的 thread/checkpoint 使用独立运行存储，不属于 `agent_` 业务表；业务查询只读取 AgentRun/Proposal，不能从 checkpoint 推断完成、批准或 current。
