...
> 状态：Python 主栈目标架构 v4，待容量基线与研发评审
> 日期：2026-08-20
> 对应产品设计：[018-AI视频生产平台目标产品与功能模块设计](018-AI视频生产平台目标产品与功能模块设计.md)
> 数据与实现细化：[020-AI视频生产平台核心领域与数据模型设计](020-AI视频生产平台核心领域与数据模型设计.md)、[021-AI视频生产平台接口工作流与功能实现设计](021-AI视频生产平台接口工作流与功能实现设计.md)
> 设计方式：从目标出发重新设计（绿地方法），不把现有技术架构、代码结构或服务边界当作前提
...
这个结论不是把所有工作放进一个 Python 进程。系统保持“一套语言、一个业务模型、多个独立进程角色”：API 只做短请求和短事务；调度/对账、Agent、外部 Provider 和 FFmpeg 分别在受限 Worker 中运行。进程可以独立扩缩和失败隔离，但首期仍是模块化单体，不按领域拆微服务。
...
- API、调度器、I/O/Provider Worker、Agent/Evaluation Worker 和 Media Worker 使用独立入口点；生产环境不由同一进程共同监督，避免任一 Worker 故障终止公网 API。
...
flowchart TB
    ...
    IOW["Python I/O / Provider Worker"]
...
| I/O/Provider Worker | 文档解析、厂商提交/查询/取消、下载并提交结果 | 保存任意业务 current、自动批准或选择 | 队列、提供商配额、网络 |
...
首期可以让 Outbox Publisher 与 Scheduler/Reconciler 共用一个部署单元，也可以让低流量 Agent 与 I/O 任务共用 Worker 镜像；但 API、调度/任务 Worker、Media Worker 不能共用生产进程生命周期。是否拆镜像由依赖和资源隔离决定，不由领域模块数量决定。

### 5.2 启动依赖顺序

1. PostgreSQL、对象存储和 RabbitMQ 先通过健康检查；Redis 不是首期启动依赖。
2. 单次 Migration Job 获取数据库锁并完成迁移；普通服务只校验 Schema，不自行并发迁移。
3. API、Outbox/Scheduler、I/O/Provider Worker、Agent/Evaluation Worker、Media Worker 在依赖健康后可并行启动，不形成“必须先启动 API 才能启动 Worker”的隐式顺序。
...
- Python API/Webhook/SSE、Outbox/Scheduler/Reconciler、I/O/Provider Worker、Agent/Evaluation Worker 和 Media Worker 使用独立进程入口并按指标扩缩。
...
1. 保留现有 FastAPI 路由、领域服务、Repository、RabbitMQ 拓扑和数据库模型，先增加/启用独立生产入口：`api`、`outbox-scheduler`、`io-worker`、`media-worker`。
...
4. 切片 B 再把 LangGraph/模型任务从通用 I/O Worker 收敛到 `agent-worker`；在此之前不创建空 Agent 服务。
5. 完成“停止任一 Worker 不影响 API、恢复后积压继续处理”和“API 重启不重复外部任务”故障测试后，再考虑拆镜像或增加副本。
...
先启用 API、Outbox/Scheduler、最小 I/O Worker 和 Media Probe Worker；完成工作区隔离、项目 Brief、来源直传/保全、叙事校对、修订基类、最小实体/状态、手工镜头方案、Operation、命令幂等、PostgreSQL Migration、Outbox、审计和基础观测。使用确定性解析或固定 Fixture，不依赖 Agent 和真实生成 Provider。

**验收**：用户可从真实来源手工得到带锚点的批准叙事和至少 10 个镜头；跨租户负向测试通过；重复命令无重复事实；Outbox/Worker 任一点重启后 Operation 可恢复。只有手工命令语义稳定后才允许 Agent 复用。

### 切片 B：受限 Agent 提案

新增独立 Agent Worker、LangGraph checkpoint、实体/镜头提案和人工逐项接受；Agent 复用切片 A 已完成的 Query 与命令 Schema，不新增旁路写入口。

**验收**：无 Agent 时手工流程仍完整；Agent 输出非法、重复、越权或过期时不改变业务事实；接受提案与手工提交得到相同领域结果；镜头重排不改变已发布有效范围。

### 切片 C：可恢复生成

扩展 I/O/Provider Worker，完成能力目录、计划冻结、用量预留、耐久 Job/Attempt、一个图像和一个视频提供商、媒体入库、候选选择。
...
- [ ] API、Outbox/Scheduler、I/O/Provider Worker、Agent/Evaluation Worker 和 Media Worker 使用独立生产进程生命周期与资源限制。
...
5. 现有 Python 服务中哪些纵向闭环可直接保留，哪些需要按 018—021 的新领域模型替换？迁移按闭环进行，不按文件夹批量重命名。
