...
flowchart LR
  ...
  A --> P[("PostgreSQL")]
  X["backend-worker"] --> P
  X --> R["Approved AI Providers / Models"]
  X --> O[("MinIO")]
  X --> F["FFmpeg/ffprobe"]
  W --> O
```
...
| `production_jobs` | SubmissionSnapshot、ProductionTask、ProductionAttempt、TaskEvent、TaskJob、TaskOutput、取消/重试/对账 | 创作基线、候选采用 |
...
前端由官方 create-next-app 建立 App Router，再由 shadcn/ui 显式选择 Radix base；`components/ui` 是生成并受控维护的基础组件层。Redux Toolkit 是唯一前端状态容器：RTK Query 保存服务端查询缓存并只对活跃任务执行 2 秒轮询，普通 Slice 只保存未提交编辑、选区和面板状态。不得使用 `redux-persist` 持久化 Task、Adoption、Delivery、签名 URL 或权限事实；刷新页面、网络断开或清空 Redux 状态后必须从 API 恢复。

## 6. 权限与安全边界

MVP 的主体只有一个由部署边界保护且不持久化的 `internal_operator`；操作仅使用 request/task/version 标识关联。

- 本地应用只绑定 loopback；root Compose 中 frontend 与 backend-api 的宿主端口显式绑定 `127.0.0.1`，backend-worker 不发布宿主端口。PostgreSQL 与 MinIO 复用本机既有配置，不由本仓库启动。
- Provider、数据库和对象存储密钥只来自当前 shell 或用户显式指定的仓库外环境文件，不进入 Git、浏览器、日志、TaskJob payload 或错误响应。
...
仓库不创建 `deploy/`。默认本地文件为根目录 `docker-compose.yml`，只含 `frontend`、`backend-api`、`backend-worker` 三个应用服务；其他环境按需叠加 `docker-compose-<env>.yml`，生产固定为 `docker-compose-prod.yml`，未进入范围的后缀文件不预建。Compose 直接读取当前 shell 中已有 PostgreSQL/MinIO endpoint、bucket 和凭据变量，不复制 `.env`、不启动基础设施容器、不创建数据卷。frontend `127.0.0.1:3000`、backend-api `127.0.0.1:8000` 是唯一本地发布端口；API/Worker 健康检查验证既有数据库和 MinIO 可达但不得打印 secret。

可观测性 allowlist 为 Compose healthcheck 和结构化日志，日志使用 `request_id/task_id/job_id/attempt_id/release_version/error_code` 关联。Plan 必须给出应用容器资源上限和只停止三应用服务的安全步骤。
...
- Worker 重启、租约过期和重复请求不得产生重复 Task、Candidate 或 Delivery。
- 渲染失败保留固定 RenderSnapshot 和既有媒体；系统自动重试在原 Task 下创建新 Attempt，用户重试创建引用原 Task 的新 Task。

初始数据库从逐表标准 SQL 驱动的 `0001_mvp` 建立。回滚停止新入口并回退应用制品，不停止或删除用户既有 PostgreSQL/MinIO，不删除 TaskJob、对象或成功产物；不兼容数据变更只能前滚修正。
...
- AC-ARCH-001-004：清除前端缓存或重启 API/Worker 后，PostgreSQL TaskJob 与 MinIO 足以恢复可判断状态。
- AC-ARCH-001-005：每个目录、接口、数据表、后台进程和 Acceptance 条目均可追溯到六模块与端到端业务流。
- AC-ARCH-001-006：安全边界限制为内部单操作者，根 Compose 与网络测试能证明应用仅 loopback 可达且未管理本机既有 PostgreSQL/MinIO 生命周期。
