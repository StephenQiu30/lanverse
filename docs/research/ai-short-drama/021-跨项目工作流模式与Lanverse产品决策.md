...
- 当前边界：不做剪辑、时间线、配音、字幕、整集成片和商业运营；必须支持逐镜视频候选选择与有序素材包导出
...
- 关键帧主选与视频主选分别存在；
- P0 建设与列表同源的生产关系画布；多图层局部重绘、蒙版和像素级自由编辑属于更深的视觉编辑能力，必须在关键帧候选闭环稳定后单独验证，不能阻塞首个可运行闭环。
...
| 当前非目标 | 剪辑、音频、成片、商业运营、社区市场 |
| 开源使用 | 只参考产品/工作流模式，不借用或移植代码 |

这组决议应作为后续需求、架构和验收文档的研究输入；如正式设计作出不同取舍，必须记录新证据、适用条件和迁移影响，而不是静默偏离。
