...
| 数据 | PostgreSQL 18.4 + SQLAlchemy 2 async + asyncpg + Alembic 1.18.5；单 Metadata 是模型设计/autogenerate 输入，已审阅 revision 是部署升级事实源；独立命令 upgrade，运行角色只检查 heads；旧 `create_all` 库经备份与结构零差异校验后才可采用 baseline |
...
| [PRD-012](../prd/012-AI短剧MVP核心制作产品任务.md) | 11 个整剧/改写/叙事/状态资产/分镜覆盖/导出 PT | PLAN-012 | active；范围已接受，MVPA-01 研究/spike 中；黄金样本阻塞 MVPA-02 |
...
| G-MVPA-001～003/006 MVP-A | 产品+制作人+技术负责人 | PLAN-012 各 DEV | G-MVPA-001 范围、G-MVPA-002 工程黄金样本和 G-MVPA-003 真实旧库迁移已关闭；制作人/QA 内容质量复核保留为分镜与分镜包产品验收；每个 DEV 在 Green 前继续完成 G-MVPA-006 的 GitHub 固定提交、许可证、源码/测试和不采用项审查 |
...
本节 DEV 行是既有 S0～S6 的工程执行台账；MVP-A 的唯一细化台账在 PLAN-012 §5，不在本文复制 12 行状态。S0～S3 已完成并链接 Acceptance；PLAN-012 的 `DEV-MVPA-01/02` 已完成 migration 与工程黄金 fixture 准入，下一任务为 `DEV-MVPA-03`。真实 DeepSeek 起点的 S2→S3 联合 E2E 已于 2026-08-04 通过；既有证据不替代新增整剧、改写、状态资产或覆盖验收。
...
| MVP-A 后续 migration 漂移 | G-MVPA-003 已用本机真实 38 表/19 行旧库关闭；后续每个新增模型仍须独立 revision、空库/当前 head/含样本库前滚与恢复验证，未知外部库不得套用本次小数据量结论 |
...
DEV-S0-01～05、DEV-S1-01～04、DEV-S2-01～06、DEV-S3-01～03 已完成，S0～S3 accepted；证据见对应 Acceptance。当前产品主线是 PLAN-012 MVP-A，`DEV-MVPA-01/02` 已完成并关闭真实 migration 与工程黄金 fixture Gate，下一任务为 `DEV-MVPA-03` 整剧导入与格式体检。制作人/QA 尚未对 mock 的内容质量签字，该项保留到分镜与分镜包产品验收，不冒充已接受内容质量。S4 在 D-004 真实账号能力、参数、额度和账单证据前保持 proposed，不与 MVP-A 并行冒充完成。
...
| 3 | 创建 database.py（历史 S0；现由 PLAN-012 migration 接管） | `backend/app/core/database.py`、`backend/alembic/` | async engine + session factory + `get_async_session()`；部署结构由 Alembic upgrade 管理，运行角色 revision fail-closed；`create_all()` 只保留在隔离测试 fixture |
