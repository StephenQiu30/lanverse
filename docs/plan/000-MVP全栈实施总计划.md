...
| DEV-ARCH-01 | Codex / in_progress | 待实测 | SYS-NFR-008/009；DEV-S2-02 工程门禁 | R0 边界守卫、R1 公开 seam、官方脚手架基线迁移及 R2 scripts 版本/提取能力拆分已完成；跨模块内部导入 allowlist 已清零。下一步执行 R3 projects/identity 能力拆分并收敛测试 fixture | 无 uv 环境/命令；requirements 锁可从 pyproject 重建；前端官方脚手架基线可验证；scripts 根 API 仅组合能力 Router 且不再保留根 service/schema；OpenAPI 生成结果无漂移；全量 `make check` 通过 |
