...
本 Plan 已具备可执行输入、任务、文件边界、验证命令和停止条件，文档状态为 `ready`。S0、S1 已通过真实实现、浏览器验收和远端 CI 接受；D-003 已按“不预设 AI 供应商、模型或配置”关闭，S2 已从 `DEV-S2-01` 开始实施。D-005 合规决定仍 open，它阻止治理、ready 资产和 S2 被 accepted，也阻止真实生成、公开交付和上线，但不阻止 Task、剧本版本和私有媒体的内部工程，边界见 [DES-005](../design/005-S2供应商与合规准入设计.md)。D-006 已选择“复用本机 PostgreSQL/Redis/RabbitMQ，仅 MinIO 默认由 Docker 提供；完整根环境 Compose 作为显式隔离兜底”。后续切片仍必须按依赖激活，不得并行预建空实现。
...
| D-003 AI 服务接入边界 | 产品+AI | 已关闭 | 不预设供应商、模型、地址、密钥名或价格 |
| D-004 Image/Video/Speech Provider | 产品+AI | S4 | submit/query/cancel/计价契约 |
| D-005 地区/授权/审核/标识/留存 | 产品+合规 | PT-GOV-001/002 与 S2 accepted；S4 真实生成；S5 公开交付 | 适用规则、责任人和测试材料；未关闭时 fail-closed |
...
本节 DEV 行是唯一工程执行台账，不在其他文件复制状态。S0、S1 已完成并链接 Acceptance；D-003 已关闭，`DEV-S2-01` 由 StephenQiu30 领取并进入 `in_progress`。D-005 不阻止前三个 S2 工程任务，但阻止治理和 ready 资产验收。`ready` 表示当前动作已有明确责任角色和输入；`blocked` 必须写明解除条件；DEV 进入 `in_progress` 前把“待指派”替换为具体负责人并记录估算；完成时把证据替换为可访问的评审、测试或 Acceptance 链接。
...
| S2 | `scheduler.py`、`worker.py`、`modules/production/` 最小 Task、`scripts/`、`media/`、`governance/`、`assets/`；RabbitMQ/MinIO 真实适配 | `app/studio/[episodeId]/script/`、`app/studio/[episodeId]/assets/`、`components/studio/`、`components/tasks/` | 版本/上传/权利/消息契约、同一 backend 镜像运行 API/Scheduler/Worker 与剧本资产 E2E |
...
| ID | 责任人/状态 | 估算 | 对应 PT | Red 与最小实现顺序 | 退出证据 |
| --- | --- | --- | --- | --- | --- |
| DEV-S2-01 | StephenQiu30 / in_progress | 8～12 小时 | PT-PROD-001、PT-MQ-001/002 | 先写 Task 事实、Outbox/Inbox、confirm 丢失、ack 前后崩溃和重复投递测试；再创建真实使用的 `scheduler.py`/`worker.py`，实现 script_extraction 最小 Publisher/Worker | 刷新和 Worker 重启可恢复；重复消息一次业务效果；API/Scheduler/Worker 使用同一 backend 镜像 |
| DEV-S2-02 | 待指派 / proposed | 待领取 | PT-SCR-001–005 | 先写版本不可变、幂等提取/决议、未决候选、current 冲突和安全清理测试；再实现 scripts 用例；只有用户提供实际 AI 服务配置后才增加对应适配器 | 导入→提取→决议→确认结构可恢复；未配置 AI 服务时明确返回能力不可用，正文不进日志/消息 |
| DEV-S2-03 | 待指派 / proposed | 待领取 | PT-MED-001–003 | 先写直传完成、hash/MIME/大小、半失败、探测、短期访问与归档测试；再实现 media 版本/位置/探测 | 字节不经 API；无永久 URL；失败不产生可用 MediaVersion |
| DEV-S2-04 | 待指派 / blocked | 待领取 | PT-GOV-001/002 | 先写授权过期/撤销、证明访问、RightsGate unavailable 和跨空间测试；再实现 Consent/Revision 与权利门禁 | D-005 关闭后才可接受；权利不足可保存 draft 但不得 ready/生产 |
| DEV-S2-05 | 待指派 / blocked | 待领取 | PT-AST-001–003、PT-AST-005 | 先写六类规格、版本/current 并发、权利/媒体 readiness 和候选幂等测试；再实现 assets 与 scripts 公开交接 | DEV-S2-04 与 D-005 关闭后解锁；至少角色/场景/声音 ready；JSONB 不绕过类型校验 |
| DEV-S2-06 | 待指派 / proposed | 待领取 | PT-PRJ-004/005 增量；S2 全部 | 接入 current ScriptVersion 和模块摘要；生成 client 后实现剧本/资产/任务 ViewModel 与 E2E | “导入 → 提取 → 决议 → 确认 → ready 资产”浏览器闭环；D-005 未关闭不得 accepted |
...
DEV-S0-01～05、DEV-S1-01～04 已完成；S0 与 S1 已接受，证据分别见 [S0](../acceptance/001-S0工程基线验收.md) 和 [S1](../acceptance/002-S1单人立项验收.md)。D-003 已按“不预设 AI 服务”关闭，DEV-S2-01 已进入 `in_progress`。D-005 仍 open：按 [DES-005](../design/005-S2供应商与合规准入设计.md) 确认首发地区、合规责任人、授权模板和留存表；关闭前可完成 Task、剧本和私有媒体工程，但治理、ready 资产和 S2 不得 accepted，真实生成与公开交付必须 fail-closed。需要真实 AI 调用的验收必须等用户提供实际服务配置后进行。
...
| 14 | 创建仓库与最小 CI 配置 | 编写根 `README.md`、`.editorconfig`、`.gitignore`、`.env.example`、`Makefile`、`.github/workflows/ci.yml` | CI 固定 Python/uv/Node 版本并使用 frozen lockfile；根 `.env.example` 是后端、前端、生成命令和 Compose 的唯一配置示例，明确区分 `DATABASE_URL` 与 `TEST_DATABASE_URL`，不含真实凭据；本步只运行已存在的 setup/静态检查，不引用未来文件或假装 skip 通过。后续 DEV 随真实检查增加同次更新 CI，DEV-S0-05 才收敛为启动 API 后生成 client 并调用 `make check`/`make docker-build`；忽略 `.venv/`、`node_modules/`、`.env`、缓存、报告和日志但不忽略 `.codex/` |
...
| 8 | 创建 Docker Compose | `docker-compose.yml`、`docker-compose-env.yml` | 业务文件定义 api/web，并在 S2 增量加入复用 backend 镜像的真实 scheduler；环境文件定义 PostgreSQL/Redis/RabbitMQ/MinIO 且仅显式启动；本地开发继续优先复用已安装服务；凭据从未提交 env 注入；worker 只在真实入口完成时加入 |
...
| 步 | 动作 | 命令 | 验证 |
| --- | --- | --- | --- |
| 1 | 确保 Makefile 目标完整 | 检查 setup/dev-api/dev-frontend/scheduler/db-init/generate-api/lint/typecheck/test/check/docker-build/e2e-install/e2e/business-up/down/env-up/down/contract-minio/contract-rabbitmq | 所有目标存在且仅做薄转发/前置检查；scheduler 与 API 复用同一 backend 镜像，docker-build 不重复构建该镜像 |
