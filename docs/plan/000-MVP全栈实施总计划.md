...
| DEV-ARCH-01 | Codex / in_progress | 待实测 | SYS-NFR-008/009；DEV-S2-02 工程门禁 | R0 边界守卫、R1 公开 seam、官方脚手架基线迁移、R2 scripts 拆分及 R3 projects/identity 拆分与测试 fixture 收敛已完成；跨模块内部导入 allowlist 已清零。下一步执行 R4 production/messaging 边界固定与 R5 总体验收 | 无 uv 环境/命令；requirements 锁可从 pyproject 重建；三个复杂模块根 API 仅组合能力 Router 且不再保留根 service/schema；integration 数据库/client fixture 唯一；OpenAPI 生成结果无漂移；全量 `make check` 通过 |
