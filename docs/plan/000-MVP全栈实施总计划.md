...
| DEV-S2-03 | StephenQiu30 / completed | 12～18 小时 | PT-MED-001–003 | UploadSession 受限直传、大小/MIME/sha256 完成校验、不可变 MediaVersion/Location、短期访问、追加版本 CAS、归档历史及 media_probe Task/Worker 已按 Red→Green 完成 | commits `eadd102`、`3a729f5`、`2c06d3b`；重复完成回读同版本，校验失败不创建 MediaVersion，存储故障保持 pending；跨空间统一 404，URL 不持久化，归档历史仍受控可读；临时真实 MinIO 的 PUT→完成→GET 2/2、ffprobe 8.1.2 契约 1/1，以及隔离 RabbitMQ→Media Worker→MinIO→ffprobe→PostgreSQL 全栈契约 1/1；`make check` 收集后端 97 项（默认门禁 91 通过/6 项显式外部契约跳过）、前端 13 项通过，生产构建通过；PT-MED-001–003 仍待产品接受 |
