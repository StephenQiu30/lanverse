...
本 Plan 已具备可执行输入、任务、文件边界、验证命令和停止条件，文档状态为 `ready`。S0、S1 已通过真实实现、浏览器验收和远端 CI 接受；D-003 已按“LangChain DeepSeek 1.1.0 + `deepseek-v4-pro` 非思考结构化输出”重新决策并关闭。D-005 已于 2026-07-30 按中国大陆公众首发、产品负责人承担合规责任角色、无标识导出禁用、未成年人材料默认阻断和逐类最短必要删除触发条件关闭，边界见 [DES-005](../design/005-S2供应商与合规准入设计.md)。`DEV-S2-04` 已完成授权与权利门禁工程验收，`DEV-S2-05` 已完成 ready 资产后端、候选交接和真实资产工作台；`DEV-S2-06` 已由真实 Playwright 运行栈覆盖资产身份、版本、current、归档恢复和删除阻断。DEV-S3-03 的无密钥本地浏览器闭环现覆盖明确标记的本地 accepted Candidate 建镜、HTML5 原生拖拽、角色/声音/场景三类 ready 资产完整规格、场景 v1→v2 引用升级、活动/历史引用分流、来源变化失效、ScriptVersion current 影响以及全部镜头生命周期。用户当前选择不配置 API Key，真实 DeepSeek 联合退出证据留作未来 Provider 门禁；候选与资产升级对话框已完成真实渲染复查，其余功能由真实浏览器交互固定，像素截图不替代功能或 Provider 证据。S4 图片/视频方向已选 Seedream/Seedance，2026-07-31 已完成官方 SDK 公开契约复核，但公开 SDK 不证明模型开通，D-004 仍受真实账号能力、参数、账单、S4/S5 和上线材料门禁约束。D-006 已按用户最终澄清确定为 local-first 加共享全栈 Compose 与生产薄覆盖层：本地开发不强制 Docker，development 可构建快速启动，production 使用独立 env、隐藏基础设施端口并只拉取已发布镜像；六服务健康依赖、持久卷和重启策略共用，基础镜像使用 `latest`，不复制完整 Compose 或维护 digest/发行版守卫。后续切片仍必须按依赖激活，不得并行预建空实现。
...
| DEV-S2-02 | StephenQiu30 / completed | 12～18 小时 | PT-SCR-001–005 | 版本、提取契约、候选决议、结构确认、current 影响摘要和安全删除已按 Red→Green 完成；真实 DeepSeek 适配与联合验收由 DEV-S2-06 收口 | 导入→发布→提取→决议→确认结构可恢复；切换并发只保留一个 winner 且不改历史结构；单版本删除只允许显式确认的未引用 draft，published/current/提取输入/确认结果均返回 blocker；2026-08-03 增量通过 scripts 公开批量计数把 active/archived 来源下的全部剧本版本接入 Episode 锁内和 Project 汇总删除门禁，current 为空也不误删；commits `206b004`、`4dd7db0` 在 S3 接入后通过 storyboards 公开契约返回真实 active Shot 影响集合，前端补齐差异、影响确认和清理动作；PT-SCR-001–005 仍待产品接受 |
| DEV-S2-03 | StephenQiu30 / completed | 12～18 小时 | PT-MED-001–003 | UploadSession 受限直传、大小/MIME/sha256 完成校验、不可变 MediaVersion/Location、短期访问、追加版本与 current/revision CAS、归档/恢复历史及 media_probe Task/Worker 已按 Red→Green 完成 | commits `eadd102`、`3a729f5`、`2c06d3b`、`ab28a62`、`fc4121e` 及 2026-08-03 工作区增量；重复完成回读同版本，校验失败不创建 MediaVersion，存储故障保持 pending；跨空间统一 404，URL 不持久化，archived 阻止追加/current 且给出恢复动作，历史仍受控可读；临时真实 MinIO 的 PUT→完成→GET 2/2、ffprobe 8.1.2 契约 1/1，以及隔离 RabbitMQ→Media Worker→MinIO→ffprobe→PostgreSQL 全栈契约 1/1；真实浏览器补证“追加 v2→ready→切回 v1→归档→恢复”；PT-MED-001–003 仍待 S2 产品接受 |
| DEV-S2-04 | StephenQiu30 / completed | 12～18 小时 | PT-GOV-001/002 | 授权过期/撤销、证明访问、RightsGate unavailable、跨空间与未成年人测试；实现 Consent/Revision、RightsGate、生成 client、真实认证和治理工作台 | commits `b817c80`、`0703886`、`d67b370`、`6b6d0fc`、`2f59205`、`80d5b54`；历史版本守卫 commit `8c6c169` 已由最新 D-006 决策取消；真实 PostgreSQL + MinIO 完成登记 r1 → 修订 r2 → 撤销 r3 → RightsGate 阻断；[授权治理增量验收](../acceptance/003-S2授权治理增量验收.md)；PT-GOV-001/002 因 PT-MED-002 尚未产品接受而保持 `in_progress` |
| DEV-S2-05 | StephenQiu30 / completed | 16～24 小时 | PT-AST-001–003、PT-AST-005 | 六类规格、版本/current 并发、权利/媒体 readiness 和候选幂等均按 Red→Green 完成；assets 与 scripts/projects 只通过公开契约交接；生成 client 后接入真实资产工作台 | commits `5ba13b7`、`402b5ae`、`deff612`、`5c62a86`、`2b5cd92` 及当前工作区增量；真实工作台补齐身份编辑、历史 current、删除预检与空身份安全删除；active/archived Asset 与全部 AssetVersion 已进入 Project 删除门禁，并发首次资产创建/项目删除无孤立事实；[资产增量验收](../acceptance/004-S2资产增量验收.md)；角色/场景/声音全部 ready 与 PT 接受保留至 DEV-S2-06 |
| DEV-S2-06 | StephenQiu30 / in_progress | 12～18 小时 | PT-PRJ-004/005 增量；S2 全部 | 已接入 current ScriptVersion、ProductionSnapshot 和真实剧本/媒体/资产/任务工作台；`langchain-deepseek==1.1.0` + `deepseek-v4-pro` 非思考结构化提取、结果未知防重调和人工修改/合并交互已按 Red→Green 完成；媒体、剧本和资产的版本影响/清理/生命周期交互已收口 | 原有组合根与 Provider commits 加 `206b004`、`4dd7db0`、`5c62a86`、`2b5cd92`、`234079e`、`e7f111f` 及当前 ProductionSnapshot 增量；[统一前端增量验收](../acceptance/005-S2统一前端增量验收.md)；当前 `make check` 后端 174 项通过/7 项显式跳过、前端 49 项通过；2026-08-03 最新显式复核中 MinIO/RabbitMQ/ffprobe/媒体联合栈为 2/2、2/2、1/1、1/1，分镜性能 1/1，完整无密钥 `make e2e` 7/7（42.7 秒）是前一提交基线；当前 Ready 分镜浏览器增量因 9000 端口外部 MinIO 冲突待重跑；PT-GOV-006 已覆盖身份、Workspace、Project/Episode、授权、媒体、任务、剧本、资产及 S3 分镜规格纵向切片，未来 S5 审核/交付实体仍须随真实命令接入；`make contract-deepseek` 当前按用户要求不配置密钥、不运行，待未来真实提取→人工决议→确认结构→设为 current 后完成 |
...
| DEV-S3-03 | StephenQiu30 / in_progress | PT-PRJ-005 增量；S3 全部 | 生成 client、候选/手工建镜、标题修改、完整规格编辑、readiness warning、拖拽及键盘重排、变换/删除预检、资产 usage/升级、剧本版本影响、规格审计、Project/Episode 分镜删除影响、Project 资产删除影响、ProductionSnapshot 批量 readiness 和批量 blocker 已完成；本地 Playwright Ready 分镜增量待恢复唯一 Lanverse MinIO 后复核，真实 Provider 起点 E2E 继续保留 | commits `afa42cd`～`b229c08`、`54e171a`、`a771af4`、`a141c57`～`09bd9e8`、`206b004`～`2b5cd92`、`234079e`、`e7f111f`、`c10b33d`、`eb2acec`、`06d1926`、`1c9e6d3`、`78fda30`、`2705ba6` 加当前工作区增量；本地 fixture 只允许 `_test` 数据库；P95、本地服务恢复、本地 accepted Candidate 建镜、角色/声音/场景 ready 资产完整规格、受候选决议与关联版本引用的空资产归档引导、HTML5 拖拽、变换字段恢复、v1→v2 引用升级、归档引用历史化、来源选择失效、ScriptVersion current 影响、ShotSpec 审计筛选、Episode/Project 的 6 镜头/7 规格及 Project 的 5 资产/5 版本删除 blocker 均有前一提交 S3 定向浏览器 1/1、默认 E2E 7/7；当前 `make check` 后端 174 项通过/7 项显式跳过、前端 49 项通过，新增 PostgreSQL 证明 12 集与 36/120 镜头的 ProductionSnapshot 常数查询；当前浏览器增量未到达 Ready 分镜断言，待环境恢复后重跑，随后仍需 S2 Provider 证据验证真实 AI confirmed Episode 全部目标镜头 ready |
...
| S0/S2/S4/S5 | 真实 PG/Redis/RabbitMQ/MinIO/Provider/FFmpeg 契约；本地仅缺 MinIO 时执行 `make minio-up`，需要完整隔离开发环境时执行 `make docker-dev-up`，生产发布执行 `make docker-prod-up`；RabbitMQ 契约使用 `lanverse_contract` 专用 vhost，拒绝 `/` | 依赖地址、环境、运行命令和非 mock 结果 |
...
| 9 | 验证依赖状态 | 本地缺 MinIO 时 `make minio-up`；完整开发环境执行 `make docker-dev-up`，生产环境执行 `make docker-prod-up`，再请求 `/readyz` | 两个入口 `--wait` 成功后六服务健康；生产无基础设施宿主机端口且不现场构建；API 返回全部依赖明细，硬依赖失败时 unavailable/503 |
...
| 步 | 动作 | 命令 | 验证 |
| --- | --- | --- | --- |
| 1 | 确保 Makefile 目标完整 | 检查 setup/dev-api/dev-frontend/scheduler/worker-io/worker-media/db-init/generate-api/lint/typecheck/test/check/docker-build/docker-dev-up/down/logs/docker-prod-up/down/logs/minio-up/down/e2e-install/e2e/contract-minio/contract-rabbitmq | 所有目标存在且仅做薄转发/前置检查；本地角色独立运行，共享服务事实源按 development/production 显式入口启动并等待健康 |
