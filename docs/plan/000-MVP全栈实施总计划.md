...
本 Plan 已具备可执行输入、任务、文件边界、验证命令和停止条件，文档状态为 `ready`。用户于 2026-07-28 明确要求把全部计划转为可执行状态，D-001 据此接受 S0 范围、非目标、技术基线和工程边界；DEV-S0-01 由 `StephenQiu30` 负责并进入 `ready`。D-006 已选择“复用本机 PostgreSQL/Redis，RabbitMQ/MinIO 使用显式 Compose profile”，因此不再阻塞任务领取；真实连接仍必须由 DEV-S0-04 验证后才能退出 S0。后续切片的 Plan 同样为可执行说明，但对应 DEV 只能在前序切片 accepted 且本切片外部门禁关闭后进入 `ready`，不得并行预建空实现。
...
| DEV-GATE-02 | 工程负责人 | completed | S0 使用本机 PostgreSQL/Redis，RabbitMQ/MinIO 仅由显式 Compose profile 提供，凭据只进 secret 注入 | D-006 closed；端口/env 契约见 §7.2.1，未来切片不提前冻结 |
...
| --- | --- | --- | --- | --- | --- |
| DEV-S0-01 | StephenQiu30 / ready | 领取时填写 | 技术启用项 | 先做环境预检并固化文档追踪守卫；再建 README、Makefile、gitignore、CI、backend/frontend 锁文件和无污染 setup | 可直接领取；`setup` 不下载 Python、不全局安装、不启容器 |
...
| DEV-S0-04 | 待指派 / proposed | 进入 in_progress 前填写 | PT-STO-001；Redis/RabbitMQ 连接启用 | 先写 PG/Redis/RabbitMQ/MinIO 真实连接和 MinIO 存储契约；再建类型化 adapter、显式 Compose profile 和 readiness | DEV-S0-02 completed 后进入 ready；D-006 已关闭，MinIO 契约通过后完成 |
...
| RabbitMQ/MinIO | Compose profiles `rabbitmq`、`minio`；RabbitMQ=`rabbitmq:4.3.4-alpine@sha256:c07a5e60f5429be18b3b7fd3a4dfa9a84c3372f88df084b6f2b22224192c360c`、host 端口 5672；MinIO=`minio/minio:RELEASE.2025-09-07T16-13-09Z-cpuv1@sha256:13582eff79c6605a2d315bdd0e70164142ea7e98fc8411e9e10d089502a6d883`、host 端口 9000/9001；账号和密钥由未提交 env 注入 | `docker compose config` 无浮动镜像和硬编码真实凭据；DEV-S0-04 真实连接和存储契约通过 |
...
当前首个可执行交付是 DEV-S0-01：建立无污染的前后端工程基线。D-001、D-006、owner 和 S0 fixture 已关闭，执行者可以直接按 §12.1 开始；随后按依赖完成 DEV-S0-02～05。S1～S6 的计划说明均已 ready，但对应 Requirement/Design/PRD 范围和外部决策仍在各切片进入前接受，不阻塞 S0。

## 12. S0 逐步骤执行剧本

D-001、D-006 和 DEV-S0-01 owner 已关闭，按编号执行 12.1～12.5。每一步包含确切命令、创建文件和验证标准；开始前只需确认工作区没有与 S0 重叠的未说明修改并填写本任务估算。RabbitMQ/MinIO 已确定使用显式 Compose profile，DEV-S0-04 创建配置后再启动并验证，不能因当前尚无 Compose 文件反向阻塞 DEV-S0-01～03。
...
| 9 | 添加后端开发依赖 | `uv add --project backend --dev --no-python-downloads ruff pyright pytest pytest-asyncio pytest-cov testcontainers httpx` | `backend/uv.lock` 更新；未触碰系统 Python 和全局 site-packages；Testcontainers 只有显式模式才启动 |
| 10 | 初始化前端项目 | `npx --yes create-next-app@16.2.12 frontend --typescript --tailwind --eslint --app --src-dir --empty --use-npm --disable-git --no-agents-md --yes`，随后删除生成的 `frontend/README.md` | `frontend/package.json` 存在；使用 React 19、Next.js 16.2.12 和 App Router；不建嵌套 Git/AGENTS；精确解析结果进入 lockfile；项目说明只维护根 README |
...
| 14 | 创建仓库与最小 CI 配置 | 编写根 `README.md`、`.editorconfig`、`.gitignore`、`Makefile`、`.github/workflows/ci.yml`，以及 `backend/.env.example`、`frontend/.env.example` | CI 固定 Python/uv/Node 版本并使用 frozen lockfile；`backend/.env.example` 明确区分 `DATABASE_URL` 与 `TEST_DATABASE_URL`，不含真实凭据；本步只运行已存在的 setup/静态检查，不引用未来文件或假装 skip 通过。后续 DEV 随真实检查增加同次更新 CI，DEV-S0-05 才收敛为启动 API 后生成 client 并调用 `make check`/`make docker-build`；忽略 `.venv/`、`node_modules/`、`.env`、缓存、报告和日志但不忽略 `.codex/` |
| 15 | 固化文档追踪守卫 | 创建 `backend/tests/architecture/test_document_traceability.py`，运行 `uv run --project backend --frozen --no-python-downloads pytest backend/tests/architecture/test_document_traceability.py` | 测试从仓库根定位 `docs/`，验证 379 个叶子、69 个 PT、PRD↔Plan 10/10、Markdown 本地链接、Plan 状态与 P1/条件任务守卫；失败只报告差异，不修改文档 |
| 16 | 运行 setup | `make setup` | 只执行版本预检、`uv sync --project backend --frozen --no-python-downloads` 与 `cd frontend && npm ci`；不全局安装、不启动容器、不自动下载 Playwright 浏览器 |
...
| 8 | 创建 Docker Compose | `deploy/compose.yaml`、`deploy/.env.example` | S0 定义 api/web，并只为本机缺少的 RabbitMQ/MinIO 提供显式 profile；PostgreSQL/Redis 继续复用本机地址，不创建重复容器；镜像使用 §7.2.1 的 tag+digest、健康检查和命名 volume，凭据从未提交 env 注入；scheduler/worker 服务在 S2 有真实入口时再加入 |
...
每个页面直接按 `frontend/src/app/(main)/<route>/` 组织（Next.js App Router），动态路由使用 `[projectId]`、`[episodeId]`；页面专属 ViewModel hook 与页面同目录。S0 只实现 `frontend/src/app/page.tsx` 的可启动根首页和 API 状态入口；`/projects` 及 `(main)` 业务布局在 S1 出现真实身份与项目用例时创建。
