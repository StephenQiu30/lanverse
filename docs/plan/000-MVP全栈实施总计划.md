...
| DEV-ARCH-01 | Codex / in_progress | 待实测 | SYS-NFR-008/009；DEV-S2-02 工程门禁 | 按 DES-006 执行 R0～R5：先用架构测试锁定当前违规，再建立公开 seam、清除跨模块内部依赖、按能力拆 scripts/projects/identity，并收敛测试 fixture | 跨模块内部导入归零；OpenAPI、数据库与消息行为无漂移；S0/S1/S2 已有回归和 `make check` 通过 |
| DEV-S2-02 | StephenQiu30 / in_progress（暂停新增业务用例） | 12～18 小时 | PT-SCR-001–005 | 已完成版本、提取和候选决议增量；等待 DEV-ARCH-01 完成后，再以 Red→Green 实现确认结构；只有用户提供实际 AI 服务配置后才增加对应适配器 | 导入→提取→决议→确认结构可恢复；未配置 AI 服务时明确返回能力不可用，正文不进日志/消息 |
