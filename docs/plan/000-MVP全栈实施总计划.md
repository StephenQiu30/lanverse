...
| --- | --- | --- | --- | --- | --- |
| DEV-S1-01 | StephenQiu30 / in_progress | 8～12 小时 | PT-IDN-001–004、PT-DAT-003 首个增量 | 加入 `uuid6`、`pwdlib[argon2]` 后先写注册/错误凭据/JWT 撤销、UUIDv7、数据库复合租户引用、能力矩阵、归档/停用和跨空间测试；再实现 identity 事实、命令和 ActorContext | 三项注册事实原子；密码摘要不泄漏；注销/改密使旧 JWT 失效；数据库与应用层共同阻止跨空间访问 |
...
DEV-S0-01～05 与对应 PT 已接受。D-002 已按用户最新决定固定为 FastAPI + JWT 本地认证；用户于 2026-07-29 授权全生命周期执行与自主配置协调，并局部接受 S1 所需的 REQ-001/002/003、DES-MOD-001/002/003/011、PRD-001/002/003/004/007/010 范围。当前首个可执行交付是 DEV-S1-01；S2～S6 仍保持未接受。
...
| 6 | 创建 backend 镜像 | `backend/Dockerfile` | 固定 Python 3.11.15 基础镜像，按 `uv.lock` frozen 安装；S0 只验证 API command，不创建 Scheduler/Worker 占位入口 |
| 7 | 创建 frontend 镜像 | `frontend/Dockerfile`、`frontend/next.config.ts` | 固定 Node 22.23.1 构建，Next standalone 运行；依赖只来自 package-lock |
| 8 | 创建 Docker Compose | `docker-compose.yml`、`docker-compose-env.yml` | 业务文件只定义 api/web；环境文件定义 PostgreSQL/Redis/RabbitMQ/MinIO 且仅显式启动；本地开发继续优先复用已安装服务；凭据从未提交 env 注入；scheduler/worker 在 S2 有真实入口时再加入 |
