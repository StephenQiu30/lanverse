...
本节 DEV 行是唯一工程执行台账，不在其他文件复制状态。S0、S1 已完成并链接 Acceptance；D-003、D-005 已关闭，`DEV-S2-01` 至 `DEV-S2-03` 已完成。`DEV-S2-04` 已解除阻塞并进入实施，依次完成授权证据、撤回历史、RightsGate 和前端闭环后再解锁 ready 资产。`ready` 表示当前动作已有明确责任角色和输入；`blocked` 必须写明解除条件；DEV 进入 `in_progress` 前把“待指派”替换为具体负责人并记录估算；完成时把证据替换为可访问的评审、测试或 Acceptance 链接。
...
| DEV-S2-04 | StephenQiu30 / in_progress | 12～18 小时 | PT-GOV-001/002 | 先写授权过期/撤销、证明访问、RightsGate unavailable 和跨空间测试；再实现 Consent/Revision 与权利门禁 | D-005 已于 2026-07-30 签认关闭；权利不足可保存 draft 但不得 ready/生产；需真实 PostgreSQL、OpenAPI/client、前端与浏览器验收证据 |
| DEV-S2-05 | 待指派 / blocked | 待领取 | PT-AST-001–003、PT-AST-005 | 先写六类规格、版本/current 并发、权利/媒体 readiness 和候选幂等测试；再实现 assets 与 scripts 公开交接 | DEV-S2-04 完成后解锁；至少角色/场景/声音 ready；JSONB 不绕过类型校验 |
...
DEV-S0-01～05、DEV-S1-01～04、DEV-S2-01～03 已完成；S0 与 S1 已接受，证据分别见 [S0](../acceptance/001-S0工程基线验收.md) 和 [S1](../acceptance/002-S1单人立项验收.md)。D-003、D-005 已关闭；PT-SCR-001～005、PT-MED-001～003 已具备工程证据但仍待产品接受。当前从 DEV-S2-04 实现 ConsentRevision、撤回历史、证明媒体可访问性和 RightsGate，完成后依次解锁 ready 资产与 S2 前端闭环。需要真实 AI 调用的验收必须等用户提供实际服务配置后进行，公开交付仍受标识、内容检查和上线材料门禁约束。
