...
| --- | --- | --- | --- | --- | --- |
| DEV-S0-01 | StephenQiu30 / completed | 未预估（流程偏差） | 技术启用项 | 先做环境预检并固化文档追踪守卫；再建 README、gitignore、CI、backend/frontend 锁文件和无污染的直接安装流程 | commits `758ba53`、`5b13d8d`；[工程基线验收](../acceptance/001-工程基线验收.md) |
...
| 10 | 启动并测试 | 在 `backend/` 执行 `.venv/bin/python -m uvicorn app.main:app --reload --host 127.0.0.1 --port 8000`，再请求 `/healthz` 与 `/readyz` | healthz=200；PG 正常而 Redis/MQ/MinIO 尚未接通时 API readyz=200 + degraded，并逐项返回 critical/status/reason；PG 或结构基线失败才返回 503 |
...
| 7 | 首次生成 client | 启动本地 API 后在 `frontend/` 执行 `OPENAPI_SCHEMA_URL=http://127.0.0.1:8000/openapi.json npm run openapi2ts` | `src/api/` 目录生成，无错误；生成命令未写死开发机 URL |
