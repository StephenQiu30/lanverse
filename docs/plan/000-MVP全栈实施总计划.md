...
| DEV-S0-04 | StephenQiu30 / completed | 未预估（流程偏差） | PT-STO-001；Redis/RabbitMQ 连接启用 | 先写 PG/Redis/RabbitMQ/MinIO 真实连接和 MinIO 存储契约；再建类型化 adapter、显式 Compose profile 和 readiness | commit `0cf1369`；真实 MinIO 契约和四依赖 ready 通过 |
...
### 7.3 S1：WP-03 单人立项（当前切片）

| ID | 责任人/状态 | 估算 | 对应 PT | Red 与最小实现顺序 | 退出证据 |
| --- | --- | --- | --- | --- | --- |
| DEV-S1-01 | StephenQiu30 / ready | 8～12 小时 | PT-IDN-001–004、PT-DAT-003 首个增量 | 加入 `uuid6` 后先写 OIDC 非法/并发首登、UUIDv7、数据库复合租户引用、能力矩阵、归档/停用和跨空间测试；再实现 identity 事实、命令和 ActorContext | 真实 OIDC；三项首登事实原子且幂等；数据库与应用层共同阻止跨空间访问 |
| DEV-S1-02 | StephenQiu30 / proposed | 8～12 小时 | PT-PRJ-001–003 | 先写权限、预算字段夹带、重排、生命周期和删除竞态测试；再实现 Project/Episode 用例 | 数据库约束与 HTTP 错误稳定；viewer/editor/owner 行为正确 |
| DEV-S1-03 | StephenQiu30 / proposed | 3～5 小时 | PT-PRJ-004/005 起始增量 | 先写空项目版本入口/Snapshot 契约；再返回服务端阶段、blockers、next_actions 和 partial_failures | 空态可指引导入剧本；前端不可 PATCH 阶段 |
| DEV-S1-04 | StephenQiu30 / proposed | 8～12 小时 | S1 全部 | 加入 Redux Toolkit/RTK Query Provider（首个真实服务端状态用例），生成 client 后实现项目入口 View/ViewModel，再写 Playwright 闭环 | 浏览器完成“首登 → Project → Episode → 导入剧本下一动作”；无 S0 空 store 或第二套查询缓存 |
...
DEV-S0-01～05 与对应 PT 已接受。D-002 已由真实 Keycloak OIDC 契约关闭；用户于 2026-07-29 授权全生命周期执行与自主配置协调，并局部接受 S1 所需的 REQ-001/002/003、DES-MOD-001/002/003/011、PRD-001/002/003/004/007/010 范围。当前首个可执行交付是 DEV-S1-01；S2～S6 仍保持未接受。
