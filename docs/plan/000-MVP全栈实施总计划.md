...
REQ-008、DES-MOD-007 的引用升级范围、DES-MOD-008 与 PRD-008 的 S3 PT 已完成设计准入，性能 profile 见 §7.5.1；只有 S2 accepted 后才能领取以下 DEV。文档准入不授权提前建立 `storyboards` 表、模块或页面。

| ID | 对应 PT | Red 与最小实现顺序 | 退出证据 |
| --- | --- | --- | --- |
| DEV-S3-01 | PT-SBD-001–004 | Red：confirmed Candidate/手工幂等、120 上限、ShotSpec v1、order_hash 并发、transform impact/hash/幂等/证据保留；Green：Shot/Spec/Transform 命令 | position 连续唯一；规格固定 Script/Scene/Dialogue/AssetVersion；变换零复制生产事实 |
| DEV-S3-02 | PT-SBD-005/006、PT-AST-004 | Red：固定 blocker/warning、依赖 unavailable、批量 query 上限、升级全或无；Green：readiness/usage/影响/升级公开 contract | 同一 evaluation hash 结果确定；36/120 Shot 无 N+1；current/授权变化不自动改旧 Spec |
| DEV-S3-03 | PT-PRJ-005 增量；S3 全部 | 生成 client 后实现分镜页、镜头列表/规格编辑、拖拽重排、变换预检、资产选择和批量 blocker，再写真实 E2E | 验收 Episode 全部目标镜头 `readiness.status=ready`；刷新/重启恢复；D-007 P95 达标 |
