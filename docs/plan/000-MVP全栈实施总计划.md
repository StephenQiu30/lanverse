...
本 Plan 已具备可执行输入、任务、文件边界、验证命令和停止条件，文档状态为 `ready`。S0、S1 已通过真实实现、浏览器验收和远端 CI 接受；D-003 已按“LangChain DeepSeek 1.1.0 + `deepseek-v4-pro` 非思考结构化输出”重新决策并关闭。D-005 已于 2026-07-30 按中国大陆公众首发、产品负责人承担合规责任角色、无标识导出禁用、未成年人材料默认阻断和逐类最短必要删除触发条件关闭，边界见 [DES-005](../design/005-S2供应商与合规准入设计.md)。`DEV-S2-04` 已完成授权与权利门禁工程验收，`DEV-S2-05` 已完成 ready 资产后端、候选交接和真实资产工作台；`DEV-S2-06` 已由真实 Playwright 运行栈覆盖资产身份、版本、current、归档恢复和删除阻断。DEV-S3-03 的无密钥本地浏览器闭环现覆盖明确标记的本地 accepted Candidate 建镜、HTML5 原生拖拽、角色/声音/场景三类 ready 资产完整规格、场景 v1→v2 引用升级、活动/历史引用分流、来源变化选择失效、ScriptVersion current 影响以及全部镜头生命周期。用户当前选择不配置 API Key，真实 DeepSeek 联合退出证据留作未来 Provider 门禁；候选与资产升级对话框已完成真实渲染复查，其余功能由真实浏览器交互固定，像素截图不替代功能或 Provider 证据。S4 图片/视频方向已选 Seedream/Seedance，2026-07-31 已完成官方 SDK 公开契约复核，但公开 SDK 不证明模型开通，D-004 仍受真实账号能力、参数、账单、S4/S5 和上线材料门禁约束。D-006 已选择“复用本机 PostgreSQL/Redis/RabbitMQ，仅 MinIO 默认由 Docker 提供；完整根环境 Compose 作为显式隔离兜底”，MinIO 只复用本机唯一实例并校验冻结发行版。后续切片仍必须按依赖激活，不得并行预建空实现。
...
本节 DEV 行是唯一工程执行台账，不在其他文件复制状态。S0、S1 已完成并链接 Acceptance；D-003、D-005 已关闭，`DEV-S2-01` 至 `DEV-S2-05` 已完成。`DEV-S2-06` 已领取并进入实施，负责组合已有剧本、媒体、授权和资产事实；角色、场景、声音 ready 及其浏览器闭环、LangChain DeepSeek 适配、人工修改/合并、剧本版本影响/清理和资产身份/版本生命周期已经完成，资产生命周期已由真实 Playwright 运行栈复核。DEV-S3-03 的无密钥页面、本地服务、本地候选建镜、三类资产完整规格、原生拖拽、资产版本升级、ScriptVersion current 影响和分镜浏览器生命周期也已完成，真实 Provider 起点 E2E 继续保留。真实 DeepSeek 联合退出证据按用户当前“不配置 API Key”的决策保留；候选与资产升级对话框已完成真实渲染复查，其余像素截图不替代已通过的功能证据。`ready` 表示当前动作已有明确责任角色和输入；`blocked` 必须写明解除条件；DEV 进入 `in_progress` 前把“待指派”替换为具体负责人并记录估算；完成时把证据替换为可访问的评审、测试或 Acceptance 链接。
...
| DEV-S2-02 | StephenQiu30 / completed | 12～18 小时 | PT-SCR-001–005 | 版本、提取契约、候选决议、结构确认、current 影响摘要和安全删除已按 Red→Green 完成；真实 DeepSeek 适配与联合验收由 DEV-S2-06 收口 | 导入→发布→提取→决议→确认结构可恢复；切换并发只保留一个 winner 且不改历史结构；删除只允许显式确认的未引用 draft，published/current/提取输入/确认结果均返回 blocker；commits `206b004`、`4dd7db0` 在 S3 接入后通过 storyboards 公开契约返回真实 active Shot 影响集合，前端补齐差异、影响确认和清理动作；PT-SCR-001–005 仍待产品接受 |
...
| DEV-S2-05 | StephenQiu30 / completed | 16～24 小时 | PT-AST-001–003、PT-AST-005 | 六类规格、版本/current 并发、权利/媒体 readiness 和候选幂等均按 Red→Green 完成；assets 与 scripts 只通过公开契约交接；生成 client 后接入真实资产工作台 | commits `5ba13b7`、`402b5ae`、`deff612`、`5c62a86`、`2b5cd92`；真实工作台补齐身份编辑、历史 current、删除预检与空身份安全删除；[资产增量验收](../acceptance/004-S2资产增量验收.md)；角色/场景/声音全部 ready 与 PT 接受保留至 DEV-S2-06 |
| DEV-S2-06 | StephenQiu30 / in_progress | 12～18 小时 | PT-PRJ-004/005 增量；S2 全部 | 已接入 current ScriptVersion、ProductionSnapshot 和真实剧本/媒体/资产/任务工作台；`langchain-deepseek==1.1.0` + `deepseek-v4-pro` 非思考结构化提取、结果未知防重调和人工修改/合并交互已按 Red→Green 完成；剧本版本影响/清理和资产生命周期交互已收口 | 原有组合根与 Provider commits 加 `206b004`、`4dd7db0`、`5c62a86`、`2b5cd92`、`234079e`、`e7f111f`；[统一前端增量验收](../acceptance/005-S2统一前端增量验收.md)；当前 `make check` 后端 147 项通过/7 项显式跳过、前端 43 项通过，默认 `make e2e` 7/7 并覆盖资产与本地分镜生命周期，既有 MinIO/RabbitMQ/ffprobe/媒体联合栈真实契约成立；`make contract-deepseek` 当前按用户要求不配置密钥、不运行，待未来真实提取→人工决议→确认结构→设为 current 后完成 |

### 7.5 S3：WP-06 分镜准备

REQ-008、DES-MOD-007 的引用升级范围、DES-MOD-008 与 PRD-008 的 S3 PT 已完成设计准入，性能 profile 见 §7.5.1。2026-07-31 用户授权在不配置 API Key 时继续非供应商功能；DEV-S3-01/02 已完成，DEV-S3-03 已完成页面、本地服务和隔离 Playwright 生命周期，剧本 current 影响已接入真实 storyboards 查询，只保留真实 Provider 起点的联合 E2E。S2 Provider 证据仍阻塞 S3 最终 accepted 与发布，不得使用假 Key、Mock Provider 或伪造 AI 候选补证。

| ID | 责任人/状态 | 对应 PT | Red 与最小实现顺序 | 退出证据 |
| --- | --- | --- | --- | --- |
| DEV-S3-01 | StephenQiu30 / completed | PT-SBD-001–004 | ShotSpec、建镜、顺序、生命周期和变换按 Red→Green 完成 | commits `33e6e22`～`66f7182`；position 连续唯一，固定 Script/Scene/Dialogue/AssetVersion，变换零复制生产事实；[S3 本地工程增量验收](../acceptance/006-S3分镜本地工程增量验收.md) |
| DEV-S3-02 | StephenQiu30 / completed | PT-SBD-005/006、PT-AST-004 | readiness、usage、影响、升级和常数查询按 Red→Green 完成 | commits `d638f03`～`441cdf6`、`7ec4b78`、`206b004`、`4dd7db0`；36/120 Shot 无 N+1，current/授权变化不自动改旧 Spec，升级冲突整批零写入，剧本 current 切换返回真实 active Shot 影响；[S3 本地工程增量验收](../acceptance/006-S3分镜本地工程增量验收.md) |
| DEV-S3-03 | StephenQiu30 / in_progress | PT-PRJ-005 增量；S3 全部 | 生成 client、候选/手工建镜、标题修改、完整规格编辑、readiness warning、拖拽及键盘重排、变换/删除预检、资产 usage/升级、剧本版本影响和批量 blocker 已完成；本地 Playwright 生命周期完成，保留真实 Provider 起点 E2E | commits `afa42cd`～`b229c08`、`54e171a`、`a771af4`、`a141c57`～`09bd9e8`、`206b004`～`2b5cd92`、`234079e`、`e7f111f`、`c10b33d`、`eb2acec`、`06d1926`、`1c9e6d3`、`78fda30`、`2705ba6`；本地 fixture 只允许 `_test` 数据库；P95、本地服务恢复、本地 accepted Candidate 建镜、角色/声音/场景 ready 资产完整规格、HTML5 拖拽、变换字段恢复、v1→v2 引用升级、归档引用历史化、来源选择失效与 ScriptVersion current 影响均进入 S3 定向浏览器 1/1，默认 E2E 7/7；最新 `make check` 后端 147 项通过/7 项显式跳过、前端 43 项通过；待 S2 Provider 证据后验证真实 AI confirmed Episode 全部目标镜头 ready |
...
| DEV-S4-02 | PT-PROD-005/006、PT-MQ-003/004、PT-SCH-001/002 | 先写 Attempt 发送前持久化、Seedream 同步结果丢失、Seedance task ID 落库前/后超时、真实账号下各状态取消竞态、毒消息、有界 prefetch 和 Fire 防重；再实现唯一 ArkMediaAdapter/Scheduler/Worker | 重启/超时/乱序无重复外部调用；有 task ID 用原 ID 对账，无可查证据进入 manual_attention |
...
| 5 | 运行显式 E2E 门禁 | `make e2e-install && make e2e` | 在 S0～S3 浏览器验收环境中安装 Chromium 并运行默认无密钥 Playwright；CI 使用固定缓存/镜像，缺浏览器不得把 E2E 报告为通过；真实 Provider E2E 仍由独立显式命令控制 |
