...
| G-MVPA-001～003/006 MVP-A | 产品+制作人+技术负责人 | PLAN-012 各 DEV | 范围已接受；提供自有黄金样本；接受 Alembic baseline 与旧库升级/恢复方案；每个 DEV 在 Green 前完成 GitHub 固定提交、许可证、源码/测试和不采用项审查 |
...
本节 DEV 行是既有 S0～S6 的工程执行台账；MVP-A 的唯一细化台账在 PLAN-012 §5，不在本文复制 12 行状态。S0～S3 已完成并链接 Acceptance；PLAN-012 的 `DEV-MVPA-01` 当前 `in_progress`，仅允许已完成上游证据 Gate 的 migration 范围，`DEV-MVPA-02` 仍受黄金样本阻塞。真实 DeepSeek 起点的 S2→S3 联合 E2E 已于 2026-08-04 通过；既有证据不替代新增整剧、改写、状态资产、覆盖或 migration 验收。
...
| MVP-A migration Gate 未关闭 | Alembic baseline、严格旧库接管和运行时 head gate 已实现；在团队自有旧库副本完成真实备份/恢复演练并保存证据前，不领取新增业务表任务 |
...
DEV-S0-01～05、DEV-S1-01～04、DEV-S2-01～06、DEV-S3-01～03 已完成，S0～S3 accepted；证据见对应 Acceptance。当前产品主线是 PLAN-012 MVP-A，`DEV-MVPA-01` 已完成成熟上游对标、Red 与 migration Green，继续等待团队自有旧库的真实备份/恢复演练关闭 G-MVPA-003；黄金样本 G-MVPA-002 仍阻止领取 `DEV-MVPA-02`。S4 在 D-004 真实账号能力、参数、额度和账单证据前保持 proposed，不与 MVP-A 并行冒充完成。
