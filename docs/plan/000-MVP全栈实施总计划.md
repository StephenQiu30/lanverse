...
- 输出：S0–S6 与 AIP Provider 控制面的全栈工程任务、文件边界、Red 测试顺序、真实依赖验证和产品接受门禁
- 下游：当前工作包实现；完成后的 Acceptance 证据

## 1. 实施结论

本总 Plan 管理跨 PRD 的排序、技术基线和公共门禁；PLAN-001～PLAN-011 分别承接对应 PRD 的执行细节。全局以 S0–S6 纵向切片为主轴，AIP Provider 控制面作为 S3 后、真实 S4 Provider 执行前的独立工作流，以 76 个 PT 为产品接受单元；既有切片使用本文 `DEV-Sx-xx`，AIP 使用 [PLAN-011](./011-AI提供方配置与启用执行计划.md) 的 `DEV-AIP-*` 唯一工程单元。一个工程任务必须同时交付后端事实、OpenAPI/生成 client、需要的前端交互、失败恢复和自动化证据；不拆成“后端阶段”与“前端阶段”。
...
| [PRD-010](../prd/010-需求设计与产品任务追踪矩阵.md) | 418 个需求叶子的 Design/PT/验收归属 | §8 | 完整性基线；实施时每个 PT 必须取回相应 ENT/FR/IF/NFR |
| [PRD-011](../prd/011-AI提供方配置与启用PRD.md) | 7 个 Provider 管理 PT、量化安全/性能/真实 DeepSeek 门禁 | §5 AIP、§7.5.2、§8；细节见 PLAN-011 | proposed；PRD 接受前 DEV-AIP 保持 pending，控制面证据不关闭 D-004 |
...
本工作流以 [PLAN-011](./011-AI提供方配置与启用执行计划.md) 为唯一详细执行台账，所有任务在 PRD-011 accepted 前保持 pending。PT-AIP-001～005 建立目录、加密配置、检查、Binding 和凭据生命周期；PT-AIP-006 以真实 DeepSeek 完成数据库 SSOT 切换；PT-AIP-007 通过生成 client 和 Workspace 页面收口。任何 catalog-only、健康或 Key 配置证据都不关闭 D-004，也不接受 Seedream/Seedance。

| DEV | 对应 PT | 当前状态 | 总计划退出事实 |
| --- | --- | --- | --- |
| DEV-AIP-01～05 | PT-AIP-001～005 | pending | 安全数据事实、预设/Connection、SSRF 检查、唯一 Binding、轮换归档分层通过 |
