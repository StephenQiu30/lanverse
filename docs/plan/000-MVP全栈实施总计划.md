...
| DEV-S4-02 | PT-PROD-005/006、PT-MQ-003/004、PT-SCH-001/002 | in_progress；queued 取消、全额释放和幂等事实已完成（[ACC-021](../acceptance/021-排队生成取消与预占释放验收.md)）；待写 Attempt 发送前持久化、Seedream 同步结果丢失、Seedance task ID 落库前/后超时、真实账号下各状态取消竞态、毒消息和有界 prefetch | queued 子集已证明 Task/Reservation/release/审计同事务且不调 Provider；全量退出仍需重启/超时/乱序无重复外部调用，有 task ID 用原 ID 对账 |
