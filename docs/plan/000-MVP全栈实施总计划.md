...
| DEV-ARCH-01 | Codex / in_progress | 待实测 | SYS-NFR-008/009；DEV-S2-02 工程门禁 | R0 已锁定边界，R1 已建立 identity seam；PyCharm 标准 venv/pip 与官方 create-next-app 基线迁移已完成，继续清除跨模块内部依赖、按能力拆 scripts/projects/identity，并收敛测试 fixture | 无 uv 环境/命令；requirements 锁可从 pyproject 重建；前端官方脚手架基线可验证；跨模块内部导入归零；全量 `make check` 通过 |
