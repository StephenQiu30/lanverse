...
本节 DEV 行是唯一工程执行台账，不在其他文件复制状态。S0、S1 已完成并链接 Acceptance；D-003、D-005 已关闭，`DEV-S2-01` 至 `DEV-S2-04` 已完成。`DEV-S2-05` 已领取并进入实施，先建立六类资产规格、版本/current 与权利/媒体 readiness，再完成角色、场景、声音 ready 闭环。`ready` 表示当前动作已有明确责任角色和输入；`blocked` 必须写明解除条件；DEV 进入 `in_progress` 前把“待指派”替换为具体负责人并记录估算；完成时把证据替换为可访问的评审、测试或 Acceptance 链接。
...
| DEV-S2-05 | StephenQiu30 / in_progress | 16～24 小时 | PT-AST-001–003、PT-AST-005 | 先写六类规格、版本/current 并发、权利/媒体 readiness 和候选幂等测试；再实现 assets 与 scripts 公开交接 | DEV-S2-04 工程门禁已关闭；至少角色/场景/声音 ready；JSONB 不绕过类型校验 |
| DEV-S2-06 | 待指派 / proposed | 待领取 | PT-PRJ-004/005 增量；S2 全部 | 接入 current ScriptVersion 和模块摘要；生成 client 后实现剧本/资产/任务 ViewModel 与 E2E | “导入 → 提取 → 决议 → 确认 → ready 资产”浏览器闭环；所有 S2 PT 按证据逐项 accepted |
...
DEV-S0-01～05、DEV-S1-01～04、DEV-S2-01～04 已完成；S0 与 S1 已接受，证据分别见 [S0](../acceptance/001-S0工程基线验收.md) 和 [S1](../acceptance/002-S1单人立项验收.md)，授权治理工程证据见 [S2 增量验收](../acceptance/003-S2授权治理增量验收.md)。D-003、D-005 已关闭；PT-SCR-001～005、PT-MED-001～003、PT-GOV-001/002 已具备工程证据但仍待 S2 产品闭环接受。当前从 DEV-S2-05 实现六类资产、不可变版本/current、媒体与权利 readiness，随后由 DEV-S2-06 完成 S2 统一前端闭环。需要真实 AI 调用的验收必须等用户提供实际服务配置后进行，公开交付仍受标识、内容检查和上线材料门禁约束。
