...
PLAN-001～011 已达到既有“可执行说明”状态；新 PLAN-012 的执行规格完整，但必须先接受 MVP-A 范围、黄金样本和正式 migration 方案。S0～S3 已 accepted；D-002 已固定为 FastAPI + JWT 本地认证，D-003 已固定为 `langchain-deepseek==1.1.0` + DeepSeek `deepseek-v4-pro` 非思考结构化输出。2026-08-13 起优先级调整为先完成 MVP-A“整剧到可信分镜包”；S4 真实图片/视频继续等待 D-004，不用替身生产事实越过门禁。
...
| 文档 | 对应输入 | 作用 | 当前可执行状态 |
| --- | --- | --- | --- |
| [PLAN-000 MVP 全栈实施总计划](./000-MVP全栈实施总计划.md) | PRD-001～PRD-012 | 统一技术基线、S0～S6/AIP/MVP-A 排序、跨模块 DEV 任务与公共门禁 | ready；S0～S3 accepted；MVP-A Gate open；S4 等待 D-004 |
