...
- [ ] `SG-I10`：接入 Bible `review_storygraph` 与有界 Candidate Repair，每轮重跑确定性 Gate。完成门：Candidate Revision/Head CAS、冻结允许集、修复预算和旧下游 stale 通过。**第一个交付单元完成（2026-08-28）**：invocation/aggregate/repair 三类不可变 Revision origin 已形成严格互斥联合，content hash 与 revision hash 分层覆盖父版本和完整来源；Backend GORM 原子发布原语锁定 expected Head、重验父 Revision 与成功 Repair Result、创建 N+1 并 CAS 切换，两个竞争修复在真实 PostgreSQL 下仅一个成功。下一单元只实现冻结 Review/Repair 输入与确定性 Gate；Patch 允许集、幂等 Receipt、stale closure 和有界重审仍保持未完成。
