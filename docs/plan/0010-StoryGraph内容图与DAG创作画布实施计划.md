...
- [ ] `SG-I20`：只消费 `SG-I19` 的 approved intent 输出，按已接受并同步的 `2051` 完成 `reference_asset` Intent/Cost/Quota/Provider Job/Artifact 执行。完成门：Provider 未知结果按同一 Job 对账，不盲目重提，Artifact 只是 Candidate。**实施中（2026-08-29）**：已完成 strict `GenerationTarget`/Snapshot/Hash、Runware REST Adapter 离线合同、唯一 GORM Catalog 中不可变 Target 持久化及 approved intent Target Builder。Builder 只接受 `storyboard.freeze_intent_set` 已提交回执，沿同一 Draft Set/Batch/Agent Invocation 重读冻结 Style，并重验 Character Asset、SpecificationVersion、AssetState；重复角色/状态需求确定性合并为一个 composite 三视图 Target，Target/Receipt 在单一 GORM 事务中幂等提交，任一 Hash 漂移在写入前失败关闭。Workflow Node 输入 Hash 已与 Target Hash 分离，Intent/Authorization/Request/Provider Submission 强制冻结同一 Target ID/Hash。下一交付单元是把 `reference_asset` Workflow Executor 接到 Builder 与既有 Cost/Quota Preparation；私有 Runware 下载/Staging、真实 Artifact/Candidate 闭环和凭据化 Runware 验收仍未完成，本项保持未勾选。
