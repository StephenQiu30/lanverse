...
- [ ] 实现 resolve_visual_foundation、plan_reference_assets、expected target set、Gate 3、Preset 切换的真实 stale 写入与原子发布。
  ...
  - [ ] 实现 `plan_reference_assets` 严格合同与 Normalizer：Backend 单次 Invocation 冻结 Anchor/State/固定 Target seed inventory、VisualFoundationCandidate、Style 与 PurposeProfile；Agent 仅选择合法 Anchor State 并返回最终键上的 fulfillment/受限用途/机械依赖，Backend 重算 expected set 和正式 Owner Ref 投影。完成跨 Go/Python Red/Green 后再接 Workflow 节点。
- [ ] 定向验证、全量 CI、Acceptance Evidence 与独立提交完成。
