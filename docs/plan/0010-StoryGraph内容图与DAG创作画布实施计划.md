...
- [ ] 实现 resolve_visual_foundation、plan_reference_assets、expected target set、Gate 3、Preset 切换的真实 stale 写入与原子发布。
  ...
  - [x] 实现 `plan_reference_assets` 跨 Go/Python 严格语义合同：Input 内容定址 Anchor/State/固定 Target seed inventory、VisualFoundationCandidate、EffectiveStyleSnapshot 与六类 PurposeProfile；Candidate 只选择实际 Anchor State，并在派生后的完整业务键集上提出 fulfillment/受限 design focus/完整 forbidden changes 和机械依赖回显。两端拒绝 Seed Root、lineage、Target set、PurposeProfile、dependency、fulfillment rank 漂移以及 Provider 字段，Schema Manifest 冻结一致。
  - [ ] 实现 `plan_reference_assets` Backend Input Compiler、正式 Owner Ref/coverage/constraints 投影与可调用 Harness/Wire：从当前 Production StoryGraph 生成上述 Seed Inventory，接入 Stage Registry/Release、持久执行与 Workflow 节点，并复用 `ExpectedReferenceTargetSet` 做最终 Plan 前像校验。
- [ ] 定向验证、全量 CI、Acceptance Evidence 与独立提交完成。
