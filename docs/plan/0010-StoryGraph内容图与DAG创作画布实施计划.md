...
- [ ] 实现 resolve_visual_foundation、plan_reference_assets、expected target set、Gate 3、Preset 切换的真实 stale 写入与原子发布。
  - [x] 固定 Backend-owned `ProjectPresetSelection` 不可变合同：只冻结同 Project/Workspace 的精确策展 Release 内容身份、`faithful|world_adaptation` 模式、选择人/UTC 时间与线性父版本，不接受自由 `visual_style`、Provider 参数、Prompt、current/latest 或任意 overrides map；严格 Decoder 与 Canonical Hash 对未知字段、父链和内容漂移失败关闭。当前尚未接入 GORM Head、项目 API、Workflow 输入或真实 stale 写入。
