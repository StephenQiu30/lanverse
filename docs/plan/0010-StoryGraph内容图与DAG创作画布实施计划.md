...
- [ ] 实现 resolve_visual_foundation、plan_reference_assets、expected target set、Gate 3、Preset 切换的真实 stale 写入与原子发布。
  ...
  - [ ] 接通 `plan_reference_assets` 精确 GORM 候选读取、正式 coverage/constraints 投影、可调用 Harness/Wire、Stage Registry/Release、持久执行与 Workflow 节点，并复用 `ExpectedReferenceTargetSet` 做最终 Plan 前像校验。
- [ ] 定向验证、全量 CI、Acceptance Evidence 与独立提交完成。
