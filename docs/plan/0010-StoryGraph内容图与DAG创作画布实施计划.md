...
- [x] 为上述八个变体建立正式 StageRelease 内容身份：Backend 固定读取已安装 Skill 的 Bundle artifact，逐项绑定 Core、Wire/Input/Output/Normalizer、Prompt Compiler、实际 Reference/Rubric、Loaded Resource Set、执行/模型/工具 Policy 与运行镜像；视觉变体固定 `visual-foundation-harness` 与 `references/visual-identity.md`。既有七个文本变体仍由 SceneAnalysis 服务使用该 Hash 和资源证明写入 GORM Release 事实；视觉变体当前只完成注册和内容身份，不表示调度或持久化已接通，且未新增字段、migration 或旁路事实源。
