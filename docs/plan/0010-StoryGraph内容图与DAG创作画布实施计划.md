...
- [x] 上架 4 个不可变、版本化、可追溯许可的 PresetRelease；首批固定为都市电影写实、古装电影写实、国漫仙侠和赛博朋克动画，全部覆盖六类 Target、默认 faithful，并绑定仓库 Skill Bundle、内容定址 QC/视觉模型能力策略及 first-party MIT NOTICE。Backend 内置目录只提供精确身份查询；项目选择已由后续 GORM Current Head 子项持久化并开放认证 API，但尚未进入 Workflow 或 Gate 3。
- [ ] 实现 resolve_visual_foundation、plan_reference_assets、expected target set、Gate 3、Preset 切换的真实 stale 写入与原子发布。
  - [x] 固定、持久化并开放 Backend-owned `ProjectPresetSelection`：不可变 Selection 只冻结同 Project/Workspace 的精确策展 Release 内容身份、`faithful|world_adaptation` 模式、选择人/UTC 时间与线性父版本，不接受自由 `visual_style`、Provider 参数、Prompt、current/latest 或任意 overrides map；严格 Decoder 与 Canonical Hash 对未知字段、父链和内容漂移失败关闭。GORM Store 通过 SERIALIZABLE 事务校验 active Project 与 owner/editor 权限，以 Current Head revision CAS 原子发布；CommandReceipt 保证同 key 重放原结果并拒绝同 key 不同输入。认证 API 提供精确策展目录及项目当前选择读写，OpenAPI 与 Web 类型已同步；真实 PostgreSQL+Temporal 系统边界验证创建、读取、幂等重放、回执唯一、idempotency 冲突和 stale revision 冲突。当前尚未接入 Workflow 输入或 Preset 切换的真实下游 stale 写入。
  ...
  - [x] 实现 `resolve_visual_foundation` 的 Backend 执行生命周期与 GORM 生产持久化：复用现有 Release/Manifest/Invocation/Attempt/Authorization/Result/Candidate/Head 表，按精确 StageRelease、Control、运行镜像和输入 Hash 签发调用；accepted replay 返回同一 Candidate，传输或运行时不确定性只记录 `outcome_unknown`。接收结果时在 SERIALIZABLE 事务中重读当前 production StoryGraph、Gate 2 exact source 与策展 PresetRelease 并重新编译输入，任一漂移返回 `stale_visual_foundation_input`，不产生 Candidate。当前尚未接入 Workflow/Temporal 生产节点，也尚未从 Project Preset Selection 冻结运行输入或进入 Gate 3。
- [ ] 定向验证、全量 CI、Acceptance Evidence 与独立提交完成。
