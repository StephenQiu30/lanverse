...
- [ ] 实现 resolve_visual_foundation、plan_reference_assets、expected target set、Gate 3、Preset 切换的真实 stale 写入与原子发布。
  ...
  - [ ] 实现 Gate 3 原子 Owner Apply：由单一 `ConfirmVisualFoundationAndReferencePlanCommand` 在同一 PostgreSQL/GORM 事务中发布 Effective Style/Policy Snapshot 与唯一 active ApprovedReferencePlanVersion/Targets；任一 Head、read set 或 expected Target 漂移整体回滚。
