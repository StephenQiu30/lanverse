...
- [ ] `SG-I10`：接入 Bible `review_storygraph` 与有界 Candidate Repair，每轮重跑确定性 Gate。完成门：Candidate Revision/Head CAS、冻结允许集、修复预算和旧下游 stale 通过。**前三个交付单元完成（2026-08-28）**：invocation/aggregate/repair 三类不可变 Revision origin 与 content/revision hash 已固定，Backend GORM 原子发布原语锁定 expected Head、重验父 Revision 与成功 Repair Result、创建 N+1 并 CAS 切换；Go/Pydantic 双侧冻结 Review target + deterministic Gate，以及 Repair target/review revision + Issue + allowlist + base fragment hash + read-only adjacency。Backend Candidate Repair Coordinator 现已在同一 GORM/PostgreSQL 事务中应用冻结 Patch、创建 N+1、写入幂等 Command Receipt，并按 exact upstream revision id/hash 为旧下游写不可变 staleness closure；闭包失败全事务回滚，相同 Patch 并发重放同 Receipt，新幂等键争用旧 Head 整体冲突。下一单元只实现 Temporal 驱动的有界 Gate/Review 闭包重跑与预算耗尽语义，因此本项仍不提前勾选。
