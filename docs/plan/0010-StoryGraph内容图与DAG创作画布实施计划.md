...
- [ ] `SG-I09`：Definition-first 接入有界 `analyze_story` map 和确定性 `reconcile_story` tree，产出带证据的 Bible/Claim Candidate Revision。完成门：Manifest/leaf 谱系、fan-in、重放、冲突和上游 stale 测试通过。**三个交付单元完成（2026-08-27）**：有界 map、固定 fan-in=2 的确定性 reduce tree、精确 Candidate/Evidence 谱系、上游 Head stale 拒绝、同名/同别名跨集不同 Key 身份守恒、并发 reduce 调度幂等，以及 map/reduce 超预算后的精确 Candidate 子区间与 versioned Manifest 路径重分片均已通过真实依赖 CI；本项仍需完成单 shard deadline/失败恢复，未提前进入 `SG-I10`。
