...
- [x] 让仓库内同一 `build-storygraph` Skill 提供 `visual_foundation` 视觉能力，并建立 Agent Visual Foundation Harness：只加载 Skill 入口与视觉规则，按 Input 的稳定 object key/内容 Hash/媒体类型/字节长度精确绑定 Backend 提供的本地图片，使用一次受限 Codex CLI vision 调用产生严格 Candidate，再复验完整输入谱系和业务不变量；缺失、乱序、外来或重复附件均失败关闭。当前只完成可调用 Harness，不表示 Backend Stage Registry、StageRelease、API/Workflow、候选持久化或 Gate 3 已接通。
