...
从剧本到镜头素材包是一条长价值流，但不应实现为一个可随意改写全部数据的“大流程服务”。Workspace 是整条价值流的顶层作用域；每个模块必须拥有明确事实，跨模块只能在同一 Workspace 内读取固定版本或提交公开命令，否则会形成越权访问、第二套任务、重复媒体、隐式主选和历史漂移。
...
| Production Control | Project 级 ProductionRun、带 project/episode/shot/package scope 的 StageRun、Gate、Checkpoint 引用、阶段转移和流程下一动作 | ProductionRun Snapshot、Stage/Gate 事件、恢复/重入命令 | Project/Episode 主数据、Script/Asset/Shot 正式事实、WorkTask/Provider 执行事实、其他 Workspace 的流程事实 |
...
- 所有箭头默认携带同一个 `workspace_id`；目标模块必须重新解析目标对象归属，缺失或不一致时拒绝整个命令/事件处理。
- Platform 是支撑端口，不是位于所有业务模块之上的“万能业务层”。
- Projects 的 ProductionSnapshot 可以订阅所有下游摘要事件，但只保存来源、口径、更新时间和派生值。
- Production Control 保存“流程应该走到哪里、为什么可以/不可以继续、等待谁确认以及如何恢复”的事实；它不复制下游业务对象，也不把 LangGraph checkpoint 当成业务状态。
- ProductionRun 属于 Project，同一 Project 同时最多一个 current 运行；Episode/Shot/Package 只作为 StageRun 的显式 scope。不同 Episode/Shot 在满足各自依赖后可独立推进，Project 聚合状态不得强制全局串行或覆盖子范围事实。
- Production Harness 只能通过各模块公开 Command/Query/Port 推进阶段；不能直接写 Scripts、Assets、Storyboards、Videos 或 Exports 的 persistence。
- Production Harness 的每个 Command/Query/Port 调用都必须携带 `workspace_id`、ActorContext 和稳定目标 ID；不能用 Project 名称、Episode 标题或 LangGraph checkpoint 推断 Workspace。
