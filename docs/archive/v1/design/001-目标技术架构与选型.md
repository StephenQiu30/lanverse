...
- 版本：v1.0
- 日期：2026-08-14
- 关联需求：CUR-00、CUR-PROD、CUR-PLT-FR-001～010、CUR-PLT-NFR-001～010
- 关联验收：AC-CUR-PLT-001-A～AC-CUR-PLT-010-C
- 下游：DES-002～DES-015

## 1. 架构基线声明

这是面向当前 Lanverse 的目标架构，不再以“完全忽略现有实现”的 Greenfield 假设作为前提。Workspace 是所有制作事实和权限校验的顶层边界，Project 是一部短剧的运行边界：`Workspace → Project → ProductionRun → StageRun(scope=project/episode/shot/package) → Skill/WorkTask`，Episode/Shot/Package 是 StageRun 的作用域目标，不是 ProductionRun 的父链。一个 Workspace 可以有多个相互隔离的 Project，每个 Project 同时最多一个 current ProductionRun 并保留历史。现有页面实现不能决定产品需求，但当前 Python 运行栈、PostgreSQL 领域事实、WorkTask/RabbitMQ 执行事实和 LangGraph Harness 构成必须正视的兼容约束。除非单独批准后端重写，否则不再维护与真实服务分叉的 TypeScript 后端目标。

本设计只使用以下输入：

1. 用户已经明确的产品范围：从 DOCX/Markdown 剧本上传开始，由 Production Harness 管理深度解析、资产、分镜、生成、审核和交付；当前不建设剪辑、无时间线、无成片、无商业运营，终点为每镜唯一主选和有序原始视频素材包；
2. CUR-00、CUR-IAM、CUR-PRJ、CUR-SCR、CUR-AST、CUR-SBD、CUR-KFR、CUR-VID、CUR-EXP、CUR-PLT、CUR-SEC、CUR-PROD，以及标记为 post-MVP 的 CUR-CAN 的不变式、功能需求、非功能需求、验收条件和开放问题；
3. 固定提交的 GitHub 一手项目/工作流证据，以及候选基础组件的官方一手资料；它们只用于确认可行模式和风险，不授权复制代码或默认架构。
4. 当前实际运行栈：Python 3.11、FastAPI、SQLAlchemy、Pydantic、RabbitMQ、LangGraph 和本机 Codex 适配器；技术替换必须有迁移收益和故障验证，不以文档中的理想语言替换可运行基础。
...
| [Temporal](https://github.com/temporalio/temporal) | 可耐久执行长业务流程并跨进程故障恢复 | 作为 MVP 后的对照候选 | 不把工作流状态直接当作产品 Task；不假定第三方副作用 Exactly-once |
...
```mermaid
flowchart LR
    WS["Workspace 上下文"] --> U["React/Next.js 创作工作台<br/>项目列表与流程页面"]
    WS --> G["API / BFF 模块化单体"]
    U --> UL["受控分片/断点上传"]
    G --> PG["PostgreSQL 业务事实 + Outbox/Inbox"]
    G --> RP["项目工作台 Read Model"]
    G --> PH["Production Harness<br/>LangGraph 主图 + Checkpoint"]
    G --> SS["SSE；轮询降级"]
    PH --> TW["文本/分析 Worker"]
    PH --> PW["Provider Worker"]
    PH --> MW["媒体 Worker"]
    PH --> EW["导出 Worker"]
...
- 业务采用 Python 模块化单体，领域模块与数据库所有权清晰，但初期不通过网络拆分。
- Workspace 是所有业务命令、查询、事件、ProductionRun、WorkTask、媒体和审计的强制作用域；任何对象关系都必须能沿 Project/Episode 或显式 Workspace 归属回溯，跨 Workspace 默认拒绝。
- Production Harness 负责项目/剧集全流程的阶段推进、人工门禁、暂停/恢复、重入和下一动作；它不直接写领域表。
- LangGraph 是 Harness 和各 Skill 的流程编排器；其 checkpoint 只保存恢复所需的运行指针、输入版本引用和阶段结果引用，不成为业务事实源。
- 长任务 Worker 与 API 进程隔离，可按文本、Provider、媒体、导出任务池独立扩缩容；WorkTask 负责一次可观察异步执行，RabbitMQ 负责投递。
- PostgreSQL 保存领域事实、ProductionRun/StageRun/Gate、WorkTask/Attempt 和审计引用；RabbitMQ、LangGraph history 和前端缓存都不能替代它。
- 对象存储保存原始和派生字节；数据库保存稳定媒体身份、版本、位置引用、大小、探测规格与 SHA-256。
- 项目页和各阶段页面使用同一领域查询/命令；未来 Canvas 只能作为 post-MVP 的布局事实和可重建 DomainNode/Edge 投影，不复制镜头、主选、Task、stale 或导出真相。
...
| API | Python FastAPI 模块化单体 | 已有服务、文档解析、LangGraph 和 Codex Python 适配器可以共享边界 | OpenAPI、SSE、模块边界与基准测试 |
| Worker | Python Worker 进程 | 复用 LangGraph、Codex、本地文档解析和 Provider 适配 | 真实 Provider/媒体/恢复故障闭环 |
| 数据 | PostgreSQL | 事务、唯一约束、JSON、锁、Outbox 和成熟恢复 | 事务/并发/恢复集成测试 |
| 生产编排 | Production Harness + LangGraph | 全流程阶段、人工门禁、checkpoint 和 Skill 子图 | LangGraph checkpoint、RabbitMQ 重启、人工恢复和 Provider unknown PoC |
| 异步投递 | WorkTask + Outbox/Inbox + RabbitMQ | 与现有服务一致，持久化用户任务并隔离 Provider Job | 重复投递、乱序回调和未知副作用故障 PoC |
...
| --- | --- | --- |
| Workspace/Project/Episode/版本/主选/快照 | Identity 与各领域 PostgreSQL 表 | Cache、页面和工作流状态会丢失或过期；跨 Workspace 访问必须在应用与数据层同时拒绝 |
| WorkTask/Attempt/Cost | Platform PostgreSQL 表 | LangGraph/RabbitMQ/Worker 的内部 Job 不等于用户任务或真实 Provider 尝试 |
...
| ProductionRun/StageRun/Gate | Production 控制模块 PostgreSQL 表，均绑定 Workspace | Harness checkpoint 只能恢复位置，不能代替用户可观察阶段、门禁和下一动作 |
...
- 项目页和各阶段页面调用现有应用命令；均必须携带 `expected_revision`、权限和审计上下文。Canvas 位置/分组属于 post-MVP；
- 所有同步 Command/Query 必须携带 `workspace_id` 与 ActorContext；服务端根据目标对象重新解析并校验 Workspace，不能信任客户端重复字段；
...
1. API 事务创建不可变业务请求、WorkTask、Cost reserve 和 StartExecution Outbox；所有记录必须带 Workspace 归属或由稳定目标反查归属。
2. Outbox 使用 `execution_id = task_id` 幂等启动执行。
...
6. 回调先验证签名并写 Inbox，再驱动执行/Task；重复和乱序是正常边界。
...
Python 是当前后端和 AI 编排的主运行时，不再作为偶发例外。Worker 必须通过公开应用端口和稳定任务契约交接，不能绕过模块所有权直接写其他模块表。只有在某个能力需要独立运行时，才拆分独立进程；拆分不能形成第二套 Task、Attempt、Media、Cost 或 Provider Catalog。
...
共享托管依赖为 PostgreSQL、RabbitMQ、对象存储、Secret Manager/KMS 和 OTel Collector。任何单一 Worker 池故障都不应使其他模块只读能力整体不可用；MVP 不额外引入独立耐久工作流服务。

## 11. 架构选项权衡

| 选项 | 优点 | 主要风险 | 结论 |
| --- | --- | --- | --- |
| Python 模块化单体 + Production Harness/LangGraph + WorkTask/RabbitMQ | 复用当前服务，支持阶段门禁、Skill 子图和异步恢复 | checkpoint 与业务事实必须严格分离 | MVP 采用；通过故障 PoC 固化边界 |
| 模块化单体 + Temporal | 长定时器、Signal、跨进程恢复能力强 | 额外基础设施和第二状态边界 | MVP 后仅在故障/规模证据不足时评估 |
| Python 模块化单体 + PostgreSQL Queue | 系统少、事务内投递简单 | 对账、长定时器、复杂恢复需自建 | 仅作为 WorkTask 投递层对照 |
...
6. 恢复：PostgreSQL 恢复后 ProductionRun/StageRun、WorkTask/Attempt 可继续或进入明确 manual action。
...
- ADR-002：FastAPI API/Worker 组合边界；
- ADR-003：Production Harness/LangGraph 与 WorkTask/RabbitMQ 边界；
