...
当前迁移入口已调整为 Go `lanverse-api`；尚未迁移的业务请求由该入口转发到内部 Python 兼容运行时。该拓扑只用于逐聚合替换，不能形成两套写入所有者，详见[迁移期后端语言切换策略](../adr/迁移期后端语言切换策略.md)。
...
产品能力以两个发布门验收：`Guided MVP` 完成 Guided Studio 的可恢复生产闭环，`Platform Complete` 再补齐完整事件投影、Search/Cost/Quota/Observability、Canvas、协作与 Copilot。发布门不等同于迁移阶段编号；当前整合顺序以[平台迁移与服务整合计划](../plans/平台迁移与服务整合计划.md)为准，详细产品边界见[产品范围与验收基线](../prd/产品范围与验收基线.md)。
...
业务模块使用 `access/production/preset/authoring/workflow/review/generation/agent/asset/media/cost/quota/eventing/search` 作为规范名；详细事实所有权、用例、状态、事件与失败路径见 [后端领域模块功能设计](../designs/后端领域模块功能设计.md)。前端用户任务模块见 [前端功能模块设计](../designs/前端功能模块设计.md)。
...
2. 每个业务资源、表、Endpoint、Workflow 类型在迁移中始终只有一个写入所有者。
...
当前已可复用：Guided Studio 的项目/剧本/Production Bible/分镜切片，PostgreSQL 领域模型，OpenAPI `/api/v1`，MinIO 预签名与存储端口，Kafka Outbox/Inbox 语义，本地 Codex Skill/Harness Schema、候选与人工决议、日志/指标和测试资产。

当前未落地或需迁移：Go Backend 五 Binary、独立 Agent、Temporal、Model Gateway、完整 Media Render、Debezium、Elasticsearch、Frontend Monorepo、TanStack Query/Zustand、Canvas、Yjs/Hocuspocus、统一 Review/QC/Quota 和完整 Compose。安全顺序见[平台迁移与服务整合计划](../plans/平台迁移与服务整合计划.md)，资源级控制见[资源写入所有权迁移台账](../plans/资源写入所有权迁移台账.md)。
