...
Go API 启动时必须通过根目录 `.env` 中的 `LV_DB_DSN` 连接可用的本机业务库，并配置 `LV_REDIS_URL` 指向本机 Redis；`.env.example` 的数据库密码只是占位值，不能直接用于连接。`/healthz` 检查进程存活，`/readyz` 检查 PostgreSQL 与 Redis；Redis 暂不可达时 API 仍运行，就绪探针返回 503。`LV_KAFKA_BROKERS` 已供 Kafka 客户端连通性测试使用，事件投递与消费在 M1-08 实现；其他中间件客户端仍在 M1-05 逐项接入。
