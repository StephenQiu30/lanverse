...
function ContentIssues({ issues }: { issues: API.CreationTextIssue[] }) {
  ...
  return <section className="space-y-2 text-sm"><h4 className="font-medium">本场待确认</h4>{issues.map((issue, index) => <p key={index}><span className="text-muted-foreground">{issue.severity === "blocker" ? "需处理" : "提示"} · </span>{issue.summary}</p>)}</section>;
}
...
export function ProposalContent({ proposal, proposals }: { proposal: API.CreationProposal; proposals: API.CreationProposal[] }) {
  ...
  if (proposal.stage === "map_manuscript") {
    ...
    return <div className="space-y-4">
      ...
      {value.episodes.map((episode, i) => <section className="space-y-2 py-4" key={episode.key}><h3 className="font-semibold">第 {episode.number ?? i + 1} 集 · {episode.title}</h3><p className="text-xs text-muted-foreground"><Range value={episode} /></p><p className="whitespace-pre-wrap text-sm">{episode.rationale}</p></section>)}
  ...
  if (proposal.stage === "analyze_episode") {
    ...
    return <div className="space-y-5"><Facts><Fact label="剧情概述">{value.summary}</Fact><Fact label="主要冲突">{value.conflict}</Fact><Fact label="转折">{value.turning_point}</Fact><Fact label="结尾悬念">{value.ending_hook}</Fact></Facts>
      <nav aria-label="场景目录" className="flex flex-wrap gap-2">{value.scenes.map((scene, i) => <a className="rounded-md bg-muted px-3 py-2 text-sm hover:bg-accent" href={`#scene-${encodeURIComponent(scene.key)}`} key={scene.key}>{i + 1}. {scene.title}</a>)}</nav>
      {value.scenes.map((scene, i) => <section className="scroll-mt-6 space-y-4 py-4" id={`scene-${encodeURIComponent(scene.key)}`} key={scene.key}>
  ...
  if (proposal.stage === "build_world") {
    ...
    return <div className="space-y-8">
      <nav aria-label="设定分类" className="flex flex-wrap gap-2">{Object.entries(kindLabels).map(([kind, label]) => <a href={`#world-${kind}`} className="rounded-md bg-muted px-3 py-2 text-sm hover:bg-accent" key={kind}>{label} · {value.entities.filter((entity) => entity.kind === kind).length}</a>)}</nav>
      {Object.entries(kindLabels).map(([kind, label]) => <section id={`world-${kind}`} className="scroll-mt-6 space-y-5" key={kind}>
        <h3 className="text-lg font-semibold">{label}资料</h3>
        {!value.entities.some((entity) => entity.kind === kind) && <p className="text-sm text-muted-foreground">本次分析没有明确的{label}记录。</p>}
        {value.entities.filter((entity) => entity.kind === kind).map((entity) => {
          const mentions = entity.mentions.map((ref) => ({ ref, mention: context.scene(ref.episode_key, ref.scene_key)?.mentions.find((item) => item.key === ref.mention_key) }));
          ...
          const relations = value.relations.filter((item) => item.subject === entity.key || item.target === entity.key);
          return <section aria-label={`${label}资料：${entity.label}`} className="space-y-5 py-4" key={entity.key}>
            <header><h4 className="text-xl font-semibold">{entity.label}</h4><p className="mt-1 text-sm text-muted-foreground">身份依据：{entity.identity_basis === "explicit" ? "原文明确" : entity.identity_basis === "inferred" ? "推断" : "待确认"}</p></header>
            {entity.uncertainty && <p className="text-sm">不确定项：{entity.uncertainty}</p>}
            <Quotes items={entity.evidence} />
            <div><h5 className="text-sm font-medium">出现与提及</h5><ul className="mt-2 space-y-2 text-sm">{mentions.map(({ ref, mention }, i) => <li key={i}>{context.location(ref.episode_key, ref.scene_key)} · {mention ? `${mention.name} · ${channelLabels[mention.presence] ?? (mention.presence === "mentioned" ? "仅被提及" : "待确认")}` : `来源内容尚未读取（${ref.mention_key}）`}</li>)}</ul></div>
            <div><h5 className="text-sm font-medium">外观与形象依据</h5>{mentions.some(({ mention }) => mention?.visual_details.length) ? mentions.map(({ mention }, i) => mention?.visual_details.length ? <Quotes items={mention.visual_details} key={i} /> : null) : <p className="mt-2 text-sm text-muted-foreground">原稿暂无明确外观证据，不自动补全。</p>}</div>
            <div className="space-y-3"><h5 className="text-sm font-medium">状态变化</h5>{states.length ? states.map((item, i) => <div className="text-sm" key={i}><p>{item.property}：{item.before ?? "未知"} → {item.after ?? "未知"}</p><p className="mt-1 text-xs text-muted-foreground">{context.location(item.episode_key, item.scene_key)} · {item.time_branch} · {item.story_time} · {knowledgeLabels[item.knowledge]} / {basisLabels[item.basis]}</p><Quotes items={item.evidence} /></div>) : <p className="text-sm text-muted-foreground">暂无明确状态变化，不代表各场状态完全一致。</p>}</div>
            <div className="space-y-3"><h5 className="text-sm font-medium">关联关系</h5>{relations.length ? relations.map((item, i) => <div className="text-sm" key={i}>{value.entities.find((e) => e.key === item.subject)?.label ?? item.subject} → {item.predicate} → {value.entities.find((e) => e.key === item.target)?.label ?? item.target}（{originLabels[item.origin]} / {basisLabels[item.basis]}）<Quotes items={item.evidence} /></div>) : <p className="text-sm text-muted-foreground">暂无明确关系。</p>}</div>
            <div className="space-y-3"><h5 className="text-sm font-medium">视觉参考需求</h5>{needs.map((item, i) => <div className="text-sm" key={i}><p>{item.description}</p><Quotes items={item.evidence} /></div>)}<p className="text-sm text-muted-foreground">{kind === "cast" ? "此处为文本设定，尚未生成角色图或三视图。" : "此处为文本设定，不代表视觉素材已生成。"}</p></div>
          </section>;
        })}
      </section>)}
      {value.unresolved_mentions.length > 0 && <section className="space-y-3"><h3 className="font-semibold">尚未归属的提及</h3>{value.unresolved_mentions.map((item, i) => <p className="text-sm" key={i}>{context.location(item.episode_key, item.scene_key)} · {context.scene(item.episode_key, item.scene_key)?.mentions.find((mention) => mention.key === item.mention_key)?.name ?? item.mention_key}</p>)}</section>}
    </div>;
  }
