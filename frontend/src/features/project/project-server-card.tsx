      ...
      <CardContent>
        ...
        <div className="mt-5 flex flex-wrap gap-4 pt-4 text-xs text-muted-foreground">
          <span className="flex items-center gap-1.5">
            ...
            {project.visual_style ?? "未设视觉风格"}
          </span>
          <span className="flex items-center gap-1.5">
            <Clock3 className="size-3.5" aria-hidden="true" />
            {Math.round(project.target_duration_ms / 1_000)} 秒/集
          </span>
        </div>
        <Link
          aria-label={`打开项目 ${project.name}`}
          className="absolute inset-0 rounded-md focus-visible:outline-2 focus-visible:outline-solid focus-visible:outline-offset-2 focus-visible:outline-ring"
