...
export const creationApi = appApi.injectEndpoints({
  endpoints: (builder) => ({
    resumeCreation: builder.mutation<
      API.CreationResumeResponse,
      { runId: string; revision: number }
    >({
      queryFn: ({ runId, revision }) =>
        runRequest(() => resumeCreationRun(runId, { expected_revision: revision })),
      invalidatesTags: (_data, _error, { runId }) => [
        { type: "CreationRuns", id: runId },
        { type: "CreationRuns", id: `proposals:${runId}` },
      ],
    }),
    ...
    currentScriptSource: builder.query<API.AcceptedScriptSourceResponse | null, string>({
      queryFn: (id) =>
        runRequest(async () => {
          try {
            return await getCurrentScriptSource(id);
          } catch (error) {
    ...
    acceptCreationSource: builder.mutation<
      API.AcceptedScriptSourceResponse,
      { projectId: string; body: API.AcceptScriptSourceRequest }
    >({
      queryFn: ({ projectId, body }) => runRequest(() => acceptScriptSource(projectId, body)),
      invalidatesTags: (_data, _error, { projectId }) => [
        { type: "ScriptDocuments", id: `source:${projectId}` },
      ],
    }),
    startCreation: builder.mutation<
      API.CreationRunResponse,
      { projectId: string; body: API.CreateCreationRunRequest }
    >({
      queryFn: ({ projectId, body }) => runRequest(() => createCreationRun(projectId, body)),
      invalidatesTags: (_data, _error, { projectId }) => [
        { type: "CreationRuns", id: `project:${projectId}` },
      ],
    }),
    retryCreationDelivery: builder.mutation<
      API.CreationRunResponse,
      { projectId: string; runId: string; revision: number }
    >({
      queryFn: ({ runId, revision }) =>
        runRequest(() => retryCreationDelivery(runId, { expected_revision: revision })),
      invalidatesTags: (_data, _error, { projectId, runId }) => [
        { type: "CreationRuns", id: `project:${projectId}` },
        { type: "CreationRuns", id: runId },
      ],
    }),
    syncCreation: builder.mutation<API.CreationSyncResult, string>({
      ...
      invalidatesTags: (_data, _error, id) => [
        { type: "CreationRuns", id },
        { type: "CreationRuns", id: `proposals:${id}` },
        "HumanTasks",
      ],
    }),
    adoptCreation: builder.mutation<
      API.CreationAdoptionReceipt,
      {
        projectId: string;
        runId: string;
        proposalId: string;
    ...
    >({
      queryFn: ({ runId, proposalId, body }) =>
        runRequest(() => adoptCreationProposal(runId, proposalId, body)),
      invalidatesTags: (_data, _error, { projectId, runId }) => [
        { type: "CreationRuns", id: runId },
        { type: "CreationRuns", id: `proposals:${runId}` },
