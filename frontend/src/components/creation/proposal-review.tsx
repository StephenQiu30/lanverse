...
} from "@/components/review/endpoints";
import { appApiErrorMessage } from "@/lib/server-state";
import { useAdoptCreationMutation } from "@/components/creation/endpoints";
import { ProposalContent, stageLabels } from "@/components/creation/proposal-content";
...
export function ProposalReview({
  ...
  const detail = useHumanTaskQuery(proposal.human_task_id, {
  ...
  const resolutions = blockers.map((issue) => ({
    code: issue.code,
    scope: issue.scope,
    reason: reasons[riskKey(issue)]?.trim() ?? "",
  }));
      ...
      () =>
        claim({
          ...
          body: {
            ...
            idempotency_key: `creation-claim:${task.id}:${task.revision}`,
          },
        }).unwrap(),
      "审核已领取，请核对正文、来源和待确认问题。",
    );
  }
  function renewTask() {
      ...
      () =>
        renew({
          ...
          body: {
            claim_token: token,
            expected_revision: task.revision,
            idempotency_key: `creation-renew:${task.id}:${task.revision}`,
          },
        }).unwrap(),
      "审核领取已续期。",
    );
    ...
    void command(
      () =>
        decide({
          projectId,
          taskId: task.id,
          workflowRunId: proposal.run_id,
          body: {
            claim_token: token,
            expected_task_revision: task.revision,
            expected_subject_revision: proposal.revision,
            expected_subject_hash: proposal.result_hash,
            decision: value,
            selected_candidate_id: null,
            idempotency_key: `creation-review:${task.id}:${proposal.revision}:${value}`,
          },
        }).unwrap(),
      value === "approved"
        ? "草案已批准；正式采纳后才会进入下一阶段。"
        : "草案已拒绝，本次创作将停止。修改原稿后可发起新的创作。",
    );
  }
    ...
    <article aria-label="提案审阅" className="flex flex-col min-w-0 gap-6 bg-transparent">
      <header className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <h2 className="text-2xl font-semibold">{proposalTitle(proposal, proposals)}</h2>
        </div>
              ...
              {issue.severity === "blocker" && !accepted && canWrite && (
                <Field className="flex flex-col gap-2">
                  ...
                  <Textarea
                    ...
                    maxLength={4000}
                    value={reasons[riskKey(issue)] ?? ""}
                    onChange={(event) =>
                      setReasons((prior) => ({ ...prior, [riskKey(issue)]: event.target.value }))
                    }
      ...
      {accepted ? (
        <section aria-label="正式采纳回执" className="flex flex-col gap-3 pt-5">
          ...
          {accepted.risk_resolutions.map((item) => (
            <p className="whitespace-pre-wrap text-sm" key={`${item.code}:${item.scope}`}>
              已处置 {item.scope}：{item.reason}
            </p>
          ))}
          {proposal.stage === "direct_scene" && (
            <p className="text-sm text-muted-foreground">文字导演意图已保存，视觉参考尚待制作。</p>
          )}
        </section>
      ) : canWrite ? (
        <section className="flex flex-col gap-3 pt-5">
            ...
            <div className="flex flex-wrap gap-2">
              {!decision &&
                ...
                (task.status === "OPEN" || task.status === "CLAIMED") && (
                  <Button disabled={busy} onClick={claimTask}>
                    领取审阅
                  </Button>
                )}
              {!decision && task.claim?.claim_token && (
                <>
                  <Button disabled={busy || !risksReady} onClick={() => decideTask("approved")}>
                    批准草案
                  </Button>
                  <Button disabled={busy} variant="outline" onClick={() => decideTask("rejected")}>
                    拒绝草案
                  </Button>
                  <Button disabled={busy} variant="ghost" onClick={renewTask}>
                    延长审阅时间
                  </Button>
                </>
              )}
              {decision?.decision === "approved" && (
                <Button disabled={busy || !risksReady} onClick={adoptProposal}>
                  正式采纳并继续
                </Button>
              )}
            </div>
          ...
          {decision?.decision === "rejected" && (
            <p className="text-sm">本提案已拒绝。请修改原稿后发起新的创作。</p>
          )}
          {!risksReady && (
            <p className="text-sm text-muted-foreground">
              填写全部必需问题的处理说明后才能批准和采纳。
            </p>
          )}
        </section>
      ) : (
