...
export function proposalContext(proposal: API.CreationProposal, proposals: API.CreationProposal[]) {
  const related = proposals.filter(
    (item) =>
      ...
      item.source_revision_id === proposal.source_revision_id &&
      item.source_hash === proposal.source_hash,
  );
  const episodes =
    (
      related.find((item) => item.stage === "map_manuscript")?.candidate as
        API.CreationTextEpisodeMap | undefined
    )?.episodes ?? [];
  const analyses = related
    .filter((item) => item.stage === "analyze_episode")
    .map((item) => item.candidate as API.CreationTextEpisodeAnalysis);
  ...
  function scene(episodeKey: string, sceneKey: string) {
    return analyses
      .find((item) => item.episode_key === episodeKey)
      ?.scenes.find((item) => item.key === sceneKey);
  }
...
export function proposalTitle(proposal: API.CreationProposal, proposals: API.CreationProposal[]) {
  ...
  if (proposal.stage === "analyze_episode")
    return context.episodeLabel(
      (proposal.candidate as API.CreationTextEpisodeAnalysis).episode_key,
    );
