...
export function CanvasBoard({
  ...
  canEdit: boolean;
  onApply: (
    operations: GeneratedAPI.CanvasOperation[],
  ) => Promise<GeneratedAPI.CanvasApplyResponse>;
}) {
  const nodes = useMemo(
    () => createNodes(project, episodes, document),
    [project, episodes, document],
  );
  ...
  function createNode(kind: NodeKind) {
    ...
    const y = size.height
      ? (size.height / 2 - viewport.y) / viewport.k - definition.height / 2
      : 120;
  ...
  function updateNode(
    node: GeneratedAPI.CanvasNode,
    changes: Partial<GeneratedAPI.CanvasNode>,
    trackLayout = false,
  ) {
    const next = { ...node, ...changes };
    void apply([{ kind: "update_node", node: next, expected_revision: node.revision }]).then(
      (result) => {
        if (result && trackLayout) {
          setUndoStack((stack) => [
            ...stack.slice(-49),
            { nodeId: node.id, before: geometry(node), after: geometry(next) },
          ]);
          setRedoStack([]);
        }
      },
  ...
  function removeNode(node: GeneratedAPI.CanvasNode) {
    ...
    void apply([{ kind: "delete_node", node_id: node.id, expected_revision: node.revision }]).then(
      (result) => {
        if (result) {
  ...
  function duplicateNode(node: GeneratedAPI.CanvasNode) {
    const copy: GeneratedAPI.CanvasNode = {
      ...node,
      id: crypto.randomUUID(),
      revision: 0,
      group_id: undefined,
  ...
  function connectNodes(fromID: string, toID: string) {
    ...
    if (
      fromID === toID ||
      document.connections.some((edge) => edge.from_node_id === fromID && edge.to_node_id === toID)
    )
      return;
    const edge: GeneratedAPI.CanvasConnection = {
      id: crypto.randomUUID(),
      from_node_id: fromID,
      to_node_id: toID,
      purpose: "visual",
  ...
  function groupSelected() {
    const chosen = selectedIds
      ...
      .filter((node): node is CanvasNode => Boolean(node?.saved && !node.saved.group_id));
    ...
    const operations: GeneratedAPI.CanvasOperation[] = [
      ...
      ...chosen.map((node) => ({
        kind: "update_node" as const,
        node: { ...node.saved!, group_id: group.id, x: node.x - group.x, y: node.y - group.y },
        expected_revision: node.saved!.revision,
      })),
    ];
    ...
    const operations: GeneratedAPI.CanvasOperation[] = children.map((child) => {
      ...
      return {
        ...
        node: {
          ...
          x: world.x - (parent?.x ?? 0),
          y: world.y - (parent?.y ?? 0),
        },
      };
    });
  ...
  function selectNode(event: MouseEvent<HTMLButtonElement>, node: CanvasNode) {
    ...
    if (connectingFrom && node.saved) {
      connectNodes(connectingFrom, node.id);
      return;
    }
    setSelectedId(node.id);
    if (event.shiftKey && node.saved)
      setSelectedIds((ids) =>
  ...
  function nodePointerDown(event: PointerEvent<HTMLButtonElement>, node: CanvasNode) {
    ...
    dragRef.current = {
      ...
      mode: (event.target as HTMLElement).closest("[data-canvas-resize]") ? "resize" : "move",
      preview: null,
    };
    ...
    drag.preview =
      drag.mode === "move"
        ...
        : {
            ...
            x: node.x,
            y: node.y,
            width: Math.max(120, Math.min(3000, node.width + dx)),
            height: Math.max(80, Math.min(3000, node.height + dy)),
          };
  ...
  function nodePointerUp(event: PointerEvent<HTMLButtonElement>) {
    ...
    const changes =
      drag.mode === "move"
        ? {
            x: saved.x + (drag.preview.x - drag.node.x),
            y: saved.y + (drag.preview.y - drag.node.y),
          }
  ...
  function wheel(event: WheelEvent<HTMLDivElement>) {
    ...
    setViewport((current) =>
      zoomAtPoint(current, current.k * Math.exp(-event.deltaY * 0.001), {
  ...
  function keyDown(event: KeyboardEvent<HTMLDivElement>) {
    ...
    if ((event.metaKey || event.ctrlKey) && event.key.toLowerCase() === "z") {
      ...
      setViewport((current) => ({
        ...current,
        x:
          current.x + (event.key === "ArrowRight" ? -delta : event.key === "ArrowLeft" ? delta : 0),
  ...
  const canGroup =
    selectedIds.filter((id) => {
      ...
      return node?.saved && !node.saved.group_id;
    }).length >= 2;
    ...
    <section aria-label="项目画布" className="space-y-4">
      <div className="flex flex-wrap items-end justify-between gap-4 px-1">
        <div>
          ...
          >
            <ArrowLeft className="size-4" aria-hidden="true" /> 返回项目
          </Link>
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            创建和组织媒体节点，所有修改由项目服务保存。
          </p>
        </div>
        <span className="rounded-full bg-muted px-3 py-1 text-xs text-muted-foreground">
          {document.nodes.length} 个创作节点 · {episodes.length} 集
        </span>
      </div>
      {error ? (
        <p
      ...
      <div
        className="relative min-h-[590px] overflow-hidden rounded-2xl bg-muted/45 md:h-[min(72vh,800px)]"
        onKeyDown={keyDown}
      >
        <div
          aria-label="无限画布；拖动平移，滚轮缩放，方向键移动，0 重置视图"
          className="absolute inset-0 touch-none overflow-hidden outline-none focus-visible:outline-2 focus-visible:outline-offset-[-2px] focus-visible:outline-ring max-md:hidden"
          onPointerCancel={() => {
            panRef.current = null;
            dragRef.current = null;
            setDragPreview(null);
          ...
          style={{
            backgroundImage:
              "radial-gradient(circle, color-mix(in oklch, var(--foreground) 20%, transparent) 0.8px, transparent 1px)",
        ...
        >
          ...
          >
            <svg
              ...
              width="1"
            >
              {contextualEpisodes.map((node) => (
                <path
                  className="stroke-foreground/25"
                  d={curve(visibleNodes[0], node)}
                  fill="none"
                  key={node.id}
                  strokeWidth="2"
                />
            ...
            {visibleNodes.map((node) => (
              <CanvasNodeCard
                ...
                selected={selectedId === node.id || selectedIds.includes(node.id)}
              />
            ))}
          </div>
        </div>
        ...
        <div className="absolute left-5 top-5 hidden items-center gap-2 rounded-xl bg-background/95 p-1.5 shadow-sm backdrop-blur-sm md:flex">
          ...
          <Button aria-label="适合画布内容" onClick={fit} size="icon" variant="ghost">
        ...
        {canEdit ? (
          <div
            aria-label="创作节点工具"
            className="absolute right-5 top-5 z-10 flex max-w-[calc(100%-10rem)] flex-wrap items-center justify-end gap-1 rounded-xl bg-background/95 p-1.5 shadow-sm backdrop-blur-sm max-md:left-4 max-md:right-4 max-md:max-w-none"
          >
            <Button
              aria-label="新增文本节点"
              disabled={busy}
            ...
            <Button
              aria-label="拆分选中分组"
              disabled={selectedNode.kind !== "group" || !selectedNode.saved || busy}
              onClick={() => selectedNode.saved && ungroup(selectedNode.saved)}
              size="icon-sm"
              variant="ghost"
            >
            ...
            <Button
              ...
              onClick={() => changeLayout(undoStack.at(-1)!, "before")}
              size="icon-sm"
              variant="ghost"
            >
            ...
            <Button
              ...
              onClick={() => changeLayout(redoStack.at(-1)!, "after")}
              size="icon-sm"
              variant="ghost"
            >
        ...
        <div className="absolute left-5 top-20 hidden max-w-[220px] rounded-xl bg-background/90 px-3 py-2 text-xs text-muted-foreground shadow-sm backdrop-blur-sm md:block">
          <Compass className="mr-1 inline size-3.5" aria-hidden="true" /> 拖动平移 · 滚轮缩放
        </div>

        <CanvasInspector
          canEdit={canEdit}
          connecting={connectingFrom === selectedNode.id}
          connections={document.connections}
          key={`${selectedNode.id}:${selectedNode.saved?.revision ?? 0}`}
          node={selectedNode}
          nodes={visibleNodes}
          onConnect={() =>
            setConnectingFrom((current) => (current === selectedNode.id ? null : selectedNode.id))
          }
          onDelete={() => selectedNode.saved && removeNode(selectedNode.saved)}
          onDisconnect={(edge) =>
            void apply([
              { kind: "delete_edge", connection_id: edge.id, expected_revision: edge.revision },
            ])
          }
          onDuplicate={() => selectedNode.saved && duplicateNode(selectedNode.saved)}
          onSave={(changes) => selectedNode.saved && updateNode(selectedNode.saved, changes)}
          pending={busy}
        />
        <CanvasMiniMap
          nodes={visibleNodes}
          onNavigate={(worldX, worldY) =>
            setViewport((current) => ({
              ...current,
              x: size.width / 2 - worldX * current.k,
              y: size.height / 2 - worldY * current.k,
            }))
          }
          size={size}
          viewport={viewport}
        ...
        <CanvasNodeList
          ...
          onSelect={(id) => {
            setSelectedId(id);
            setSelectedIds(savedByID.has(id) ? [id] : []);
          }}
