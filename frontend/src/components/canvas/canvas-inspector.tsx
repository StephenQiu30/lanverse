...
export function CanvasInspector({
  ...
  const dirty =
    title !== node.title ||
    content !== (node.saved?.content ?? "") ||
    prompt !== (node.saved?.prompt ?? "");

  return (
    <aside
      aria-label="选中节点详情"
      className="absolute bottom-5 left-5 right-5 z-10 max-h-[55%] overflow-y-auto rounded-2xl bg-background/95 p-5 shadow-lg backdrop-blur-sm md:bottom-5 md:right-auto md:w-72"
    >
      ...
      {!node.saved ? (
        <>
          ...
          {node.href ? (
            <Link
              className="mt-4 inline-flex items-center gap-1 text-sm font-medium hover:underline focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-ring"
              href={node.href}
            >
        ...
        <div className="mt-4 space-y-3">
          ...
          {canEdit ? (
            <div className="flex flex-wrap gap-1.5">
              <Button
                disabled={!dirty || !title.trim() || pending}
                onClick={() => onSave({ title: title.trim(), content, prompt })}
                size="sm"
              >
                保存
              </Button>
              ...
              <Button
                aria-label={connecting ? "取消连接" : "连接到其他节点"}
                disabled={pending}
                onClick={onConnect}
                size="icon-sm"
                variant="ghost"
              ...
              {outgoing.map((edge) => (
                <div className="flex items-center justify-between gap-2 text-xs" key={edge.id}>
                  <span className="truncate">
                    → {nodes.find((item) => item.id === edge.to_node_id)?.title ?? "已移除节点"}
                  </span>
                  {canEdit ? (
                    <Button
                      aria-label="删除视觉连线"
