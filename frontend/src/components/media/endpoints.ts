...
export const mediaApi = appApi.injectEndpoints({
  endpoints: (builder) => ({
    ...
    >({
      queryFn: (body) => runRequest(() => initializeUploadApiMediaUploadsPost(body)),
    }),
    ...
    >({
      ...
      invalidatesTags: (_result, _error, { workspaceId }) => [{ type: "Media", id: workspaceId }],
    }),
    mediaVersion: builder.query<API.MediaVersionResponse, string>({
      queryFn: (versionId) =>
        runRequest(() => getMediaApiMediaVersionIdGet({ version_id: versionId })),
      providesTags: (_result, _error, versionId) => [{ type: "Media", id: versionId }],
    }),
  }),
});
