...
if (!["install", "approved", "adopted", "readonly"].includes(mode))
  throw new Error("Unknown preview state");
...
const analysis = {
  episode_key: "ep",
  summary: "阿宁带伤归来，门外有人送来一封信。",
  conflict: "是否打开来信",
  turning_point: "门外传来敲门声",
  ending_hook: "送信人没有回答",
  excluded: [],
  issues: [],
  scenes: [
...
const world = {
  entities: [
    ...
    },
    {
      ...
      identity_basis: "explicit",
      uncertainty: null,
      mentions: [{ episode_key: "ep", scene_key: "door", mention_key: "house" }],
      evidence: [evidence],
    },
  ],
  relations: [],
  ...
  asset_needs: [
    {
      entity_key: "aning",
      description: "红衣与伤后包扎形象，保持同一角色身份。",
      evidence: [evidence],
    },
  ],
  unresolved_mentions: [],
...
const direction = {
  episode_key: "ep",
  scene_key: "door",
  dramatic_intent: "让观众感受到开门前的犹豫",
  audience_knows: ["阿宁刚刚受伤归来"],
  withhold: ["送信人的身份"],
  blocking: [{ mention_key: "person", position: "门内右侧", facing: "房门", action: "手伸向门把" }],
  issues: [],
  shots: [
    {
      key: "hand",
      ...
      action: "手指停在门把上",
      beat_keys: ["wait"],
      audio: [{ dialogue_key: "question", channel: "onscreen" }],
      visible_mentions: ["person"],
      detail_evidence: [evidence],
      duration_min_ms: 1000,
      duration_max_ms: 3000,
      timing_basis: "停顿和对白",
      screen_direction: "从右向左",
      entry_state: "手伸向门",
      exit_state: "手停在门把上",
      panel_caption: "包扎的手停在门把上。",
    },
    {
      key: "door",
      purpose: "保留未知",
...
const receipt = {
  submission_id: "fixture-adoption",
  formal_refs: [{ type: "text_intent_version", id: "fixture-text-intent", revision: 1 }],
  risk_resolutions: [
    {
      code: "continuity_mapping_pending",
      scope: "ep/door",
      reason: "已核对，保留伤后状态并隐藏来客身份。",
    },
  ],
};
const proposals = stages.map((stage, index) => ({
  id: `fixture-proposal-${index}`,
  run_id: runId,
  ...
  source_hash: hash,
  invocation_id: `fixture-invocation-${index}`,
  input_hash: hash,
  release_hash: hash,
  human_task_id: `fixture-task-${index}`,
      ...
      ? [
          {
            ...
            scope: "ep/door",
            severity: "blocker",
            summary: "核对人物伤后状态与来客身份披露",
          },
        ]
      : [],
  status: index < 3 || mode === "adopted" ? "accepted" : "needs_review",
  acceptance: index < 3 || mode === "adopted" ? receipt : null,
  created_at: created,
}));
const run = {
  id: runId,
...
const me = {
  ...
  workspace: {
    id: projectId,
    name: "隔离测试 · 非真实作品",
    role: mode === "readonly" ? "viewer" : "owner",
    status: "active",
    revision: 1,
  },
};
const project = {
  ...
  workspace_id: projectId,
  name: "雨夜来信 · 流程测试",
  description: "仅用于前端流程验证，未运行模型、未写入数据库。",
  aspect_ratio: "9:16",
  language: "zh-CN",
  visual_style: "水墨",
...
function browser(...args) {
  return execFileSync("agent-browser", ["--session", session, ...args], {
    ...
    env: {
      ...process.env,
      AGENT_BROWSER_INIT_SCRIPTS: fileURLToPath(
        new URL("./creation-preview-init.js", import.meta.url),
      ),
    },
  });
}
...

{
  route("/api/auth/refresh", {
    ...me,
    access_token: "isolated-browser-fixture-not-a-real-token",
    token_type: "bearer",
    expires_in: 1800,
  });
  route("/api/me", me);
  route(`/api/projects/${projectId}`, project);
  route(`/api/projects/${projectId}/current-script-document`, {
    document: { title: "雨夜来信.md" },
    revision: {
      id: sourceId,
      normalized_hash: hash,
      normalized_text: `第一集 雨夜来信\n${evidence.quote}\n阿宁：这么晚了，是谁？`,
    },
  });
  route(`/api/projects/${projectId}/current-script-source`, {
    identity: { version_id: sourceId, content_hash: hash },
  });
  ...
  const episode = {
    ...
    position: 1,
    target_duration_ms: 90000,
    current_script_version_id: sourceId,
  };
  route(`/api/projects/${projectId}/episodes`, [episode]);
  route(`/api/episodes/${episodeId}`, episode);
  route(`/api/episodes/${episodeId}/structure`, {
    id: "fixture-structure",
    status: "confirmed",
    revision: 1,
    script_version_id: sourceId,
    scenes: [
      {
        id: "fixture-door",
        heading: "旧宅门前",
        position: 1,
        narrative_units: [{ id: "fixture-action", kind: "action", text: evidence.quote }],
        dialogues: [{ id: "fixture-dialogue", speaker: "阿宁", text: "这么晚了，是谁？" }],
        tasks: [
          {
            id: "fixture-breakdown",
            kind: "shot_breakdown",
            label: "拆解场景分镜",
            status: "accepted",
            required: true,
...
route(`/api/creation-runs/${runId}/execution`, {
  status: mode === "adopted" ? "completed" : "waiting_review",
  stage: "direct_scene",
  call_limit: 20,
  reserved_calls: 4,
  last_error: "",
  can_resume: false,
  steps: stages.map((stage) => ({ step_key: stage, state: "needs_review" })),
});
for (let index = 0; index < stages.length; index++) {
  ...
  route(`/api/human-tasks/fixture-task-${index}`, {
    task,
    decision: mode === "approved" || mode === "adopted" ? decision : null,
    coordination: null,
  });
...
browser(
  "network",
  "route",
  "http://127.0.0.1:8123/__browser_fixture/**",
  "--body",
  JSON.stringify({
    error: { code: "fixture_unconfigured", message: "未配置的测试接口，不访问真实后端" },
  }),
);
