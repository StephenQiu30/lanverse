...
describe("Production Bible review contract", () => {
  ...
  it("sends the exact decision and propagates transport options and failures", async () => {
    ...
    expect(requestMock).toHaveBeenCalledWith(`/api/production-bibles/${bibleId}/review-decisions`, {
      method: "POST",
      data: body,
      signal: controller.signal,
    });
    ...
    await expect(decideProductionBibleReviewIssue({ bible_id: bibleId }, body)).rejects.toBe(
      failure,
    );
  });
  ...
  it("refreshes project and Bible queries after a persisted decision", async () => {
    ...
    requestMock.mockImplementation(async (_url: string, options?: { method?: string }) => {
      ...
      return {
        data: {
          id: bibleId,
          revision,
          review_decisions: revision === 3 ? { "issue-1": "accepted" } : {},
        },
      };
    });

    const current = store.dispatch(
      productionBibleApi.endpoints.currentProductionBible.initiate(projectId),
    );
    ...
    await expect(
      store
        .dispatch(
          productionBibleApi.endpoints.decideProductionBibleReviewIssue.initiate({
            projectId,
            bibleId,
            body,
          }),
    ...
    await vi.waitFor(() => {
      expect(
        productionBibleApi.endpoints.currentProductionBible.select(projectId)(store.getState()).data
          ?.revision,
      ).toBe(3);
      expect(
        productionBibleApi.endpoints.productionBible.select(bibleId)(store.getState()).data
          ?.review_decisions,
      ).toEqual({ "issue-1": "accepted" });
    });
  ...
  it("keeps conflict details and never returns success for a rejected command", async () => {
    requestMock.mockRejectedValueOnce(
      new ApiClientError("版本已变化", "resource_conflict", "refresh", { expected_revision: 3 }),
    );
    await expect(
      store
        .dispatch(
