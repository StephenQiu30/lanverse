...
describe("共享视觉基础", () => {
  it("内容卡片无边框且不以轮廓阴影模拟边框", () => {
    render(
      <Card aria-label="剧本" role="region">
        <CardContent>剧本正文</CardContent>
      </Card>,
    );
    expect(screen.getByRole("region", { name: "剧本" })).toHaveClass(
      "bg-transparent",
      "border-0",
      "shadow-none",
    );
  ...
  it("次级操作使用轻边界并保留链接语义", () => {
    ...
    expect(screen.getByRole("link", { name: "继续制作" })).toHaveClass(
      "shadow-border",
  ...
  it("表单轻边界不丢失必填、禁用与错误语义", () => {
    render(
      <>
        <Input aria-label="邮箱" required />
        <Textarea aria-label="剧本正文" aria-invalid="true" />
