...
const run = {
  ...
  source: {
    ...
    revision: 1,
    content_hash: hash,
    span_index_id: projectId,
  },
  flow_type: "lanverse.creation.text-storyboard.production",
  workflow_id: `lanverse:creation:${runId}`,
  status: "accepted",
  revision: 2,
  last_error: "",
  acceptance: null,
...
const proposal: API.CreationProposal = {
  ...
  step_id: projectId,
  stage: "direct_scene",
  step_key: "direct_scene/ep1/door",
  revision: 1,
  result_hash: hash,
  candidate_hash: hash,
  source_revision_id: revisionId,
  source_hash: hash,
  invocation_id: "scene-door",
  input_hash: hash,
  release_hash: hash,
  human_task_id: projectId,
  candidate: {
    episode_key: "ep1",
    scene_key: "door",
    dramatic_intent: "门外来信引发犹豫",
    audience_knows: ["甲在等一封信"],
    withhold: ["来信人的身份"],
    blocking: [],
    issues: [],
    shots: [
      {
        ...
        timing_basis: "等待与停顿",
        screen_direction: "从左向右",
        entry_state: "手伸向门",
        exit_state: "停在门把上",
        panel_caption: "门把上的手，停顿。",
      },
    ],
  },
  evidence: [
...
describe("文本核心闭环", () => {
  beforeEach(() => {
    ...
    requestMock.mockImplementation(async (url: string, options?: { method?: string }) => {
      if (url.endsWith("/current-script-source"))
        throw new ApiClientError("not_found", "not_found");
      if (url.endsWith("/creation-runs") && options?.method === "GET") return { data: [] };
      if (url.endsWith("/script-sources"))
        return { data: { identity: { version_id: revisionId, content_hash: hash } } };
  ...
  it("先正式固定源，再用同一版本和哈希创建运行", async () => {
    ...
    render(
      <AppProviders>
        <TextCreationWorkspace projectId={projectId} source={source} canWrite />
      </AppProviders>,
    );
    await user.click(await screen.findByRole("button", { name: "固定原稿并开始创作" }));
    await waitFor(() =>
      expect(requestMock).toHaveBeenCalledWith(
        `/api/projects/${projectId}/creation-runs`,
        expect.objectContaining({
          method: "POST",
          data: expect.objectContaining({ document_revision_id: revisionId, source_hash: hash }),
        }),
      ),
    );
    const acceptance = requestMock.mock.calls.findIndex(
      ([url]) => url === `/api/projects/${projectId}/script-sources`,
    );
    const creation = requestMock.mock.calls.findIndex(
      ([url, options]) =>
        url === `/api/projects/${projectId}/creation-runs` && options.method === "POST",
    );
  ...
  it("只读成员可查回运行但不能启动或同步提案", async () => {
    ...
    render(
      <AppProviders>
        <TextCreationWorkspace projectId={projectId} source={source} canWrite={false} />
      </AppProviders>,
    );
  ...
  it("需要明确处置问题，批准后再单独正式采纳", async () => {
    ...
    const task = {
      ...
      status: "CLAIMED",
      revision: 2,
      claim: { claim_token: runId },
      subject_revision: 1,
      subject_hash: hash,
    };
    requestMock.mockImplementation(
      async (url: string, options?: { method?: string; data?: unknown }) => {
        if (url.endsWith("/decisions")) {
          decision = { id: runId, decision: "approved" };
          return { data: { task, decision, coordination: null } };
        }
        if (url.endsWith("/adopt"))
          return {
            data: {
              submission_id: runId,
              formal_refs: [{ type: "text_intent_version", id: revisionId, revision: 1 }],
              risk_resolutions: [
                {
                  code: "continuity_mapping_pending",
                  scope: "ep1/door",
                  reason: "已核对，门外身份保持隐藏，手的位置连续。",
                },
              ],
            },
          };
        if (url === `/api/human-tasks/${projectId}`)
          return { data: { task, decision, coordination: null } };
    ...
    render(
      <AppProviders>
        <ProposalReview proposal={proposal} proposals={[proposal]} projectId={projectId} canWrite />
      </AppProviders>,
    );
    ...
    expect(await screen.findByRole("region", { name: "正式采纳回执" })).toHaveTextContent(
      "text_intent_version",
    );
    expect(requestMock).toHaveBeenCalledWith(
      `/api/creation-runs/${runId}/proposals/${revisionId}/adopt`,
      expect.objectContaining({
      ...
      if (url.endsWith("/execution"))
        return {
          data: {
            ...
            reserved_calls: 3,
            steps: [
              { step_key: "analyze_episode/ep1", state: "needs_review" },
              { step_key: "analyze_episode/ep2", state: "running" },
            ],
          },
        };
      throw new ApiClientError("not_found", "not_found");
  ...
  it("未知推理显示原运行受阻且不提供重复推理按钮", async () => {
      ...
      if (url.endsWith("/execution"))
        return {
          data: {
            ...
            call_limit: 20,
            reserved_calls: 4,
            last_error: "invocation_outcome_unknown",
            steps: [{ state: "unknown" }],
          },
        };
      throw new ApiClientError("not_found", "not_found");
    ...
    expect(
      screen.queryByRole("button", { name: /重新生成|重试模型|恢复原运行交接/ }),
