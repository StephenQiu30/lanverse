...
function proposal(stage: API.CreationProposal["stage"], candidate: API.CreationProposal["candidate"]): API.CreationProposal {
  return { ...base, id: stage, stage, candidate } as API.CreationProposal;
}
const map = proposal("map_manuscript", { mode: "preserve", episodes: [{ key: "ep", number: 1, title: "雨夜", first_block: 0, last_block: 0, rationale: "保留原稿" }], excluded: [], issues: [] });
const analysis = proposal("analyze_episode", { episode_key: "ep", summary: "来信", conflict: "犹豫", turning_point: "敲门", ending_hook: "谁来了", excluded: [], issues: [], scenes: [{ key: "door", title: "旧宅门前", first_block: 0, last_block: 0, summary: "阿宁受伤归来", time_label: "夜", time_branch: "main", presentation: "present", beats: [], dialogues: [], mentions: [{ key: "person", name: "阿宁", kind: "cast", presence: "onscreen", evidence, visual_details: [evidence] }], issues: [{ code: "time", scope: "door", severity: "warning", summary: "此场时间尚待核对" }] }] });
const world = proposal("build_world", { entities: [{ key: "aning", label: "阿宁", kind: "cast", identity_basis: "explicit", uncertainty: null, evidence: [evidence], mentions: [{ episode_key: "ep", scene_key: "door", mention_key: "person" }] }], unresolved_mentions: [], relations: [], asset_needs: [{ entity_key: "aning", description: "红衣受伤造型参考", evidence: [evidence] }], state_events: [{ entity_key: "aning", episode_key: "ep", scene_key: "door", time_branch: "main", story_time: "归来时", property: "手臂", before: "完好", after: "受伤", knowledge: "known", basis: "narration", evidence: [evidence] }], issues: [] });
...
describe("创作内容完整展示", () => {
  it("用剧集和场景名称查找并切换内容，空搜索可恢复", async () => {
    ...
    function Directory() {
      ...
      return <ProposalNavigation proposals={[map, analysis, world]} selectedId={selected} onSelect={select} />;
    }
    render(<Directory />);
  ...
  it("同一人物不同状态仍只有一份身份资料，不合并为一个当前状态", () => {
    ...
    const later = { ...candidate.state_events[0], story_time: "次日", before: "受伤", after: "包扎" };
    const changed = { ...world, candidate: { ...candidate, state_events: [...candidate.state_events, later] } };
  ...
  it("不使用其他运行的同名内部标识解析角色来源", () => {
    render(<ProposalContent proposal={world} proposals={[{ ...analysis, run_id: "another-run" }, world]} />);
