...
function testToneWav(): Buffer {
  ...
  for (let index = 0; index < sampleCount; index += 1) {
    const sample = Math.round(6_553 * Math.sin((2 * Math.PI * 440 * index) / sampleRate));
...
async function fillAssetSpec(page: Page, fixture: AssetFixture) {
...
async function createReadyAsset(page: Page, fixture: AssetFixture) {
  ...
  await createDialog.getByLabel("标签（逗号分隔）").fill("S2验收，第一季");
