...
用户明确要求采用无边框设计。本节是 Lanverse 的实际实施约束，优先于下方第三方参考中的卡片描边示例。

- 继承 Geist 字体、黑白灰色阶、字体层级、克制圆角、宽松留白和蓝色键盘焦点。
- 页面内容区、Card、指标、剧集/角色/分镜条目与阶段介绍，不使用闭合 CSS 边框，也不使用 `0 0 0 1px` 阴影或 ring 模拟边框；以标题、间距和对齐表达分组。
- 页眉、页脚不使用贯穿页面的装饰分隔线；内容随主体滚动，保持现有导航行为。
- 按钮、输入框、选择器与浮层保留必要的控件识别、轻阴影和焦点反馈；不能为了无边框移除键盘焦点、错误提示或禁用状态。
