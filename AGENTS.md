...
1. `Plan` 中每个可独立验证的工作包、切片或测试任务视为一个小任务；开始前明确其输入、改动范围、验收项和验证命令。
2. 小任务按“确认基线 → Red → Green → Refactor → 验收回填 → Git 交付”顺序执行，不跨任务混做，也不提前实现后续切片。
3. 每个小任务完成后立即运行与风险匹配的测试、lint、类型检查、构建或手工验收，并把精确命令和结果回填到 Acceptance/Workpad。
4. 验证通过后立即检查 diff，排除无关文件、secret、缓存、日志、构建产物和临时文件；按提交纪律创建职责单一的提交并推送到 GitHub，不积攒多个已完成任务为一个模糊提交。
5. 用户明确要求直接更新 `main` 时，提交并推送 `main`；其他功能任务推送符合规范的 feature 分支并创建或更新 PR，不擅自合并。
6. 推送后必须验证本地 `HEAD` 与对应远端分支一致，并再次运行 `git status --short --branch`。小任务只有在验证通过、提交已推送且任务产生的工作区改动全部清理后才算完成。
7. “工作区干净”不等于删除或吞并用户已有改动。发现任务开始前已存在的改动时必须保留并隔离，只提交当前任务文件；无法隔离时停止提交并明确报告。
8. 推送被拒绝时先按 `pull` 流程同步并重新验证；权限、认证或远端保护规则无法解决时记录为真实阻塞，不得把仅存在本地的提交报告为已交付。
...
9. 每个已完成小任务都必须在当前回合形成可追踪提交并推送到 GitHub；不得以“稍后统一提交”为常规交付方式。
10. 交付前后都必须确认工作区状态；最终报告应给出提交 SHA、远端分支、PR/合并状态以及 `HEAD == remote`、工作区是否干净。
