...
9. 任何产生仓库修改的任务都必须在验证通过、diff 复核无误后提交全部任务内改动；不得只暂存、留下本任务未提交 diff，或把多个无关职责塞入同一提交。
10. 最终答复前必须运行 `git status --short`；本任务产生的工作区必须干净。存在用户既有或其他任务的修改时不得擅自提交、回滚或清理，应保留并逐项报告，不能虚假声称整个工作区干净。
