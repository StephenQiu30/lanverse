...
8. FastAPI 路由与 Pydantic 是唯一 HTTP 契约源；开发环境启用 Swagger `/docs` 与 `/openapi.json`。前端通过 `LANVERSE_OPENAPI_URL`（本地默认 `http://127.0.0.1:8000/openapi.json`）让 `@umijs/openapi` 直接读取运行中 API 的 Swagger URL，并将请求函数与 DTO 直接生成到 `frontend/src/services/`，不得再套 `generated/` 或 `api/` 子目录；不提交或消费静态 OpenAPI 中间文件。自动生成文件带固定生成标识且不得手改，生成命令只替换这些文件，不得覆盖同目录的手写 RTK Query 层；页面只能经该手写层访问，禁止手写第二套 URL、DTO 或降级 OpenAPI。
