...
4. 应用实现进入已接受的 Plan 后，应用根仅允许 `backend/` 与 `frontend/`；后端源码直接位于 `backend/src/`，不得再套 `lanverse/` 包装层，API、TaskJob Worker、业务模块和 Provider/MinIO/FFmpeg Adapter 均归入 `backend/`；前端源码固定在 `frontend/src/`。不得创建根 `src/`、`deploy/`、顶层 `apps/`、`packages/` 或独立 Worker 仓库。
...
8. FastAPI 路由与 Pydantic 是唯一 HTTP 契约源；开发环境启用 Swagger `/docs` 与 `/openapi.json`。前端通过 `LANVERSE_OPENAPI_URL`（本地默认 `http://127.0.0.1:8000/openapi.json`）让 `@umijs/openapi` 直接读取运行中 API 的 Swagger URL，并利用生成器原生目录直接生成 `frontend/src/api/`；不得再套 `services/` 或 `generated/`。`src/api/` 只能包含整体覆盖的 Swagger 生成文件且不得手改，不放手写请求接口；不提交或消费静态 OpenAPI 中间文件。页面只能经 `src/store/backend-api.ts` 的 RTK Query 缓存/轮询层访问生成函数，禁止手写第二套 URL、DTO 或降级 OpenAPI。
...
10. 后端采用单一 FastAPI 技术分层：`main.py`、`api/routes/`、`schemas/`、`services/`、`repositories/`、`db/`、`workers/`、`integrations/`、`core/` 与 `domain/`；`api/errors.py` 是唯一全局业务异常→Problem Details 映射与注册入口，`api/responses.py` 统一定义 HTTP 错误状态枚举和 OpenAPI 错误响应生成函数，禁止在各 Router 复制 `ERROR_RESPONSES` 或创建按业务拆分的 `*_errors.py`。Service 只抛业务语义异常，不保存 HTTP 状态。禁止恢复逐业务模块重复的 `domain/application/infrastructure/transport/public.py` 五层结构，也禁止新增 `other`、运行时 `test` 路由或重复通用工具层。MVP 使用 Task HTTP 轮询，不创建 WebSocket/socket 子系统。
