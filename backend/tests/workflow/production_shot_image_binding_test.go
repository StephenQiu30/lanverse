...
func seedFormalStoryboardShot(
	...
	records := []any{
		...
		&model.StoryboardDraftBatch{
			...
			EpisodeID: episodeID, StructureID: structureID, ScriptVersionID: scriptVersionID,
			WorkflowRunID: uuid.New(), NodeRunID: uuid.New(), ManifestID: uuid.New(), ManifestVersion: 1,
			GraphVersionID: uuid.New(), GraphVersionNo: 1,
			SceneStoryNodeKey: "sgn_" + strings.Repeat("7", 64),
			Status:            "ready", InputHash: strings.Repeat("7", 64), ResultHash: &resultHash,
			Candidate: []byte(`{"scene_story_node_key":"sgn_7777777777777777777777777777777777777777777777777777777777777777","shot_intents":[],"asset_readiness":"ready"}`), Decisions: []byte(`{}`), Revision: 4,
