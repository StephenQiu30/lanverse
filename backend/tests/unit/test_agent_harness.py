...
async def test_harness_accepts_a_langgraph_checkpointer_for_resumable_runs() -> None:
    run = await AgentHarness(checkpointer=InMemorySaver()).run(
        skill=_skill(),
...
async def test_harness_rejects_invalid_structured_output() -> None:
    with pytest.raises(AgentExecutionError, match="invalid structured result") as error:
...
def test_harness_denies_tools_outside_skill_allowlist() -> None:
    with pytest.raises(AgentExecutionError) as error:
        AgentHarness().require_tools(_skill(), ["database.write"])

    assert error.value.code == "agent_tool_denied"
