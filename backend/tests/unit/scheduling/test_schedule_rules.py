...
def test_cron_skips_dst_gap_and_only_uses_first_fold() -> None:
    ...
    first_fold = next_cron_occurrence("30 1 * * *", "America/New_York", after=before_fall_fold)
    assert first_fold == datetime(2026, 11, 1, 5, 30, tzinfo=UTC)
    assert next_cron_occurrence("30 1 * * *", "America/New_York", after=first_fold) == datetime(
