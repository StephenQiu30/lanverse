...
from app.modules.agents.contracts import AgentExecutionContext, AgentExecutionError, SkillDefinition
...
def test_segment_script_preserves_episode_and_scene_boundaries() -> None:
    ...
    assert any(
        chunk.episode_number == 1 and "EXT. ROAD - NIGHT" in chunk.text
        for chunk in chunks
    )
...
async def test_workflow_maps_ranges_references_and_deduplicates_assets() -> None:
    ...
    def chunker(value: str) -> tuple[ScriptExtractionChunk, ...]:
        return (
            ...
            ScriptExtractionChunk(
                "ep001-c002", 1, midpoint, len(value), value[midpoint:]
            ),
        )
    ...
    dialogue = next(
        ...
        if candidate.proposal.kind == "dialogue"
    )
    ...
    shot = next(
        candidate for candidate in result.candidates if candidate.proposal.kind == "shot"
    )
...
async def test_workflow_rejects_empty_aggregated_result() -> None:
    ...
    with pytest.raises(AgentExecutionError, match="no candidates") as error:
...
async def test_workflow_keeps_deep_episode_world_and_character_structure() -> None:
    ...
    workflow = ScriptStructureExtractionWorkflow(
        ...
        chunker=lambda value: (
            ScriptExtractionChunk("ep001-c001", 1, 0, len(value), value),
        ),
    )
