...
func seedSearchOwners(t *testing.T, create func(any) error) searchOwnerFixture {
	...
	for _, value := range []any{
		...
		&model.DocumentRevision{ID: fixture.revisionID, WorkspaceID: fixture.workspaceID, DocumentID: fixture.documentID, VersionNo: 1, SourceType: "text", RawText: text, RawHash: searchHashText(text), NormalizedText: text, NormalizedHash: searchHashText(text), NormalizerVersion: "unicode-codepoint-normalizer", NormalizationMap: datatypes.JSON([]byte(`{}`)), CodepointCount: len([]rune(text)), AnalysisStatus: "deterministic", AnalyzerVersion: "screenplay-structure-analyzer", Blocks: datatypes.JSON([]byte(`[]`)), Issues: datatypes.JSON([]byte(`[]`)), CreatedBy: fixture.userID, CreatedAt: now},
