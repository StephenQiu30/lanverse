...
async def test_real_redis_registration_challenge_and_ticket_are_atomic_and_private() -> None:
    ...
    pattern = (
        f"lanverse:{environment}:identity:registration_verification:v1:*"
    )
    try:
        ...
        winner = tickets[confirmations.index(next(
            result for result in confirmations if result.status == "confirmed"
        ))]
