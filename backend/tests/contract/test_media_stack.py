    ...
    try:
        ...
        async with session_factory() as session:
            async with session.begin():
                upload = await session.get(
                    UploadSession, upload_id, with_for_update=True
                )
        ...
        async with session_factory() as session:
            ...
            task = await session.scalar(
                select(Task).where(Task.task_type == "upload_expiration")
            )
...
async def test_workspace_cleanup_removes_only_unversioned_bytes_in_the_real_media_stack(
    ...
    try:
        ...
        async def initialize_and_put(
            *, content: bytes, idempotency_key: str
        ) -> tuple[UUID, str]:
            initialized = await client.post(
                "/api/v1/media/uploads",
                headers=headers,
        ...
        async with session_factory() as session:
            task = await session.scalar(
                select(Task).where(Task.task_type == "upload_cleanup")
            )
        ...
        async with session_factory() as session:
            ...
            assert version is not None and version.sha256 == hashlib.sha256(
                ONE_PIXEL_PNG
            ).hexdigest()
...
async def test_media_location_migrates_rolls_back_and_retires_in_the_real_stack(
    ...
    try:
        ...
        async with session_factory() as session:
            migration_event = await session.scalar(
                select(OutboxEvent).where(
                    OutboxEvent.aggregate_id == migration_task_id
                )
            )
        ...
        safe_locations = await client.get(
        ...
        async with session_factory() as session:
            async with session.begin():
                source = await session.get(
                    MediaLocation, source_location_id, with_for_update=True
                )
                ...
                retirement_schedule = await session.scalar(
                    select(Schedule).where(
                        ...
                        == f"media.location.retire:{migrated_location_id}"
                    )
                )
        ...
        async with session_factory() as session:
            ...
            retirement_event = await session.scalar(
                select(OutboxEvent).where(
                    OutboxEvent.aggregate_id == retirement_task.id
                )
            )
