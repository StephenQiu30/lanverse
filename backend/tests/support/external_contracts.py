...
DEFAULT_RABBITMQ_CONTRACT_URL = "amqp://guest:guest@127.0.0.1:5672/lanverse_contract"


def require_isolated_rabbitmq_url(url: str) -> str:
    vhost = unquote(urlparse(url).path).strip("/")
    if not vhost:
        raise ValueError("real messaging contracts require an isolated RabbitMQ vhost")
    return url
