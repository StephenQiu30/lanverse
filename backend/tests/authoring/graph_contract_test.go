...
func TestSystemCatalogCoversScriptToStoryboardJourney(t *testing.T) {
	...
	if catalog.Key != "lanverse.production" || catalog.Version != "30.0.0" || len(catalog.ContentHash) != 64 {
...
func TestVisualStagesConsumeExactStoryGraphAndProjectPresetSelection(t *testing.T) {
	...
	want := map[string]struct {
		...
		"agent.reference_briefs": {
			executor: "activity.compile_reference_briefs", cachePolicy: "by_inputs", riskLevel: "external_ai",
			inputs:    map[string]string{"owners": "visual_reference_owner_set"},
