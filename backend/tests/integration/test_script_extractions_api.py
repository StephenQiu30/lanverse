...
async def test_start_extraction_is_atomic_concurrent_and_body_free(
    ...
    headers, workspace_id = await _identity(client, email="extraction-owner@example.com")
    episode, imported, published = await _published_script(client, headers, workspace_id)
...
async def test_extraction_transaction_rolls_back_all_three_facts(
    ...
    headers, workspace_id = await _identity(client, email="extraction-rollback@example.com")
...
async def test_unconfigured_worker_updates_batch_and_task_once_and_is_private(
    ...
    headers, workspace_id = await _identity(client, email="extraction-worker-owner@example.com")
    _, _, published = await _published_script(
    ...
    async with session_factory() as session:
        event = await session.scalar(
            select(OutboxEvent).where(OutboxEvent.aggregate_id == batch["task"]["id"])
        )
        assert event is not None
    ...
    stranger_headers, _ = await _identity(client, email="extraction-worker-stranger@example.com")
    hidden = await client.get(f"/api/v1/extraction-batches/{batch['id']}", headers=stranger_headers)
...
async def test_worker_redelivery_after_result_commit_failure_does_not_call_provider_again(
    ...
    headers, workspace_id = await _identity(client, email="extraction-unknown-worker@example.com")
    _, _, published = await _published_script(
        client,
        headers,
        workspace_id,
...
async def test_extraction_candidates_are_typed_idempotent_paginated_and_private(
    ...
    headers, workspace_id = await _identity(client, email="candidate-owner@example.com")
    _, _, published = await _published_script(
    ...
    detail = await client.get(f"/api/v1/extraction-candidates/{scene['id']}", headers=headers)
    ...
    completed = await client.get(f"/api/v1/extraction-batches/{batch['id']}", headers=headers)
    ...
    stranger_headers, _ = await _identity(client, email="candidate-stranger@example.com")
    ...
    async with session_factory() as session:
        ...
        assert await session.scalar(select(func.count()).select_from(candidate_table)) == 5
        assert (
...
async def test_extraction_result_rejects_invalid_ranges_and_changed_replay(
    ...
    headers, workspace_id = await _identity(client, email="candidate-conflict@example.com")
    _, _, published = await _published_script(
...
async def test_candidate_decision_is_append_only_idempotent_and_refreshable(
    ...
    headers, workspace_id = await _identity(client, email="decision-owner@example.com")
    ...
    refreshed = await client.get(f"/api/v1/extraction-candidates/{scene['id']}", headers=headers)
    assert refreshed.status_code == 200
    assert refreshed.json()["data"] == result["candidate"]

    stranger_headers, _ = await _identity(client, email="decision-stranger@example.com")
    ...
    async with session_factory() as session:
        ...
        assert await session.scalar(select(func.count()).select_from(decision_table)) == 1
...
async def test_candidate_decisions_validate_changes_merge_ignore_and_scope(
    ...
    headers, workspace_id = await _identity(client, email="decision-validation@example.com")
    _, candidates = await _completed_candidate_batch(
        client,
        session_factory,
        headers,
    ...
    async with session_factory() as session:
        decision_table = Base.metadata.tables["scr_candidate_decisions"]
        assert await session.scalar(select(func.count()).select_from(decision_table)) == 3
