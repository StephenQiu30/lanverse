...
async def test_manual_spec_append_requires_and_persists_narrative_references(
    ...
    missing_references = await client.post(
        ...
        headers=headers,
        json={
            "expected_current_spec_version_id": mapped_data["current_spec_version_id"],
            "spec": spec["spec"],
            "asset_references": [],
        },
    )
...
async def test_many_to_many_coverage_is_bidirectional_and_gates_readiness(
    ...
    blocker_codes = {item["code"] for item in readiness.json()["data"]["blocking_reasons"]}
    ...
    after_codes = {item["code"] for item in readiness_after.json()["data"]["blocking_reasons"]}
...
async def test_one_unit_can_support_two_shots_but_primary_is_unique(
    ...
    assert duplicate_primary.json()["error"]["details"]["reason"] == ("duplicate_primary_reference")
...
async def test_omission_and_invented_approvals_are_append_only_and_become_stale(
    ...
    cast(dict[str, object], changed_invented_spec["visual"])["subject_placements"] = []
...
async def test_mapping_rejects_stale_cas_and_cross_episode_unit(
    ...
    assert cross_episode.json()["error"]["details"]["reason"] == ("unit_version_outside_episode")
...
async def test_split_rejects_narrative_loss_then_preserves_the_exact_partition(
    ...
    assert rejected.json()["error"]["details"] == {"reason": "split_narrative_mismatch"}
    ...
    assert sorted(item["unit_version_id"] for item in result_report["references"]) == sorted(
        item["unit_version_id"] for item in references
    )
