    ...
    async with session_factory() as session, session.begin():
        ...
        rows = list(
            await session.execute(
                ...
                .where(
                    NarrativeUnitVersion.script_version_id
            ...
            "shots": [
                {
                    ...
                    "narrative_unit_version_ids": (
                        unit_ids if position == 1 else unit_ids[:1]
                    ),
                    "spec": {
                        "script_reference": {
                            "confirmed_script_version_id": structure[
                                "script_version_id"
                            ],
...
async def test_whole_script_reaches_stable_export_after_restart(
    ...
    assert changed_preflight.json()["data"]["input_hash"] != second_export[
        "input_hash"
    ]
    ...
    async with session_factory() as session:
        assert (
            await session.scalar(
                select(func.count()).select_from(StoryboardExportManifest)
            )
            == 1
        )
        assert (
            await session.scalar(select(func.count()).select_from(InboxDelivery))
            == 1
        )
