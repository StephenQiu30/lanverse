    ...
    async with session_factory() as session:
        ...
        schedule = await session.scalar(
            select(Schedule).where(
                Schedule.schedule_key == f"media.location.retire:{source.id}"
            )
        )
        ...
        task = await session.scalar(
            select(Task).where(Task.id == target.migration_task_id)
        )
...
async def test_location_migration_storage_failure_requeues_without_database_switch(
    ...
    async with session_factory() as session:
        ...
        assert [(location.id, location.status) for location in locations] == [
            (source_id, "active")
        ]
        task = await session.scalar(
            select(Task).where(
                Task.idempotency_key == "location-migrate-storage-failure"
            )
        )
...
async def test_competing_location_task_cannot_replace_the_winner_active_location(
    ...
    actor, version_id, source_id, source_key, content, winner_body = (
        await _location_fixture(
            session_factory,
