...
async def test_confirmation_materializes_one_identity_with_multiple_asset_states(
    ...
    assert confirmed["entities"][0]["asset_id"] == str(
        result.entity_asset_ids["character:aurelia"]
    )
    ...
    async with session_factory() as session:
        ...
        assert await session.scalar(
            select(func.count()).select_from(AuditEvent).where(
                AuditEvent.action == "asset.created"
            )
        ) == 1
        assert await session.scalar(
            select(func.count()).select_from(AuditEvent).where(
                AuditEvent.action == "asset.state_created"
            )
        ) == 1
        assert await session.scalar(
            select(func.count()).select_from(AuditEvent).where(
                AuditEvent.action == "asset.version_created"
            )
        ) == 2
        assert await session.scalar(
            select(func.count()).select_from(AuditEvent).where(
                AuditEvent.action == "script.production_bible_confirmed"
            )
        ) == 1
...
async def test_existing_identity_match_requires_explicit_link_instead_of_duplication(
    ...
    async with session_factory() as session:
        plan = await plan_production_bible_materialization(session, bible_id)
    assert not plan.confirmable
    assert "existing_identity_requires_explicit_link" in {
...
async def test_review_issue_resolution_repairs_state_and_preserves_idempotent_hash_chain(
    ...
    resolved_states = {
        state["state_key"]: state for state in resolved["entities"][0]["states"]
    }
    ...
    async with session_factory() as session:
        ...
        assert await session.scalar(
            select(func.count()).select_from(AuditEvent).where(
                AuditEvent.action == "script.production_bible_review_issue_resolved"
            )
        ) == 1
