...
async def test_consent_registration_read_revision_and_revoke_are_append_only(
    ...
    assert [item["action"] for item in detail.json()["data"]["revisions"]] == ["register"]
    ...
    assert [item["action"] for item in updated.json()["data"]["revisions"]] == [
        "register",
        "update",
    ]
    ...
    async with session_factory() as session:
        ...
        assert await session.scalar(select(func.count()).select_from(ConsentRevision)) == 3
...
async def test_consent_commands_enforce_schema_capabilities_and_workspace_isolation(
    ...
    created = await client.post("/api/v1/consents", headers=owner_headers, json=payload)
    assert created.status_code == 201
...
async def test_script_version_resolver_uses_the_real_version_boundary(
    ...
    payload["scope"] = _scope(script_version_id, subject_type="SCRIPT_VERSION")
    payload["idempotency_key"] = "script-consent-001"
    created = await client.post("/api/v1/consents", headers=headers, json=payload)
    assert created.status_code == 201
