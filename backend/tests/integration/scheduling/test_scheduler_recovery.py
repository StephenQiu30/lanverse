...
async def test_owner_configures_registered_cleanup_cron_without_exposing_payload(
    ...
    headers, workspace_id = await _identity(
        client, email="scheduler-configuration@example.com"
    )
    other_headers, _ = await _identity(
        client, email="scheduler-configuration-other@example.com"
    )
...
async def test_misfire_policies_have_bounded_postgresql_concurrency_evidence(
    ...
    headers, workspace_id = await _identity(
        client, email=f"scheduler-policy-{policy}@example.com"
    )
    await _create_cleanup_schedule(
    ...
    async with session_factory() as session:
        ...
        assert await session.scalar(
            select(func.count())
            .select_from(Task)
            .where(Task.request_id == workspace_id)
        ) == expected_fires
        assert await session.scalar(
            select(func.count())
...
async def test_expired_lease_and_task_boundary_rollback_recover_without_duplicates(
    ...
    headers, workspace_id = await _identity(
        client, email="scheduler-boundary-recovery@example.com"
    )
    await _create_cleanup_schedule(
...
async def test_failure_limit_and_unregistered_handler_stop_automatic_dispatch(
    ...
    headers, workspace_id = await _identity(
        client, email="scheduler-manual-attention@example.com"
    )
    await _create_cleanup_schedule(
            ...
            async with session_factory() as session:
                async with session.begin():
                    stored = await session.get(
                        Schedule, schedule.id, with_for_update=True
                    )
                    assert stored is not None
                    if expected_count == 1:
    ...
    assert retry_records[-1].getMessage() == (
    ...
    public = next(
        item for item in listed.json()["data"]["items"] if item["id"] == str(schedule.id)
    )
    ...
    invalid_schedule = await _cleanup_schedule(
        session_factory, invalid_workspace_id
    )
    ...
    async with session_factory() as session:
        async with session.begin():
            stored = await session.get(
                Schedule, invalid_schedule.id, with_for_update=True
            )
            assert stored is not None
            stored.payload = {
...
async def test_dispatch_can_use_postgresql_clock_as_the_authority(
    ...
    headers, workspace_id = await _identity(
        client, email="scheduler-database-clock@example.com"
    )
    await _create_cleanup_schedule(
