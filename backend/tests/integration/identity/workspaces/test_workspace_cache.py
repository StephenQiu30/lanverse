...
async def test_cache_is_never_read_before_membership_and_failures_do_not_change_facts(
    ...
    hidden = await client.get(f"/api/v1/workspaces/{workspace_id}", headers=other_headers)
    ...
    cache.fail_get = True
    recovered = await client.get(f"/api/v1/workspaces/{workspace_id}", headers=headers)
