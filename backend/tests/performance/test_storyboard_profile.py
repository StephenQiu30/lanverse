...
async def _seed_confirmed_episode(
    ...
    *,
    ...
    headers = {"authorization": f"Bearer {create_access_token(actor_id, 1, token_settings)}"}
    project = _data(
        await client.post(
            "/api/v1/projects",
            headers=headers,
        ...
        for position in range(1, shot_count + 1):
            ...
            references.extend(
                AssetReference(
                    ...
                    asset_state_id=asset_versions[request.asset_version_id].asset_state_id,
...
async def _exercise_reorder(
    ...
    *,
    ...
    for _ in range(30):
        ...
        assert [item["position"] for item in reordered["items"]] == list(range(1, 121))
...
async def test_fixed_storyboard_performance_profile(
    ...
    for shot_count, request_ids in sample_request_ids.items():
        combined_samples = [
            sum(durations[request_id] for request_id in profile_ids) for profile_ids in request_ids
        ]
