...
async def _prepare(
    ...
    for shot, version in rows:
        ...
        shots.append(
            ExportShot(
                ...
                narrative_references=tuple(
                    narrative_refs_by_spec.get(version.id, [])
                ),
            )
        )
    snapshot = ExportSnapshot(
        ...
        assets=tuple(
            ...
            for value in (
                assets_by_version[version_id] for version_id in asset_version_ids
            )
        ),
        shots=tuple(shots),
    )
...
def _preflight_response(
    ...
    return ExportPreflightResponse(
        ...
        coverage_evaluation_hash=(
            snapshot.coverage_evaluation_hash if snapshot else None
        ),
        readiness_evaluation_hash=(
            snapshot.readiness_evaluation_hash if snapshot else None
        ),
        blockers=[
            ExportBlockerResponse.model_validate(item.model_dump())
...
def _job_response(
    ...
    return ExportResponse(
        ...
        status=cast(
            Literal["queued", "running", "succeeded", "failed"], job.status
        ),
