...
def _unavailable_report(episode_id: UUID) -> CoverageReportResponse:
    basis_hash = canonical_payload_hash(
        {"episode_id": str(episode_id), "status": "unavailable"}
    )
    ...
    evaluation_hash = canonical_payload_hash(
        {"basis_hash": basis_hash, "status": "unavailable"}
    )
...
def _evaluate_report(
    ...
    spec_to_shot = {
        version.id: shot.id for shot, version in rows if version is not None
    }
    ...
    stale_decision_ids = [
        ...
        or (
            decision.unit_version_id is not None
            and decision.unit_version_id not in current_units
        )
        or (
        ...
        shot_items.append(
            ShotCoverageResponse(
                ...
                unit_version_ids=list(
                    dict.fromkeys(
                        reference.unit_version_id for reference in shot_references
                    )
                ),
            )
        )
    ...
    summary = CoverageSummaryResponse(
        ...
        approved_omitted=sum(
            item.status == "approved_omitted" for item in required_items
        ),
        ...
        approved_invented=sum(
            item.status == "approved_invented" for item in shot_items
        ),
    ...
    *,
    ...
    async with session.begin():
        ...
        version = ShotSpecVersion(
            ...
            shot_id=shot.id,
            version_no=(
                await storyboard_repository.latest_spec_version_number(session, shot.id)
            )
            + 1,
...
async def decide_coverage(
    ...
    *,
    ...
    async with session.begin():
        ...
        shot_by_spec = {
            shot.spec_version_id: shot
            for shot in report.shots
            if shot.spec_version_id is not None
        }
        ...
        append_audit_event(
            ...
            metadata={
                ...
                "unit_version_id": (
                    str(decision.unit_version_id)
                    if decision.unit_version_id is not None
                    else None
                ),
