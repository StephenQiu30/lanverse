...
async def _prepare_envelope(
    ...
    *,
    ...
    delivery, is_first_delivery = await start_inbox_delivery(session, envelope, consumer_name, now)
    ...
    if envelope.event_type != "script_extraction.requested":
        return reject_inbox_delivery(delivery, error_code="unsupported_message_type", now=now)
    if envelope.schema_version != 1:
        ...
        if changed:
            await synchronize_extraction_batch_status(session, task.request_id, "failed", now=now)
