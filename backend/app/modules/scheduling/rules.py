...
def _parse_cron(expression: str) -> _CronSpec:
    ...
    return _CronSpec(
        ...
        month_days=_parse_cron_field(
            month_day, minimum=1, maximum=31, field="day-of-month"
        ),
    ...
    for day_offset in range(366 * 8 + 1):
        ...
        for hour in sorted(spec.hours):
            for minute in sorted(spec.minutes):
                candidate = _wall_time_to_first_fold_utc(
                    candidate_date, hour, minute, zone
                )
...
def _interval_plan(
    *,
    ...
    return DispatchPlan(
        fire_times=tuple(
            next_fire_at + timedelta(seconds=index * seconds)
    ...
    *,
    ...
    if misfire_policy == "skip":
        ...
        candidate = _previous_cron_occurrence(
            expression, timezone, at_or_before=now
        )
        ...
        latest = _previous_cron_occurrence(
            expression, timezone, at_or_before=now
        )
