...
async def list_consents(
    ...
    *,
    ...
    total = await session.scalar(select(func.count()).select_from(Consent).where(*filters))
...
async def find_revision(session: AsyncSession, revision_id: UUID) -> ConsentRevision | None:
    return await session.scalar(select(ConsentRevision).where(ConsentRevision.id == revision_id))


async def list_revisions(session: AsyncSession, consent_id: UUID) -> list[ConsentRevision]:
    return list(
...
async def find_revisions(session: AsyncSession, revision_ids: list[UUID]) -> list[ConsentRevision]:
    if not revision_ids:
        return []
    return list(
        await session.scalars(select(ConsentRevision).where(ConsentRevision.id.in_(revision_ids)))
    )


async def list_proofs(session: AsyncSession, revision_ids: list[UUID]) -> list[ConsentProof]:
    if not revision_ids:
        return []
    return list(
        await session.scalars(
