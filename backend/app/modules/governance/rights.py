...
def evaluate_grants(grants: list[ConsentGrant], usage: RightsUsage) -> RightsGateResult:
    if not grants:
    ...
    valid_ids = tuple(grant.consent_id for grant in grants if _blocker(grant, usage) is None)
    if valid_ids:
        return RightsGateResult(allowed=True, blockers=(), consent_ids=valid_ids)

    blockers = tuple(blocker for grant in grants if (blocker := _blocker(grant, usage)) is not None)
