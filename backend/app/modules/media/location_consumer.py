...
async def consume_media_location_migration(
    ...
    *,
    ...
    if (
        ...
        or task.usage_type
        not in {"media_location_migration", "media_location_rollback"}
    ...
    found = await repository.find_media_version(
        session, task.request_id, for_update=True
    )
    location = await repository.find_media_location(
        session, task.usage_id, for_update=True
    )
    if (
        found is None
    ...
    version = found[0]
    if (
        location.storage_profile != storage_profile
    ...
    if task.usage_type == "media_location_migration":
        ...
        target_key = (
            f"workspaces/{version.workspace_id}/media/{version.id}/locations/{task.id}"
        )
        try:
        ...
        if not valid:
            ...
            return await _integrity_failure(
                session, envelope, task_id=task.id, now=now
            )
        ...
        if (
            active.storage_profile != storage_profile
