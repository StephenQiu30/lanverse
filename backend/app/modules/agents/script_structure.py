...
def _validate_chunk_result(result: ScriptExtractionResult, chunk_text: str) -> None:
    ...
    candidate_by_key = {
        candidate.candidate_key: candidate for candidate in result.candidates
    }
...
def _merge_strings(
    ...
    *,
    ...
    incoming = (
        [str(item) for item in cast(list[object], values)]
...
def _aggregate_results(
    ...
    for item in ordered_results:
        ...
        for candidate in result.candidates:
            candidate_data = cast(
                dict[str, object], candidate.model_dump(mode="json")
            )
            ...
            source_range["end"] = (
                _state_int(source_range["end"], "source_range.end") + chunk_start
            )
            ...
            if candidate.proposal.kind == "continuity":
                ...
                if continuity_scope == "episode":
                    ...
                    continuity_key = "world:" + " ".join(
                        str(proposal.get("topic") or proposal.get("issue") or "")
                        .casefold()
                        .split()
                    )
                else:
                ...
                if existing_continuity is not None:
                    existing_proposal = cast(
                        dict[str, object], existing_continuity["proposal"]
                    )
            ...
            if candidate.proposal.kind == "asset":
                ...
                if isinstance(first_seen_episode, int) and isinstance(
                    raw_episode_numbers, list
                ):
                ...
                if existing is not None:
                    existing_proposal = cast(
                        dict[str, object], existing["proposal"]
                    )
        ...
        async def extract_chunk(state: _ScriptStructureState) -> dict[str, object]:
            ...
            try:
                ...
                raise AgentExecutionError(
                    outcome="failed",
                    code="agent_output_invalid",
                    summary="Agent returned an invalid chunk result",
                    retryable=False,
                    next_action="start_new_skill_run",
                ) from error
            return {
