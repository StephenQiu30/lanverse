...
_CHINESE_MARKER = re.compile(r"第([0-9０-９零〇一二两三四五六七八九十百千]+)(?:集|话|章)")
...
def _structural_issues(
    ...
    duplicate_index = next(
        (index for index, value in enumerate(marker_values) if value in marker_values[:index]),
        None,
    )
    ...
    if duplicate_index is not None:
        ...
        if out_of_order_index is not None:
            ...
            gap_index = next(
                (index for index, value in enumerate(marker_values) if value != index + 1),
                None,
            )
    ...
    for index, located in enumerate(markers):
        segment_end = markers[index + 1].line_start if index + 1 < len(markers) else len(text)
...
def analyze_document(text: str) -> DocumentAnalysis:
    ...
    located_markers = tuple(
        marker for line in lines if (marker := _marker_for_line(line)) is not None
    )
    issues: list[FormatIssueDraft] = []
    if text.startswith("\ufeff"):
        issues.append(
    ...
    marker_by_line = {located.marker.line_number: located.marker for located in located_markers}
    first_marker_line = located_markers[0].marker.line_number if located_markers else None
    blocks = tuple(
        NarrativeBlockDraft(
            position=position,
            kind=kind,
        ...
        for kind, metadata in [_block_kind(line, marker_by_line, first_marker_line)]
    )
