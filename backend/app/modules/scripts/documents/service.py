    ...
    canonical = json.dumps(
        {
            ...
            "source_media_version_id": (
                str(request.media_version_id) if request.media_version_id is not None else None
            ),
            "title": request.title,
...
def _require_analysis_invariants(text: str, analysis: DocumentAnalysis) -> None:
    reconstructed = "".join(
        text[block.start_codepoint : block.end_codepoint] for block in analysis.blocks
    )
    if (
        reconstructed != text
        or analysis.content_hash != hashlib.sha256(text.encode("utf-8")).hexdigest()
    ):
        raise ApiError(
    ...
    *,
    ...
    async with session.begin():
        ...
        inserted_id = await session.scalar(
            ...
            .on_conflict_do_nothing(constraint="uq_scr_document_project_idempotency")
