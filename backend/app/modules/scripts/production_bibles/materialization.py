    ...
    *,
    ...
    for raw_item in raw_items:
        ...
        if evidence.source_end > len(source_text) or (
            source_text[evidence.source_start : evidence.source_end]
...
async def _existing_states(
    ...
    *,
    ...
    if not asset_ids:
        return []
    query = select(AssetState).where(AssetState.asset_id.in_(asset_ids)).order_by(
        AssetState.asset_id,
...
async def _existing_versions(
    ...
    *,
    ...
    if not version_ids:
        return []
    query = select(AssetVersion).where(AssetVersion.id.in_(version_ids)).order_by(
        AssetVersion.id
    )
    if for_update:
        query = query.with_for_update().execution_options(populate_existing=True)
...
async def _prepare_materialization(
    ...
    *,
    ...
    existing_state_by_key = {
        (state.asset_id, state.state_key): state for state in existing_states
    }
    mapped_version_ids = [
    ...
    for entity in entities:
        ...
        if linked_asset is not None and (
            linked_asset.id,
            "base",
        ...
        for state in entity_states:
            ...
            prepared_states.append(
                _PreparedState(row=state, spec=state_spec, action="create")
            )
    ...
    unique_issues = list(
        {
            (issue.code, issue.scope, issue.subject_key, issue.summary): issue
            for issue in issues
        }.values()
    )
    unique_issues.sort(
...
def _snapshot_result(
    ...
    return {
        ...
        "entity_asset_ids": {
            key: str(value) for key, value in sorted(entity_asset_ids.items())
        },
        "state_bindings": {
            key: {
...
def _result_from_snapshot(
    ...
    *,
    ...
    try:
        ...
        entity_asset_ids = {
            key: UUID(str(value)) for key, value in raw_entities.items()
        }
        state_bindings = {
            key: ProductionBibleStateBinding(
                asset_state_id=UUID(
                    str(cast(dict[str, object], value)["asset_state_id"])
                ),
                asset_version_id=UUID(
                    str(cast(dict[str, object], value)["asset_version_id"])
                ),
            )
    ...
    async with session.begin():
        ...
        append_audit_event(
            ...
            metadata={
                ...
                "state_count": sum(
                    len(entity.states) for entity in prepared.entities
                ),
            },
            occurred_at=now,
        )
