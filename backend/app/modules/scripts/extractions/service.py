...
_CANDIDATE_PROPOSAL_ADAPTER: TypeAdapter[CandidateProposal] = TypeAdapter(CandidateProposal)
...
def _same_decision(
    ...
    return existing.action == requested.action and existing.payload == _decision_payload(requested)
...
async def enqueue_extraction(
    ...
    *,
    ...
    source = await repository.find_source(session, version.source_id, for_update=True)
    if source is None:
    ...
    inserted_id = await session.scalar(
        ...
        .on_conflict_do_nothing(constraint="uq_scr_batch_version_idempotency")
    ...
    batch = await repository.find_extraction_batch(session, inserted_id, for_update=True)
    if batch is None:
        raise ApiError(
            ErrorCode.INTERNAL_ERROR,
            "Extraction batch state is unavailable",
...
async def start_extraction(
    ...
    *,
    ...
    async with session.begin():
        result = await enqueue_extraction(
            session,
            claims,
            version_id,
            request,
...
async def get_extraction_batch(
    ...
    await require_resource_access(session, claims, batch.workspace_id, "Extraction batch")
    task = await get_task(session, claims, batch.task_id)
    return _batch_response(batch, task)
...
async def synchronize_extraction_batch_status(
    ...
    *,
    ...
    batch = await repository.find_extraction_batch(session, batch_id, for_update=True)
    if batch is None:
        return
    batch.status = status
...
async def get_extraction_candidate(
    ...
    await require_resource_access(session, claims, candidate.workspace_id, "Extraction candidate")
...
async def decide_extraction_candidate(
    ...
    async with session.begin():
        ...
        if candidate.kind == "asset" and decision.action in {
            ...
            input_version = await repository.find_version(session, batch.script_version_id)
            source = (
                None
