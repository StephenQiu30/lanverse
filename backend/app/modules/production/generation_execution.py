...
def generation_provider_request_key(
    *,
    ...
    source = f"generation-attempt-v1:{workspace_id}:{task_id}:{sequence}:{input_hash}"
...
async def fail_generation_attempt_without_provider(
    ...
    *,
    ...
    if attempt.status == "failed" and task.status == "failed" and reservation.status == "released":
        return
    if (
    ...
    if (
        await repository.find_release_cost_entry(
            session,
            workspace_id,
            reservation.id,
        )
