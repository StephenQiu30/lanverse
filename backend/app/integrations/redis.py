...
"""
    ...
    def _high_cost_key(self, scope: str, identity: str) -> str:
        return f"lanverse:{self._environment}:high_cost_guard:v1:{scope}:{identity}"
    ...
    async def set(
        ...
        *,
        ...
        started = perf_counter()
        result = "error"
        ttl = ttl_seconds + (secrets.randbelow(jitter_seconds + 1) if jitter_seconds else 0)
        try:
    ...
    def _parse_high_cost_result(raw_result: object) -> HighCostGuardResult:
        ...
        try:
            ...
            outcome_raw = parts[1]
            outcome = (
                outcome_raw.decode("utf-8") if isinstance(outcome_raw, bytes) else str(outcome_raw)
            )
