...
type ScriptAnalysisPort interface {
	...
	ApproveEpisodeBreakdown(context.Context, uuid.UUID) (Analysis, error)
	ReviseNarrativeDraft(context.Context, uuid.UUID, string, []NarrativeOperation) (Analysis, error)
	ApproveNarrative(context.Context, uuid.UUID, string) (Analysis, error)
