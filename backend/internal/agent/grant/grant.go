...
func (signer *Signer) IssueReferencePlanDispatchAuthorization(
	...
	if err := claims.ValidateForReferencePlan(invocation, claimVersion, signer.now().UTC().Unix()); err != nil {
		return contract.SceneAnalysisDispatchAuthorization{}, err
	}
	return signer.encodeSceneAnalysisDispatchAuthorization(claims)
}

func (signer *Signer) VerifyReferencePlanDispatchAuthorization(
	value string,
	invocation contract.ReferencePlanInvocation,
	claimVersion int64,
) error {
	...
	if err = claims.ValidateForReferencePlan(invocation, claimVersion, signer.now().UTC().Unix()); err != nil {
		return errors.New("dispatch authorization does not authorize Reference Plan invocation")
	}
	return nil
}

func (signer *Signer) IssueReferenceBriefDispatchAuthorization(
	...
	if err := claims.ValidateForReferenceBrief(invocation, claimVersion, signer.now().UTC().Unix()); err != nil {
		return contract.SceneAnalysisDispatchAuthorization{}, err
	}
	return signer.encodeSceneAnalysisDispatchAuthorization(claims)
}

func (signer *Signer) VerifyReferenceBriefDispatchAuthorization(
	value string,
	invocation contract.ReferenceBriefInvocation,
	claimVersion int64,
) error {
	...
	if err = claims.ValidateForReferenceBrief(invocation, claimVersion, signer.now().UTC().Unix()); err != nil {
		return errors.New("dispatch authorization does not authorize Reference Brief invocation")
	}
	return nil
}
