...
var (
	...
	ErrImmutableSceneAnalysisRead      = errors.New("SceneAnalysisInvocationRead is immutable")
)

type SceneAnalysisRelease struct {
	ID                  uuid.UUID      `gorm:"type:uuid;primaryKey"`
	StageKey            string         `gorm:"type:varchar(64);not null;uniqueIndex:uq_agt_scene_release_variant,priority:1;check:ck_agt_scene_release_stage,stage_key IN ('propose_script_spans','extract_scene_facts','resolve_identities','review_candidate','derive_production_entities')"`
...
type SceneAnalysisInvocationRecord struct {
	...
	StageKey          string               `gorm:"type:varchar(64);not null;uniqueIndex:uq_agt_scene_invocation_node_input,priority:3;check:ck_agt_scene_invocation_stage,stage_key IN ('propose_script_spans','extract_scene_facts','resolve_identities','review_candidate','derive_production_entities')"`
...
type SceneAnalysisInvocationRead struct {
	...
	SourceResultHash      string                         `gorm:"type:char(64);not null;check:ck_agt_scene_upstream_result_hash,char_length(source_result_hash) = 64"`
	CreatedAt             time.Time                      `gorm:"type:timestamptz;not null"`
	...
	CandidateRevision     SceneAnalysisCandidateRevision `gorm:"foreignKey:CandidateRevisionID;references:ID;constraint:OnUpdate:CASCADE,OnDelete:RESTRICT"`
...
func (SceneAnalysisInvocationRead) TableName() string {
	return "agt_scene_analysis_invocation_reads"
}
func (*SceneAnalysisInvocationRead) BeforeUpdate(*gorm.DB) error {
	return ErrImmutableSceneAnalysisRead
}
func (*SceneAnalysisInvocationRead) BeforeDelete(*gorm.DB) error {
	return ErrImmutableSceneAnalysisRead
}
...
type SceneAnalysisCandidateRevision struct {
	...
	CandidateType         string                        `gorm:"type:varchar(80);not null;check:ck_agt_scene_candidate_type,candidate_type IN ('script_span_candidate','scene_fact_candidate','identity_resolution_candidate','structure_identity_review_candidate','production_entity_fragment_candidate')"`
