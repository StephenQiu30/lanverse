...
type AgentInvocation struct {
	...
	Status      string         `gorm:"type:varchar(20);not null;index:ix_agt_invocations_status_created,priority:1;check:ck_agt_invocation_status,status IN ('queued','running','succeeded','failed','unknown')"`
	...
	CreatedAt   time.Time      `gorm:"type:timestamptz;not null;index:ix_agt_invocations_status_created,priority:3"`
	UpdatedAt   time.Time      `gorm:"type:timestamptz;not null"`
	Workspace   Workspace      `gorm:"foreignKey:WorkspaceID;references:ID;constraint:OnUpdate:CASCADE,OnDelete:RESTRICT"`
}
