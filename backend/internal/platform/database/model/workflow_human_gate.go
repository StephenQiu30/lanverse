...
type WorkflowHumanGateInput struct {
	...
	GateKey           string            `gorm:"type:varchar(80);not null;check:ck_wrk_human_gate_input_key,gate_key = 'structure_identity'"`
	GateInstanceKey   string            `gorm:"type:varchar(220);not null;uniqueIndex:uq_wrk_human_gate_input_instance"`
	SubjectType       string            `gorm:"type:varchar(80);not null;check:ck_wrk_human_gate_input_subject,subject_type = 'structure_identity'"`
