...
func (service *ControlService) Cancel(
	...
	if service == nil || service.repository == nil || service.controller == nil || service.config.Now == nil ||
		...
		command.ExpectedRevision < 1 || command.IdempotencyKey == "" || len(command.IdempotencyKey) > 200 {
		return domain.ControlIntent{}, invalid("Invalid workflow cancel input")
	}
	commandInputHash, err := platformcommand.InputHash(struct {
		...
		ExpectedRevision: command.ExpectedRevision, Action: domain.ControlActionCancel,
	})
	if err != nil {
		return domain.ControlIntent{}, err
	}
	now := service.config.Now().UTC()
	intentID := stableID("workflow-control-intent", command.WorkspaceID, command.WorkflowRunID, domain.ControlActionCancel)
	desired := domain.ControlPreparation{Intent: domain.ControlIntent{
		...
		ControlID: stableID("workflow-control", intentID), Action: domain.ControlActionCancel,
	...
	if prepared.Intent.CommandInputHash != commandInputHash || prepared.Intent.InputHash != request.InputHash ||
		prepared.Intent.WorkflowRunID != command.WorkflowRunID || prepared.Intent.Action != domain.ControlActionCancel {
...
func finalizeControl(
	...
	case controlErr != nil || outcome == domain.ControlOutcomeUnknown:
		...
		run.Status, run.ProgressStage = "NEEDS_ATTENTION", "cancel_unknown"
		run.NextAction, run.Error = stringPointer("reconcile_cancel"), errorPayload("cancel_outcome_unknown")
	case outcome == domain.ControlOutcomeRequested && observation.ObservedInputHash == intent.InputHash:
		intent.Status = "pending"
		run.Status, run.ProgressStage = "PAUSED", "cancel_requested"
		run.NextAction, run.Error = stringPointer("reconcile_cancel"), nil
	case (outcome == domain.ControlOutcomeApplied || outcome == domain.ControlOutcomeAlreadyApplied) &&
	...
	default:
		...
		if run.Status != "SUCCEEDED" && run.Status != "FAILED" && run.Status != "CANCELLED" {
			run.Status, run.ProgressStage = "NEEDS_ATTENTION", "cancel_conflict"
