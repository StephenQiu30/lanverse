...
func main() {
	...
	bibleWorker := bibleapp.NewWorker(bibleStore, agentRuntime, func() time.Time { return time.Now().UTC() }, configuration.AgentPollInterval, logger)
	...
	storyboardWorker := storyboardapp.NewWorker(storyboardStore, agentRuntime, func() time.Time { return time.Now().UTC() }, configuration.AgentPollInterval, logger)
